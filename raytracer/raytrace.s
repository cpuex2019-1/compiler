Init: # initialize float value and heap pointer
	addi	$3, $0, 0
	addi	$30, $0, 170
	addi	$29, $0, 0
	addi	$31, $0, 0
#	outb	$30 # atsunobu request
	addi	$4, $0, 4592
	ori	$30, $0, 17152
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16230
	slli	$30, $30, 16
	ori	$30, $30, 26214
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 17174
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 49942
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15820
	slli	$30, $30, 16
	ori	$30, $30, 52429
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 49152
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15232
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 19646
	slli	$30, $30, 16
	ori	$30, $30, 48160
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 20078
	slli	$30, $30, 16
	ori	$30, $30, 27432
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16800
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15692
	slli	$30, $30, 16
	ori	$30, $30, 52429
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16000
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16672
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16025
	slli	$30, $30, 16
	ori	$30, $30, 39322
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 17279
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15897
	slli	$30, $30, 16
	ori	$30, $30, 39322
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16457
	slli	$30, $30, 16
	ori	$30, $30, 4059
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16880
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16752
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 14545
	slli	$30, $30, 16
	ori	$30, $30, 46871
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 48588
	slli	$30, $30, 16
	ori	$30, $30, 52429
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15395
	slli	$30, $30, 16
	ori	$30, $30, 55050
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 48716
	slli	$30, $30, 16
	ori	$30, $30, 52429
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16384
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 49992
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 17224
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16585
	slli	$30, $30, 16
	ori	$30, $30, 4059
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15502
	slli	$30, $30, 16
	ori	$30, $30, 64053
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16412
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15733
	slli	$30, $30, 16
	ori	$30, $30, 59333
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15799
	slli	$30, $30, 16
	ori	$30, $30, 54894
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15843
	slli	$30, $30, 16
	ori	$30, $30, 36408
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15890
	slli	$30, $30, 16
	ori	$30, $30, 18725
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15948
	slli	$30, $30, 16
	ori	$30, $30, 52429
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16042
	slli	$30, $30, 16
	ori	$30, $30, 43690
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16096
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 49024
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	addi	$30, $0, 0
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 14669
	slli	$30, $30, 16
	ori	$30, $30, 25782
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15368
	slli	$30, $30, 16
	ori	$30, $30, 34406
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15914
	slli	$30, $30, 16
	ori	$30, $30, 43692
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16256
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15027
	slli	$30, $30, 16
	ori	$30, $30, 33030
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 15658
	slli	$30, $30, 16
	ori	$30, $30, 42889
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16128
	slli	$30, $30, 16
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16201
	slli	$30, $30, 16
	ori	$30, $30, 4059
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16329
	slli	$30, $30, 16
	ori	$30, $30, 4059
	sw	$30, 0($4)
	addi	$4, $4, 8
	ori	$30, $0, 16457
	slli	$30, $30, 16
	ori	$30, $30, 4059
	sw	$30, 0($4)
	addi	$4, $4, 8
	j Main
sin_sub2.2684:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36255
	fsub	$f1, $f2, $f1
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36256
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36256:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36255:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36257
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36257:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
sin_sub1.2687:
	lf	$f2, 4968($0) # 3.141593
	sltf	$30, $f1, $f2
	bne	$30, $0, eq_else.36258
	fsub	$f1, $f1, $f2
	fneg	$f0, $f0
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36259
	fsub	$f1, $f2, $f1
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36260
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36260:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36259:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36261
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36261:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36258:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36262
	fsub	$f1, $f2, $f1
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36263
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36263:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36262:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36264
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36264:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
cos_sub2.2695:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36265
	fsub	$f1, $f2, $f1
	fneg	$f0, $f0
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36266
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36266:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36265:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36267
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36267:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
cos_sub1.2698:
	lf	$f2, 4968($0) # 3.141593
	sltf	$30, $f1, $f2
	bne	$30, $0, eq_else.36268
	fsub	$f1, $f1, $f2
	fneg	$f0, $f0
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36269
	fsub	$f1, $f2, $f1
	fneg	$f0, $f0
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36270
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36270:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36269:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36271
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36271:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36268:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36272
	fsub	$f1, $f2, $f1
	fneg	$f0, $f0
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36273
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36273:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36272:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36274
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f1, $f1
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f1, $f1, $f1
	fmul	$f3, $f1, $f1
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
eq_else.36274:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f1, $f2, $f1
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f1, $f1
	fmul	$f5, $f1, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f1, $f2
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f1, $f1
	fmul	$f5, $f3, $f3
	fmul	$f1, $f1, $f3
	fmul	$f1, $f1, $f5
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	jr $31
atan.2705:
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36275
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36276
eq_else.36275:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36276:
	fmul	$f0, $f1, $f0
	lf	$f2, 4872($0) # 0.437500
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.36277
	lf	$f2, 4816($0) # 2.437500
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.36278
	lf	$f2, 4960($0) # 1.570796
	lf	$f3, 4920($0) # 1.000000
	fdiv	$f0, $f3, $f0
	fmul	$f3, $f0, $f0
	fmul	$f3, $f0, $f3
	lf	$f4, 4864($0) # 0.333333
	fmul	$f3, $f3, $f4
	fsub	$f3, $f0, $f3
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4856($0) # 0.200000
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f4, $f0, $f4
	fmul	$f4, $f4, $f5
	lf	$f5, 4848($0) # 0.142857
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4840($0) # 0.111111
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f0
	fmul	$f4, $f5, $f4
	lf	$f5, 4832($0) # 0.089764
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f5, $f4, $f4
	fmul	$f0, $f5, $f0
	fmul	$f0, $f0, $f4
	lf	$f4, 4824($0) # 0.060035
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	jr $31
eq_else.36278:
	lf	$f2, 4952($0) # 0.785398
	lf	$f3, 4920($0) # 1.000000
	fsub	$f3, $f0, $f3
	lf	$f4, 4920($0) # 1.000000
	fadd	$f0, $f0, $f4
	fdiv	$f0, $f3, $f0
	fmul	$f3, $f0, $f0
	fmul	$f3, $f0, $f3
	lf	$f4, 4864($0) # 0.333333
	fmul	$f3, $f3, $f4
	fsub	$f3, $f0, $f3
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4856($0) # 0.200000
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f4, $f0, $f4
	fmul	$f4, $f4, $f5
	lf	$f5, 4848($0) # 0.142857
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4840($0) # 0.111111
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f0
	fmul	$f4, $f5, $f4
	lf	$f5, 4832($0) # 0.089764
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f5, $f4, $f4
	fmul	$f0, $f5, $f0
	fmul	$f0, $f0, $f4
	lf	$f4, 4824($0) # 0.060035
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	fadd	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	jr $31
eq_else.36277:
	fmul	$f2, $f0, $f0
	fmul	$f2, $f0, $f2
	lf	$f3, 4864($0) # 0.333333
	fmul	$f2, $f2, $f3
	fsub	$f2, $f0, $f2
	fmul	$f3, $f0, $f0
	fmul	$f3, $f3, $f3
	fmul	$f3, $f0, $f3
	lf	$f4, 4856($0) # 0.200000
	fmul	$f3, $f3, $f4
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f4, $f3, $f3
	fmul	$f3, $f0, $f3
	fmul	$f3, $f3, $f4
	lf	$f4, 4848($0) # 0.142857
	fmul	$f3, $f3, $f4
	fsub	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f3, $f3, $f3
	fmul	$f3, $f3, $f3
	fmul	$f3, $f0, $f3
	lf	$f4, 4840($0) # 0.111111
	fmul	$f3, $f3, $f4
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f4, $f3, $f3
	fmul	$f4, $f4, $f4
	fmul	$f4, $f4, $f0
	fmul	$f3, $f4, $f3
	lf	$f4, 4832($0) # 0.089764
	fmul	$f3, $f3, $f4
	fsub	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f3, $f3, $f3
	fmul	$f4, $f3, $f3
	fmul	$f0, $f4, $f0
	fmul	$f0, $f0, $f3
	lf	$f3, 4824($0) # 0.060035
	fmul	$f0, $f0, $f3
	fadd	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	jr $31
read_screen_settings.2844:
	inf	$f0
	sf	$f0, 3244($0)
	inf	$f0
	sf	$f0, 3252($0)
	inf	$f0
	sf	$f0, 3260($0)
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36279
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36280
eq_else.36279:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36280:
	fmul	$f2, $f2, $f0
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	lf	$f3, 4968($0) # 3.141593
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36281
	fsub	$f1, $f2, $f3
	lf	$f2, 4880($0) # -1.000000
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f1, $f4
	bne	$30, $0, eq_else.36283
	fsub	$f1, $f3, $f1
	lf	$f2, 4920($0) # 1.000000
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36285
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36286
eq_else.36285:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36286:
	j	eq_cont.36284
eq_else.36283:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36287
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36288
eq_else.36287:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36288:
eq_cont.36284:
	j	eq_cont.36282
eq_else.36281:
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f2, $f4
	bne	$30, $0, eq_else.36289
	fsub	$f1, $f3, $f2
	lf	$f2, 4880($0) # -1.000000
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36291
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36292
eq_else.36291:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36292:
	j	eq_cont.36290
eq_else.36289:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f2
	bne	$30, $0, eq_else.36293
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f2, $f2
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f2, $f2
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f2, $f2, $f2
	fmul	$f4, $f2, $f2
	fmul	$f2, $f2, $f4
	fmul	$f2, $f2, $f5
	fsub	$f2, $f3, $f2
	fmul	$f1, $f1, $f2
	j	eq_cont.36294
eq_else.36293:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f2, $f3, $f2
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f2, $f2
	fmul	$f6, $f2, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f2, $f3
	fmul	$f6, $f2, $f2
	fmul	$f6, $f6, $f6
	fmul	$f6, $f2, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f2, $f2
	fmul	$f6, $f4, $f4
	fmul	$f2, $f2, $f4
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f5
	fsub	$f2, $f3, $f2
	fmul	$f1, $f1, $f2
eq_cont.36294:
eq_cont.36290:
eq_cont.36282:
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36295
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36296
eq_else.36295:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36296:
	fmul	$f0, $f2, $f0
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f0, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f0, $f0, $f3
	lf	$f3, 4968($0) # 3.141593
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.36297
	fsub	$f0, $f0, $f3
	fneg	$f2, $f2
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f0, $f4
	bne	$30, $0, eq_else.36299
	fsub	$f0, $f3, $f0
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36301
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f0, $f0
	fmul	$f6, $f0, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f0, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f6, $f0, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f6, $f4, $f4
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f6
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
	j	eq_cont.36302
eq_else.36301:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f0, $f3, $f0
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f0, $f0
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f0, $f0, $f0
	fmul	$f4, $f0, $f0
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
eq_cont.36302:
	j	eq_cont.36300
eq_else.36299:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36303
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f0, $f0
	fmul	$f6, $f0, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f0, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f6, $f0, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f6, $f4, $f4
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f6
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
	j	eq_cont.36304
eq_else.36303:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f0, $f3, $f0
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f0, $f0
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f0, $f0, $f0
	fmul	$f4, $f0, $f0
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
eq_cont.36304:
eq_cont.36300:
	j	eq_cont.36298
eq_else.36297:
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f0, $f4
	bne	$30, $0, eq_else.36305
	fsub	$f0, $f3, $f0
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36307
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f0, $f0
	fmul	$f6, $f0, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f0, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f6, $f0, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f6, $f4, $f4
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f6
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
	j	eq_cont.36308
eq_else.36307:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f0, $f3, $f0
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f0, $f0
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f0, $f0, $f0
	fmul	$f4, $f0, $f0
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
eq_cont.36308:
	j	eq_cont.36306
eq_else.36305:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36309
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f0, $f0
	fmul	$f6, $f0, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f0, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f6, $f0, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f6, $f4, $f4
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f6
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
	j	eq_cont.36310
eq_else.36309:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f0, $f3, $f0
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f0, $f0
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f0, $f0
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f0, $f0, $f0
	fmul	$f4, $f0, $f0
	fmul	$f0, $f0, $f4
	fmul	$f0, $f0, $f5
	fsub	$f0, $f3, $f0
	fmul	$f0, $f2, $f0
eq_cont.36310:
eq_cont.36306:
eq_cont.36298:
	inf	$f2
	lf	$f3, 4808($0) # 0.017453
	fmul	$f2, $f2, $f3
	lf	$f3, 4920($0) # 1.000000
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36311
	lf	$f4, 4880($0) # -1.000000
	j	eq_cont.36312
eq_else.36311:
	lf	$f4, 4920($0) # 1.000000
eq_cont.36312:
	fmul	$f4, $f4, $f2
	lf	$f5, 4800($0) # 6.283185
	fdiv	$f6, $f4, $f5
	floor	$f6, $f6
	fmul	$f5, $f6, $f5
	fsub	$f4, $f4, $f5
	lf	$f5, 4968($0) # 3.141593
	sltf	$30, $f4, $f5
	bne	$30, $0, eq_else.36313
	fsub	$f3, $f4, $f5
	lf	$f4, 4880($0) # -1.000000
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f3, $f6
	bne	$30, $0, eq_else.36315
	fsub	$f3, $f5, $f3
	lf	$f4, 4920($0) # 1.000000
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36317
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36318
eq_else.36317:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36318:
	j	eq_cont.36316
eq_else.36315:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36319
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36320
eq_else.36319:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36320:
eq_cont.36316:
	j	eq_cont.36314
eq_else.36313:
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f4, $f6
	bne	$30, $0, eq_else.36321
	fsub	$f3, $f5, $f4
	lf	$f4, 4880($0) # -1.000000
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36323
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36324
eq_else.36323:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36324:
	j	eq_cont.36322
eq_else.36321:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36325
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f4, $f4
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f4, $f4
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f4, $f4, $f4
	fmul	$f6, $f4, $f4
	fmul	$f4, $f4, $f6
	fmul	$f4, $f4, $f7
	fsub	$f4, $f5, $f4
	fmul	$f3, $f3, $f4
	j	eq_cont.36326
eq_else.36325:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f4, $f5, $f4
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f4, $f4
	fmul	$f8, $f4, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f4, $f5
	fmul	$f8, $f4, $f4
	fmul	$f8, $f8, $f8
	fmul	$f8, $f4, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f4, $f4
	fmul	$f8, $f6, $f6
	fmul	$f4, $f4, $f6
	fmul	$f4, $f4, $f8
	fmul	$f4, $f4, $f7
	fsub	$f4, $f5, $f4
	fmul	$f3, $f3, $f4
eq_cont.36326:
eq_cont.36322:
eq_cont.36314:
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36327
	lf	$f4, 4880($0) # -1.000000
	j	eq_cont.36328
eq_else.36327:
	lf	$f4, 4920($0) # 1.000000
eq_cont.36328:
	fmul	$f2, $f4, $f2
	lf	$f5, 4800($0) # 6.283185
	fdiv	$f6, $f2, $f5
	floor	$f6, $f6
	fmul	$f5, $f6, $f5
	fsub	$f2, $f2, $f5
	lf	$f5, 4968($0) # 3.141593
	sltf	$30, $f2, $f5
	bne	$30, $0, eq_else.36329
	fsub	$f2, $f2, $f5
	fneg	$f4, $f4
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f2, $f6
	bne	$30, $0, eq_else.36331
	fsub	$f2, $f5, $f2
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f2
	bne	$30, $0, eq_else.36333
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f2, $f2
	fmul	$f8, $f2, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f2, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f8, $f2, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	fmul	$f8, $f6, $f6
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f8
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
	j	eq_cont.36334
eq_else.36333:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f2, $f5, $f2
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f2, $f2
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f2
	fmul	$f6, $f2, $f2
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
eq_cont.36334:
	j	eq_cont.36332
eq_else.36331:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f2
	bne	$30, $0, eq_else.36335
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f2, $f2
	fmul	$f8, $f2, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f2, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f8, $f2, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	fmul	$f8, $f6, $f6
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f8
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
	j	eq_cont.36336
eq_else.36335:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f2, $f5, $f2
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f2, $f2
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f2
	fmul	$f6, $f2, $f2
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
eq_cont.36336:
eq_cont.36332:
	j	eq_cont.36330
eq_else.36329:
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f2, $f6
	bne	$30, $0, eq_else.36337
	fsub	$f2, $f5, $f2
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f2
	bne	$30, $0, eq_else.36339
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f2, $f2
	fmul	$f8, $f2, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f2, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f8, $f2, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	fmul	$f8, $f6, $f6
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f8
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
	j	eq_cont.36340
eq_else.36339:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f2, $f5, $f2
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f2, $f2
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f2
	fmul	$f6, $f2, $f2
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
eq_cont.36340:
	j	eq_cont.36338
eq_else.36337:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f2
	bne	$30, $0, eq_else.36341
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f2, $f2
	fmul	$f8, $f2, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f2, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f8, $f2, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	fmul	$f8, $f6, $f6
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f8
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
	j	eq_cont.36342
eq_else.36341:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f2, $f5, $f2
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f2, $f2
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f2, $f2
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f2
	fmul	$f6, $f2, $f2
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f7
	fsub	$f2, $f5, $f2
	fmul	$f2, $f4, $f2
eq_cont.36342:
eq_cont.36338:
eq_cont.36330:
	fmul	$f4, $f1, $f2
	lf	$f5, 4792($0) # 200.000000
	fmul	$f4, $f4, $f5
	sf	$f4, 3792($0)
	lf	$f4, 4784($0) # -200.000000
	fmul	$f4, $f0, $f4
	sf	$f4, 3800($0)
	fmul	$f4, $f1, $f3
	lf	$f5, 4792($0) # 200.000000
	fmul	$f4, $f4, $f5
	sf	$f4, 3808($0)
	sf	$f3, 3744($0)
	lf	$f4, 4888($0) # 0.000000
	sf	$f4, 3752($0)
	fneg	$f4, $f2
	sf	$f4, 3760($0)
	fneg	$f4, $f0
	fmul	$f2, $f4, $f2
	sf	$f2, 3768($0)
	fneg	$f1, $f1
	sf	$f1, 3776($0)
	fneg	$f0, $f0
	fmul	$f0, $f0, $f3
	sf	$f0, 3784($0)
	lf	$f0, 3244($0)
	lf	$f1, 3792($0)
	fsub	$f0, $f0, $f1
	sf	$f0, 3268($0)
	lf	$f0, 3252($0)
	lf	$f1, 3800($0)
	fsub	$f0, $f0, $f1
	sf	$f0, 3276($0)
	lf	$f0, 3260($0)
	lf	$f1, 3808($0)
	fsub	$f0, $f0, $f1
	sf	$f0, 3284($0)
	jr $31
rotate_quadratic_matrix.2848:
	lf	$f0, 0($5)
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36344
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36345
eq_else.36344:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36345:
	fmul	$f0, $f2, $f0
	lf	$f2, 4800($0) # 6.283185
	fdiv	$f3, $f0, $f2
	floor	$f3, $f3
	fmul	$f2, $f3, $f2
	fsub	$f0, $f0, $f2
	lf	$f2, 4968($0) # 3.141593
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.36346
	fsub	$f0, $f0, $f2
	lf	$f1, 4880($0) # -1.000000
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.36348
	fsub	$f0, $f2, $f0
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36350
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.36351
eq_else.36350:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.36351:
	j	eq_cont.36349
eq_else.36348:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36352
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.36353
eq_else.36352:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.36353:
eq_cont.36349:
	j	eq_cont.36347
eq_else.36346:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.36354
	fsub	$f0, $f2, $f0
	lf	$f1, 4880($0) # -1.000000
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36356
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.36357
eq_else.36356:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.36357:
	j	eq_cont.36355
eq_else.36354:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.36358
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.36359
eq_else.36358:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.36359:
eq_cont.36355:
eq_cont.36347:
	lf	$f1, 0($5)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36360
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36361
eq_else.36360:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36361:
	fmul	$f1, $f2, $f1
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f1, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f1, $f1, $f3
	lf	$f3, 4968($0) # 3.141593
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.36362
	fsub	$f1, $f1, $f3
	fneg	$f2, $f2
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f1, $f4
	bne	$30, $0, eq_else.36364
	fsub	$f1, $f3, $f1
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36366
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36367
eq_else.36366:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36367:
	j	eq_cont.36365
eq_else.36364:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36368
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36369
eq_else.36368:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36369:
eq_cont.36365:
	j	eq_cont.36363
eq_else.36362:
	lf	$f3, 4968($0) # 3.141593
	lf	$f4, 4960($0) # 1.570796
	sltf	$30, $f1, $f4
	bne	$30, $0, eq_else.36370
	fsub	$f1, $f3, $f1
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36372
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36373
eq_else.36372:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36373:
	j	eq_cont.36371
eq_else.36370:
	lf	$f3, 4952($0) # 0.785398
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36374
	lf	$f3, 4912($0) # 0.166667
	lf	$f4, 4904($0) # 0.008333
	lf	$f5, 4896($0) # 0.000196
	fmul	$f6, $f1, $f1
	fmul	$f6, $f1, $f6
	fmul	$f3, $f6, $f3
	fsub	$f3, $f1, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f6, $f1, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f6, $f4, $f4
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f6
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.36375
eq_else.36374:
	lf	$f3, 4960($0) # 1.570796
	fsub	$f1, $f3, $f1
	lf	$f3, 4944($0) # 0.500000
	lf	$f4, 4936($0) # 0.041664
	lf	$f5, 4928($0) # 0.001370
	lf	$f6, 4920($0) # 1.000000
	fmul	$f7, $f1, $f1
	fmul	$f3, $f7, $f3
	fsub	$f3, $f6, $f3
	fmul	$f6, $f1, $f1
	fmul	$f6, $f6, $f6
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f1
	fmul	$f4, $f1, $f1
	fmul	$f1, $f1, $f4
	fmul	$f1, $f1, $f5
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.36375:
eq_cont.36371:
eq_cont.36363:
	lf	$f2, 8($5)
	lf	$f3, 4920($0) # 1.000000
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36376
	lf	$f4, 4880($0) # -1.000000
	j	eq_cont.36377
eq_else.36376:
	lf	$f4, 4920($0) # 1.000000
eq_cont.36377:
	fmul	$f2, $f4, $f2
	lf	$f4, 4800($0) # 6.283185
	fdiv	$f5, $f2, $f4
	floor	$f5, $f5
	fmul	$f4, $f5, $f4
	fsub	$f2, $f2, $f4
	lf	$f4, 4968($0) # 3.141593
	sltf	$30, $f2, $f4
	bne	$30, $0, eq_else.36378
	fsub	$f2, $f2, $f4
	lf	$f3, 4880($0) # -1.000000
	lf	$f4, 4968($0) # 3.141593
	lf	$f5, 4960($0) # 1.570796
	sltf	$30, $f2, $f5
	bne	$30, $0, eq_else.36380
	fsub	$f2, $f4, $f2
	lf	$f3, 4920($0) # 1.000000
	lf	$f4, 4952($0) # 0.785398
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36382
	lf	$f4, 4944($0) # 0.500000
	lf	$f5, 4936($0) # 0.041664
	lf	$f6, 4928($0) # 0.001370
	lf	$f7, 4920($0) # 1.000000
	fmul	$f8, $f2, $f2
	fmul	$f4, $f8, $f4
	fsub	$f4, $f7, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f2, $f2, $f2
	fmul	$f5, $f2, $f2
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
	j	eq_cont.36383
eq_else.36382:
	lf	$f4, 4960($0) # 1.570796
	fsub	$f2, $f4, $f2
	lf	$f4, 4912($0) # 0.166667
	lf	$f5, 4904($0) # 0.008333
	lf	$f6, 4896($0) # 0.000196
	fmul	$f7, $f2, $f2
	fmul	$f7, $f2, $f7
	fmul	$f4, $f7, $f4
	fsub	$f4, $f2, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f7, $f2, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f5, $f2, $f2
	fmul	$f7, $f5, $f5
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f7
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
eq_cont.36383:
	j	eq_cont.36381
eq_else.36380:
	lf	$f4, 4952($0) # 0.785398
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36384
	lf	$f4, 4944($0) # 0.500000
	lf	$f5, 4936($0) # 0.041664
	lf	$f6, 4928($0) # 0.001370
	lf	$f7, 4920($0) # 1.000000
	fmul	$f8, $f2, $f2
	fmul	$f4, $f8, $f4
	fsub	$f4, $f7, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f2, $f2, $f2
	fmul	$f5, $f2, $f2
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
	j	eq_cont.36385
eq_else.36384:
	lf	$f4, 4960($0) # 1.570796
	fsub	$f2, $f4, $f2
	lf	$f4, 4912($0) # 0.166667
	lf	$f5, 4904($0) # 0.008333
	lf	$f6, 4896($0) # 0.000196
	fmul	$f7, $f2, $f2
	fmul	$f7, $f2, $f7
	fmul	$f4, $f7, $f4
	fsub	$f4, $f2, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f7, $f2, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f5, $f2, $f2
	fmul	$f7, $f5, $f5
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f7
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
eq_cont.36385:
eq_cont.36381:
	j	eq_cont.36379
eq_else.36378:
	lf	$f4, 4968($0) # 3.141593
	lf	$f5, 4960($0) # 1.570796
	sltf	$30, $f2, $f5
	bne	$30, $0, eq_else.36386
	fsub	$f2, $f4, $f2
	lf	$f3, 4880($0) # -1.000000
	lf	$f4, 4952($0) # 0.785398
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36388
	lf	$f4, 4944($0) # 0.500000
	lf	$f5, 4936($0) # 0.041664
	lf	$f6, 4928($0) # 0.001370
	lf	$f7, 4920($0) # 1.000000
	fmul	$f8, $f2, $f2
	fmul	$f4, $f8, $f4
	fsub	$f4, $f7, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f2, $f2, $f2
	fmul	$f5, $f2, $f2
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
	j	eq_cont.36389
eq_else.36388:
	lf	$f4, 4960($0) # 1.570796
	fsub	$f2, $f4, $f2
	lf	$f4, 4912($0) # 0.166667
	lf	$f5, 4904($0) # 0.008333
	lf	$f6, 4896($0) # 0.000196
	fmul	$f7, $f2, $f2
	fmul	$f7, $f2, $f7
	fmul	$f4, $f7, $f4
	fsub	$f4, $f2, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f7, $f2, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f5, $f2, $f2
	fmul	$f7, $f5, $f5
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f7
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
eq_cont.36389:
	j	eq_cont.36387
eq_else.36386:
	lf	$f4, 4952($0) # 0.785398
	sltf	$30, $f4, $f2
	bne	$30, $0, eq_else.36390
	lf	$f4, 4944($0) # 0.500000
	lf	$f5, 4936($0) # 0.041664
	lf	$f6, 4928($0) # 0.001370
	lf	$f7, 4920($0) # 1.000000
	fmul	$f8, $f2, $f2
	fmul	$f4, $f8, $f4
	fsub	$f4, $f7, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f2, $f2, $f2
	fmul	$f5, $f2, $f2
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
	j	eq_cont.36391
eq_else.36390:
	lf	$f4, 4960($0) # 1.570796
	fsub	$f2, $f4, $f2
	lf	$f4, 4912($0) # 0.166667
	lf	$f5, 4904($0) # 0.008333
	lf	$f6, 4896($0) # 0.000196
	fmul	$f7, $f2, $f2
	fmul	$f7, $f2, $f7
	fmul	$f4, $f7, $f4
	fsub	$f4, $f2, $f4
	fmul	$f7, $f2, $f2
	fmul	$f7, $f7, $f7
	fmul	$f7, $f2, $f7
	fmul	$f5, $f7, $f5
	fadd	$f4, $f4, $f5
	fmul	$f5, $f2, $f2
	fmul	$f7, $f5, $f5
	fmul	$f2, $f2, $f5
	fmul	$f2, $f2, $f7
	fmul	$f2, $f2, $f6
	fsub	$f2, $f4, $f2
	fmul	$f2, $f3, $f2
eq_cont.36391:
eq_cont.36387:
eq_cont.36379:
	lf	$f3, 8($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.36392
	lf	$f4, 4880($0) # -1.000000
	j	eq_cont.36393
eq_else.36392:
	lf	$f4, 4920($0) # 1.000000
eq_cont.36393:
	fmul	$f3, $f4, $f3
	lf	$f5, 4800($0) # 6.283185
	fdiv	$f6, $f3, $f5
	floor	$f6, $f6
	fmul	$f5, $f6, $f5
	fsub	$f3, $f3, $f5
	lf	$f5, 4968($0) # 3.141593
	sltf	$30, $f3, $f5
	bne	$30, $0, eq_else.36394
	fsub	$f3, $f3, $f5
	fneg	$f4, $f4
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f3, $f6
	bne	$30, $0, eq_else.36396
	fsub	$f3, $f5, $f3
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36398
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36399
eq_else.36398:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36399:
	j	eq_cont.36397
eq_else.36396:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36400
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36401
eq_else.36400:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36401:
eq_cont.36397:
	j	eq_cont.36395
eq_else.36394:
	lf	$f5, 4968($0) # 3.141593
	lf	$f6, 4960($0) # 1.570796
	sltf	$30, $f3, $f6
	bne	$30, $0, eq_else.36402
	fsub	$f3, $f5, $f3
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36404
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36405
eq_else.36404:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36405:
	j	eq_cont.36403
eq_else.36402:
	lf	$f5, 4952($0) # 0.785398
	sltf	$30, $f5, $f3
	bne	$30, $0, eq_else.36406
	lf	$f5, 4912($0) # 0.166667
	lf	$f6, 4904($0) # 0.008333
	lf	$f7, 4896($0) # 0.000196
	fmul	$f8, $f3, $f3
	fmul	$f8, $f3, $f8
	fmul	$f5, $f8, $f5
	fsub	$f5, $f3, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f8, $f3, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f6, $f3, $f3
	fmul	$f8, $f6, $f6
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f8
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
	j	eq_cont.36407
eq_else.36406:
	lf	$f5, 4960($0) # 1.570796
	fsub	$f3, $f5, $f3
	lf	$f5, 4944($0) # 0.500000
	lf	$f6, 4936($0) # 0.041664
	lf	$f7, 4928($0) # 0.001370
	lf	$f8, 4920($0) # 1.000000
	fmul	$f9, $f3, $f3
	fmul	$f5, $f9, $f5
	fsub	$f5, $f8, $f5
	fmul	$f8, $f3, $f3
	fmul	$f8, $f8, $f8
	fmul	$f6, $f8, $f6
	fadd	$f5, $f5, $f6
	fmul	$f3, $f3, $f3
	fmul	$f6, $f3, $f3
	fmul	$f3, $f3, $f6
	fmul	$f3, $f3, $f7
	fsub	$f3, $f5, $f3
	fmul	$f3, $f4, $f3
eq_cont.36407:
eq_cont.36403:
eq_cont.36395:
	lf	$f4, 16($5)
	lf	$f5, 4920($0) # 1.000000
	lf	$f6, 4888($0) # 0.000000
	sltf	$30, $f6, $f4
	bne	$30, $0, eq_else.36408
	lf	$f6, 4880($0) # -1.000000
	j	eq_cont.36409
eq_else.36408:
	lf	$f6, 4920($0) # 1.000000
eq_cont.36409:
	fmul	$f4, $f6, $f4
	lf	$f6, 4800($0) # 6.283185
	fdiv	$f7, $f4, $f6
	floor	$f7, $f7
	fmul	$f6, $f7, $f6
	fsub	$f4, $f4, $f6
	lf	$f6, 4968($0) # 3.141593
	sltf	$30, $f4, $f6
	bne	$30, $0, eq_else.36410
	fsub	$f4, $f4, $f6
	lf	$f5, 4880($0) # -1.000000
	lf	$f6, 4968($0) # 3.141593
	lf	$f7, 4960($0) # 1.570796
	sltf	$30, $f4, $f7
	bne	$30, $0, eq_else.36412
	fsub	$f4, $f6, $f4
	lf	$f5, 4920($0) # 1.000000
	lf	$f6, 4952($0) # 0.785398
	sltf	$30, $f6, $f4
	bne	$30, $0, eq_else.36414
	lf	$f6, 4944($0) # 0.500000
	lf	$f7, 4936($0) # 0.041664
	lf	$f8, 4928($0) # 0.001370
	lf	$f9, 4920($0) # 1.000000
	fmul	$f10, $f4, $f4
	fmul	$f6, $f10, $f6
	fsub	$f6, $f9, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f4
	fmul	$f7, $f4, $f4
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
	j	eq_cont.36415
eq_else.36414:
	lf	$f6, 4960($0) # 1.570796
	fsub	$f4, $f6, $f4
	lf	$f6, 4912($0) # 0.166667
	lf	$f7, 4904($0) # 0.008333
	lf	$f8, 4896($0) # 0.000196
	fmul	$f9, $f4, $f4
	fmul	$f9, $f4, $f9
	fmul	$f6, $f9, $f6
	fsub	$f6, $f4, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f9, $f4, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	fmul	$f9, $f7, $f7
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f9
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
eq_cont.36415:
	j	eq_cont.36413
eq_else.36412:
	lf	$f6, 4952($0) # 0.785398
	sltf	$30, $f6, $f4
	bne	$30, $0, eq_else.36416
	lf	$f6, 4944($0) # 0.500000
	lf	$f7, 4936($0) # 0.041664
	lf	$f8, 4928($0) # 0.001370
	lf	$f9, 4920($0) # 1.000000
	fmul	$f10, $f4, $f4
	fmul	$f6, $f10, $f6
	fsub	$f6, $f9, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f4
	fmul	$f7, $f4, $f4
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
	j	eq_cont.36417
eq_else.36416:
	lf	$f6, 4960($0) # 1.570796
	fsub	$f4, $f6, $f4
	lf	$f6, 4912($0) # 0.166667
	lf	$f7, 4904($0) # 0.008333
	lf	$f8, 4896($0) # 0.000196
	fmul	$f9, $f4, $f4
	fmul	$f9, $f4, $f9
	fmul	$f6, $f9, $f6
	fsub	$f6, $f4, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f9, $f4, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	fmul	$f9, $f7, $f7
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f9
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
eq_cont.36417:
eq_cont.36413:
	j	eq_cont.36411
eq_else.36410:
	lf	$f6, 4968($0) # 3.141593
	lf	$f7, 4960($0) # 1.570796
	sltf	$30, $f4, $f7
	bne	$30, $0, eq_else.36418
	fsub	$f4, $f6, $f4
	lf	$f5, 4880($0) # -1.000000
	lf	$f6, 4952($0) # 0.785398
	sltf	$30, $f6, $f4
	bne	$30, $0, eq_else.36420
	lf	$f6, 4944($0) # 0.500000
	lf	$f7, 4936($0) # 0.041664
	lf	$f8, 4928($0) # 0.001370
	lf	$f9, 4920($0) # 1.000000
	fmul	$f10, $f4, $f4
	fmul	$f6, $f10, $f6
	fsub	$f6, $f9, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f4
	fmul	$f7, $f4, $f4
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
	j	eq_cont.36421
eq_else.36420:
	lf	$f6, 4960($0) # 1.570796
	fsub	$f4, $f6, $f4
	lf	$f6, 4912($0) # 0.166667
	lf	$f7, 4904($0) # 0.008333
	lf	$f8, 4896($0) # 0.000196
	fmul	$f9, $f4, $f4
	fmul	$f9, $f4, $f9
	fmul	$f6, $f9, $f6
	fsub	$f6, $f4, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f9, $f4, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	fmul	$f9, $f7, $f7
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f9
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
eq_cont.36421:
	j	eq_cont.36419
eq_else.36418:
	lf	$f6, 4952($0) # 0.785398
	sltf	$30, $f6, $f4
	bne	$30, $0, eq_else.36422
	lf	$f6, 4944($0) # 0.500000
	lf	$f7, 4936($0) # 0.041664
	lf	$f8, 4928($0) # 0.001370
	lf	$f9, 4920($0) # 1.000000
	fmul	$f10, $f4, $f4
	fmul	$f6, $f10, $f6
	fsub	$f6, $f9, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f4
	fmul	$f7, $f4, $f4
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
	j	eq_cont.36423
eq_else.36422:
	lf	$f6, 4960($0) # 1.570796
	fsub	$f4, $f6, $f4
	lf	$f6, 4912($0) # 0.166667
	lf	$f7, 4904($0) # 0.008333
	lf	$f8, 4896($0) # 0.000196
	fmul	$f9, $f4, $f4
	fmul	$f9, $f4, $f9
	fmul	$f6, $f9, $f6
	fsub	$f6, $f4, $f6
	fmul	$f9, $f4, $f4
	fmul	$f9, $f9, $f9
	fmul	$f9, $f4, $f9
	fmul	$f7, $f9, $f7
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	fmul	$f9, $f7, $f7
	fmul	$f4, $f4, $f7
	fmul	$f4, $f4, $f9
	fmul	$f4, $f4, $f8
	fsub	$f4, $f6, $f4
	fmul	$f4, $f5, $f4
eq_cont.36423:
eq_cont.36419:
eq_cont.36411:
	lf	$f5, 16($5)
	lf	$f6, 4888($0) # 0.000000
	sltf	$30, $f6, $f5
	bne	$30, $0, eq_else.36424
	lf	$f6, 4880($0) # -1.000000
	j	eq_cont.36425
eq_else.36424:
	lf	$f6, 4920($0) # 1.000000
eq_cont.36425:
	fmul	$f5, $f6, $f5
	lf	$f7, 4800($0) # 6.283185
	fdiv	$f8, $f5, $f7
	floor	$f8, $f8
	fmul	$f7, $f8, $f7
	fsub	$f5, $f5, $f7
	lf	$f7, 4968($0) # 3.141593
	sltf	$30, $f5, $f7
	bne	$30, $0, eq_else.36426
	fsub	$f5, $f5, $f7
	fneg	$f6, $f6
	lf	$f7, 4968($0) # 3.141593
	lf	$f8, 4960($0) # 1.570796
	sltf	$30, $f5, $f8
	bne	$30, $0, eq_else.36428
	fsub	$f5, $f7, $f5
	lf	$f7, 4952($0) # 0.785398
	sltf	$30, $f7, $f5
	bne	$30, $0, eq_else.36430
	lf	$f7, 4912($0) # 0.166667
	lf	$f8, 4904($0) # 0.008333
	lf	$f9, 4896($0) # 0.000196
	fmul	$f10, $f5, $f5
	fmul	$f10, $f5, $f10
	fmul	$f7, $f10, $f7
	fsub	$f7, $f5, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f10, $f5, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f8, $f5, $f5
	fmul	$f10, $f8, $f8
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f10
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
	j	eq_cont.36431
eq_else.36430:
	lf	$f7, 4960($0) # 1.570796
	fsub	$f5, $f7, $f5
	lf	$f7, 4944($0) # 0.500000
	lf	$f8, 4936($0) # 0.041664
	lf	$f9, 4928($0) # 0.001370
	lf	$f10, 4920($0) # 1.000000
	fmul	$f11, $f5, $f5
	fmul	$f7, $f11, $f7
	fsub	$f7, $f10, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f5, $f5, $f5
	fmul	$f8, $f5, $f5
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
eq_cont.36431:
	j	eq_cont.36429
eq_else.36428:
	lf	$f7, 4952($0) # 0.785398
	sltf	$30, $f7, $f5
	bne	$30, $0, eq_else.36432
	lf	$f7, 4912($0) # 0.166667
	lf	$f8, 4904($0) # 0.008333
	lf	$f9, 4896($0) # 0.000196
	fmul	$f10, $f5, $f5
	fmul	$f10, $f5, $f10
	fmul	$f7, $f10, $f7
	fsub	$f7, $f5, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f10, $f5, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f8, $f5, $f5
	fmul	$f10, $f8, $f8
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f10
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
	j	eq_cont.36433
eq_else.36432:
	lf	$f7, 4960($0) # 1.570796
	fsub	$f5, $f7, $f5
	lf	$f7, 4944($0) # 0.500000
	lf	$f8, 4936($0) # 0.041664
	lf	$f9, 4928($0) # 0.001370
	lf	$f10, 4920($0) # 1.000000
	fmul	$f11, $f5, $f5
	fmul	$f7, $f11, $f7
	fsub	$f7, $f10, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f5, $f5, $f5
	fmul	$f8, $f5, $f5
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
eq_cont.36433:
eq_cont.36429:
	j	eq_cont.36427
eq_else.36426:
	lf	$f7, 4968($0) # 3.141593
	lf	$f8, 4960($0) # 1.570796
	sltf	$30, $f5, $f8
	bne	$30, $0, eq_else.36434
	fsub	$f5, $f7, $f5
	lf	$f7, 4952($0) # 0.785398
	sltf	$30, $f7, $f5
	bne	$30, $0, eq_else.36436
	lf	$f7, 4912($0) # 0.166667
	lf	$f8, 4904($0) # 0.008333
	lf	$f9, 4896($0) # 0.000196
	fmul	$f10, $f5, $f5
	fmul	$f10, $f5, $f10
	fmul	$f7, $f10, $f7
	fsub	$f7, $f5, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f10, $f5, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f8, $f5, $f5
	fmul	$f10, $f8, $f8
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f10
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
	j	eq_cont.36437
eq_else.36436:
	lf	$f7, 4960($0) # 1.570796
	fsub	$f5, $f7, $f5
	lf	$f7, 4944($0) # 0.500000
	lf	$f8, 4936($0) # 0.041664
	lf	$f9, 4928($0) # 0.001370
	lf	$f10, 4920($0) # 1.000000
	fmul	$f11, $f5, $f5
	fmul	$f7, $f11, $f7
	fsub	$f7, $f10, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f5, $f5, $f5
	fmul	$f8, $f5, $f5
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
eq_cont.36437:
	j	eq_cont.36435
eq_else.36434:
	lf	$f7, 4952($0) # 0.785398
	sltf	$30, $f7, $f5
	bne	$30, $0, eq_else.36438
	lf	$f7, 4912($0) # 0.166667
	lf	$f8, 4904($0) # 0.008333
	lf	$f9, 4896($0) # 0.000196
	fmul	$f10, $f5, $f5
	fmul	$f10, $f5, $f10
	fmul	$f7, $f10, $f7
	fsub	$f7, $f5, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f10, $f5, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f8, $f5, $f5
	fmul	$f10, $f8, $f8
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f10
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
	j	eq_cont.36439
eq_else.36438:
	lf	$f7, 4960($0) # 1.570796
	fsub	$f5, $f7, $f5
	lf	$f7, 4944($0) # 0.500000
	lf	$f8, 4936($0) # 0.041664
	lf	$f9, 4928($0) # 0.001370
	lf	$f10, 4920($0) # 1.000000
	fmul	$f11, $f5, $f5
	fmul	$f7, $f11, $f7
	fsub	$f7, $f10, $f7
	fmul	$f10, $f5, $f5
	fmul	$f10, $f10, $f10
	fmul	$f8, $f10, $f8
	fadd	$f7, $f7, $f8
	fmul	$f5, $f5, $f5
	fmul	$f8, $f5, $f5
	fmul	$f5, $f5, $f8
	fmul	$f5, $f5, $f9
	fsub	$f5, $f7, $f5
	fmul	$f5, $f6, $f5
eq_cont.36439:
eq_cont.36435:
eq_cont.36427:
	fmul	$f6, $f2, $f4
	fmul	$f7, $f1, $f3
	fmul	$f7, $f7, $f4
	fmul	$f8, $f0, $f5
	fsub	$f7, $f7, $f8
	fmul	$f8, $f0, $f3
	fmul	$f8, $f8, $f4
	fmul	$f9, $f1, $f5
	fadd	$f8, $f8, $f9
	fmul	$f9, $f2, $f5
	fmul	$f10, $f1, $f3
	fmul	$f10, $f10, $f5
	fmul	$f11, $f0, $f4
	fadd	$f10, $f10, $f11
	fmul	$f11, $f0, $f3
	fmul	$f5, $f11, $f5
	fmul	$f4, $f1, $f4
	fsub	$f4, $f5, $f4
	fneg	$f3, $f3
	fmul	$f1, $f1, $f2
	fmul	$f0, $f0, $f2
	lf	$f2, 0($2)
	lf	$f5, 8($2)
	lf	$f11, 16($2)
	fmul	$f12, $f6, $f6
	fmul	$f12, $f2, $f12
	fmul	$f13, $f9, $f9
	fmul	$f13, $f5, $f13
	fadd	$f12, $f12, $f13
	fmul	$f13, $f3, $f3
	fmul	$f13, $f11, $f13
	fadd	$f12, $f12, $f13
	sf	$f12, 0($2)
	fmul	$f12, $f7, $f7
	fmul	$f12, $f2, $f12
	fmul	$f13, $f10, $f10
	fmul	$f13, $f5, $f13
	fadd	$f12, $f12, $f13
	fmul	$f13, $f1, $f1
	fmul	$f13, $f11, $f13
	fadd	$f12, $f12, $f13
	sf	$f12, 8($2)
	fmul	$f12, $f8, $f8
	fmul	$f12, $f2, $f12
	fmul	$f13, $f4, $f4
	fmul	$f13, $f5, $f13
	fadd	$f12, $f12, $f13
	fmul	$f13, $f0, $f0
	fmul	$f13, $f11, $f13
	fadd	$f12, $f12, $f13
	sf	$f12, 16($2)
	lf	$f12, 4776($0) # 2.000000
	fmul	$f13, $f2, $f7
	fmul	$f13, $f13, $f8
	fmul	$f14, $f5, $f10
	fmul	$f14, $f14, $f4
	fadd	$f13, $f13, $f14
	fmul	$f14, $f11, $f1
	fmul	$f14, $f14, $f0
	fadd	$f13, $f13, $f14
	fmul	$f12, $f12, $f13
	sf	$f12, 0($5)
	lf	$f12, 4776($0) # 2.000000
	fmul	$f13, $f2, $f6
	fmul	$f8, $f13, $f8
	fmul	$f13, $f5, $f9
	fmul	$f4, $f13, $f4
	fadd	$f4, $f8, $f4
	fmul	$f8, $f11, $f3
	fmul	$f0, $f8, $f0
	fadd	$f0, $f4, $f0
	fmul	$f0, $f12, $f0
	sf	$f0, 8($5)
	lf	$f0, 4776($0) # 2.000000
	fmul	$f2, $f2, $f6
	fmul	$f2, $f2, $f7
	fmul	$f4, $f5, $f9
	fmul	$f4, $f4, $f10
	fadd	$f2, $f2, $f4
	fmul	$f3, $f11, $f3
	fmul	$f1, $f3, $f1
	fadd	$f1, $f2, $f1
	fmul	$f0, $f0, $f1
	sf	$f0, 16($5)
	jr $31
read_nth_object.2851:
	in	$5
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36441
	addi	$2, $0, 0
	jr $31
eq_else.36441:
	in	$6
	in	$7
	in	$8
	addi	$9, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 0($3)
	sw	$5, 4($3)
	sw	$7, 8($3)
	sw	$6, 12($3)
	sw	$8, 16($3)
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	inf	$f0
	sf	$f0, 0($2)
	inf	$f0
	sf	$f0, 8($2)
	inf	$f0
	sf	$f0, 16($2)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 20($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	inf	$f0
	sf	$f0, 0($2)
	inf	$f0
	sf	$f0, 8($2)
	inf	$f0
	sf	$f0, 16($2)
	inf	$f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.36442
	addi	$5, $0, 0
	j	eq_cont.36443
eq_else.36442:
	addi	$5, $0, 1
eq_cont.36443:
	addi	$6, $0, 2
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 24($3)
	sw	$5, 28($3)
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	inf	$f0
	sf	$f0, 0($2)
	inf	$f0
	sf	$f0, 8($2)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 32($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	inf	$f0
	sf	$f0, 0($2)
	inf	$f0
	sf	$f0, 8($2)
	inf	$f0
	sf	$f0, 16($2)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 36($3)
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 16($3)
	bne	$5, $0, eq_else.36444
	j	eq_cont.36445
eq_else.36444:
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	sf	$f0, 0($2)
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	sf	$f0, 8($2)
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	sf	$f0, 16($2)
eq_cont.36445:
	lw	$6, 12($3)
	addi	$30, $0, 2
	bne	$6, $30, eq_else.36446
	addi	$7, $0, 1
	j	eq_cont.36447
eq_else.36446:
	lw	$7, 28($3)
eq_cont.36447:
	addi	$8, $0, 4
	lf	$f0, 4888($0) # 0.000000
	sw	$7, 40($3)
	sw	$2, 44($3)
	mov	$2, $8
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $4
	addi	$4, $4, 48
	sw	$2, 40($5)
	lw	$2, 44($3)
	sw	$2, 36($5)
	lw	$6, 36($3)
	sw	$6, 32($5)
	lw	$6, 32($3)
	sw	$6, 28($5)
	lw	$6, 40($3)
	sw	$6, 24($5)
	lw	$6, 24($3)
	sw	$6, 20($5)
	lw	$6, 20($3)
	sw	$6, 16($5)
	lw	$7, 16($3)
	sw	$7, 12($5)
	lw	$8, 8($3)
	sw	$8, 8($5)
	lw	$8, 12($3)
	sw	$8, 4($5)
	lw	$9, 4($3)
	sw	$9, 0($5)
	lw	$9, 0($3)
	slli	$9, $9, 2
	addi	$9, $9, 3004
	sw	$5, 0($9)
	addi	$30, $0, 3
	bne	$8, $30, eq_else.36448
	lf	$f0, 0($6)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36450
	addi	$5, $0, 1
	j	eq_cont.36451
eq_else.36450:
	addi	$5, $0, 0
eq_cont.36451:
	bne	$5, $0, eq_else.36452
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36454
	addi	$5, $0, 1
	j	eq_cont.36455
eq_else.36454:
	addi	$5, $0, 0
eq_cont.36455:
	bne	$5, $0, eq_else.36456
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36458
	addi	$5, $0, 0
	j	eq_cont.36459
eq_else.36458:
	addi	$5, $0, 1
eq_cont.36459:
	bne	$5, $0, eq_else.36460
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36461
eq_else.36460:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36461:
	j	eq_cont.36457
eq_else.36456:
	lf	$f1, 4888($0) # 0.000000
eq_cont.36457:
	fmul	$f0, $f0, $f0
	fdiv	$f0, $f1, $f0
	j	eq_cont.36453
eq_else.36452:
	lf	$f0, 4888($0) # 0.000000
eq_cont.36453:
	sf	$f0, 0($6)
	lf	$f0, 8($6)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36462
	addi	$5, $0, 1
	j	eq_cont.36463
eq_else.36462:
	addi	$5, $0, 0
eq_cont.36463:
	bne	$5, $0, eq_else.36464
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36466
	addi	$5, $0, 1
	j	eq_cont.36467
eq_else.36466:
	addi	$5, $0, 0
eq_cont.36467:
	bne	$5, $0, eq_else.36468
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36470
	addi	$5, $0, 0
	j	eq_cont.36471
eq_else.36470:
	addi	$5, $0, 1
eq_cont.36471:
	bne	$5, $0, eq_else.36472
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36473
eq_else.36472:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36473:
	j	eq_cont.36469
eq_else.36468:
	lf	$f1, 4888($0) # 0.000000
eq_cont.36469:
	fmul	$f0, $f0, $f0
	fdiv	$f0, $f1, $f0
	j	eq_cont.36465
eq_else.36464:
	lf	$f0, 4888($0) # 0.000000
eq_cont.36465:
	sf	$f0, 8($6)
	lf	$f0, 16($6)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36474
	addi	$5, $0, 1
	j	eq_cont.36475
eq_else.36474:
	addi	$5, $0, 0
eq_cont.36475:
	bne	$5, $0, eq_else.36476
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36478
	addi	$5, $0, 1
	j	eq_cont.36479
eq_else.36478:
	addi	$5, $0, 0
eq_cont.36479:
	bne	$5, $0, eq_else.36480
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36482
	addi	$5, $0, 0
	j	eq_cont.36483
eq_else.36482:
	addi	$5, $0, 1
eq_cont.36483:
	bne	$5, $0, eq_else.36484
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36485
eq_else.36484:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36485:
	j	eq_cont.36481
eq_else.36480:
	lf	$f1, 4888($0) # 0.000000
eq_cont.36481:
	fmul	$f0, $f0, $f0
	fdiv	$f0, $f1, $f0
	j	eq_cont.36477
eq_else.36476:
	lf	$f0, 4888($0) # 0.000000
eq_cont.36477:
	sf	$f0, 16($6)
	j	eq_cont.36449
eq_else.36448:
	addi	$30, $0, 2
	bne	$8, $30, eq_else.36486
	lw	$5, 28($3)
	bne	$5, $0, eq_else.36488
	addi	$5, $0, 1
	j	eq_cont.36489
eq_else.36488:
	addi	$5, $0, 0
eq_cont.36489:
	lf	$f0, 0($6)
	fmul	$f0, $f0, $f0
	lf	$f1, 8($6)
	fmul	$f1, $f1, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($6)
	fmul	$f1, $f1, $f1
	fadd	$f0, $f0, $f1
	sqrt	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36490
	addi	$8, $0, 1
	j	eq_cont.36491
eq_else.36490:
	addi	$8, $0, 0
eq_cont.36491:
	bne	$8, $0, eq_else.36492
	bne	$5, $0, eq_else.36494
	lf	$f1, 4920($0) # 1.000000
	fdiv	$f0, $f1, $f0
	j	eq_cont.36495
eq_else.36494:
	lf	$f1, 4880($0) # -1.000000
	fdiv	$f0, $f1, $f0
eq_cont.36495:
	j	eq_cont.36493
eq_else.36492:
	lf	$f0, 4920($0) # 1.000000
eq_cont.36493:
	lf	$f1, 0($6)
	fmul	$f1, $f1, $f0
	sf	$f1, 0($6)
	lf	$f1, 8($6)
	fmul	$f1, $f1, $f0
	sf	$f1, 8($6)
	lf	$f1, 16($6)
	fmul	$f0, $f1, $f0
	sf	$f0, 16($6)
	j	eq_cont.36487
eq_else.36486:
eq_cont.36487:
eq_cont.36449:
	bne	$7, $0, eq_else.36496
	j	eq_cont.36497
eq_else.36496:
	mov	$5, $2
	mov	$2, $6
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	rotate_quadratic_matrix.2848
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.36497:
	addi	$2, $0, 1
	jr $31
read_object.2853:
	addi	$30, $0, 60
	bge	$2, $30, ge.36498
	sw	$2, 0($3)
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	read_nth_object.2851
	addi	$3, $3, -8
	lw	$31, 4($3)
	bne	$2, $0, eq_else.36499
	addi	$2, $0, 3000
	lw	$5, 0($3)
	sw	$5, 0($2)
	jr $31
eq_else.36499:
	lw	$2, 0($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36501
	sw	$2, 4($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	read_nth_object.2851
	addi	$3, $3, -16
	lw	$31, 12($3)
	bne	$2, $0, eq_else.36502
	addi	$2, $0, 3000
	lw	$5, 4($3)
	sw	$5, 0($2)
	jr $31
eq_else.36502:
	lw	$2, 4($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36504
	sw	$2, 8($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	read_nth_object.2851
	addi	$3, $3, -16
	lw	$31, 12($3)
	bne	$2, $0, eq_else.36505
	addi	$2, $0, 3000
	lw	$5, 8($3)
	sw	$5, 0($2)
	jr $31
eq_else.36505:
	lw	$2, 8($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36507
	sw	$2, 12($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	read_nth_object.2851
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.36508
	addi	$2, $0, 3000
	lw	$5, 12($3)
	sw	$5, 0($2)
	jr $31
eq_else.36508:
	lw	$2, 12($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36510
	sw	$2, 16($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	read_nth_object.2851
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.36511
	addi	$2, $0, 3000
	lw	$5, 16($3)
	sw	$5, 0($2)
	jr $31
eq_else.36511:
	lw	$2, 16($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36513
	sw	$2, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	read_nth_object.2851
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.36514
	addi	$2, $0, 3000
	lw	$5, 20($3)
	sw	$5, 0($2)
	jr $31
eq_else.36514:
	lw	$2, 20($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36516
	sw	$2, 24($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	read_nth_object.2851
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.36517
	addi	$2, $0, 3000
	lw	$5, 24($3)
	sw	$5, 0($2)
	jr $31
eq_else.36517:
	lw	$2, 24($3)
	addi	$2, $2, 1
	addi	$30, $0, 60
	bge	$2, $30, ge.36519
	sw	$2, 28($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	read_nth_object.2851
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36520
	addi	$2, $0, 3000
	lw	$5, 28($3)
	sw	$5, 0($2)
	jr $31
eq_else.36520:
	lw	$2, 28($3)
	addi	$2, $2, 1
	j	read_object.2853
ge.36519:
	jr $31
ge.36516:
	jr $31
ge.36513:
	jr $31
ge.36510:
	jr $31
ge.36507:
	jr $31
ge.36504:
	jr $31
ge.36501:
	jr $31
ge.36498:
	jr $31
read_net_item.2857:
	in	$5
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36530
	addi	$2, $2, 1
	addi	$5, $0, -1
	j	min_caml_create_array
eq_else.36530:
	addi	$6, $2, 1
	in	$7
	sw	$5, 0($3)
	sw	$2, 4($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36531
	addi	$6, $6, 1
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36532
eq_else.36531:
	addi	$8, $6, 1
	in	$9
	sw	$7, 8($3)
	sw	$6, 12($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36533
	addi	$8, $8, 1
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36534
eq_else.36533:
	addi	$10, $8, 1
	in	$11
	sw	$9, 16($3)
	sw	$8, 20($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.36535
	addi	$10, $10, 1
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36536
eq_else.36535:
	addi	$12, $10, 1
	in	$13
	sw	$11, 24($3)
	sw	$10, 28($3)
	addi	$30, $0, -1
	bne	$13, $30, eq_else.36537
	addi	$12, $12, 1
	addi	$13, $0, -1
	mov	$5, $13
	mov	$2, $12
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.36538
eq_else.36537:
	addi	$14, $12, 1
	in	$15
	sw	$13, 32($3)
	sw	$12, 36($3)
	addi	$30, $0, -1
	bne	$15, $30, eq_else.36539
	addi	$14, $14, 1
	addi	$15, $0, -1
	mov	$5, $15
	mov	$2, $14
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.36540
eq_else.36539:
	addi	$16, $14, 1
	in	$17
	sw	$15, 40($3)
	sw	$14, 44($3)
	addi	$30, $0, -1
	bne	$17, $30, eq_else.36541
	addi	$16, $16, 1
	addi	$17, $0, -1
	mov	$5, $17
	mov	$2, $16
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36542
eq_else.36541:
	addi	$18, $16, 1
	in	$19
	sw	$17, 48($3)
	sw	$16, 52($3)
	addi	$30, $0, -1
	bne	$19, $30, eq_else.36543
	addi	$18, $18, 1
	addi	$19, $0, -1
	mov	$5, $19
	mov	$2, $18
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36544
eq_else.36543:
	addi	$20, $18, 1
	sw	$19, 56($3)
	sw	$18, 60($3)
	mov	$2, $20
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	read_net_item.2857
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 60($3)
	slli	$5, $5, 2
	lw	$6, 56($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36544:
	lw	$5, 52($3)
	slli	$5, $5, 2
	lw	$6, 48($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36542:
	lw	$5, 44($3)
	slli	$5, $5, 2
	lw	$6, 40($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36540:
	lw	$5, 36($3)
	slli	$5, $5, 2
	lw	$6, 32($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36538:
	lw	$5, 28($3)
	slli	$5, $5, 2
	lw	$6, 24($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36536:
	lw	$5, 20($3)
	slli	$5, $5, 2
	lw	$6, 16($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36534:
	lw	$5, 12($3)
	slli	$5, $5, 2
	lw	$6, 8($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36532:
	lw	$5, 4($3)
	slli	$5, $5, 2
	lw	$6, 0($3)
	add $30, $2, $5
	sw	$6, 0($30)
	jr $31
read_or_network.2859:
	in	$5
	sw	$2, 0($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36545
	addi	$5, $0, 1
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	mov	$5, $2
	j	eq_cont.36546
eq_else.36545:
	in	$6
	sw	$5, 4($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36547
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36548
eq_else.36547:
	in	$7
	sw	$6, 8($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36549
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36550
eq_else.36549:
	in	$8
	sw	$7, 12($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36551
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36552
eq_else.36551:
	in	$9
	sw	$8, 16($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36553
	addi	$9, $0, 5
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36554
eq_else.36553:
	in	$10
	sw	$9, 20($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36555
	addi	$10, $0, 6
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36556
eq_else.36555:
	in	$11
	sw	$10, 24($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.36557
	addi	$11, $0, 7
	addi	$12, $0, -1
	mov	$5, $12
	mov	$2, $11
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36558
eq_else.36557:
	addi	$12, $0, 7
	sw	$11, 28($3)
	mov	$2, $12
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	read_net_item.2857
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$5, 24($2)
eq_cont.36558:
	lw	$5, 24($3)
	sw	$5, 20($2)
eq_cont.36556:
	lw	$5, 20($3)
	sw	$5, 16($2)
eq_cont.36554:
	lw	$5, 16($3)
	sw	$5, 12($2)
eq_cont.36552:
	lw	$5, 12($3)
	sw	$5, 8($2)
eq_cont.36550:
	lw	$5, 8($3)
	sw	$5, 4($2)
eq_cont.36548:
	lw	$5, 4($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36546:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36559
	lw	$2, 0($3)
	addi	$2, $2, 1
	j	min_caml_create_array
eq_else.36559:
	lw	$2, 0($3)
	addi	$6, $2, 1
	in	$7
	sw	$5, 32($3)
	sw	$6, 36($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36560
	addi	$7, $0, 1
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $2
	j	eq_cont.36561
eq_else.36560:
	in	$8
	sw	$7, 40($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36562
	addi	$8, $0, 2
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.36563
eq_else.36562:
	in	$9
	sw	$8, 44($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36564
	addi	$9, $0, 3
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36565
eq_else.36564:
	in	$10
	sw	$9, 48($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36566
	addi	$10, $0, 4
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36567
eq_else.36566:
	in	$11
	sw	$10, 52($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.36568
	addi	$11, $0, 5
	addi	$12, $0, -1
	mov	$5, $12
	mov	$2, $11
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36569
eq_else.36568:
	in	$12
	sw	$11, 56($3)
	addi	$30, $0, -1
	bne	$12, $30, eq_else.36570
	addi	$12, $0, 6
	addi	$13, $0, -1
	mov	$5, $13
	mov	$2, $12
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36571
eq_else.36570:
	addi	$13, $0, 6
	sw	$12, 60($3)
	mov	$2, $13
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	read_net_item.2857
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 60($3)
	sw	$5, 20($2)
eq_cont.36571:
	lw	$5, 56($3)
	sw	$5, 16($2)
eq_cont.36569:
	lw	$5, 52($3)
	sw	$5, 12($2)
eq_cont.36567:
	lw	$5, 48($3)
	sw	$5, 8($2)
eq_cont.36565:
	lw	$5, 44($3)
	sw	$5, 4($2)
eq_cont.36563:
	lw	$5, 40($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36561:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36572
	lw	$2, 36($3)
	addi	$2, $2, 1
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.36573
eq_else.36572:
	lw	$2, 36($3)
	addi	$6, $2, 1
	in	$7
	sw	$5, 64($3)
	sw	$6, 68($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36574
	addi	$7, $0, 1
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $2
	j	eq_cont.36575
eq_else.36574:
	in	$8
	sw	$7, 72($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36576
	addi	$8, $0, 2
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.36577
eq_else.36576:
	in	$9
	sw	$8, 76($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36578
	addi	$9, $0, 3
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	j	eq_cont.36579
eq_else.36578:
	in	$10
	sw	$9, 80($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36580
	addi	$10, $0, 4
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	j	eq_cont.36581
eq_else.36580:
	in	$11
	sw	$10, 84($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.36582
	addi	$11, $0, 5
	addi	$12, $0, -1
	mov	$5, $12
	mov	$2, $11
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36583
eq_else.36582:
	addi	$12, $0, 5
	sw	$11, 88($3)
	mov	$2, $12
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	read_net_item.2857
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$5, 88($3)
	sw	$5, 16($2)
eq_cont.36583:
	lw	$5, 84($3)
	sw	$5, 12($2)
eq_cont.36581:
	lw	$5, 80($3)
	sw	$5, 8($2)
eq_cont.36579:
	lw	$5, 76($3)
	sw	$5, 4($2)
eq_cont.36577:
	lw	$5, 72($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36575:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36584
	lw	$2, 68($3)
	addi	$2, $2, 1
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36585
eq_else.36584:
	lw	$2, 68($3)
	addi	$6, $2, 1
	in	$7
	sw	$5, 92($3)
	sw	$6, 96($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36586
	addi	$7, $0, 1
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	mov	$5, $2
	j	eq_cont.36587
eq_else.36586:
	in	$8
	sw	$7, 100($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36588
	addi	$8, $0, 2
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	j	eq_cont.36589
eq_else.36588:
	in	$9
	sw	$8, 104($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36590
	addi	$9, $0, 3
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	j	eq_cont.36591
eq_else.36590:
	in	$10
	sw	$9, 108($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36592
	addi	$10, $0, 4
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	j	eq_cont.36593
eq_else.36592:
	addi	$11, $0, 4
	sw	$10, 112($3)
	mov	$2, $11
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	read_net_item.2857
	addi	$3, $3, -120
	lw	$31, 116($3)
	lw	$5, 112($3)
	sw	$5, 12($2)
eq_cont.36593:
	lw	$5, 108($3)
	sw	$5, 8($2)
eq_cont.36591:
	lw	$5, 104($3)
	sw	$5, 4($2)
eq_cont.36589:
	lw	$5, 100($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36587:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36594
	lw	$2, 96($3)
	addi	$2, $2, 1
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	j	eq_cont.36595
eq_else.36594:
	lw	$2, 96($3)
	addi	$6, $2, 1
	sw	$5, 116($3)
	mov	$2, $6
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	read_or_network.2859
	addi	$3, $3, -128
	lw	$31, 124($3)
	lw	$5, 96($3)
	slli	$5, $5, 2
	lw	$6, 116($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36595:
	lw	$5, 68($3)
	slli	$5, $5, 2
	lw	$6, 92($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36585:
	lw	$5, 36($3)
	slli	$5, $5, 2
	lw	$6, 64($3)
	add $30, $2, $5
	sw	$6, 0($30)
eq_cont.36573:
	lw	$5, 0($3)
	slli	$5, $5, 2
	lw	$6, 32($3)
	add $30, $2, $5
	sw	$6, 0($30)
	jr $31
read_and_network.2861:
	in	$5
	sw	$2, 0($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36596
	addi	$5, $0, 1
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	j	eq_cont.36597
eq_else.36596:
	in	$6
	sw	$5, 4($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36598
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36599
eq_else.36598:
	in	$7
	sw	$6, 8($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36600
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36601
eq_else.36600:
	in	$8
	sw	$7, 12($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36602
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36603
eq_else.36602:
	in	$9
	sw	$8, 16($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36604
	addi	$9, $0, 5
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36605
eq_else.36604:
	in	$10
	sw	$9, 20($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36606
	addi	$10, $0, 6
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36607
eq_else.36606:
	in	$11
	sw	$10, 24($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.36608
	addi	$11, $0, 7
	addi	$12, $0, -1
	mov	$5, $12
	mov	$2, $11
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36609
eq_else.36608:
	addi	$12, $0, 7
	sw	$11, 28($3)
	mov	$2, $12
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	read_net_item.2857
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$5, 24($2)
eq_cont.36609:
	lw	$5, 24($3)
	sw	$5, 20($2)
eq_cont.36607:
	lw	$5, 20($3)
	sw	$5, 16($2)
eq_cont.36605:
	lw	$5, 16($3)
	sw	$5, 12($2)
eq_cont.36603:
	lw	$5, 12($3)
	sw	$5, 8($2)
eq_cont.36601:
	lw	$5, 8($3)
	sw	$5, 4($2)
eq_cont.36599:
	lw	$5, 4($3)
	sw	$5, 0($2)
eq_cont.36597:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36610
	jr $31
eq_else.36610:
	lw	$5, 0($3)
	slli	$6, $5, 2
	addi	$6, $6, 3324
	sw	$2, 0($6)
	addi	$2, $5, 1
	in	$5
	sw	$2, 32($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36612
	addi	$5, $0, 1
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.36613
eq_else.36612:
	in	$6
	sw	$5, 36($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36614
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.36615
eq_else.36614:
	in	$7
	sw	$6, 40($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36616
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.36617
eq_else.36616:
	in	$8
	sw	$7, 44($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36618
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36619
eq_else.36618:
	in	$9
	sw	$8, 48($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36620
	addi	$9, $0, 5
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36621
eq_else.36620:
	in	$10
	sw	$9, 52($3)
	addi	$30, $0, -1
	bne	$10, $30, eq_else.36622
	addi	$10, $0, 6
	addi	$11, $0, -1
	mov	$5, $11
	mov	$2, $10
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36623
eq_else.36622:
	addi	$11, $0, 6
	sw	$10, 56($3)
	mov	$2, $11
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	read_net_item.2857
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 56($3)
	sw	$5, 20($2)
eq_cont.36623:
	lw	$5, 52($3)
	sw	$5, 16($2)
eq_cont.36621:
	lw	$5, 48($3)
	sw	$5, 12($2)
eq_cont.36619:
	lw	$5, 44($3)
	sw	$5, 8($2)
eq_cont.36617:
	lw	$5, 40($3)
	sw	$5, 4($2)
eq_cont.36615:
	lw	$5, 36($3)
	sw	$5, 0($2)
eq_cont.36613:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36624
	jr $31
eq_else.36624:
	lw	$5, 32($3)
	slli	$6, $5, 2
	addi	$6, $6, 3324
	sw	$2, 0($6)
	addi	$2, $5, 1
	in	$5
	sw	$2, 60($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36626
	addi	$5, $0, 1
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.36627
eq_else.36626:
	in	$6
	sw	$5, 64($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36628
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.36629
eq_else.36628:
	in	$7
	sw	$6, 68($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36630
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.36631
eq_else.36630:
	in	$8
	sw	$7, 72($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36632
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.36633
eq_else.36632:
	in	$9
	sw	$8, 76($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36634
	addi	$9, $0, 5
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	j	eq_cont.36635
eq_else.36634:
	addi	$10, $0, 5
	sw	$9, 80($3)
	mov	$2, $10
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	read_net_item.2857
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	sw	$5, 16($2)
eq_cont.36635:
	lw	$5, 76($3)
	sw	$5, 12($2)
eq_cont.36633:
	lw	$5, 72($3)
	sw	$5, 8($2)
eq_cont.36631:
	lw	$5, 68($3)
	sw	$5, 4($2)
eq_cont.36629:
	lw	$5, 64($3)
	sw	$5, 0($2)
eq_cont.36627:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36636
	jr $31
eq_else.36636:
	lw	$5, 60($3)
	slli	$6, $5, 2
	addi	$6, $6, 3324
	sw	$2, 0($6)
	addi	$2, $5, 1
	in	$5
	sw	$2, 84($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36638
	addi	$5, $0, 1
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36639
eq_else.36638:
	in	$6
	sw	$5, 88($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36640
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36641
eq_else.36640:
	in	$7
	sw	$6, 92($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36642
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	j	eq_cont.36643
eq_else.36642:
	in	$8
	sw	$7, 96($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36644
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	j	eq_cont.36645
eq_else.36644:
	addi	$9, $0, 4
	sw	$8, 100($3)
	mov	$2, $9
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	read_net_item.2857
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 100($3)
	sw	$5, 12($2)
eq_cont.36645:
	lw	$5, 96($3)
	sw	$5, 8($2)
eq_cont.36643:
	lw	$5, 92($3)
	sw	$5, 4($2)
eq_cont.36641:
	lw	$5, 88($3)
	sw	$5, 0($2)
eq_cont.36639:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36646
	jr $31
eq_else.36646:
	lw	$5, 84($3)
	slli	$6, $5, 2
	addi	$6, $6, 3324
	sw	$2, 0($6)
	addi	$2, $5, 1
	j	read_and_network.2861
read_parameter.2863:
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	read_screen_settings.2844
	addi	$3, $3, -8
	lw	$31, 4($3)
	in	$2
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36648
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36649
eq_else.36648:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36649:
	fmul	$f2, $f1, $f0
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	lf	$f3, 4968($0) # 3.141593
	sf	$f0, 0($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36650
	fsub	$f2, $f2, $f3
	fneg	$f1, $f1
	movf	$f0, $f1
	movf	$f1, $f2
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	sin_sub2.2684
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.36651
eq_else.36650:
	movf	$f0, $f1
	movf	$f1, $f2
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	sin_sub2.2684
	addi	$3, $3, -16
	lw	$31, 12($3)
eq_cont.36651:
	fneg	$f0, $f0
	sf	$f0, 3300($0)
	inf	$f0
	lf	$f1, 4808($0) # 0.017453
	fmul	$f0, $f0, $f1
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	lf	$f3, 0($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36652
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36653
eq_else.36652:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36653:
	fmul	$f2, $f2, $f3
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	lf	$f3, 4968($0) # 3.141593
	sf	$f0, 8($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36654
	fsub	$f1, $f2, $f3
	lf	$f2, 4880($0) # -1.000000
	movf	$f0, $f2
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	cos_sub2.2695
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.36655
eq_else.36654:
	movf	$f0, $f1
	movf	$f1, $f2
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	cos_sub2.2695
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.36655:
	lf	$f1, 4888($0) # 0.000000
	lf	$f2, 8($3)
	sltf	$30, $f1, $f2
	bne	$30, $0, eq_else.36656
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.36657
eq_else.36656:
	lf	$f1, 4920($0) # 1.000000
eq_cont.36657:
	fmul	$f3, $f1, $f2
	lf	$f4, 4800($0) # 6.283185
	fdiv	$f5, $f3, $f4
	floor	$f5, $f5
	fmul	$f4, $f5, $f4
	fsub	$f3, $f3, $f4
	lf	$f4, 4968($0) # 3.141593
	sf	$f0, 16($3)
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.36658
	fsub	$f3, $f3, $f4
	fneg	$f1, $f1
	movf	$f0, $f1
	movf	$f1, $f3
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	sin_sub2.2684
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36659
eq_else.36658:
	movf	$f0, $f1
	movf	$f1, $f3
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	sin_sub2.2684
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.36659:
	lf	$f1, 16($3)
	fmul	$f0, $f1, $f0
	sf	$f0, 3292($0)
	lf	$f0, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	lf	$f3, 8($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36660
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.36661
eq_else.36660:
	lf	$f2, 4920($0) # 1.000000
eq_cont.36661:
	fmul	$f2, $f2, $f3
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	lf	$f3, 4968($0) # 3.141593
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.36662
	fsub	$f0, $f2, $f3
	lf	$f2, 4880($0) # -1.000000
	movf	$f1, $f0
	movf	$f0, $f2
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	cos_sub2.2695
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.36663
eq_else.36662:
	movf	$f1, $f2
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	cos_sub2.2695
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.36663:
	lf	$f1, 16($3)
	fmul	$f0, $f1, $f0
	sf	$f0, 3308($0)
	inf	$f0
	sf	$f0, 3316($0)
	addi	$2, $0, 0
	sw	$2, 24($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	read_nth_object.2851
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.36664
	addi	$2, $0, 3000
	lw	$5, 24($3)
	sw	$5, 0($2)
	j	eq_cont.36665
eq_else.36664:
	addi	$2, $0, 1
	sw	$2, 28($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	read_nth_object.2851
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36666
	addi	$2, $0, 3000
	lw	$5, 28($3)
	sw	$5, 0($2)
	j	eq_cont.36667
eq_else.36666:
	addi	$2, $0, 2
	sw	$2, 32($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	read_nth_object.2851
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36668
	addi	$2, $0, 3000
	lw	$5, 32($3)
	sw	$5, 0($2)
	j	eq_cont.36669
eq_else.36668:
	addi	$2, $0, 3
	sw	$2, 36($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	read_nth_object.2851
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.36670
	addi	$2, $0, 3000
	lw	$5, 36($3)
	sw	$5, 0($2)
	j	eq_cont.36671
eq_else.36670:
	addi	$2, $0, 4
	sw	$2, 40($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	read_nth_object.2851
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.36672
	addi	$2, $0, 3000
	lw	$5, 40($3)
	sw	$5, 0($2)
	j	eq_cont.36673
eq_else.36672:
	addi	$2, $0, 5
	sw	$2, 44($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	read_nth_object.2851
	addi	$3, $3, -56
	lw	$31, 52($3)
	bne	$2, $0, eq_else.36674
	addi	$2, $0, 3000
	lw	$5, 44($3)
	sw	$5, 0($2)
	j	eq_cont.36675
eq_else.36674:
	addi	$2, $0, 6
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	read_object.2853
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.36675:
eq_cont.36673:
eq_cont.36671:
eq_cont.36669:
eq_cont.36667:
eq_cont.36665:
	in	$2
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36676
	addi	$2, $0, 1
	addi	$5, $0, -1
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36677
eq_else.36676:
	in	$5
	sw	$2, 48($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36678
	addi	$5, $0, 2
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.36679
eq_else.36678:
	in	$6
	sw	$5, 52($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36680
	addi	$6, $0, 3
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36681
eq_else.36680:
	in	$7
	sw	$6, 56($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36682
	addi	$7, $0, 4
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.36683
eq_else.36682:
	in	$8
	sw	$7, 60($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36684
	addi	$8, $0, 5
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.36685
eq_else.36684:
	in	$9
	sw	$8, 64($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36686
	addi	$9, $0, 6
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.36687
eq_else.36686:
	addi	$10, $0, 6
	sw	$9, 68($3)
	mov	$2, $10
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	read_net_item.2857
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	sw	$5, 20($2)
eq_cont.36687:
	lw	$5, 64($3)
	sw	$5, 16($2)
eq_cont.36685:
	lw	$5, 60($3)
	sw	$5, 12($2)
eq_cont.36683:
	lw	$5, 56($3)
	sw	$5, 8($2)
eq_cont.36681:
	lw	$5, 52($3)
	sw	$5, 4($2)
eq_cont.36679:
	lw	$5, 48($3)
	sw	$5, 0($2)
eq_cont.36677:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36688
	j	eq_cont.36689
eq_else.36688:
	addi	$5, $0, 3324
	sw	$2, 0($5)
	in	$2
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36690
	addi	$2, $0, 1
	addi	$5, $0, -1
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.36691
eq_else.36690:
	in	$5
	sw	$2, 72($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36692
	addi	$5, $0, 2
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.36693
eq_else.36692:
	in	$6
	sw	$5, 76($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36694
	addi	$6, $0, 3
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	j	eq_cont.36695
eq_else.36694:
	in	$7
	sw	$6, 80($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36696
	addi	$7, $0, 4
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	j	eq_cont.36697
eq_else.36696:
	in	$8
	sw	$7, 84($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36698
	addi	$8, $0, 5
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36699
eq_else.36698:
	addi	$9, $0, 5
	sw	$8, 88($3)
	mov	$2, $9
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	read_net_item.2857
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$5, 88($3)
	sw	$5, 16($2)
eq_cont.36699:
	lw	$5, 84($3)
	sw	$5, 12($2)
eq_cont.36697:
	lw	$5, 80($3)
	sw	$5, 8($2)
eq_cont.36695:
	lw	$5, 76($3)
	sw	$5, 4($2)
eq_cont.36693:
	lw	$5, 72($3)
	sw	$5, 0($2)
eq_cont.36691:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36700
	j	eq_cont.36701
eq_else.36700:
	addi	$5, $0, 3328
	sw	$2, 0($5)
	in	$2
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36702
	addi	$2, $0, 1
	addi	$5, $0, -1
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.36703
eq_else.36702:
	in	$5
	sw	$2, 92($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36704
	addi	$5, $0, 2
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	j	eq_cont.36705
eq_else.36704:
	in	$6
	sw	$5, 96($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36706
	addi	$6, $0, 3
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	j	eq_cont.36707
eq_else.36706:
	in	$7
	sw	$6, 100($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36708
	addi	$7, $0, 4
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	j	eq_cont.36709
eq_else.36708:
	addi	$8, $0, 4
	sw	$7, 104($3)
	mov	$2, $8
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	read_net_item.2857
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 104($3)
	sw	$5, 12($2)
eq_cont.36709:
	lw	$5, 100($3)
	sw	$5, 8($2)
eq_cont.36707:
	lw	$5, 96($3)
	sw	$5, 4($2)
eq_cont.36705:
	lw	$5, 92($3)
	sw	$5, 0($2)
eq_cont.36703:
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36710
	j	eq_cont.36711
eq_else.36710:
	addi	$5, $0, 3332
	sw	$2, 0($5)
	addi	$2, $0, 3
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	read_and_network.2861
	addi	$3, $3, -112
	lw	$31, 108($3)
eq_cont.36711:
eq_cont.36701:
eq_cont.36689:
	in	$2
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36712
	addi	$2, $0, 1
	addi	$5, $0, -1
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $2
	j	eq_cont.36713
eq_else.36712:
	in	$5
	sw	$2, 108($3)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.36714
	addi	$5, $0, 2
	addi	$6, $0, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	j	eq_cont.36715
eq_else.36714:
	in	$6
	sw	$5, 112($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36716
	addi	$6, $0, 3
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	j	eq_cont.36717
eq_else.36716:
	in	$7
	sw	$6, 116($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36718
	addi	$7, $0, 4
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	min_caml_create_array
	addi	$3, $3, -128
	lw	$31, 124($3)
	j	eq_cont.36719
eq_else.36718:
	in	$8
	sw	$7, 120($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36720
	addi	$8, $0, 5
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	min_caml_create_array
	addi	$3, $3, -128
	lw	$31, 124($3)
	j	eq_cont.36721
eq_else.36720:
	in	$9
	sw	$8, 124($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36722
	addi	$9, $0, 6
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	j	eq_cont.36723
eq_else.36722:
	addi	$10, $0, 6
	sw	$9, 128($3)
	mov	$2, $10
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	read_net_item.2857
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 128($3)
	sw	$5, 20($2)
eq_cont.36723:
	lw	$5, 124($3)
	sw	$5, 16($2)
eq_cont.36721:
	lw	$5, 120($3)
	sw	$5, 12($2)
eq_cont.36719:
	lw	$5, 116($3)
	sw	$5, 8($2)
eq_cont.36717:
	lw	$5, 112($3)
	sw	$5, 4($2)
eq_cont.36715:
	lw	$5, 108($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36713:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36724
	addi	$2, $0, 1
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	j	eq_cont.36725
eq_else.36724:
	in	$2
	sw	$5, 132($3)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36726
	addi	$2, $0, 1
	addi	$6, $0, -1
	mov	$5, $6
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	mov	$5, $2
	j	eq_cont.36727
eq_else.36726:
	in	$6
	sw	$2, 136($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36728
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	j	eq_cont.36729
eq_else.36728:
	in	$7
	sw	$6, 140($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36730
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	j	eq_cont.36731
eq_else.36730:
	in	$8
	sw	$7, 144($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36732
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	j	eq_cont.36733
eq_else.36732:
	in	$9
	sw	$8, 148($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.36734
	addi	$9, $0, 5
	addi	$10, $0, -1
	mov	$5, $10
	mov	$2, $9
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	j	eq_cont.36735
eq_else.36734:
	addi	$10, $0, 5
	sw	$9, 152($3)
	mov	$2, $10
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	read_net_item.2857
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 152($3)
	sw	$5, 16($2)
eq_cont.36735:
	lw	$5, 148($3)
	sw	$5, 12($2)
eq_cont.36733:
	lw	$5, 144($3)
	sw	$5, 8($2)
eq_cont.36731:
	lw	$5, 140($3)
	sw	$5, 4($2)
eq_cont.36729:
	lw	$5, 136($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36727:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36736
	addi	$2, $0, 2
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	j	eq_cont.36737
eq_else.36736:
	in	$2
	sw	$5, 156($3)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36738
	addi	$2, $0, 1
	addi	$6, $0, -1
	mov	$5, $6
	sw	$31, 164($3)
	addi	$3, $3, 168
	jal	min_caml_create_array
	addi	$3, $3, -168
	lw	$31, 164($3)
	mov	$5, $2
	j	eq_cont.36739
eq_else.36738:
	in	$6
	sw	$2, 160($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.36740
	addi	$6, $0, 2
	addi	$7, $0, -1
	mov	$5, $7
	mov	$2, $6
	sw	$31, 164($3)
	addi	$3, $3, 168
	jal	min_caml_create_array
	addi	$3, $3, -168
	lw	$31, 164($3)
	j	eq_cont.36741
eq_else.36740:
	in	$7
	sw	$6, 164($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.36742
	addi	$7, $0, 3
	addi	$8, $0, -1
	mov	$5, $8
	mov	$2, $7
	sw	$31, 172($3)
	addi	$3, $3, 176
	jal	min_caml_create_array
	addi	$3, $3, -176
	lw	$31, 172($3)
	j	eq_cont.36743
eq_else.36742:
	in	$8
	sw	$7, 168($3)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.36744
	addi	$8, $0, 4
	addi	$9, $0, -1
	mov	$5, $9
	mov	$2, $8
	sw	$31, 172($3)
	addi	$3, $3, 176
	jal	min_caml_create_array
	addi	$3, $3, -176
	lw	$31, 172($3)
	j	eq_cont.36745
eq_else.36744:
	addi	$9, $0, 4
	sw	$8, 172($3)
	mov	$2, $9
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	read_net_item.2857
	addi	$3, $3, -184
	lw	$31, 180($3)
	lw	$5, 172($3)
	sw	$5, 12($2)
eq_cont.36745:
	lw	$5, 168($3)
	sw	$5, 8($2)
eq_cont.36743:
	lw	$5, 164($3)
	sw	$5, 4($2)
eq_cont.36741:
	lw	$5, 160($3)
	sw	$5, 0($2)
	mov	$5, $2
eq_cont.36739:
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.36746
	addi	$2, $0, 3
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	min_caml_create_array
	addi	$3, $3, -184
	lw	$31, 180($3)
	j	eq_cont.36747
eq_else.36746:
	addi	$2, $0, 3
	sw	$5, 176($3)
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	read_or_network.2859
	addi	$3, $3, -184
	lw	$31, 180($3)
	lw	$5, 176($3)
	sw	$5, 8($2)
eq_cont.36747:
	lw	$5, 156($3)
	sw	$5, 4($2)
eq_cont.36737:
	lw	$5, 132($3)
	sw	$5, 0($2)
eq_cont.36725:
	sw	$2, 3524($0)
	jr $31
solver_rect_surface.2865:
	slli	$9, $6, 3
	add	$30, $5, $9
	lf	$f3, 0($30)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36749
	addi	$9, $0, 1
	j	eq_cont.36750
eq_else.36749:
	addi	$9, $0, 0
eq_cont.36750:
	bne	$9, $0, eq_else.36751
	lw	$9, 16($2)
	lw	$2, 24($2)
	slli	$10, $6, 3
	add	$30, $5, $10
	lf	$f3, 0($30)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.36752
	addi	$10, $0, 0
	j	eq_cont.36753
eq_else.36752:
	addi	$10, $0, 1
eq_cont.36753:
	bne	$2, $0, eq_else.36754
	mov	$2, $10
	j	eq_cont.36755
eq_else.36754:
	bne	$10, $0, eq_else.36756
	addi	$2, $0, 1
	j	eq_cont.36757
eq_else.36756:
	addi	$2, $0, 0
eq_cont.36757:
eq_cont.36755:
	slli	$10, $6, 3
	add	$30, $9, $10
	lf	$f3, 0($30)
	bne	$2, $0, eq_else.36758
	fneg	$f3, $f3
	j	eq_cont.36759
eq_else.36758:
eq_cont.36759:
	fsub	$f0, $f3, $f0
	slli	$2, $6, 3
	add	$30, $5, $2
	lf	$f3, 0($30)
	fdiv	$f0, $f0, $f3
	slli	$2, $7, 3
	add	$30, $5, $2
	lf	$f3, 0($30)
	fmul	$f3, $f0, $f3
	fadd	$f1, $f3, $f1
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36760
	addi	$2, $0, 0
	j	eq_cont.36761
eq_else.36760:
	addi	$2, $0, 1
eq_cont.36761:
	bne	$2, $0, eq_else.36762
	fneg	$f1, $f1
	j	eq_cont.36763
eq_else.36762:
eq_cont.36763:
	slli	$2, $7, 3
	add	$30, $9, $2
	lf	$f3, 0($30)
	fsub	$f1, $f3, $f1
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f1
	bne	$30, $0, eq_else.36764
	addi	$2, $0, 0
	j	eq_cont.36765
eq_else.36764:
	addi	$2, $0, 1
eq_cont.36765:
	bne	$2, $0, eq_else.36766
	addi	$2, $0, 0
	j	eq_cont.36767
eq_else.36766:
	addi	$2, $0, 1
eq_cont.36767:
	bne	$2, $0, eq_else.36768
	addi	$2, $0, 0
	jr $31
eq_else.36768:
	slli	$2, $8, 3
	add	$30, $5, $2
	lf	$f1, 0($30)
	fmul	$f1, $f0, $f1
	fadd	$f1, $f1, $f2
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36769
	addi	$2, $0, 0
	j	eq_cont.36770
eq_else.36769:
	addi	$2, $0, 1
eq_cont.36770:
	bne	$2, $0, eq_else.36771
	fneg	$f1, $f1
	j	eq_cont.36772
eq_else.36771:
eq_cont.36772:
	slli	$2, $8, 3
	add	$30, $9, $2
	lf	$f2, 0($30)
	fsub	$f1, $f2, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.36773
	addi	$2, $0, 0
	j	eq_cont.36774
eq_else.36773:
	addi	$2, $0, 1
eq_cont.36774:
	bne	$2, $0, eq_else.36775
	addi	$2, $0, 0
	j	eq_cont.36776
eq_else.36775:
	addi	$2, $0, 1
eq_cont.36776:
	bne	$2, $0, eq_else.36777
	addi	$2, $0, 0
	jr $31
eq_else.36777:
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.36751:
	addi	$2, $0, 0
	jr $31
solver_rect.2874:
	lf	$f3, 0($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36778
	addi	$6, $0, 1
	j	eq_cont.36779
eq_else.36778:
	addi	$6, $0, 0
eq_cont.36779:
	bne	$6, $0, eq_else.36780
	lw	$6, 16($2)
	lw	$7, 24($2)
	lf	$f3, 0($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.36782
	addi	$8, $0, 0
	j	eq_cont.36783
eq_else.36782:
	addi	$8, $0, 1
eq_cont.36783:
	bne	$7, $0, eq_else.36784
	mov	$7, $8
	j	eq_cont.36785
eq_else.36784:
	bne	$8, $0, eq_else.36786
	addi	$7, $0, 1
	j	eq_cont.36787
eq_else.36786:
	addi	$7, $0, 0
eq_cont.36787:
eq_cont.36785:
	lf	$f3, 0($6)
	bne	$7, $0, eq_else.36788
	fneg	$f3, $f3
	j	eq_cont.36789
eq_else.36788:
eq_cont.36789:
	fsub	$f3, $f3, $f0
	lf	$f4, 0($5)
	fdiv	$f3, $f3, $f4
	lf	$f4, 8($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f1
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36790
	addi	$7, $0, 0
	j	eq_cont.36791
eq_else.36790:
	addi	$7, $0, 1
eq_cont.36791:
	bne	$7, $0, eq_else.36792
	fneg	$f4, $f4
	j	eq_cont.36793
eq_else.36792:
eq_cont.36793:
	lf	$f5, 8($6)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36794
	addi	$7, $0, 0
	j	eq_cont.36795
eq_else.36794:
	addi	$7, $0, 1
eq_cont.36795:
	bne	$7, $0, eq_else.36796
	addi	$7, $0, 0
	j	eq_cont.36797
eq_else.36796:
	addi	$7, $0, 1
eq_cont.36797:
	bne	$7, $0, eq_else.36798
	addi	$6, $0, 0
	j	eq_cont.36799
eq_else.36798:
	lf	$f4, 16($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f2
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36800
	addi	$7, $0, 0
	j	eq_cont.36801
eq_else.36800:
	addi	$7, $0, 1
eq_cont.36801:
	bne	$7, $0, eq_else.36802
	fneg	$f4, $f4
	j	eq_cont.36803
eq_else.36802:
eq_cont.36803:
	lf	$f5, 16($6)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36804
	addi	$6, $0, 0
	j	eq_cont.36805
eq_else.36804:
	addi	$6, $0, 1
eq_cont.36805:
	bne	$6, $0, eq_else.36806
	addi	$6, $0, 0
	j	eq_cont.36807
eq_else.36806:
	addi	$6, $0, 1
eq_cont.36807:
	bne	$6, $0, eq_else.36808
	addi	$6, $0, 0
	j	eq_cont.36809
eq_else.36808:
	addi	$6, $0, 3528
	sf	$f3, 0($6)
	addi	$6, $0, 1
eq_cont.36809:
eq_cont.36799:
	j	eq_cont.36781
eq_else.36780:
	addi	$6, $0, 0
eq_cont.36781:
	bne	$6, $0, eq_else.36810
	lf	$f3, 8($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36811
	addi	$6, $0, 1
	j	eq_cont.36812
eq_else.36811:
	addi	$6, $0, 0
eq_cont.36812:
	bne	$6, $0, eq_else.36813
	lw	$6, 16($2)
	lw	$7, 24($2)
	lf	$f3, 8($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.36815
	addi	$8, $0, 0
	j	eq_cont.36816
eq_else.36815:
	addi	$8, $0, 1
eq_cont.36816:
	bne	$7, $0, eq_else.36817
	mov	$7, $8
	j	eq_cont.36818
eq_else.36817:
	bne	$8, $0, eq_else.36819
	addi	$7, $0, 1
	j	eq_cont.36820
eq_else.36819:
	addi	$7, $0, 0
eq_cont.36820:
eq_cont.36818:
	lf	$f3, 8($6)
	bne	$7, $0, eq_else.36821
	fneg	$f3, $f3
	j	eq_cont.36822
eq_else.36821:
eq_cont.36822:
	fsub	$f3, $f3, $f1
	lf	$f4, 8($5)
	fdiv	$f3, $f3, $f4
	lf	$f4, 16($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f2
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36823
	addi	$7, $0, 0
	j	eq_cont.36824
eq_else.36823:
	addi	$7, $0, 1
eq_cont.36824:
	bne	$7, $0, eq_else.36825
	fneg	$f4, $f4
	j	eq_cont.36826
eq_else.36825:
eq_cont.36826:
	lf	$f5, 16($6)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36827
	addi	$7, $0, 0
	j	eq_cont.36828
eq_else.36827:
	addi	$7, $0, 1
eq_cont.36828:
	bne	$7, $0, eq_else.36829
	addi	$7, $0, 0
	j	eq_cont.36830
eq_else.36829:
	addi	$7, $0, 1
eq_cont.36830:
	bne	$7, $0, eq_else.36831
	addi	$6, $0, 0
	j	eq_cont.36832
eq_else.36831:
	lf	$f4, 0($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f0
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36833
	addi	$7, $0, 0
	j	eq_cont.36834
eq_else.36833:
	addi	$7, $0, 1
eq_cont.36834:
	bne	$7, $0, eq_else.36835
	fneg	$f4, $f4
	j	eq_cont.36836
eq_else.36835:
eq_cont.36836:
	lf	$f5, 0($6)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36837
	addi	$6, $0, 0
	j	eq_cont.36838
eq_else.36837:
	addi	$6, $0, 1
eq_cont.36838:
	bne	$6, $0, eq_else.36839
	addi	$6, $0, 0
	j	eq_cont.36840
eq_else.36839:
	addi	$6, $0, 1
eq_cont.36840:
	bne	$6, $0, eq_else.36841
	addi	$6, $0, 0
	j	eq_cont.36842
eq_else.36841:
	addi	$6, $0, 3528
	sf	$f3, 0($6)
	addi	$6, $0, 1
eq_cont.36842:
eq_cont.36832:
	j	eq_cont.36814
eq_else.36813:
	addi	$6, $0, 0
eq_cont.36814:
	bne	$6, $0, eq_else.36843
	lf	$f3, 16($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36844
	addi	$6, $0, 1
	j	eq_cont.36845
eq_else.36844:
	addi	$6, $0, 0
eq_cont.36845:
	bne	$6, $0, eq_else.36846
	lw	$6, 16($2)
	lw	$2, 24($2)
	lf	$f3, 16($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.36848
	addi	$7, $0, 0
	j	eq_cont.36849
eq_else.36848:
	addi	$7, $0, 1
eq_cont.36849:
	bne	$2, $0, eq_else.36850
	mov	$2, $7
	j	eq_cont.36851
eq_else.36850:
	bne	$7, $0, eq_else.36852
	addi	$2, $0, 1
	j	eq_cont.36853
eq_else.36852:
	addi	$2, $0, 0
eq_cont.36853:
eq_cont.36851:
	lf	$f3, 16($6)
	bne	$2, $0, eq_else.36854
	fneg	$f3, $f3
	j	eq_cont.36855
eq_else.36854:
eq_cont.36855:
	fsub	$f2, $f3, $f2
	lf	$f3, 16($5)
	fdiv	$f2, $f2, $f3
	lf	$f3, 0($5)
	fmul	$f3, $f2, $f3
	fadd	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36856
	addi	$2, $0, 0
	j	eq_cont.36857
eq_else.36856:
	addi	$2, $0, 1
eq_cont.36857:
	bne	$2, $0, eq_else.36858
	fneg	$f0, $f0
	j	eq_cont.36859
eq_else.36858:
eq_cont.36859:
	lf	$f3, 0($6)
	fsub	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36860
	addi	$2, $0, 0
	j	eq_cont.36861
eq_else.36860:
	addi	$2, $0, 1
eq_cont.36861:
	bne	$2, $0, eq_else.36862
	addi	$2, $0, 0
	j	eq_cont.36863
eq_else.36862:
	addi	$2, $0, 1
eq_cont.36863:
	bne	$2, $0, eq_else.36864
	addi	$2, $0, 0
	j	eq_cont.36865
eq_else.36864:
	lf	$f0, 8($5)
	fmul	$f0, $f2, $f0
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36866
	addi	$2, $0, 0
	j	eq_cont.36867
eq_else.36866:
	addi	$2, $0, 1
eq_cont.36867:
	bne	$2, $0, eq_else.36868
	fneg	$f0, $f0
	j	eq_cont.36869
eq_else.36868:
eq_cont.36869:
	lf	$f1, 8($6)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36870
	addi	$2, $0, 0
	j	eq_cont.36871
eq_else.36870:
	addi	$2, $0, 1
eq_cont.36871:
	bne	$2, $0, eq_else.36872
	addi	$2, $0, 0
	j	eq_cont.36873
eq_else.36872:
	addi	$2, $0, 1
eq_cont.36873:
	bne	$2, $0, eq_else.36874
	addi	$2, $0, 0
	j	eq_cont.36875
eq_else.36874:
	addi	$2, $0, 3528
	sf	$f2, 0($2)
	addi	$2, $0, 1
eq_cont.36875:
eq_cont.36865:
	j	eq_cont.36847
eq_else.36846:
	addi	$2, $0, 0
eq_cont.36847:
	bne	$2, $0, eq_else.36876
	addi	$2, $0, 0
	jr $31
eq_else.36876:
	addi	$2, $0, 3
	jr $31
eq_else.36843:
	addi	$2, $0, 2
	jr $31
eq_else.36810:
	addi	$2, $0, 1
	jr $31
solver_second.2899:
	lf	$f3, 0($5)
	lf	$f4, 8($5)
	lf	$f5, 16($5)
	fmul	$f6, $f3, $f3
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.36877
	movf	$f3, $f6
	j	eq_cont.36878
eq_else.36877:
	fmul	$f7, $f4, $f5
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$6, 36($2)
	lf	$f4, 16($6)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.36878:
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36879
	addi	$6, $0, 1
	j	eq_cont.36880
eq_else.36879:
	addi	$6, $0, 0
eq_cont.36880:
	bne	$6, $0, eq_else.36881
	lf	$f4, 0($5)
	lf	$f5, 8($5)
	lf	$f6, 16($5)
	fmul	$f7, $f4, $f0
	lw	$5, 16($2)
	lf	$f8, 0($5)
	fmul	$f7, $f7, $f8
	fmul	$f8, $f5, $f1
	lw	$5, 16($2)
	lf	$f9, 8($5)
	fmul	$f8, $f8, $f9
	fadd	$f7, $f7, $f8
	fmul	$f8, $f6, $f2
	lw	$5, 16($2)
	lf	$f9, 16($5)
	fmul	$f8, $f8, $f9
	fadd	$f7, $f7, $f8
	lw	$5, 12($2)
	bne	$5, $0, eq_else.36882
	movf	$f4, $f7
	j	eq_cont.36883
eq_else.36882:
	fmul	$f8, $f6, $f1
	fmul	$f9, $f5, $f2
	fadd	$f8, $f8, $f9
	lw	$5, 36($2)
	lf	$f9, 0($5)
	fmul	$f8, $f8, $f9
	fmul	$f9, $f4, $f2
	fmul	$f6, $f6, $f0
	fadd	$f6, $f9, $f6
	lw	$5, 36($2)
	lf	$f9, 8($5)
	fmul	$f6, $f6, $f9
	fadd	$f6, $f8, $f6
	fmul	$f4, $f4, $f1
	fmul	$f5, $f5, $f0
	fadd	$f4, $f4, $f5
	lw	$5, 36($2)
	lf	$f5, 16($5)
	fmul	$f4, $f4, $f5
	fadd	$f4, $f6, $f4
	lf	$f5, 4944($0) # 0.500000
	fmul	$f4, $f4, $f5
	fadd	$f4, $f7, $f4
eq_cont.36883:
	fmul	$f5, $f0, $f0
	lw	$5, 16($2)
	lf	$f6, 0($5)
	fmul	$f5, $f5, $f6
	fmul	$f6, $f1, $f1
	lw	$5, 16($2)
	lf	$f7, 8($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	lw	$5, 16($2)
	lf	$f7, 16($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	lw	$5, 12($2)
	bne	$5, $0, eq_else.36884
	movf	$f0, $f5
	j	eq_cont.36885
eq_else.36884:
	fmul	$f6, $f1, $f2
	lw	$5, 36($2)
	lf	$f7, 0($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f0
	lw	$5, 36($2)
	lf	$f6, 8($5)
	fmul	$f2, $f2, $f6
	fadd	$f2, $f5, $f2
	fmul	$f0, $f0, $f1
	lw	$5, 36($2)
	lf	$f1, 16($5)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.36885:
	lw	$5, 4($2)
	addi	$30, $0, 3
	bne	$5, $30, eq_else.36886
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.36887
eq_else.36886:
eq_cont.36887:
	fmul	$f1, $f4, $f4
	fmul	$f0, $f3, $f0
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36888
	addi	$5, $0, 0
	j	eq_cont.36889
eq_else.36888:
	addi	$5, $0, 1
eq_cont.36889:
	bne	$5, $0, eq_else.36890
	addi	$2, $0, 0
	jr $31
eq_else.36890:
	sqrt	$f0, $f0
	lw	$2, 24($2)
	bne	$2, $0, eq_else.36891
	fneg	$f0, $f0
	j	eq_cont.36892
eq_else.36891:
eq_cont.36892:
	fsub	$f0, $f0, $f4
	fdiv	$f0, $f0, $f3
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.36881:
	addi	$2, $0, 0
	jr $31
solver.2905:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 0($6)
	lw	$7, 20($2)
	lf	$f1, 0($7)
	fsub	$f0, $f0, $f1
	lf	$f1, 8($6)
	lw	$7, 20($2)
	lf	$f2, 8($7)
	fsub	$f1, $f1, $f2
	lf	$f2, 16($6)
	lw	$6, 20($2)
	lf	$f3, 16($6)
	fsub	$f2, $f2, $f3
	lw	$6, 4($2)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.36893
	addi	$6, $0, 0
	addi	$7, $0, 1
	addi	$8, $0, 2
	sf	$f0, 0($3)
	sf	$f2, 8($3)
	sf	$f1, 16($3)
	sw	$5, 24($3)
	sw	$2, 28($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solver_rect_surface.2865
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36894
	addi	$6, $0, 1
	addi	$7, $0, 2
	addi	$8, $0, 0
	lf	$f0, 16($3)
	lf	$f1, 8($3)
	lf	$f2, 0($3)
	lw	$2, 28($3)
	lw	$5, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solver_rect_surface.2865
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36895
	addi	$6, $0, 2
	addi	$7, $0, 0
	addi	$8, $0, 1
	lf	$f0, 8($3)
	lf	$f1, 0($3)
	lf	$f2, 16($3)
	lw	$2, 28($3)
	lw	$5, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solver_rect_surface.2865
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.36896
	addi	$2, $0, 0
	jr $31
eq_else.36896:
	addi	$2, $0, 3
	jr $31
eq_else.36895:
	addi	$2, $0, 2
	jr $31
eq_else.36894:
	addi	$2, $0, 1
	jr $31
eq_else.36893:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.36897
	lw	$2, 16($2)
	lf	$f3, 0($5)
	lf	$f4, 0($2)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($5)
	lf	$f5, 8($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($5)
	lf	$f5, 16($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.36898
	addi	$5, $0, 0
	j	eq_cont.36899
eq_else.36898:
	addi	$5, $0, 1
eq_cont.36899:
	bne	$5, $0, eq_else.36900
	addi	$2, $0, 0
	jr $31
eq_else.36900:
	lf	$f4, 0($2)
	fmul	$f0, $f4, $f0
	lf	$f4, 8($2)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	fdiv	$f0, $f0, $f3
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.36897:
	lf	$f3, 0($5)
	lf	$f4, 8($5)
	lf	$f5, 16($5)
	fmul	$f6, $f3, $f3
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.36901
	movf	$f3, $f6
	j	eq_cont.36902
eq_else.36901:
	fmul	$f7, $f4, $f5
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$6, 36($2)
	lf	$f4, 16($6)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.36902:
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36903
	addi	$6, $0, 1
	j	eq_cont.36904
eq_else.36903:
	addi	$6, $0, 0
eq_cont.36904:
	bne	$6, $0, eq_else.36905
	lf	$f4, 0($5)
	lf	$f5, 8($5)
	lf	$f6, 16($5)
	fmul	$f7, $f4, $f0
	lw	$5, 16($2)
	lf	$f8, 0($5)
	fmul	$f7, $f7, $f8
	fmul	$f8, $f5, $f1
	lw	$5, 16($2)
	lf	$f9, 8($5)
	fmul	$f8, $f8, $f9
	fadd	$f7, $f7, $f8
	fmul	$f8, $f6, $f2
	lw	$5, 16($2)
	lf	$f9, 16($5)
	fmul	$f8, $f8, $f9
	fadd	$f7, $f7, $f8
	lw	$5, 12($2)
	bne	$5, $0, eq_else.36906
	movf	$f4, $f7
	j	eq_cont.36907
eq_else.36906:
	fmul	$f8, $f6, $f1
	fmul	$f9, $f5, $f2
	fadd	$f8, $f8, $f9
	lw	$5, 36($2)
	lf	$f9, 0($5)
	fmul	$f8, $f8, $f9
	fmul	$f9, $f4, $f2
	fmul	$f6, $f6, $f0
	fadd	$f6, $f9, $f6
	lw	$5, 36($2)
	lf	$f9, 8($5)
	fmul	$f6, $f6, $f9
	fadd	$f6, $f8, $f6
	fmul	$f4, $f4, $f1
	fmul	$f5, $f5, $f0
	fadd	$f4, $f4, $f5
	lw	$5, 36($2)
	lf	$f5, 16($5)
	fmul	$f4, $f4, $f5
	fadd	$f4, $f6, $f4
	lf	$f5, 4944($0) # 0.500000
	fmul	$f4, $f4, $f5
	fadd	$f4, $f7, $f4
eq_cont.36907:
	fmul	$f5, $f0, $f0
	lw	$5, 16($2)
	lf	$f6, 0($5)
	fmul	$f5, $f5, $f6
	fmul	$f6, $f1, $f1
	lw	$5, 16($2)
	lf	$f7, 8($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	lw	$5, 16($2)
	lf	$f7, 16($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	lw	$5, 12($2)
	bne	$5, $0, eq_else.36908
	movf	$f0, $f5
	j	eq_cont.36909
eq_else.36908:
	fmul	$f6, $f1, $f2
	lw	$5, 36($2)
	lf	$f7, 0($5)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f0
	lw	$5, 36($2)
	lf	$f6, 8($5)
	fmul	$f2, $f2, $f6
	fadd	$f2, $f5, $f2
	fmul	$f0, $f0, $f1
	lw	$5, 36($2)
	lf	$f1, 16($5)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.36909:
	lw	$5, 4($2)
	addi	$30, $0, 3
	bne	$5, $30, eq_else.36910
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.36911
eq_else.36910:
eq_cont.36911:
	fmul	$f1, $f4, $f4
	fmul	$f0, $f3, $f0
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36912
	addi	$5, $0, 0
	j	eq_cont.36913
eq_else.36912:
	addi	$5, $0, 1
eq_cont.36913:
	bne	$5, $0, eq_else.36914
	addi	$2, $0, 0
	jr $31
eq_else.36914:
	sqrt	$f0, $f0
	lw	$2, 24($2)
	bne	$2, $0, eq_else.36915
	fneg	$f0, $f0
	j	eq_cont.36916
eq_else.36915:
eq_cont.36916:
	fsub	$f0, $f0, $f4
	fdiv	$f0, $f0, $f3
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.36905:
	addi	$2, $0, 0
	jr $31
solver_rect_fast.2909:
	lf	$f3, 0($6)
	fsub	$f3, $f3, $f0
	lf	$f4, 8($6)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f1
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36917
	addi	$7, $0, 0
	j	eq_cont.36918
eq_else.36917:
	addi	$7, $0, 1
eq_cont.36918:
	bne	$7, $0, eq_else.36919
	fneg	$f4, $f4
	j	eq_cont.36920
eq_else.36919:
eq_cont.36920:
	lw	$7, 16($2)
	lf	$f5, 8($7)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36921
	addi	$7, $0, 0
	j	eq_cont.36922
eq_else.36921:
	addi	$7, $0, 1
eq_cont.36922:
	bne	$7, $0, eq_else.36923
	addi	$7, $0, 0
	j	eq_cont.36924
eq_else.36923:
	addi	$7, $0, 1
eq_cont.36924:
	bne	$7, $0, eq_else.36925
	addi	$7, $0, 0
	j	eq_cont.36926
eq_else.36925:
	lf	$f4, 16($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f2
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36927
	addi	$7, $0, 0
	j	eq_cont.36928
eq_else.36927:
	addi	$7, $0, 1
eq_cont.36928:
	bne	$7, $0, eq_else.36929
	fneg	$f4, $f4
	j	eq_cont.36930
eq_else.36929:
eq_cont.36930:
	lw	$7, 16($2)
	lf	$f5, 16($7)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36931
	addi	$7, $0, 0
	j	eq_cont.36932
eq_else.36931:
	addi	$7, $0, 1
eq_cont.36932:
	bne	$7, $0, eq_else.36933
	addi	$7, $0, 0
	j	eq_cont.36934
eq_else.36933:
	addi	$7, $0, 1
eq_cont.36934:
	bne	$7, $0, eq_else.36935
	addi	$7, $0, 0
	j	eq_cont.36936
eq_else.36935:
	lf	$f4, 8($6)
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	sltf	$29, $f4, $f5
	bne	$30, $29, eq_else.36937
	addi	$7, $0, 1
	j	eq_cont.36938
eq_else.36937:
	addi	$7, $0, 0
eq_cont.36938:
	bne	$7, $0, eq_else.36939
	addi	$7, $0, 1
	j	eq_cont.36940
eq_else.36939:
	addi	$7, $0, 0
eq_cont.36940:
eq_cont.36936:
eq_cont.36926:
	bne	$7, $0, eq_else.36941
	lf	$f3, 16($6)
	fsub	$f3, $f3, $f1
	lf	$f4, 24($6)
	fmul	$f3, $f3, $f4
	lf	$f4, 0($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f0
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36942
	addi	$7, $0, 0
	j	eq_cont.36943
eq_else.36942:
	addi	$7, $0, 1
eq_cont.36943:
	bne	$7, $0, eq_else.36944
	fneg	$f4, $f4
	j	eq_cont.36945
eq_else.36944:
eq_cont.36945:
	lw	$7, 16($2)
	lf	$f5, 0($7)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36946
	addi	$7, $0, 0
	j	eq_cont.36947
eq_else.36946:
	addi	$7, $0, 1
eq_cont.36947:
	bne	$7, $0, eq_else.36948
	addi	$7, $0, 0
	j	eq_cont.36949
eq_else.36948:
	addi	$7, $0, 1
eq_cont.36949:
	bne	$7, $0, eq_else.36950
	addi	$7, $0, 0
	j	eq_cont.36951
eq_else.36950:
	lf	$f4, 16($5)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f2
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36952
	addi	$7, $0, 0
	j	eq_cont.36953
eq_else.36952:
	addi	$7, $0, 1
eq_cont.36953:
	bne	$7, $0, eq_else.36954
	fneg	$f4, $f4
	j	eq_cont.36955
eq_else.36954:
eq_cont.36955:
	lw	$7, 16($2)
	lf	$f5, 16($7)
	fsub	$f4, $f5, $f4
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	bne	$30, $0, eq_else.36956
	addi	$7, $0, 0
	j	eq_cont.36957
eq_else.36956:
	addi	$7, $0, 1
eq_cont.36957:
	bne	$7, $0, eq_else.36958
	addi	$7, $0, 0
	j	eq_cont.36959
eq_else.36958:
	addi	$7, $0, 1
eq_cont.36959:
	bne	$7, $0, eq_else.36960
	addi	$7, $0, 0
	j	eq_cont.36961
eq_else.36960:
	lf	$f4, 24($6)
	lf	$f5, 4888($0) # 0.000000
	sltf	$30, $f5, $f4
	sltf	$29, $f4, $f5
	bne	$30, $29, eq_else.36962
	addi	$7, $0, 1
	j	eq_cont.36963
eq_else.36962:
	addi	$7, $0, 0
eq_cont.36963:
	bne	$7, $0, eq_else.36964
	addi	$7, $0, 1
	j	eq_cont.36965
eq_else.36964:
	addi	$7, $0, 0
eq_cont.36965:
eq_cont.36961:
eq_cont.36951:
	bne	$7, $0, eq_else.36966
	lf	$f3, 32($6)
	fsub	$f2, $f3, $f2
	lf	$f3, 40($6)
	fmul	$f2, $f2, $f3
	lf	$f3, 0($5)
	fmul	$f3, $f2, $f3
	fadd	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36967
	addi	$7, $0, 0
	j	eq_cont.36968
eq_else.36967:
	addi	$7, $0, 1
eq_cont.36968:
	bne	$7, $0, eq_else.36969
	fneg	$f0, $f0
	j	eq_cont.36970
eq_else.36969:
eq_cont.36970:
	lw	$7, 16($2)
	lf	$f3, 0($7)
	fsub	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.36971
	addi	$7, $0, 0
	j	eq_cont.36972
eq_else.36971:
	addi	$7, $0, 1
eq_cont.36972:
	bne	$7, $0, eq_else.36973
	addi	$7, $0, 0
	j	eq_cont.36974
eq_else.36973:
	addi	$7, $0, 1
eq_cont.36974:
	bne	$7, $0, eq_else.36975
	addi	$2, $0, 0
	j	eq_cont.36976
eq_else.36975:
	lf	$f0, 8($5)
	fmul	$f0, $f2, $f0
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36977
	addi	$5, $0, 0
	j	eq_cont.36978
eq_else.36977:
	addi	$5, $0, 1
eq_cont.36978:
	bne	$5, $0, eq_else.36979
	fneg	$f0, $f0
	j	eq_cont.36980
eq_else.36979:
eq_cont.36980:
	lw	$2, 16($2)
	lf	$f1, 8($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36981
	addi	$2, $0, 0
	j	eq_cont.36982
eq_else.36981:
	addi	$2, $0, 1
eq_cont.36982:
	bne	$2, $0, eq_else.36983
	addi	$2, $0, 0
	j	eq_cont.36984
eq_else.36983:
	addi	$2, $0, 1
eq_cont.36984:
	bne	$2, $0, eq_else.36985
	addi	$2, $0, 0
	j	eq_cont.36986
eq_else.36985:
	lf	$f0, 40($6)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.36987
	addi	$2, $0, 1
	j	eq_cont.36988
eq_else.36987:
	addi	$2, $0, 0
eq_cont.36988:
	bne	$2, $0, eq_else.36989
	addi	$2, $0, 1
	j	eq_cont.36990
eq_else.36989:
	addi	$2, $0, 0
eq_cont.36990:
eq_cont.36986:
eq_cont.36976:
	bne	$2, $0, eq_else.36991
	addi	$2, $0, 0
	jr $31
eq_else.36991:
	addi	$2, $0, 3528
	sf	$f2, 0($2)
	addi	$2, $0, 3
	jr $31
eq_else.36966:
	addi	$2, $0, 3528
	sf	$f3, 0($2)
	addi	$2, $0, 2
	jr $31
eq_else.36941:
	addi	$2, $0, 3528
	sf	$f3, 0($2)
	addi	$2, $0, 1
	jr $31
solver_second_fast.2922:
	lf	$f3, 0($5)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.36992
	addi	$6, $0, 1
	j	eq_cont.36993
eq_else.36992:
	addi	$6, $0, 0
eq_cont.36993:
	bne	$6, $0, eq_else.36994
	lf	$f4, 8($5)
	fmul	$f4, $f4, $f0
	lf	$f5, 16($5)
	fmul	$f5, $f5, $f1
	fadd	$f4, $f4, $f5
	lf	$f5, 24($5)
	fmul	$f5, $f5, $f2
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f5, $f5, $f6
	fmul	$f6, $f1, $f1
	lw	$6, 16($2)
	lf	$f7, 8($6)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	lw	$6, 16($2)
	lf	$f7, 16($6)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	lw	$6, 12($2)
	bne	$6, $0, eq_else.36995
	movf	$f0, $f5
	j	eq_cont.36996
eq_else.36995:
	fmul	$f6, $f1, $f2
	lw	$6, 36($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f0
	lw	$6, 36($2)
	lf	$f6, 8($6)
	fmul	$f2, $f2, $f6
	fadd	$f2, $f5, $f2
	fmul	$f0, $f0, $f1
	lw	$6, 36($2)
	lf	$f1, 16($6)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.36996:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.36997
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.36998
eq_else.36997:
eq_cont.36998:
	fmul	$f1, $f4, $f4
	fmul	$f0, $f3, $f0
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.36999
	addi	$6, $0, 0
	j	eq_cont.37000
eq_else.36999:
	addi	$6, $0, 1
eq_cont.37000:
	bne	$6, $0, eq_else.37001
	addi	$2, $0, 0
	jr $31
eq_else.37001:
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37002
	sqrt	$f0, $f0
	fsub	$f0, $f4, $f0
	lf	$f1, 32($5)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	j	eq_cont.37003
eq_else.37002:
	sqrt	$f0, $f0
	fadd	$f0, $f4, $f0
	lf	$f1, 32($5)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
eq_cont.37003:
	addi	$2, $0, 1
	jr $31
eq_else.36994:
	addi	$2, $0, 0
	jr $31
solver_fast.2928:
	slli	$7, $2, 2
	addi	$7, $7, 3004
	lw	$7, 0($7)
	lf	$f0, 0($6)
	lw	$8, 20($7)
	lf	$f1, 0($8)
	fsub	$f0, $f0, $f1
	lf	$f1, 8($6)
	lw	$8, 20($7)
	lf	$f2, 8($8)
	fsub	$f1, $f1, $f2
	lf	$f2, 16($6)
	lw	$6, 20($7)
	lf	$f3, 16($6)
	fsub	$f2, $f2, $f3
	lw	$6, 4($5)
	slli	$2, $2, 2
	add	$30, $6, $2
	lw	$6, 0($30)
	lw	$2, 4($7)
	addi	$30, $0, 1
	bne	$2, $30, eq_else.37004
	lw	$5, 0($5)
	mov	$2, $7
	j	solver_rect_fast.2909
eq_else.37004:
	addi	$30, $0, 2
	bne	$2, $30, eq_else.37005
	lf	$f3, 0($6)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37006
	addi	$2, $0, 0
	j	eq_cont.37007
eq_else.37006:
	addi	$2, $0, 1
eq_cont.37007:
	bne	$2, $0, eq_else.37008
	addi	$2, $0, 0
	jr $31
eq_else.37008:
	lf	$f3, 8($6)
	fmul	$f0, $f3, $f0
	lf	$f3, 16($6)
	fmul	$f1, $f3, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.37005:
	lf	$f3, 0($6)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.37009
	addi	$2, $0, 1
	j	eq_cont.37010
eq_else.37009:
	addi	$2, $0, 0
eq_cont.37010:
	bne	$2, $0, eq_else.37011
	lf	$f4, 8($6)
	fmul	$f4, $f4, $f0
	lf	$f5, 16($6)
	fmul	$f5, $f5, $f1
	fadd	$f4, $f4, $f5
	lf	$f5, 24($6)
	fmul	$f5, $f5, $f2
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	lw	$2, 16($7)
	lf	$f6, 0($2)
	fmul	$f5, $f5, $f6
	fmul	$f6, $f1, $f1
	lw	$2, 16($7)
	lf	$f7, 8($2)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f6, $f2, $f2
	lw	$2, 16($7)
	lf	$f7, 16($2)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	lw	$2, 12($7)
	bne	$2, $0, eq_else.37012
	movf	$f0, $f5
	j	eq_cont.37013
eq_else.37012:
	fmul	$f6, $f1, $f2
	lw	$2, 36($7)
	lf	$f7, 0($2)
	fmul	$f6, $f6, $f7
	fadd	$f5, $f5, $f6
	fmul	$f2, $f2, $f0
	lw	$2, 36($7)
	lf	$f6, 8($2)
	fmul	$f2, $f2, $f6
	fadd	$f2, $f5, $f2
	fmul	$f0, $f0, $f1
	lw	$2, 36($7)
	lf	$f1, 16($2)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.37013:
	lw	$2, 4($7)
	addi	$30, $0, 3
	bne	$2, $30, eq_else.37014
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.37015
eq_else.37014:
eq_cont.37015:
	fmul	$f1, $f4, $f4
	fmul	$f0, $f3, $f0
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37016
	addi	$2, $0, 0
	j	eq_cont.37017
eq_else.37016:
	addi	$2, $0, 1
eq_cont.37017:
	bne	$2, $0, eq_else.37018
	addi	$2, $0, 0
	jr $31
eq_else.37018:
	lw	$2, 24($7)
	bne	$2, $0, eq_else.37019
	sqrt	$f0, $f0
	fsub	$f0, $f4, $f0
	lf	$f1, 32($6)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	j	eq_cont.37020
eq_else.37019:
	sqrt	$f0, $f0
	fadd	$f0, $f4, $f0
	lf	$f1, 32($6)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
eq_cont.37020:
	addi	$2, $0, 1
	jr $31
eq_else.37011:
	addi	$2, $0, 0
	jr $31
solver_fast2.2946:
	slli	$6, $2, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	lw	$7, 40($6)
	lf	$f0, 0($7)
	lf	$f1, 8($7)
	lf	$f2, 16($7)
	lw	$8, 4($5)
	slli	$2, $2, 2
	add	$30, $8, $2
	lw	$2, 0($30)
	lw	$8, 4($6)
	addi	$30, $0, 1
	bne	$8, $30, eq_else.37021
	lw	$5, 0($5)
	mov	$27, $6
	mov	$6, $2
	mov	$2, $27
	j	solver_rect_fast.2909
eq_else.37021:
	addi	$30, $0, 2
	bne	$8, $30, eq_else.37022
	lf	$f0, 0($2)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37023
	addi	$5, $0, 0
	j	eq_cont.37024
eq_else.37023:
	addi	$5, $0, 1
eq_cont.37024:
	bne	$5, $0, eq_else.37025
	addi	$2, $0, 0
	jr $31
eq_else.37025:
	lf	$f0, 0($2)
	lf	$f1, 24($7)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	addi	$2, $0, 1
	jr $31
eq_else.37022:
	lf	$f3, 0($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.37026
	addi	$5, $0, 1
	j	eq_cont.37027
eq_else.37026:
	addi	$5, $0, 0
eq_cont.37027:
	bne	$5, $0, eq_else.37028
	lf	$f4, 8($2)
	fmul	$f0, $f4, $f0
	lf	$f4, 16($2)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 24($7)
	fmul	$f2, $f0, $f0
	fmul	$f1, $f3, $f1
	fsub	$f1, $f2, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37029
	addi	$5, $0, 0
	j	eq_cont.37030
eq_else.37029:
	addi	$5, $0, 1
eq_cont.37030:
	bne	$5, $0, eq_else.37031
	addi	$2, $0, 0
	jr $31
eq_else.37031:
	lw	$5, 24($6)
	bne	$5, $0, eq_else.37032
	sqrt	$f1, $f1
	fsub	$f0, $f0, $f1
	lf	$f1, 32($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
	j	eq_cont.37033
eq_else.37032:
	sqrt	$f1, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 32($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3528
	sf	$f0, 0($2)
eq_cont.37033:
	addi	$2, $0, 1
	jr $31
eq_else.37028:
	addi	$2, $0, 0
	jr $31
setup_rect_table.2949:
	addi	$6, $0, 6
	lf	$f0, 4888($0) # 0.000000
	sw	$5, 0($3)
	sw	$2, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	lf	$f0, 0($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.37034
	addi	$6, $0, 1
	j	eq_cont.37035
eq_else.37034:
	addi	$6, $0, 0
eq_cont.37035:
	bne	$6, $0, eq_else.37036
	lw	$6, 0($3)
	lw	$7, 24($6)
	lf	$f0, 0($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37038
	addi	$8, $0, 0
	j	eq_cont.37039
eq_else.37038:
	addi	$8, $0, 1
eq_cont.37039:
	bne	$7, $0, eq_else.37040
	mov	$7, $8
	j	eq_cont.37041
eq_else.37040:
	bne	$8, $0, eq_else.37042
	addi	$7, $0, 1
	j	eq_cont.37043
eq_else.37042:
	addi	$7, $0, 0
eq_cont.37043:
eq_cont.37041:
	lw	$8, 16($6)
	lf	$f0, 0($8)
	bne	$7, $0, eq_else.37044
	fneg	$f0, $f0
	j	eq_cont.37045
eq_else.37044:
eq_cont.37045:
	sf	$f0, 0($2)
	lf	$f0, 4920($0) # 1.000000
	lf	$f1, 0($5)
	fdiv	$f0, $f0, $f1
	sf	$f0, 8($2)
	j	eq_cont.37037
eq_else.37036:
	lf	$f0, 4888($0) # 0.000000
	sf	$f0, 8($2)
eq_cont.37037:
	lf	$f0, 8($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.37046
	addi	$6, $0, 1
	j	eq_cont.37047
eq_else.37046:
	addi	$6, $0, 0
eq_cont.37047:
	bne	$6, $0, eq_else.37048
	lw	$6, 0($3)
	lw	$7, 24($6)
	lf	$f0, 8($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37050
	addi	$8, $0, 0
	j	eq_cont.37051
eq_else.37050:
	addi	$8, $0, 1
eq_cont.37051:
	bne	$7, $0, eq_else.37052
	mov	$7, $8
	j	eq_cont.37053
eq_else.37052:
	bne	$8, $0, eq_else.37054
	addi	$7, $0, 1
	j	eq_cont.37055
eq_else.37054:
	addi	$7, $0, 0
eq_cont.37055:
eq_cont.37053:
	lw	$8, 16($6)
	lf	$f0, 8($8)
	bne	$7, $0, eq_else.37056
	fneg	$f0, $f0
	j	eq_cont.37057
eq_else.37056:
eq_cont.37057:
	sf	$f0, 16($2)
	lf	$f0, 4920($0) # 1.000000
	lf	$f1, 8($5)
	fdiv	$f0, $f0, $f1
	sf	$f0, 24($2)
	j	eq_cont.37049
eq_else.37048:
	lf	$f0, 4888($0) # 0.000000
	sf	$f0, 24($2)
eq_cont.37049:
	lf	$f0, 16($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.37058
	addi	$6, $0, 1
	j	eq_cont.37059
eq_else.37058:
	addi	$6, $0, 0
eq_cont.37059:
	bne	$6, $0, eq_else.37060
	lw	$6, 0($3)
	lw	$7, 24($6)
	lf	$f0, 16($5)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37062
	addi	$8, $0, 0
	j	eq_cont.37063
eq_else.37062:
	addi	$8, $0, 1
eq_cont.37063:
	bne	$7, $0, eq_else.37064
	mov	$7, $8
	j	eq_cont.37065
eq_else.37064:
	bne	$8, $0, eq_else.37066
	addi	$7, $0, 1
	j	eq_cont.37067
eq_else.37066:
	addi	$7, $0, 0
eq_cont.37067:
eq_cont.37065:
	lw	$6, 16($6)
	lf	$f0, 16($6)
	bne	$7, $0, eq_else.37068
	fneg	$f0, $f0
	j	eq_cont.37069
eq_else.37068:
eq_cont.37069:
	sf	$f0, 32($2)
	lf	$f0, 4920($0) # 1.000000
	lf	$f1, 16($5)
	fdiv	$f0, $f0, $f1
	sf	$f0, 40($2)
	j	eq_cont.37061
eq_else.37060:
	lf	$f0, 4888($0) # 0.000000
	sf	$f0, 40($2)
eq_cont.37061:
	jr $31
setup_surface_table.2952:
	addi	$6, $0, 4
	lf	$f0, 4888($0) # 0.000000
	sw	$5, 0($3)
	sw	$2, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	lf	$f0, 0($5)
	lw	$6, 0($3)
	lw	$7, 16($6)
	lf	$f1, 0($7)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lw	$7, 16($6)
	lf	$f2, 8($7)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lw	$5, 16($6)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37070
	addi	$5, $0, 0
	j	eq_cont.37071
eq_else.37070:
	addi	$5, $0, 1
eq_cont.37071:
	bne	$5, $0, eq_else.37072
	lf	$f0, 4888($0) # 0.000000
	sf	$f0, 0($2)
	j	eq_cont.37073
eq_else.37072:
	lf	$f1, 4880($0) # -1.000000
	fdiv	$f1, $f1, $f0
	sf	$f1, 0($2)
	lw	$5, 16($6)
	lf	$f1, 0($5)
	fdiv	$f1, $f1, $f0
	fneg	$f1, $f1
	sf	$f1, 8($2)
	lw	$5, 16($6)
	lf	$f1, 8($5)
	fdiv	$f1, $f1, $f0
	fneg	$f1, $f1
	sf	$f1, 16($2)
	lw	$5, 16($6)
	lf	$f1, 16($5)
	fdiv	$f0, $f1, $f0
	fneg	$f0, $f0
	sf	$f0, 24($2)
eq_cont.37073:
	jr $31
setup_second_table.2955:
	addi	$6, $0, 5
	lf	$f0, 4888($0) # 0.000000
	sw	$5, 0($3)
	sw	$2, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	lf	$f0, 0($5)
	lf	$f1, 8($5)
	lf	$f2, 16($5)
	fmul	$f3, $f0, $f0
	lw	$6, 0($3)
	lw	$7, 16($6)
	lf	$f4, 0($7)
	fmul	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	lw	$7, 16($6)
	lf	$f5, 8($7)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f2, $f2
	lw	$7, 16($6)
	lf	$f5, 16($7)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$7, 12($6)
	bne	$7, $0, eq_else.37074
	movf	$f0, $f3
	j	eq_cont.37075
eq_else.37074:
	fmul	$f4, $f1, $f2
	lw	$7, 36($6)
	lf	$f5, 0($7)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f2, $f2, $f0
	lw	$7, 36($6)
	lf	$f4, 8($7)
	fmul	$f2, $f2, $f4
	fadd	$f2, $f3, $f2
	fmul	$f0, $f0, $f1
	lw	$7, 36($6)
	lf	$f1, 16($7)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.37075:
	lf	$f1, 0($5)
	lw	$7, 16($6)
	lf	$f2, 0($7)
	fmul	$f1, $f1, $f2
	fneg	$f1, $f1
	lf	$f2, 8($5)
	lw	$7, 16($6)
	lf	$f3, 8($7)
	fmul	$f2, $f2, $f3
	fneg	$f2, $f2
	lf	$f3, 16($5)
	lw	$7, 16($6)
	lf	$f4, 16($7)
	fmul	$f3, $f3, $f4
	fneg	$f3, $f3
	sf	$f0, 0($2)
	lw	$7, 12($6)
	bne	$7, $0, eq_else.37076
	sf	$f1, 8($2)
	sf	$f2, 16($2)
	sf	$f3, 24($2)
	j	eq_cont.37077
eq_else.37076:
	lf	$f4, 16($5)
	lw	$7, 36($6)
	lf	$f5, 8($7)
	fmul	$f4, $f4, $f5
	lf	$f5, 8($5)
	lw	$7, 36($6)
	lf	$f6, 16($7)
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	lf	$f5, 4944($0) # 0.500000
	fmul	$f4, $f4, $f5
	fsub	$f1, $f1, $f4
	sf	$f1, 8($2)
	lf	$f1, 16($5)
	lw	$7, 36($6)
	lf	$f4, 0($7)
	fmul	$f1, $f1, $f4
	lf	$f4, 0($5)
	lw	$7, 36($6)
	lf	$f5, 16($7)
	fmul	$f4, $f4, $f5
	fadd	$f1, $f1, $f4
	lf	$f4, 4944($0) # 0.500000
	fmul	$f1, $f1, $f4
	fsub	$f1, $f2, $f1
	sf	$f1, 16($2)
	lf	$f1, 8($5)
	lw	$7, 36($6)
	lf	$f2, 0($7)
	fmul	$f1, $f1, $f2
	lf	$f2, 0($5)
	lw	$5, 36($6)
	lf	$f4, 8($5)
	fmul	$f2, $f2, $f4
	fadd	$f1, $f1, $f2
	lf	$f2, 4944($0) # 0.500000
	fmul	$f1, $f1, $f2
	fsub	$f1, $f3, $f1
	sf	$f1, 24($2)
eq_cont.37077:
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.37078
	addi	$5, $0, 1
	j	eq_cont.37079
eq_else.37078:
	addi	$5, $0, 0
eq_cont.37079:
	bne	$5, $0, eq_else.37080
	lf	$f1, 4920($0) # 1.000000
	fdiv	$f0, $f1, $f0
	sf	$f0, 32($2)
	j	eq_cont.37081
eq_else.37080:
eq_cont.37081:
	jr $31
iter_setup_dirvec_constants.2958:
	bge	$5, $0, ge.37082
	jr $31
ge.37082:
	slli	$6, $5, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	lw	$7, 4($2)
	lw	$8, 0($2)
	lw	$9, 4($6)
	sw	$2, 0($3)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37084
	sw	$7, 4($3)
	sw	$5, 8($3)
	mov	$5, $6
	mov	$2, $8
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	setup_rect_table.2949
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 8($3)
	slli	$6, $5, 2
	lw	$7, 4($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.37085
eq_else.37084:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37086
	addi	$9, $0, 4
	lf	$f0, 4888($0) # 0.000000
	sw	$7, 4($3)
	sw	$5, 8($3)
	sw	$6, 12($3)
	sw	$8, 16($3)
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	lf	$f0, 0($5)
	lw	$6, 12($3)
	lw	$7, 16($6)
	lf	$f1, 0($7)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lw	$7, 16($6)
	lf	$f2, 8($7)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lw	$5, 16($6)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37088
	addi	$5, $0, 0
	j	eq_cont.37089
eq_else.37088:
	addi	$5, $0, 1
eq_cont.37089:
	bne	$5, $0, eq_else.37090
	lf	$f0, 4888($0) # 0.000000
	sf	$f0, 0($2)
	j	eq_cont.37091
eq_else.37090:
	lf	$f1, 4880($0) # -1.000000
	fdiv	$f1, $f1, $f0
	sf	$f1, 0($2)
	lw	$5, 16($6)
	lf	$f1, 0($5)
	fdiv	$f1, $f1, $f0
	fneg	$f1, $f1
	sf	$f1, 8($2)
	lw	$5, 16($6)
	lf	$f1, 8($5)
	fdiv	$f1, $f1, $f0
	fneg	$f1, $f1
	sf	$f1, 16($2)
	lw	$5, 16($6)
	lf	$f1, 16($5)
	fdiv	$f0, $f1, $f0
	fneg	$f0, $f0
	sf	$f0, 24($2)
eq_cont.37091:
	lw	$5, 8($3)
	slli	$6, $5, 2
	lw	$7, 4($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.37087
eq_else.37086:
	sw	$7, 4($3)
	sw	$5, 8($3)
	mov	$5, $6
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_second_table.2955
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 8($3)
	slli	$6, $5, 2
	lw	$7, 4($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.37087:
eq_cont.37085:
	addi	$2, $5, -1
	bge	$2, $0, ge.37092
	jr $31
ge.37092:
	slli	$5, $2, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lw	$6, 0($3)
	lw	$7, 4($6)
	lw	$8, 0($6)
	lw	$9, 4($5)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37094
	sw	$7, 20($3)
	sw	$2, 24($3)
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_rect_table.2949
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	slli	$6, $5, 2
	lw	$7, 20($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.37095
eq_else.37094:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37096
	sw	$7, 20($3)
	sw	$2, 24($3)
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_surface_table.2952
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	slli	$6, $5, 2
	lw	$7, 20($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.37097
eq_else.37096:
	sw	$7, 20($3)
	sw	$2, 24($3)
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_second_table.2955
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	slli	$6, $5, 2
	lw	$7, 20($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.37097:
eq_cont.37095:
	addi	$5, $5, -1
	lw	$2, 0($3)
	j	iter_setup_dirvec_constants.2958
setup_startp_constants.2963:
	bge	$5, $0, ge.37098
	jr $31
ge.37098:
	slli	$6, $5, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	lw	$7, 40($6)
	lw	$8, 4($6)
	lf	$f0, 0($2)
	lw	$9, 20($6)
	lf	$f1, 0($9)
	fsub	$f0, $f0, $f1
	sf	$f0, 0($7)
	lf	$f0, 8($2)
	lw	$9, 20($6)
	lf	$f1, 8($9)
	fsub	$f0, $f0, $f1
	sf	$f0, 8($7)
	lf	$f0, 16($2)
	lw	$9, 20($6)
	lf	$f1, 16($9)
	fsub	$f0, $f0, $f1
	sf	$f0, 16($7)
	addi	$30, $0, 2
	bne	$8, $30, eq_else.37100
	lw	$6, 16($6)
	lf	$f0, 0($7)
	lf	$f1, 8($7)
	lf	$f2, 16($7)
	lf	$f3, 0($6)
	fmul	$f0, $f3, $f0
	lf	$f3, 8($6)
	fmul	$f1, $f3, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 24($7)
	j	eq_cont.37101
eq_else.37100:
	addi	$30, $0, 2
	ble	$8, $30, le_else.37102
	lf	$f0, 0($7)
	lf	$f1, 8($7)
	lf	$f2, 16($7)
	fmul	$f3, $f0, $f0
	lw	$9, 16($6)
	lf	$f4, 0($9)
	fmul	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	lw	$9, 16($6)
	lf	$f5, 8($9)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f2, $f2
	lw	$9, 16($6)
	lf	$f5, 16($9)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$9, 12($6)
	bne	$9, $0, eq_else.37104
	movf	$f0, $f3
	j	eq_cont.37105
eq_else.37104:
	fmul	$f4, $f1, $f2
	lw	$9, 36($6)
	lf	$f5, 0($9)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f2, $f2, $f0
	lw	$9, 36($6)
	lf	$f4, 8($9)
	fmul	$f2, $f2, $f4
	fadd	$f2, $f3, $f2
	fmul	$f0, $f0, $f1
	lw	$6, 36($6)
	lf	$f1, 16($6)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.37105:
	addi	$30, $0, 3
	bne	$8, $30, eq_else.37106
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.37107
eq_else.37106:
eq_cont.37107:
	sf	$f0, 24($7)
	j	le_cont.37103
le_else.37102:
le_cont.37103:
eq_cont.37101:
	addi	$5, $5, -1
	j	setup_startp_constants.2963
is_rect_outside.2968:
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37108
	addi	$5, $0, 0
	j	eq_cont.37109
eq_else.37108:
	addi	$5, $0, 1
eq_cont.37109:
	bne	$5, $0, eq_else.37110
	fneg	$f0, $f0
	j	eq_cont.37111
eq_else.37110:
eq_cont.37111:
	lw	$5, 16($2)
	lf	$f3, 0($5)
	fsub	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37112
	addi	$5, $0, 0
	j	eq_cont.37113
eq_else.37112:
	addi	$5, $0, 1
eq_cont.37113:
	bne	$5, $0, eq_else.37114
	addi	$5, $0, 0
	j	eq_cont.37115
eq_else.37114:
	addi	$5, $0, 1
eq_cont.37115:
	bne	$5, $0, eq_else.37116
	addi	$5, $0, 0
	j	eq_cont.37117
eq_else.37116:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37118
	addi	$5, $0, 0
	j	eq_cont.37119
eq_else.37118:
	addi	$5, $0, 1
eq_cont.37119:
	bne	$5, $0, eq_else.37120
	fneg	$f0, $f1
	j	eq_cont.37121
eq_else.37120:
	movf	$f0, $f1
eq_cont.37121:
	lw	$5, 16($2)
	lf	$f1, 8($5)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37122
	addi	$5, $0, 0
	j	eq_cont.37123
eq_else.37122:
	addi	$5, $0, 1
eq_cont.37123:
	bne	$5, $0, eq_else.37124
	addi	$5, $0, 0
	j	eq_cont.37125
eq_else.37124:
	addi	$5, $0, 1
eq_cont.37125:
	bne	$5, $0, eq_else.37126
	addi	$5, $0, 0
	j	eq_cont.37127
eq_else.37126:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37128
	addi	$5, $0, 0
	j	eq_cont.37129
eq_else.37128:
	addi	$5, $0, 1
eq_cont.37129:
	bne	$5, $0, eq_else.37130
	fneg	$f0, $f2
	j	eq_cont.37131
eq_else.37130:
	movf	$f0, $f2
eq_cont.37131:
	lw	$5, 16($2)
	lf	$f1, 16($5)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37132
	addi	$5, $0, 0
	j	eq_cont.37133
eq_else.37132:
	addi	$5, $0, 1
eq_cont.37133:
	bne	$5, $0, eq_else.37134
	addi	$5, $0, 0
	j	eq_cont.37135
eq_else.37134:
	addi	$5, $0, 1
eq_cont.37135:
eq_cont.37127:
eq_cont.37117:
	bne	$5, $0, eq_else.37136
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37137
	addi	$2, $0, 1
	jr $31
eq_else.37137:
	addi	$2, $0, 0
	jr $31
eq_else.37136:
	lw	$2, 24($2)
	jr $31
is_outside.2983:
	lw	$5, 20($2)
	lf	$f3, 0($5)
	fsub	$f0, $f0, $f3
	lw	$5, 20($2)
	lf	$f3, 8($5)
	fsub	$f1, $f1, $f3
	lw	$5, 20($2)
	lf	$f3, 16($5)
	fsub	$f2, $f2, $f3
	lw	$5, 4($2)
	addi	$30, $0, 1
	bne	$5, $30, eq_else.37138
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37139
	addi	$5, $0, 0
	j	eq_cont.37140
eq_else.37139:
	addi	$5, $0, 1
eq_cont.37140:
	bne	$5, $0, eq_else.37141
	fneg	$f0, $f0
	j	eq_cont.37142
eq_else.37141:
eq_cont.37142:
	lw	$5, 16($2)
	lf	$f3, 0($5)
	fsub	$f0, $f3, $f0
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37143
	addi	$5, $0, 0
	j	eq_cont.37144
eq_else.37143:
	addi	$5, $0, 1
eq_cont.37144:
	bne	$5, $0, eq_else.37145
	addi	$5, $0, 0
	j	eq_cont.37146
eq_else.37145:
	addi	$5, $0, 1
eq_cont.37146:
	bne	$5, $0, eq_else.37147
	addi	$5, $0, 0
	j	eq_cont.37148
eq_else.37147:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37149
	addi	$5, $0, 0
	j	eq_cont.37150
eq_else.37149:
	addi	$5, $0, 1
eq_cont.37150:
	bne	$5, $0, eq_else.37151
	fneg	$f0, $f1
	j	eq_cont.37152
eq_else.37151:
	movf	$f0, $f1
eq_cont.37152:
	lw	$5, 16($2)
	lf	$f1, 8($5)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37153
	addi	$5, $0, 0
	j	eq_cont.37154
eq_else.37153:
	addi	$5, $0, 1
eq_cont.37154:
	bne	$5, $0, eq_else.37155
	addi	$5, $0, 0
	j	eq_cont.37156
eq_else.37155:
	addi	$5, $0, 1
eq_cont.37156:
	bne	$5, $0, eq_else.37157
	addi	$5, $0, 0
	j	eq_cont.37158
eq_else.37157:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37159
	addi	$5, $0, 0
	j	eq_cont.37160
eq_else.37159:
	addi	$5, $0, 1
eq_cont.37160:
	bne	$5, $0, eq_else.37161
	fneg	$f0, $f2
	j	eq_cont.37162
eq_else.37161:
	movf	$f0, $f2
eq_cont.37162:
	lw	$5, 16($2)
	lf	$f1, 16($5)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37163
	addi	$5, $0, 0
	j	eq_cont.37164
eq_else.37163:
	addi	$5, $0, 1
eq_cont.37164:
	bne	$5, $0, eq_else.37165
	addi	$5, $0, 0
	j	eq_cont.37166
eq_else.37165:
	addi	$5, $0, 1
eq_cont.37166:
eq_cont.37158:
eq_cont.37148:
	bne	$5, $0, eq_else.37167
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37168
	addi	$2, $0, 1
	jr $31
eq_else.37168:
	addi	$2, $0, 0
	jr $31
eq_else.37167:
	lw	$2, 24($2)
	jr $31
eq_else.37138:
	addi	$30, $0, 2
	bne	$5, $30, eq_else.37169
	lw	$5, 16($2)
	lf	$f3, 0($5)
	fmul	$f0, $f3, $f0
	lf	$f3, 8($5)
	fmul	$f1, $f3, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lw	$2, 24($2)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37170
	addi	$5, $0, 0
	j	eq_cont.37171
eq_else.37170:
	addi	$5, $0, 1
eq_cont.37171:
	bne	$2, $0, eq_else.37172
	mov	$2, $5
	j	eq_cont.37173
eq_else.37172:
	bne	$5, $0, eq_else.37174
	addi	$2, $0, 1
	j	eq_cont.37175
eq_else.37174:
	addi	$2, $0, 0
eq_cont.37175:
eq_cont.37173:
	bne	$2, $0, eq_else.37176
	addi	$2, $0, 1
	jr $31
eq_else.37176:
	addi	$2, $0, 0
	jr $31
eq_else.37169:
	fmul	$f3, $f0, $f0
	lw	$5, 16($2)
	lf	$f4, 0($5)
	fmul	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	lw	$5, 16($2)
	lf	$f5, 8($5)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f2, $f2
	lw	$5, 16($2)
	lf	$f5, 16($5)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$5, 12($2)
	bne	$5, $0, eq_else.37177
	movf	$f0, $f3
	j	eq_cont.37178
eq_else.37177:
	fmul	$f4, $f1, $f2
	lw	$5, 36($2)
	lf	$f5, 0($5)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f2, $f2, $f0
	lw	$5, 36($2)
	lf	$f4, 8($5)
	fmul	$f2, $f2, $f4
	fadd	$f2, $f3, $f2
	fmul	$f0, $f0, $f1
	lw	$5, 36($2)
	lf	$f1, 16($5)
	fmul	$f0, $f0, $f1
	fadd	$f0, $f2, $f0
eq_cont.37178:
	lw	$5, 4($2)
	addi	$30, $0, 3
	bne	$5, $30, eq_else.37179
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f0, $f1
	j	eq_cont.37180
eq_else.37179:
eq_cont.37180:
	lw	$2, 24($2)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37181
	addi	$5, $0, 0
	j	eq_cont.37182
eq_else.37181:
	addi	$5, $0, 1
eq_cont.37182:
	bne	$2, $0, eq_else.37183
	mov	$2, $5
	j	eq_cont.37184
eq_else.37183:
	bne	$5, $0, eq_else.37185
	addi	$2, $0, 1
	j	eq_cont.37186
eq_else.37185:
	addi	$2, $0, 0
eq_cont.37186:
eq_cont.37184:
	bne	$2, $0, eq_else.37187
	addi	$2, $0, 1
	jr $31
eq_else.37187:
	addi	$2, $0, 0
	jr $31
check_all_inside.2988:
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.37188
	addi	$2, $0, 1
	jr $31
eq_else.37188:
	slli	$6, $6, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	lw	$7, 20($6)
	lf	$f3, 0($7)
	fsub	$f3, $f0, $f3
	lw	$7, 20($6)
	lf	$f4, 8($7)
	fsub	$f4, $f1, $f4
	lw	$7, 20($6)
	lf	$f5, 16($7)
	fsub	$f5, $f2, $f5
	lw	$7, 4($6)
	addi	$30, $0, 1
	bne	$7, $30, eq_else.37189
	lf	$f6, 4888($0) # 0.000000
	sltf	$30, $f6, $f3
	bne	$30, $0, eq_else.37191
	addi	$7, $0, 0
	j	eq_cont.37192
eq_else.37191:
	addi	$7, $0, 1
eq_cont.37192:
	bne	$7, $0, eq_else.37193
	fneg	$f3, $f3
	j	eq_cont.37194
eq_else.37193:
eq_cont.37194:
	lw	$7, 16($6)
	lf	$f6, 0($7)
	fsub	$f3, $f6, $f3
	lf	$f6, 4888($0) # 0.000000
	sltf	$30, $f6, $f3
	bne	$30, $0, eq_else.37195
	addi	$7, $0, 0
	j	eq_cont.37196
eq_else.37195:
	addi	$7, $0, 1
eq_cont.37196:
	bne	$7, $0, eq_else.37197
	addi	$7, $0, 0
	j	eq_cont.37198
eq_else.37197:
	addi	$7, $0, 1
eq_cont.37198:
	bne	$7, $0, eq_else.37199
	addi	$7, $0, 0
	j	eq_cont.37200
eq_else.37199:
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37201
	addi	$7, $0, 0
	j	eq_cont.37202
eq_else.37201:
	addi	$7, $0, 1
eq_cont.37202:
	bne	$7, $0, eq_else.37203
	fneg	$f3, $f4
	j	eq_cont.37204
eq_else.37203:
	movf	$f3, $f4
eq_cont.37204:
	lw	$7, 16($6)
	lf	$f4, 8($7)
	fsub	$f3, $f4, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37205
	addi	$7, $0, 0
	j	eq_cont.37206
eq_else.37205:
	addi	$7, $0, 1
eq_cont.37206:
	bne	$7, $0, eq_else.37207
	addi	$7, $0, 0
	j	eq_cont.37208
eq_else.37207:
	addi	$7, $0, 1
eq_cont.37208:
	bne	$7, $0, eq_else.37209
	addi	$7, $0, 0
	j	eq_cont.37210
eq_else.37209:
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f5
	bne	$30, $0, eq_else.37211
	addi	$7, $0, 0
	j	eq_cont.37212
eq_else.37211:
	addi	$7, $0, 1
eq_cont.37212:
	bne	$7, $0, eq_else.37213
	fneg	$f3, $f5
	j	eq_cont.37214
eq_else.37213:
	movf	$f3, $f5
eq_cont.37214:
	lw	$7, 16($6)
	lf	$f4, 16($7)
	fsub	$f3, $f4, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37215
	addi	$7, $0, 0
	j	eq_cont.37216
eq_else.37215:
	addi	$7, $0, 1
eq_cont.37216:
	bne	$7, $0, eq_else.37217
	addi	$7, $0, 0
	j	eq_cont.37218
eq_else.37217:
	addi	$7, $0, 1
eq_cont.37218:
eq_cont.37210:
eq_cont.37200:
	bne	$7, $0, eq_else.37219
	lw	$6, 24($6)
	bne	$6, $0, eq_else.37221
	addi	$6, $0, 1
	j	eq_cont.37222
eq_else.37221:
	addi	$6, $0, 0
eq_cont.37222:
	j	eq_cont.37220
eq_else.37219:
	lw	$6, 24($6)
eq_cont.37220:
	j	eq_cont.37190
eq_else.37189:
	addi	$30, $0, 2
	bne	$7, $30, eq_else.37223
	lw	$7, 16($6)
	lf	$f6, 0($7)
	fmul	$f3, $f6, $f3
	lf	$f6, 8($7)
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	lf	$f4, 16($7)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$6, 24($6)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37225
	addi	$7, $0, 0
	j	eq_cont.37226
eq_else.37225:
	addi	$7, $0, 1
eq_cont.37226:
	bne	$6, $0, eq_else.37227
	mov	$6, $7
	j	eq_cont.37228
eq_else.37227:
	bne	$7, $0, eq_else.37229
	addi	$6, $0, 1
	j	eq_cont.37230
eq_else.37229:
	addi	$6, $0, 0
eq_cont.37230:
eq_cont.37228:
	bne	$6, $0, eq_else.37231
	addi	$6, $0, 1
	j	eq_cont.37232
eq_else.37231:
	addi	$6, $0, 0
eq_cont.37232:
	j	eq_cont.37224
eq_else.37223:
	fmul	$f6, $f3, $f3
	lw	$7, 16($6)
	lf	$f7, 0($7)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$7, 16($6)
	lf	$f8, 8($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$7, 16($6)
	lf	$f8, 16($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$7, 12($6)
	bne	$7, $0, eq_else.37233
	movf	$f3, $f6
	j	eq_cont.37234
eq_else.37233:
	fmul	$f7, $f4, $f5
	lw	$7, 36($6)
	lf	$f8, 0($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$7, 36($6)
	lf	$f7, 8($7)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$7, 36($6)
	lf	$f4, 16($7)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.37234:
	lw	$7, 4($6)
	addi	$30, $0, 3
	bne	$7, $30, eq_else.37235
	lf	$f4, 4920($0) # 1.000000
	fsub	$f3, $f3, $f4
	j	eq_cont.37236
eq_else.37235:
eq_cont.37236:
	lw	$6, 24($6)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37237
	addi	$7, $0, 0
	j	eq_cont.37238
eq_else.37237:
	addi	$7, $0, 1
eq_cont.37238:
	bne	$6, $0, eq_else.37239
	mov	$6, $7
	j	eq_cont.37240
eq_else.37239:
	bne	$7, $0, eq_else.37241
	addi	$6, $0, 1
	j	eq_cont.37242
eq_else.37241:
	addi	$6, $0, 0
eq_cont.37242:
eq_cont.37240:
	bne	$6, $0, eq_else.37243
	addi	$6, $0, 1
	j	eq_cont.37244
eq_else.37243:
	addi	$6, $0, 0
eq_cont.37244:
eq_cont.37224:
eq_cont.37190:
	bne	$6, $0, eq_else.37245
	addi	$2, $2, 1
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.37246
	addi	$2, $0, 1
	jr $31
eq_else.37246:
	slli	$6, $6, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	sf	$f2, 0($3)
	sf	$f1, 8($3)
	sf	$f0, 16($3)
	sw	$5, 24($3)
	sw	$2, 28($3)
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	is_outside.2983
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.37247
	lw	$2, 28($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 24($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37248
	addi	$2, $0, 1
	jr $31
eq_else.37248:
	slli	$5, $5, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lw	$7, 20($5)
	lf	$f0, 0($7)
	lf	$f1, 16($3)
	fsub	$f0, $f1, $f0
	lw	$7, 20($5)
	lf	$f2, 8($7)
	lf	$f3, 8($3)
	fsub	$f2, $f3, $f2
	lw	$7, 20($5)
	lf	$f4, 16($7)
	lf	$f5, 0($3)
	fsub	$f4, $f5, $f4
	lw	$7, 4($5)
	sw	$2, 32($3)
	addi	$30, $0, 1
	bne	$7, $30, eq_else.37249
	mov	$2, $5
	movf	$f1, $f2
	movf	$f2, $f4
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	is_rect_outside.2968
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.37250
eq_else.37249:
	addi	$30, $0, 2
	bne	$7, $30, eq_else.37251
	lw	$7, 16($5)
	lf	$f6, 0($7)
	fmul	$f0, $f6, $f0
	lf	$f6, 8($7)
	fmul	$f2, $f6, $f2
	fadd	$f0, $f0, $f2
	lf	$f2, 16($7)
	fmul	$f2, $f2, $f4
	fadd	$f0, $f0, $f2
	lw	$5, 24($5)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37253
	addi	$7, $0, 0
	j	eq_cont.37254
eq_else.37253:
	addi	$7, $0, 1
eq_cont.37254:
	bne	$5, $0, eq_else.37255
	mov	$5, $7
	j	eq_cont.37256
eq_else.37255:
	bne	$7, $0, eq_else.37257
	addi	$5, $0, 1
	j	eq_cont.37258
eq_else.37257:
	addi	$5, $0, 0
eq_cont.37258:
eq_cont.37256:
	bne	$5, $0, eq_else.37259
	addi	$2, $0, 1
	j	eq_cont.37260
eq_else.37259:
	addi	$2, $0, 0
eq_cont.37260:
	j	eq_cont.37252
eq_else.37251:
	fmul	$f6, $f0, $f0
	lw	$7, 16($5)
	lf	$f7, 0($7)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f2, $f2
	lw	$7, 16($5)
	lf	$f8, 8($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$7, 16($5)
	lf	$f8, 16($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$7, 12($5)
	bne	$7, $0, eq_else.37261
	movf	$f0, $f6
	j	eq_cont.37262
eq_else.37261:
	fmul	$f7, $f2, $f4
	lw	$7, 36($5)
	lf	$f8, 0($7)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f0
	lw	$7, 36($5)
	lf	$f7, 8($7)
	fmul	$f4, $f4, $f7
	fadd	$f4, $f6, $f4
	fmul	$f0, $f0, $f2
	lw	$7, 36($5)
	lf	$f2, 16($7)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f4, $f0
eq_cont.37262:
	lw	$7, 4($5)
	addi	$30, $0, 3
	bne	$7, $30, eq_else.37263
	lf	$f2, 4920($0) # 1.000000
	fsub	$f0, $f0, $f2
	j	eq_cont.37264
eq_else.37263:
eq_cont.37264:
	lw	$5, 24($5)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37265
	addi	$7, $0, 0
	j	eq_cont.37266
eq_else.37265:
	addi	$7, $0, 1
eq_cont.37266:
	bne	$5, $0, eq_else.37267
	mov	$5, $7
	j	eq_cont.37268
eq_else.37267:
	bne	$7, $0, eq_else.37269
	addi	$5, $0, 1
	j	eq_cont.37270
eq_else.37269:
	addi	$5, $0, 0
eq_cont.37270:
eq_cont.37268:
	bne	$5, $0, eq_else.37271
	addi	$2, $0, 1
	j	eq_cont.37272
eq_else.37271:
	addi	$2, $0, 0
eq_cont.37272:
eq_cont.37252:
eq_cont.37250:
	bne	$2, $0, eq_else.37273
	lw	$2, 32($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 24($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37274
	addi	$2, $0, 1
	jr $31
eq_else.37274:
	slli	$5, $5, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lf	$f0, 16($3)
	lf	$f1, 8($3)
	lf	$f2, 0($3)
	sw	$2, 36($3)
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	is_outside.2983
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.37275
	lw	$2, 36($3)
	addi	$2, $2, 1
	lf	$f0, 16($3)
	lf	$f1, 8($3)
	lf	$f2, 0($3)
	lw	$5, 24($3)
	j	check_all_inside.2988
eq_else.37275:
	addi	$2, $0, 0
	jr $31
eq_else.37273:
	addi	$2, $0, 0
	jr $31
eq_else.37247:
	addi	$2, $0, 0
	jr $31
eq_else.37245:
	addi	$2, $0, 0
	jr $31
shadow_check_and_group.2994:
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.37276
	addi	$2, $0, 0
	jr $31
eq_else.37276:
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	slli	$7, $6, 2
	addi	$7, $7, 3004
	lw	$7, 0($7)
	addi	$8, $0, 3548
	lf	$f0, 0($8)
	lw	$8, 20($7)
	lf	$f1, 0($8)
	fsub	$f0, $f0, $f1
	addi	$8, $0, 3556
	lf	$f1, 0($8)
	lw	$8, 20($7)
	lf	$f2, 8($8)
	fsub	$f1, $f1, $f2
	addi	$8, $0, 3564
	lf	$f2, 0($8)
	lw	$8, 20($7)
	lf	$f3, 16($8)
	fsub	$f2, $f2, $f3
	addi	$8, $0, 3860
	lw	$8, 4($8)
	slli	$9, $6, 2
	add	$30, $8, $9
	lw	$8, 0($30)
	lw	$9, 4($7)
	sw	$5, 0($3)
	sw	$2, 4($3)
	sw	$6, 8($3)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37277
	addi	$9, $0, 3860
	lw	$9, 0($9)
	mov	$6, $8
	mov	$5, $9
	mov	$2, $7
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solver_rect_fast.2909
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.37278
eq_else.37277:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37279
	lf	$f3, 0($8)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37281
	addi	$7, $0, 0
	j	eq_cont.37282
eq_else.37281:
	addi	$7, $0, 1
eq_cont.37282:
	bne	$7, $0, eq_else.37283
	addi	$2, $0, 0
	j	eq_cont.37284
eq_else.37283:
	lf	$f3, 8($8)
	fmul	$f0, $f3, $f0
	lf	$f3, 16($8)
	fmul	$f1, $f3, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($8)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$7, $0, 3528
	sf	$f0, 0($7)
	addi	$2, $0, 1
eq_cont.37284:
	j	eq_cont.37280
eq_else.37279:
	mov	$5, $8
	mov	$2, $7
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solver_second_fast.2922
	addi	$3, $3, -16
	lw	$31, 12($3)
eq_cont.37280:
eq_cont.37278:
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	bne	$2, $0, eq_else.37285
	addi	$2, $0, 0
	j	eq_cont.37286
eq_else.37285:
	lf	$f1, 4768($0) # -0.200000
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37287
	addi	$2, $0, 0
	j	eq_cont.37288
eq_else.37287:
	addi	$2, $0, 1
eq_cont.37288:
	bne	$2, $0, eq_else.37289
	addi	$2, $0, 0
	j	eq_cont.37290
eq_else.37289:
	addi	$2, $0, 1
eq_cont.37290:
eq_cont.37286:
	bne	$2, $0, eq_else.37291
	lw	$2, 8($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37292
	addi	$2, $0, 0
	jr $31
eq_else.37292:
	lw	$2, 4($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37293
	addi	$2, $0, 0
	jr $31
eq_else.37293:
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$7, $0, 3860
	addi	$8, $0, 3548
	sw	$2, 12($3)
	sw	$5, 16($3)
	mov	$6, $8
	mov	$2, $5
	mov	$5, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_fast.2928
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	bne	$2, $0, eq_else.37294
	addi	$2, $0, 0
	j	eq_cont.37295
eq_else.37294:
	lf	$f1, 4768($0) # -0.200000
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37296
	addi	$2, $0, 0
	j	eq_cont.37297
eq_else.37296:
	addi	$2, $0, 1
eq_cont.37297:
	bne	$2, $0, eq_else.37298
	addi	$2, $0, 0
	j	eq_cont.37299
eq_else.37298:
	addi	$2, $0, 1
eq_cont.37299:
eq_cont.37295:
	bne	$2, $0, eq_else.37300
	lw	$2, 16($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37301
	addi	$2, $0, 0
	jr $31
eq_else.37301:
	lw	$2, 12($3)
	addi	$2, $2, 1
	lw	$5, 0($3)
	j	shadow_check_and_group.2994
eq_else.37300:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f1, $f1, $f0
	addi	$2, $0, 3548
	lf	$f2, 0($2)
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f2, $f2, $f0
	addi	$2, $0, 3556
	lf	$f3, 0($2)
	fadd	$f2, $f2, $f3
	addi	$2, $0, 3308
	lf	$f3, 0($2)
	fmul	$f0, $f3, $f0
	addi	$2, $0, 3564
	lf	$f3, 0($2)
	fadd	$f0, $f0, $f3
	lw	$5, 0($3)
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37302
	addi	$2, $0, 1
	j	eq_cont.37303
eq_else.37302:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$6, 20($2)
	lf	$f3, 0($6)
	fsub	$f3, $f1, $f3
	lw	$6, 20($2)
	lf	$f4, 8($6)
	fsub	$f4, $f2, $f4
	lw	$6, 20($2)
	lf	$f5, 16($6)
	fsub	$f5, $f0, $f5
	lw	$6, 4($2)
	sf	$f0, 24($3)
	sf	$f2, 32($3)
	sf	$f1, 40($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37305
	movf	$f2, $f5
	movf	$f1, $f4
	movf	$f0, $f3
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	is_rect_outside.2968
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.37306
eq_else.37305:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37307
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f3, $f6, $f3
	lf	$f6, 8($6)
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	lf	$f4, 16($6)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37309
	addi	$6, $0, 0
	j	eq_cont.37310
eq_else.37309:
	addi	$6, $0, 1
eq_cont.37310:
	bne	$2, $0, eq_else.37311
	mov	$2, $6
	j	eq_cont.37312
eq_else.37311:
	bne	$6, $0, eq_else.37313
	addi	$2, $0, 1
	j	eq_cont.37314
eq_else.37313:
	addi	$2, $0, 0
eq_cont.37314:
eq_cont.37312:
	bne	$2, $0, eq_else.37315
	addi	$2, $0, 1
	j	eq_cont.37316
eq_else.37315:
	addi	$2, $0, 0
eq_cont.37316:
	j	eq_cont.37308
eq_else.37307:
	fmul	$f6, $f3, $f3
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.37317
	movf	$f3, $f6
	j	eq_cont.37318
eq_else.37317:
	fmul	$f7, $f4, $f5
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$6, 36($2)
	lf	$f4, 16($6)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.37318:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37319
	lf	$f4, 4920($0) # 1.000000
	fsub	$f3, $f3, $f4
	j	eq_cont.37320
eq_else.37319:
eq_cont.37320:
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37321
	addi	$6, $0, 0
	j	eq_cont.37322
eq_else.37321:
	addi	$6, $0, 1
eq_cont.37322:
	bne	$2, $0, eq_else.37323
	mov	$2, $6
	j	eq_cont.37324
eq_else.37323:
	bne	$6, $0, eq_else.37325
	addi	$2, $0, 1
	j	eq_cont.37326
eq_else.37325:
	addi	$2, $0, 0
eq_cont.37326:
eq_cont.37324:
	bne	$2, $0, eq_else.37327
	addi	$2, $0, 1
	j	eq_cont.37328
eq_else.37327:
	addi	$2, $0, 0
eq_cont.37328:
eq_cont.37308:
eq_cont.37306:
	bne	$2, $0, eq_else.37329
	lw	$5, 0($3)
	lw	$2, 4($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37331
	addi	$2, $0, 1
	j	eq_cont.37332
eq_else.37331:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	is_outside.2983
	addi	$3, $3, -56
	lw	$31, 52($3)
	bne	$2, $0, eq_else.37333
	addi	$2, $0, 2
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	lw	$5, 0($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	check_all_inside.2988
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.37334
eq_else.37333:
	addi	$2, $0, 0
eq_cont.37334:
eq_cont.37332:
	j	eq_cont.37330
eq_else.37329:
	addi	$2, $0, 0
eq_cont.37330:
eq_cont.37303:
	bne	$2, $0, eq_else.37335
	lw	$2, 12($3)
	addi	$2, $2, 1
	lw	$5, 0($3)
	j	shadow_check_and_group.2994
eq_else.37335:
	addi	$2, $0, 1
	jr $31
eq_else.37291:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f1, $f1, $f0
	addi	$2, $0, 3548
	lf	$f2, 0($2)
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f2, $f2, $f0
	addi	$2, $0, 3556
	lf	$f3, 0($2)
	fadd	$f2, $f2, $f3
	addi	$2, $0, 3308
	lf	$f3, 0($2)
	fmul	$f0, $f3, $f0
	addi	$2, $0, 3564
	lf	$f3, 0($2)
	fadd	$f0, $f0, $f3
	lw	$2, 0($3)
	lw	$5, 0($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37336
	addi	$2, $0, 1
	j	eq_cont.37337
eq_else.37336:
	slli	$5, $5, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	sf	$f0, 48($3)
	sf	$f2, 56($3)
	sf	$f1, 64($3)
	mov	$2, $5
	movf	$f30, $f2
	movf	$f2, $f0
	movf	$f0, $f1
	movf	$f1, $f30
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	is_outside.2983
	addi	$3, $3, -80
	lw	$31, 76($3)
	bne	$2, $0, eq_else.37338
	lw	$2, 0($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37340
	addi	$2, $0, 1
	j	eq_cont.37341
eq_else.37340:
	slli	$5, $5, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lw	$6, 20($5)
	lf	$f0, 0($6)
	lf	$f1, 64($3)
	fsub	$f0, $f1, $f0
	lw	$6, 20($5)
	lf	$f2, 8($6)
	lf	$f3, 56($3)
	fsub	$f2, $f3, $f2
	lw	$6, 20($5)
	lf	$f4, 16($6)
	lf	$f5, 48($3)
	fsub	$f4, $f5, $f4
	lw	$6, 4($5)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37342
	mov	$2, $5
	movf	$f1, $f2
	movf	$f2, $f4
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	is_rect_outside.2968
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.37343
eq_else.37342:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37344
	lw	$6, 16($5)
	lf	$f6, 0($6)
	fmul	$f0, $f6, $f0
	lf	$f6, 8($6)
	fmul	$f2, $f6, $f2
	fadd	$f0, $f0, $f2
	lf	$f2, 16($6)
	fmul	$f2, $f2, $f4
	fadd	$f0, $f0, $f2
	lw	$5, 24($5)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37346
	addi	$6, $0, 0
	j	eq_cont.37347
eq_else.37346:
	addi	$6, $0, 1
eq_cont.37347:
	bne	$5, $0, eq_else.37348
	mov	$5, $6
	j	eq_cont.37349
eq_else.37348:
	bne	$6, $0, eq_else.37350
	addi	$5, $0, 1
	j	eq_cont.37351
eq_else.37350:
	addi	$5, $0, 0
eq_cont.37351:
eq_cont.37349:
	bne	$5, $0, eq_else.37352
	addi	$2, $0, 1
	j	eq_cont.37353
eq_else.37352:
	addi	$2, $0, 0
eq_cont.37353:
	j	eq_cont.37345
eq_else.37344:
	fmul	$f6, $f0, $f0
	lw	$6, 16($5)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f2, $f2
	lw	$6, 16($5)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($5)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($5)
	bne	$6, $0, eq_else.37354
	movf	$f0, $f6
	j	eq_cont.37355
eq_else.37354:
	fmul	$f7, $f2, $f4
	lw	$6, 36($5)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f0
	lw	$6, 36($5)
	lf	$f7, 8($6)
	fmul	$f4, $f4, $f7
	fadd	$f4, $f6, $f4
	fmul	$f0, $f0, $f2
	lw	$6, 36($5)
	lf	$f2, 16($6)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f4, $f0
eq_cont.37355:
	lw	$6, 4($5)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37356
	lf	$f2, 4920($0) # 1.000000
	fsub	$f0, $f0, $f2
	j	eq_cont.37357
eq_else.37356:
eq_cont.37357:
	lw	$5, 24($5)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37358
	addi	$6, $0, 0
	j	eq_cont.37359
eq_else.37358:
	addi	$6, $0, 1
eq_cont.37359:
	bne	$5, $0, eq_else.37360
	mov	$5, $6
	j	eq_cont.37361
eq_else.37360:
	bne	$6, $0, eq_else.37362
	addi	$5, $0, 1
	j	eq_cont.37363
eq_else.37362:
	addi	$5, $0, 0
eq_cont.37363:
eq_cont.37361:
	bne	$5, $0, eq_else.37364
	addi	$2, $0, 1
	j	eq_cont.37365
eq_else.37364:
	addi	$2, $0, 0
eq_cont.37365:
eq_cont.37345:
eq_cont.37343:
	bne	$2, $0, eq_else.37366
	lw	$2, 0($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37368
	addi	$2, $0, 1
	j	eq_cont.37369
eq_else.37368:
	slli	$5, $5, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lf	$f0, 64($3)
	lf	$f1, 56($3)
	lf	$f2, 48($3)
	mov	$2, $5
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	is_outside.2983
	addi	$3, $3, -80
	lw	$31, 76($3)
	bne	$2, $0, eq_else.37370
	addi	$2, $0, 3
	lf	$f0, 64($3)
	lf	$f1, 56($3)
	lf	$f2, 48($3)
	lw	$5, 0($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	check_all_inside.2988
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.37371
eq_else.37370:
	addi	$2, $0, 0
eq_cont.37371:
eq_cont.37369:
	j	eq_cont.37367
eq_else.37366:
	addi	$2, $0, 0
eq_cont.37367:
eq_cont.37341:
	j	eq_cont.37339
eq_else.37338:
	addi	$2, $0, 0
eq_cont.37339:
eq_cont.37337:
	bne	$2, $0, eq_else.37372
	lw	$2, 4($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37373
	addi	$2, $0, 0
	jr $31
eq_else.37373:
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$7, $0, 3860
	addi	$8, $0, 3548
	sw	$2, 72($3)
	sw	$5, 76($3)
	mov	$6, $8
	mov	$2, $5
	mov	$5, $7
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	solver_fast.2928
	addi	$3, $3, -88
	lw	$31, 84($3)
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	bne	$2, $0, eq_else.37374
	addi	$2, $0, 0
	j	eq_cont.37375
eq_else.37374:
	lf	$f1, 4768($0) # -0.200000
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37376
	addi	$2, $0, 0
	j	eq_cont.37377
eq_else.37376:
	addi	$2, $0, 1
eq_cont.37377:
	bne	$2, $0, eq_else.37378
	addi	$2, $0, 0
	j	eq_cont.37379
eq_else.37378:
	addi	$2, $0, 1
eq_cont.37379:
eq_cont.37375:
	bne	$2, $0, eq_else.37380
	lw	$2, 76($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37381
	addi	$2, $0, 0
	jr $31
eq_else.37381:
	lw	$2, 72($3)
	addi	$2, $2, 1
	lw	$5, 0($3)
	j	shadow_check_and_group.2994
eq_else.37380:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f1, $f1, $f0
	addi	$2, $0, 3548
	lf	$f2, 0($2)
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f2, $f2, $f0
	addi	$2, $0, 3556
	lf	$f3, 0($2)
	fadd	$f2, $f2, $f3
	addi	$2, $0, 3308
	lf	$f3, 0($2)
	fmul	$f0, $f3, $f0
	addi	$2, $0, 3564
	lf	$f3, 0($2)
	fadd	$f0, $f0, $f3
	lw	$5, 0($3)
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37382
	addi	$2, $0, 1
	j	eq_cont.37383
eq_else.37382:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$6, 20($2)
	lf	$f3, 0($6)
	fsub	$f3, $f1, $f3
	lw	$6, 20($2)
	lf	$f4, 8($6)
	fsub	$f4, $f2, $f4
	lw	$6, 20($2)
	lf	$f5, 16($6)
	fsub	$f5, $f0, $f5
	lw	$6, 4($2)
	sf	$f0, 80($3)
	sf	$f2, 88($3)
	sf	$f1, 96($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37384
	movf	$f2, $f5
	movf	$f1, $f4
	movf	$f0, $f3
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	is_rect_outside.2968
	addi	$3, $3, -112
	lw	$31, 108($3)
	j	eq_cont.37385
eq_else.37384:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37386
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f3, $f6, $f3
	lf	$f6, 8($6)
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	lf	$f4, 16($6)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37388
	addi	$6, $0, 0
	j	eq_cont.37389
eq_else.37388:
	addi	$6, $0, 1
eq_cont.37389:
	bne	$2, $0, eq_else.37390
	mov	$2, $6
	j	eq_cont.37391
eq_else.37390:
	bne	$6, $0, eq_else.37392
	addi	$2, $0, 1
	j	eq_cont.37393
eq_else.37392:
	addi	$2, $0, 0
eq_cont.37393:
eq_cont.37391:
	bne	$2, $0, eq_else.37394
	addi	$2, $0, 1
	j	eq_cont.37395
eq_else.37394:
	addi	$2, $0, 0
eq_cont.37395:
	j	eq_cont.37387
eq_else.37386:
	fmul	$f6, $f3, $f3
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.37396
	movf	$f3, $f6
	j	eq_cont.37397
eq_else.37396:
	fmul	$f7, $f4, $f5
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$6, 36($2)
	lf	$f4, 16($6)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.37397:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37398
	lf	$f4, 4920($0) # 1.000000
	fsub	$f3, $f3, $f4
	j	eq_cont.37399
eq_else.37398:
eq_cont.37399:
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37400
	addi	$6, $0, 0
	j	eq_cont.37401
eq_else.37400:
	addi	$6, $0, 1
eq_cont.37401:
	bne	$2, $0, eq_else.37402
	mov	$2, $6
	j	eq_cont.37403
eq_else.37402:
	bne	$6, $0, eq_else.37404
	addi	$2, $0, 1
	j	eq_cont.37405
eq_else.37404:
	addi	$2, $0, 0
eq_cont.37405:
eq_cont.37403:
	bne	$2, $0, eq_else.37406
	addi	$2, $0, 1
	j	eq_cont.37407
eq_else.37406:
	addi	$2, $0, 0
eq_cont.37407:
eq_cont.37387:
eq_cont.37385:
	bne	$2, $0, eq_else.37408
	lw	$5, 0($3)
	lw	$2, 4($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37410
	addi	$2, $0, 1
	j	eq_cont.37411
eq_else.37410:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 96($3)
	lf	$f1, 88($3)
	lf	$f2, 80($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	is_outside.2983
	addi	$3, $3, -112
	lw	$31, 108($3)
	bne	$2, $0, eq_else.37412
	addi	$2, $0, 2
	lf	$f0, 96($3)
	lf	$f1, 88($3)
	lf	$f2, 80($3)
	lw	$5, 0($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	check_all_inside.2988
	addi	$3, $3, -112
	lw	$31, 108($3)
	j	eq_cont.37413
eq_else.37412:
	addi	$2, $0, 0
eq_cont.37413:
eq_cont.37411:
	j	eq_cont.37409
eq_else.37408:
	addi	$2, $0, 0
eq_cont.37409:
eq_cont.37383:
	bne	$2, $0, eq_else.37414
	lw	$2, 72($3)
	addi	$2, $2, 1
	lw	$5, 0($3)
	j	shadow_check_and_group.2994
eq_else.37414:
	addi	$2, $0, 1
	jr $31
eq_else.37372:
	addi	$2, $0, 1
	jr $31
shadow_check_one_or_group.2997:
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.37415
	addi	$2, $0, 0
	jr $31
eq_else.37415:
	slli	$6, $6, 2
	addi	$6, $6, 3324
	lw	$6, 0($6)
	lw	$7, 0($6)
	sw	$5, 0($3)
	sw	$2, 4($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37416
	addi	$2, $0, 0
	j	eq_cont.37417
eq_else.37416:
	lw	$7, 0($6)
	addi	$8, $0, 3860
	addi	$9, $0, 3548
	sw	$6, 8($3)
	sw	$7, 12($3)
	mov	$6, $9
	mov	$5, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_fast.2928
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	bne	$2, $0, eq_else.37418
	addi	$2, $0, 0
	j	eq_cont.37419
eq_else.37418:
	lf	$f1, 4768($0) # -0.200000
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37420
	addi	$2, $0, 0
	j	eq_cont.37421
eq_else.37420:
	addi	$2, $0, 1
eq_cont.37421:
	bne	$2, $0, eq_else.37422
	addi	$2, $0, 0
	j	eq_cont.37423
eq_else.37422:
	addi	$2, $0, 1
eq_cont.37423:
eq_cont.37419:
	bne	$2, $0, eq_else.37424
	lw	$2, 12($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37426
	addi	$2, $0, 0
	j	eq_cont.37427
eq_else.37426:
	addi	$2, $0, 1
	lw	$5, 8($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	shadow_check_and_group.2994
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37427:
	j	eq_cont.37425
eq_else.37424:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f1, $f1, $f0
	addi	$2, $0, 3548
	lf	$f2, 0($2)
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f2, $f2, $f0
	addi	$2, $0, 3556
	lf	$f3, 0($2)
	fadd	$f2, $f2, $f3
	addi	$2, $0, 3308
	lf	$f3, 0($2)
	fmul	$f0, $f3, $f0
	addi	$2, $0, 3564
	lf	$f3, 0($2)
	fadd	$f0, $f0, $f3
	lw	$5, 8($3)
	lw	$2, 0($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37428
	addi	$2, $0, 1
	j	eq_cont.37429
eq_else.37428:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$6, 20($2)
	lf	$f3, 0($6)
	fsub	$f3, $f1, $f3
	lw	$6, 20($2)
	lf	$f4, 8($6)
	fsub	$f4, $f2, $f4
	lw	$6, 20($2)
	lf	$f5, 16($6)
	fsub	$f5, $f0, $f5
	lw	$6, 4($2)
	sf	$f0, 16($3)
	sf	$f2, 24($3)
	sf	$f1, 32($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37430
	movf	$f2, $f5
	movf	$f1, $f4
	movf	$f0, $f3
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	is_rect_outside.2968
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.37431
eq_else.37430:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37432
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f3, $f6, $f3
	lf	$f6, 8($6)
	fmul	$f4, $f6, $f4
	fadd	$f3, $f3, $f4
	lf	$f4, 16($6)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37434
	addi	$6, $0, 0
	j	eq_cont.37435
eq_else.37434:
	addi	$6, $0, 1
eq_cont.37435:
	bne	$2, $0, eq_else.37436
	mov	$2, $6
	j	eq_cont.37437
eq_else.37436:
	bne	$6, $0, eq_else.37438
	addi	$2, $0, 1
	j	eq_cont.37439
eq_else.37438:
	addi	$2, $0, 0
eq_cont.37439:
eq_cont.37437:
	bne	$2, $0, eq_else.37440
	addi	$2, $0, 1
	j	eq_cont.37441
eq_else.37440:
	addi	$2, $0, 0
eq_cont.37441:
	j	eq_cont.37433
eq_else.37432:
	fmul	$f6, $f3, $f3
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f5, $f5
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.37442
	movf	$f3, $f6
	j	eq_cont.37443
eq_else.37442:
	fmul	$f7, $f4, $f5
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f5, $f5, $f3
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f5, $f5, $f7
	fadd	$f5, $f6, $f5
	fmul	$f3, $f3, $f4
	lw	$6, 36($2)
	lf	$f4, 16($6)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f5, $f3
eq_cont.37443:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37444
	lf	$f4, 4920($0) # 1.000000
	fsub	$f3, $f3, $f4
	j	eq_cont.37445
eq_else.37444:
eq_cont.37445:
	lw	$2, 24($2)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37446
	addi	$6, $0, 0
	j	eq_cont.37447
eq_else.37446:
	addi	$6, $0, 1
eq_cont.37447:
	bne	$2, $0, eq_else.37448
	mov	$2, $6
	j	eq_cont.37449
eq_else.37448:
	bne	$6, $0, eq_else.37450
	addi	$2, $0, 1
	j	eq_cont.37451
eq_else.37450:
	addi	$2, $0, 0
eq_cont.37451:
eq_cont.37449:
	bne	$2, $0, eq_else.37452
	addi	$2, $0, 1
	j	eq_cont.37453
eq_else.37452:
	addi	$2, $0, 0
eq_cont.37453:
eq_cont.37433:
eq_cont.37431:
	bne	$2, $0, eq_else.37454
	lw	$5, 8($3)
	lw	$2, 4($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37456
	addi	$2, $0, 1
	j	eq_cont.37457
eq_else.37456:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 32($3)
	lf	$f1, 24($3)
	lf	$f2, 16($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	is_outside.2983
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.37458
	addi	$2, $0, 2
	lf	$f0, 32($3)
	lf	$f1, 24($3)
	lf	$f2, 16($3)
	lw	$5, 8($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	check_all_inside.2988
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.37459
eq_else.37458:
	addi	$2, $0, 0
eq_cont.37459:
eq_cont.37457:
	j	eq_cont.37455
eq_else.37454:
	addi	$2, $0, 0
eq_cont.37455:
eq_cont.37429:
	bne	$2, $0, eq_else.37460
	addi	$2, $0, 1
	lw	$5, 8($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	shadow_check_and_group.2994
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.37461
eq_else.37460:
	addi	$2, $0, 1
eq_cont.37461:
eq_cont.37425:
eq_cont.37417:
	bne	$2, $0, eq_else.37462
	lw	$2, 4($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37463
	addi	$2, $0, 0
	jr $31
eq_else.37463:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	sw	$2, 40($3)
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	shadow_check_and_group.2994
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.37464
	lw	$2, 40($3)
	addi	$2, $2, 1
	lw	$5, 0($3)
	j	shadow_check_one_or_group.2997
eq_else.37464:
	addi	$2, $0, 1
	jr $31
eq_else.37462:
	addi	$2, $0, 1
	jr $31
shadow_check_one_or_matrix.3000:
	slli	$6, $2, 2
	add	$30, $5, $6
	lw	$6, 0($30)
	lw	$7, 0($6)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37465
	addi	$2, $0, 0
	jr $31
eq_else.37465:
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.37466
	addi	$2, $0, 1
	j	eq_cont.37467
eq_else.37466:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	addi	$9, $0, 3548
	lf	$f0, 0($9)
	lw	$9, 20($8)
	lf	$f1, 0($9)
	fsub	$f0, $f0, $f1
	addi	$9, $0, 3556
	lf	$f1, 0($9)
	lw	$9, 20($8)
	lf	$f2, 8($9)
	fsub	$f1, $f1, $f2
	addi	$9, $0, 3564
	lf	$f2, 0($9)
	lw	$9, 20($8)
	lf	$f3, 16($9)
	fsub	$f2, $f2, $f3
	addi	$9, $0, 3860
	lw	$9, 4($9)
	slli	$7, $7, 2
	add	$30, $9, $7
	lw	$7, 0($30)
	lw	$9, 4($8)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37468
	addi	$9, $0, 3860
	lw	$9, 0($9)
	mov	$6, $7
	mov	$5, $9
	mov	$2, $8
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solver_rect_fast.2909
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.37469
eq_else.37468:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37470
	lf	$f3, 0($7)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f3, $f4
	bne	$30, $0, eq_else.37472
	addi	$8, $0, 0
	j	eq_cont.37473
eq_else.37472:
	addi	$8, $0, 1
eq_cont.37473:
	bne	$8, $0, eq_else.37474
	addi	$2, $0, 0
	j	eq_cont.37475
eq_else.37474:
	lf	$f3, 8($7)
	fmul	$f0, $f3, $f0
	lf	$f3, 16($7)
	fmul	$f1, $f3, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($7)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$7, $0, 3528
	sf	$f0, 0($7)
	addi	$2, $0, 1
eq_cont.37475:
	j	eq_cont.37471
eq_else.37470:
	mov	$5, $7
	mov	$2, $8
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solver_second_fast.2922
	addi	$3, $3, -16
	lw	$31, 12($3)
eq_cont.37471:
eq_cont.37469:
	bne	$2, $0, eq_else.37476
	addi	$2, $0, 0
	j	eq_cont.37477
eq_else.37476:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37478
	addi	$2, $0, 0
	j	eq_cont.37479
eq_else.37478:
	addi	$2, $0, 1
eq_cont.37479:
	bne	$2, $0, eq_else.37480
	addi	$2, $0, 0
	j	eq_cont.37481
eq_else.37480:
	addi	$2, $0, 1
eq_cont.37481:
	bne	$2, $0, eq_else.37482
	addi	$2, $0, 0
	j	eq_cont.37483
eq_else.37482:
	lw	$2, 0($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37484
	addi	$2, $0, 0
	j	eq_cont.37485
eq_else.37484:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	shadow_check_and_group.2994
	addi	$3, $3, -16
	lw	$31, 12($3)
	bne	$2, $0, eq_else.37486
	addi	$2, $0, 2
	lw	$5, 0($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -16
	lw	$31, 12($3)
	j	eq_cont.37487
eq_else.37486:
	addi	$2, $0, 1
eq_cont.37487:
eq_cont.37485:
	bne	$2, $0, eq_else.37488
	addi	$2, $0, 0
	j	eq_cont.37489
eq_else.37488:
	addi	$2, $0, 1
eq_cont.37489:
eq_cont.37483:
eq_cont.37477:
eq_cont.37467:
	bne	$2, $0, eq_else.37490
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$7, 0($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37491
	addi	$2, $0, 0
	jr $31
eq_else.37491:
	sw	$5, 12($3)
	sw	$2, 16($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.37492
	addi	$2, $0, 1
	j	eq_cont.37493
eq_else.37492:
	addi	$8, $0, 3860
	addi	$9, $0, 3548
	mov	$6, $9
	mov	$5, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_fast.2928
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.37494
	addi	$2, $0, 0
	j	eq_cont.37495
eq_else.37494:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37496
	addi	$2, $0, 0
	j	eq_cont.37497
eq_else.37496:
	addi	$2, $0, 1
eq_cont.37497:
	bne	$2, $0, eq_else.37498
	addi	$2, $0, 0
	j	eq_cont.37499
eq_else.37498:
	addi	$2, $0, 1
eq_cont.37499:
	bne	$2, $0, eq_else.37500
	addi	$2, $0, 0
	j	eq_cont.37501
eq_else.37500:
	addi	$2, $0, 1
	lw	$5, 12($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.37502
	addi	$2, $0, 0
	j	eq_cont.37503
eq_else.37502:
	addi	$2, $0, 1
eq_cont.37503:
eq_cont.37501:
eq_cont.37495:
eq_cont.37493:
	bne	$2, $0, eq_else.37504
	lw	$2, 16($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	j	shadow_check_one_or_matrix.3000
eq_else.37504:
	addi	$2, $0, 1
	lw	$5, 12($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.37505
	lw	$2, 16($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	j	shadow_check_one_or_matrix.3000
eq_else.37505:
	addi	$2, $0, 1
	jr $31
eq_else.37490:
	lw	$2, 0($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37506
	addi	$2, $0, 0
	j	eq_cont.37507
eq_else.37506:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	mov	$2, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	shadow_check_and_group.2994
	addi	$3, $3, -24
	lw	$31, 20($3)
	bne	$2, $0, eq_else.37508
	addi	$2, $0, 2
	lw	$5, 0($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.37509
eq_else.37508:
	addi	$2, $0, 1
eq_cont.37509:
eq_cont.37507:
	bne	$2, $0, eq_else.37510
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$7, 0($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37511
	addi	$2, $0, 0
	jr $31
eq_else.37511:
	sw	$5, 20($3)
	sw	$2, 24($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.37512
	addi	$2, $0, 1
	j	eq_cont.37513
eq_else.37512:
	addi	$8, $0, 3860
	addi	$9, $0, 3548
	mov	$6, $9
	mov	$5, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solver_fast.2928
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.37514
	addi	$2, $0, 0
	j	eq_cont.37515
eq_else.37514:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37516
	addi	$2, $0, 0
	j	eq_cont.37517
eq_else.37516:
	addi	$2, $0, 1
eq_cont.37517:
	bne	$2, $0, eq_else.37518
	addi	$2, $0, 0
	j	eq_cont.37519
eq_else.37518:
	addi	$2, $0, 1
eq_cont.37519:
	bne	$2, $0, eq_else.37520
	addi	$2, $0, 0
	j	eq_cont.37521
eq_else.37520:
	addi	$2, $0, 1
	lw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.37522
	addi	$2, $0, 0
	j	eq_cont.37523
eq_else.37522:
	addi	$2, $0, 1
eq_cont.37523:
eq_cont.37521:
eq_cont.37515:
eq_cont.37513:
	bne	$2, $0, eq_else.37524
	lw	$2, 24($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	j	shadow_check_one_or_matrix.3000
eq_else.37524:
	addi	$2, $0, 1
	lw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.37525
	lw	$2, 24($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	j	shadow_check_one_or_matrix.3000
eq_else.37525:
	addi	$2, $0, 1
	jr $31
eq_else.37510:
	addi	$2, $0, 1
	jr $31
solve_each_element.3003:
	slli	$7, $2, 2
	add	$30, $5, $7
	lw	$7, 0($30)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37526
	jr $31
eq_else.37526:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	addi	$9, $0, 3696
	lf	$f0, 0($9)
	lw	$9, 20($8)
	lf	$f1, 0($9)
	fsub	$f0, $f0, $f1
	addi	$9, $0, 3704
	lf	$f1, 0($9)
	lw	$9, 20($8)
	lf	$f2, 8($9)
	fsub	$f1, $f1, $f2
	addi	$9, $0, 3712
	lf	$f2, 0($9)
	lw	$9, 20($8)
	lf	$f3, 16($9)
	fsub	$f2, $f2, $f3
	lw	$9, 4($8)
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	sw	$7, 12($3)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37528
	mov	$5, $6
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_rect.2874
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.37529
eq_else.37528:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37530
	lw	$8, 16($8)
	lf	$f3, 0($6)
	lf	$f4, 0($8)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($6)
	lf	$f5, 8($8)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($6)
	lf	$f5, 16($8)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37532
	addi	$9, $0, 0
	j	eq_cont.37533
eq_else.37532:
	addi	$9, $0, 1
eq_cont.37533:
	bne	$9, $0, eq_else.37534
	addi	$2, $0, 0
	j	eq_cont.37535
eq_else.37534:
	lf	$f4, 0($8)
	fmul	$f0, $f4, $f0
	lf	$f4, 8($8)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($8)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	fdiv	$f0, $f0, $f3
	addi	$8, $0, 3528
	sf	$f0, 0($8)
	addi	$2, $0, 1
eq_cont.37535:
	j	eq_cont.37531
eq_else.37530:
	mov	$5, $6
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_second.2899
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37531:
eq_cont.37529:
	bne	$2, $0, eq_else.37536
	lw	$2, 12($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37537
	jr $31
eq_else.37537:
	lw	$2, 8($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	solve_each_element.3003
eq_else.37536:
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	lf	$f1, 4888($0) # 0.000000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37539
	addi	$5, $0, 0
	j	eq_cont.37540
eq_else.37539:
	addi	$5, $0, 1
eq_cont.37540:
	bne	$5, $0, eq_else.37541
	addi	$5, $0, 0
	j	eq_cont.37542
eq_else.37541:
	addi	$5, $0, 1
eq_cont.37542:
	bne	$5, $0, eq_else.37543
	j	eq_cont.37544
eq_else.37543:
	addi	$5, $0, 3540
	lf	$f1, 0($5)
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37545
	addi	$5, $0, 0
	j	eq_cont.37546
eq_else.37545:
	addi	$5, $0, 1
eq_cont.37546:
	bne	$5, $0, eq_else.37547
	addi	$5, $0, 0
	j	eq_cont.37548
eq_else.37547:
	addi	$5, $0, 1
eq_cont.37548:
	bne	$5, $0, eq_else.37549
	j	eq_cont.37550
eq_else.37549:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	lw	$6, 0($3)
	lf	$f1, 0($6)
	fmul	$f1, $f1, $f0
	addi	$5, $0, 3696
	lf	$f2, 0($5)
	fadd	$f1, $f1, $f2
	lf	$f2, 8($6)
	fmul	$f2, $f2, $f0
	addi	$5, $0, 3704
	lf	$f3, 0($5)
	fadd	$f2, $f2, $f3
	lf	$f3, 16($6)
	fmul	$f3, $f3, $f0
	addi	$5, $0, 3712
	lf	$f4, 0($5)
	fadd	$f3, $f3, $f4
	lw	$5, 4($3)
	lw	$7, 0($5)
	sw	$2, 16($3)
	sf	$f3, 24($3)
	sf	$f2, 32($3)
	sf	$f1, 40($3)
	sf	$f0, 48($3)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37552
	addi	$2, $0, 1
	j	eq_cont.37553
eq_else.37552:
	slli	$7, $7, 2
	addi	$7, $7, 3004
	lw	$7, 0($7)
	mov	$2, $7
	movf	$f0, $f1
	movf	$f1, $f2
	movf	$f2, $f3
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_outside.2983
	addi	$3, $3, -64
	lw	$31, 60($3)
	bne	$2, $0, eq_else.37554
	lw	$5, 4($3)
	lw	$2, 4($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37556
	addi	$2, $0, 1
	j	eq_cont.37557
eq_else.37556:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$6, 20($2)
	lf	$f0, 0($6)
	lf	$f1, 40($3)
	fsub	$f0, $f1, $f0
	lw	$6, 20($2)
	lf	$f2, 8($6)
	lf	$f3, 32($3)
	fsub	$f2, $f3, $f2
	lw	$6, 20($2)
	lf	$f4, 16($6)
	lf	$f5, 24($3)
	fsub	$f4, $f5, $f4
	lw	$6, 4($2)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37558
	movf	$f1, $f2
	movf	$f2, $f4
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_rect_outside.2968
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.37559
eq_else.37558:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37560
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f0, $f6, $f0
	lf	$f6, 8($6)
	fmul	$f2, $f6, $f2
	fadd	$f0, $f0, $f2
	lf	$f2, 16($6)
	fmul	$f2, $f2, $f4
	fadd	$f0, $f0, $f2
	lw	$2, 24($2)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37562
	addi	$6, $0, 0
	j	eq_cont.37563
eq_else.37562:
	addi	$6, $0, 1
eq_cont.37563:
	bne	$2, $0, eq_else.37564
	mov	$2, $6
	j	eq_cont.37565
eq_else.37564:
	bne	$6, $0, eq_else.37566
	addi	$2, $0, 1
	j	eq_cont.37567
eq_else.37566:
	addi	$2, $0, 0
eq_cont.37567:
eq_cont.37565:
	bne	$2, $0, eq_else.37568
	addi	$2, $0, 1
	j	eq_cont.37569
eq_else.37568:
	addi	$2, $0, 0
eq_cont.37569:
	j	eq_cont.37561
eq_else.37560:
	fmul	$f6, $f0, $f0
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f2, $f2
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.37570
	movf	$f0, $f6
	j	eq_cont.37571
eq_else.37570:
	fmul	$f7, $f2, $f4
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f0
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f4, $f4, $f7
	fadd	$f4, $f6, $f4
	fmul	$f0, $f0, $f2
	lw	$6, 36($2)
	lf	$f2, 16($6)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f4, $f0
eq_cont.37571:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37572
	lf	$f2, 4920($0) # 1.000000
	fsub	$f0, $f0, $f2
	j	eq_cont.37573
eq_else.37572:
eq_cont.37573:
	lw	$2, 24($2)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37574
	addi	$6, $0, 0
	j	eq_cont.37575
eq_else.37574:
	addi	$6, $0, 1
eq_cont.37575:
	bne	$2, $0, eq_else.37576
	mov	$2, $6
	j	eq_cont.37577
eq_else.37576:
	bne	$6, $0, eq_else.37578
	addi	$2, $0, 1
	j	eq_cont.37579
eq_else.37578:
	addi	$2, $0, 0
eq_cont.37579:
eq_cont.37577:
	bne	$2, $0, eq_else.37580
	addi	$2, $0, 1
	j	eq_cont.37581
eq_else.37580:
	addi	$2, $0, 0
eq_cont.37581:
eq_cont.37561:
eq_cont.37559:
	bne	$2, $0, eq_else.37582
	lw	$5, 4($3)
	lw	$2, 8($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37584
	addi	$2, $0, 1
	j	eq_cont.37585
eq_else.37584:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_outside.2983
	addi	$3, $3, -64
	lw	$31, 60($3)
	bne	$2, $0, eq_else.37586
	addi	$2, $0, 3
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	lw	$5, 4($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	check_all_inside.2988
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.37587
eq_else.37586:
	addi	$2, $0, 0
eq_cont.37587:
eq_cont.37585:
	j	eq_cont.37583
eq_else.37582:
	addi	$2, $0, 0
eq_cont.37583:
eq_cont.37557:
	j	eq_cont.37555
eq_else.37554:
	addi	$2, $0, 0
eq_cont.37555:
eq_cont.37553:
	bne	$2, $0, eq_else.37588
	j	eq_cont.37589
eq_else.37588:
	addi	$2, $0, 3540
	lf	$f0, 48($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3548
	lf	$f0, 40($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3556
	lf	$f0, 32($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3564
	lf	$f0, 24($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3572
	lw	$5, 12($3)
	sw	$5, 0($2)
	addi	$2, $0, 3536
	lw	$5, 16($3)
	sw	$5, 0($2)
eq_cont.37589:
eq_cont.37550:
eq_cont.37544:
	lw	$2, 8($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	solve_each_element.3003
solve_one_or_network.3007:
	slli	$7, $2, 2
	add	$30, $5, $7
	lw	$7, 0($30)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37590
	jr $31
eq_else.37590:
	slli	$7, $7, 2
	addi	$7, $7, 3324
	lw	$7, 0($7)
	addi	$8, $0, 0
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	mov	$5, $7
	mov	$2, $8
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solve_each_element.3003
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37592
	jr $31
eq_else.37592:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 12($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37594
	jr $31
eq_else.37594:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 16($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 16($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37596
	jr $31
eq_else.37596:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 20($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37598
	jr $31
eq_else.37598:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 24($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37600
	jr $31
eq_else.37600:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 28($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element.3003
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 28($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37602
	jr $31
eq_else.37602:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 32($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element.3003
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37604
	jr $31
eq_else.37604:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 36($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	solve_each_element.3003
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 36($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	solve_one_or_network.3007
trace_or_matrix.3011:
	slli	$7, $2, 2
	add	$30, $5, $7
	lw	$7, 0($30)
	lw	$8, 0($7)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.37606
	jr $31
eq_else.37606:
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	addi	$30, $0, 99
	bne	$8, $30, eq_else.37608
	lw	$8, 4($7)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.37610
	j	eq_cont.37611
eq_else.37610:
	slli	$8, $8, 2
	addi	$8, $8, 3324
	lw	$8, 0($8)
	addi	$9, $0, 0
	sw	$7, 12($3)
	mov	$5, $8
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37612
	j	eq_cont.37613
eq_else.37612:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37614
	j	eq_cont.37615
eq_else.37614:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37616
	j	eq_cont.37617
eq_else.37616:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37618
	j	eq_cont.37619
eq_else.37618:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37620
	j	eq_cont.37621
eq_else.37620:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 28($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37622
	j	eq_cont.37623
eq_else.37622:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$2, $0, 8
	lw	$5, 12($3)
	lw	$6, 0($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_one_or_network.3007
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37623:
eq_cont.37621:
eq_cont.37619:
eq_cont.37617:
eq_cont.37615:
eq_cont.37613:
eq_cont.37611:
	j	eq_cont.37609
eq_else.37608:
	slli	$8, $8, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	addi	$9, $0, 3696
	lf	$f0, 0($9)
	lw	$9, 20($8)
	lf	$f1, 0($9)
	fsub	$f0, $f0, $f1
	addi	$9, $0, 3704
	lf	$f1, 0($9)
	lw	$9, 20($8)
	lf	$f2, 8($9)
	fsub	$f1, $f1, $f2
	addi	$9, $0, 3712
	lf	$f2, 0($9)
	lw	$9, 20($8)
	lf	$f3, 16($9)
	fsub	$f2, $f2, $f3
	lw	$9, 4($8)
	sw	$7, 12($3)
	addi	$30, $0, 1
	bne	$9, $30, eq_else.37624
	mov	$5, $6
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_rect.2874
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.37625
eq_else.37624:
	addi	$30, $0, 2
	bne	$9, $30, eq_else.37626
	lw	$8, 16($8)
	lf	$f3, 0($6)
	lf	$f4, 0($8)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($6)
	lf	$f5, 8($8)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($6)
	lf	$f5, 16($8)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37628
	addi	$9, $0, 0
	j	eq_cont.37629
eq_else.37628:
	addi	$9, $0, 1
eq_cont.37629:
	bne	$9, $0, eq_else.37630
	addi	$2, $0, 0
	j	eq_cont.37631
eq_else.37630:
	lf	$f4, 0($8)
	fmul	$f0, $f4, $f0
	lf	$f4, 8($8)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 16($8)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	fdiv	$f0, $f0, $f3
	addi	$8, $0, 3528
	sf	$f0, 0($8)
	addi	$2, $0, 1
eq_cont.37631:
	j	eq_cont.37627
eq_else.37626:
	mov	$5, $6
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_second.2899
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37627:
eq_cont.37625:
	bne	$2, $0, eq_else.37632
	j	eq_cont.37633
eq_else.37632:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37634
	addi	$2, $0, 0
	j	eq_cont.37635
eq_else.37634:
	addi	$2, $0, 1
eq_cont.37635:
	bne	$2, $0, eq_else.37636
	addi	$2, $0, 0
	j	eq_cont.37637
eq_else.37636:
	addi	$2, $0, 1
eq_cont.37637:
	bne	$2, $0, eq_else.37638
	j	eq_cont.37639
eq_else.37638:
	lw	$2, 12($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37640
	j	eq_cont.37641
eq_else.37640:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37642
	j	eq_cont.37643
eq_else.37642:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37644
	j	eq_cont.37645
eq_else.37644:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37646
	j	eq_cont.37647
eq_else.37646:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37648
	j	eq_cont.37649
eq_else.37648:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37650
	j	eq_cont.37651
eq_else.37650:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 28($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37652
	j	eq_cont.37653
eq_else.37652:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element.3003
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$2, $0, 8
	lw	$5, 12($3)
	lw	$6, 0($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_one_or_network.3007
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37653:
eq_cont.37651:
eq_cont.37649:
eq_cont.37647:
eq_cont.37645:
eq_cont.37643:
eq_cont.37641:
eq_cont.37639:
eq_cont.37633:
eq_cont.37609:
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$7, 0($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37654
	jr $31
eq_else.37654:
	sw	$2, 16($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.37656
	lw	$7, 4($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37658
	j	eq_cont.37659
eq_else.37658:
	slli	$7, $7, 2
	addi	$7, $7, 3324
	lw	$7, 0($7)
	addi	$8, $0, 0
	lw	$9, 0($3)
	sw	$5, 20($3)
	mov	$6, $9
	mov	$5, $7
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37660
	j	eq_cont.37661
eq_else.37660:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37662
	j	eq_cont.37663
eq_else.37662:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37664
	j	eq_cont.37665
eq_else.37664:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37666
	j	eq_cont.37667
eq_else.37666:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37668
	j	eq_cont.37669
eq_else.37668:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 7
	lw	$5, 20($3)
	lw	$6, 0($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_one_or_network.3007
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.37669:
eq_cont.37667:
eq_cont.37665:
eq_cont.37663:
eq_cont.37661:
eq_cont.37659:
	j	eq_cont.37657
eq_else.37656:
	addi	$8, $0, 3696
	lw	$9, 0($3)
	sw	$5, 20($3)
	mov	$6, $8
	mov	$5, $9
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solver.2905
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.37670
	j	eq_cont.37671
eq_else.37670:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37672
	addi	$2, $0, 0
	j	eq_cont.37673
eq_else.37672:
	addi	$2, $0, 1
eq_cont.37673:
	bne	$2, $0, eq_else.37674
	addi	$2, $0, 0
	j	eq_cont.37675
eq_else.37674:
	addi	$2, $0, 1
eq_cont.37675:
	bne	$2, $0, eq_else.37676
	j	eq_cont.37677
eq_else.37676:
	lw	$2, 20($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37678
	j	eq_cont.37679
eq_else.37678:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37680
	j	eq_cont.37681
eq_else.37680:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37682
	j	eq_cont.37683
eq_else.37682:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37684
	j	eq_cont.37685
eq_else.37684:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37686
	j	eq_cont.37687
eq_else.37686:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37688
	j	eq_cont.37689
eq_else.37688:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element.3003
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 7
	lw	$5, 20($3)
	lw	$6, 0($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_one_or_network.3007
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.37689:
eq_cont.37687:
eq_cont.37685:
eq_cont.37683:
eq_cont.37681:
eq_cont.37679:
eq_cont.37677:
eq_cont.37671:
eq_cont.37657:
	lw	$2, 16($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	trace_or_matrix.3011
solve_each_element_fast.3017:
	lw	$7, 0($6)
	slli	$8, $2, 2
	add	$30, $5, $8
	lw	$8, 0($30)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.37690
	jr $31
eq_else.37690:
	slli	$9, $8, 2
	addi	$9, $9, 3004
	lw	$9, 0($9)
	lw	$10, 40($9)
	lf	$f0, 0($10)
	lf	$f1, 8($10)
	lf	$f2, 16($10)
	lw	$11, 4($6)
	slli	$12, $8, 2
	add	$30, $11, $12
	lw	$11, 0($30)
	lw	$12, 4($9)
	sw	$7, 0($3)
	sw	$6, 4($3)
	sw	$5, 8($3)
	sw	$2, 12($3)
	sw	$8, 16($3)
	addi	$30, $0, 1
	bne	$12, $30, eq_else.37692
	lw	$10, 0($6)
	mov	$6, $11
	mov	$5, $10
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_rect_fast.2909
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.37693
eq_else.37692:
	addi	$30, $0, 2
	bne	$12, $30, eq_else.37694
	lf	$f0, 0($11)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37696
	addi	$9, $0, 0
	j	eq_cont.37697
eq_else.37696:
	addi	$9, $0, 1
eq_cont.37697:
	bne	$9, $0, eq_else.37698
	addi	$2, $0, 0
	j	eq_cont.37699
eq_else.37698:
	lf	$f0, 0($11)
	lf	$f1, 24($10)
	fmul	$f0, $f0, $f1
	addi	$9, $0, 3528
	sf	$f0, 0($9)
	addi	$2, $0, 1
eq_cont.37699:
	j	eq_cont.37695
eq_else.37694:
	lf	$f3, 0($11)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.37700
	addi	$12, $0, 1
	j	eq_cont.37701
eq_else.37700:
	addi	$12, $0, 0
eq_cont.37701:
	bne	$12, $0, eq_else.37702
	lf	$f4, 8($11)
	fmul	$f0, $f4, $f0
	lf	$f4, 16($11)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($11)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 24($10)
	fmul	$f2, $f0, $f0
	fmul	$f1, $f3, $f1
	fsub	$f1, $f2, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37704
	addi	$10, $0, 0
	j	eq_cont.37705
eq_else.37704:
	addi	$10, $0, 1
eq_cont.37705:
	bne	$10, $0, eq_else.37706
	addi	$2, $0, 0
	j	eq_cont.37707
eq_else.37706:
	lw	$9, 24($9)
	bne	$9, $0, eq_else.37708
	sqrt	$f1, $f1
	fsub	$f0, $f0, $f1
	lf	$f1, 32($11)
	fmul	$f0, $f0, $f1
	addi	$9, $0, 3528
	sf	$f0, 0($9)
	j	eq_cont.37709
eq_else.37708:
	sqrt	$f1, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 32($11)
	fmul	$f0, $f0, $f1
	addi	$9, $0, 3528
	sf	$f0, 0($9)
eq_cont.37709:
	addi	$2, $0, 1
eq_cont.37707:
	j	eq_cont.37703
eq_else.37702:
	addi	$2, $0, 0
eq_cont.37703:
eq_cont.37695:
eq_cont.37693:
	bne	$2, $0, eq_else.37710
	lw	$2, 16($3)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$2, 24($2)
	bne	$2, $0, eq_else.37711
	jr $31
eq_else.37711:
	lw	$2, 12($3)
	addi	$2, $2, 1
	lw	$5, 8($3)
	lw	$6, 4($3)
	j	solve_each_element_fast.3017
eq_else.37710:
	addi	$5, $0, 3528
	lf	$f0, 0($5)
	lf	$f1, 4888($0) # 0.000000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37713
	addi	$5, $0, 0
	j	eq_cont.37714
eq_else.37713:
	addi	$5, $0, 1
eq_cont.37714:
	bne	$5, $0, eq_else.37715
	addi	$5, $0, 0
	j	eq_cont.37716
eq_else.37715:
	addi	$5, $0, 1
eq_cont.37716:
	bne	$5, $0, eq_else.37717
	j	eq_cont.37718
eq_else.37717:
	addi	$5, $0, 3540
	lf	$f1, 0($5)
	fsub	$f1, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37719
	addi	$5, $0, 0
	j	eq_cont.37720
eq_else.37719:
	addi	$5, $0, 1
eq_cont.37720:
	bne	$5, $0, eq_else.37721
	addi	$5, $0, 0
	j	eq_cont.37722
eq_else.37721:
	addi	$5, $0, 1
eq_cont.37722:
	bne	$5, $0, eq_else.37723
	j	eq_cont.37724
eq_else.37723:
	lf	$f1, 4760($0) # 0.010000
	fadd	$f0, $f0, $f1
	lw	$5, 0($3)
	lf	$f1, 0($5)
	fmul	$f1, $f1, $f0
	addi	$6, $0, 3720
	lf	$f2, 0($6)
	fadd	$f1, $f1, $f2
	lf	$f2, 8($5)
	fmul	$f2, $f2, $f0
	addi	$6, $0, 3728
	lf	$f3, 0($6)
	fadd	$f2, $f2, $f3
	lf	$f3, 16($5)
	fmul	$f3, $f3, $f0
	addi	$5, $0, 3736
	lf	$f4, 0($5)
	fadd	$f3, $f3, $f4
	lw	$5, 8($3)
	lw	$6, 0($5)
	sw	$2, 20($3)
	sf	$f3, 24($3)
	sf	$f2, 32($3)
	sf	$f1, 40($3)
	sf	$f0, 48($3)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.37725
	addi	$2, $0, 1
	j	eq_cont.37726
eq_else.37725:
	slli	$6, $6, 2
	addi	$6, $6, 3004
	lw	$6, 0($6)
	mov	$2, $6
	movf	$f0, $f1
	movf	$f1, $f2
	movf	$f2, $f3
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_outside.2983
	addi	$3, $3, -64
	lw	$31, 60($3)
	bne	$2, $0, eq_else.37727
	lw	$5, 8($3)
	lw	$2, 4($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37729
	addi	$2, $0, 1
	j	eq_cont.37730
eq_else.37729:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$6, 20($2)
	lf	$f0, 0($6)
	lf	$f1, 40($3)
	fsub	$f0, $f1, $f0
	lw	$6, 20($2)
	lf	$f2, 8($6)
	lf	$f3, 32($3)
	fsub	$f2, $f3, $f2
	lw	$6, 20($2)
	lf	$f4, 16($6)
	lf	$f5, 24($3)
	fsub	$f4, $f5, $f4
	lw	$6, 4($2)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37731
	movf	$f1, $f2
	movf	$f2, $f4
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_rect_outside.2968
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.37732
eq_else.37731:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37733
	lw	$6, 16($2)
	lf	$f6, 0($6)
	fmul	$f0, $f6, $f0
	lf	$f6, 8($6)
	fmul	$f2, $f6, $f2
	fadd	$f0, $f0, $f2
	lf	$f2, 16($6)
	fmul	$f2, $f2, $f4
	fadd	$f0, $f0, $f2
	lw	$2, 24($2)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37735
	addi	$6, $0, 0
	j	eq_cont.37736
eq_else.37735:
	addi	$6, $0, 1
eq_cont.37736:
	bne	$2, $0, eq_else.37737
	mov	$2, $6
	j	eq_cont.37738
eq_else.37737:
	bne	$6, $0, eq_else.37739
	addi	$2, $0, 1
	j	eq_cont.37740
eq_else.37739:
	addi	$2, $0, 0
eq_cont.37740:
eq_cont.37738:
	bne	$2, $0, eq_else.37741
	addi	$2, $0, 1
	j	eq_cont.37742
eq_else.37741:
	addi	$2, $0, 0
eq_cont.37742:
	j	eq_cont.37734
eq_else.37733:
	fmul	$f6, $f0, $f0
	lw	$6, 16($2)
	lf	$f7, 0($6)
	fmul	$f6, $f6, $f7
	fmul	$f7, $f2, $f2
	lw	$6, 16($2)
	lf	$f8, 8($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f7, $f4, $f4
	lw	$6, 16($2)
	lf	$f8, 16($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	lw	$6, 12($2)
	bne	$6, $0, eq_else.37743
	movf	$f0, $f6
	j	eq_cont.37744
eq_else.37743:
	fmul	$f7, $f2, $f4
	lw	$6, 36($2)
	lf	$f8, 0($6)
	fmul	$f7, $f7, $f8
	fadd	$f6, $f6, $f7
	fmul	$f4, $f4, $f0
	lw	$6, 36($2)
	lf	$f7, 8($6)
	fmul	$f4, $f4, $f7
	fadd	$f4, $f6, $f4
	fmul	$f0, $f0, $f2
	lw	$6, 36($2)
	lf	$f2, 16($6)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f4, $f0
eq_cont.37744:
	lw	$6, 4($2)
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37745
	lf	$f2, 4920($0) # 1.000000
	fsub	$f0, $f0, $f2
	j	eq_cont.37746
eq_else.37745:
eq_cont.37746:
	lw	$2, 24($2)
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37747
	addi	$6, $0, 0
	j	eq_cont.37748
eq_else.37747:
	addi	$6, $0, 1
eq_cont.37748:
	bne	$2, $0, eq_else.37749
	mov	$2, $6
	j	eq_cont.37750
eq_else.37749:
	bne	$6, $0, eq_else.37751
	addi	$2, $0, 1
	j	eq_cont.37752
eq_else.37751:
	addi	$2, $0, 0
eq_cont.37752:
eq_cont.37750:
	bne	$2, $0, eq_else.37753
	addi	$2, $0, 1
	j	eq_cont.37754
eq_else.37753:
	addi	$2, $0, 0
eq_cont.37754:
eq_cont.37734:
eq_cont.37732:
	bne	$2, $0, eq_else.37755
	lw	$5, 8($3)
	lw	$2, 8($5)
	addi	$30, $0, -1
	bne	$2, $30, eq_else.37757
	addi	$2, $0, 1
	j	eq_cont.37758
eq_else.37757:
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	is_outside.2983
	addi	$3, $3, -64
	lw	$31, 60($3)
	bne	$2, $0, eq_else.37759
	addi	$2, $0, 3
	lf	$f0, 40($3)
	lf	$f1, 32($3)
	lf	$f2, 24($3)
	lw	$5, 8($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	check_all_inside.2988
	addi	$3, $3, -64
	lw	$31, 60($3)
	j	eq_cont.37760
eq_else.37759:
	addi	$2, $0, 0
eq_cont.37760:
eq_cont.37758:
	j	eq_cont.37756
eq_else.37755:
	addi	$2, $0, 0
eq_cont.37756:
eq_cont.37730:
	j	eq_cont.37728
eq_else.37727:
	addi	$2, $0, 0
eq_cont.37728:
eq_cont.37726:
	bne	$2, $0, eq_else.37761
	j	eq_cont.37762
eq_else.37761:
	addi	$2, $0, 3540
	lf	$f0, 48($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3548
	lf	$f0, 40($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3556
	lf	$f0, 32($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3564
	lf	$f0, 24($3)
	sf	$f0, 0($2)
	addi	$2, $0, 3572
	lw	$5, 16($3)
	sw	$5, 0($2)
	addi	$2, $0, 3536
	lw	$5, 20($3)
	sw	$5, 0($2)
eq_cont.37762:
eq_cont.37724:
eq_cont.37718:
	lw	$2, 12($3)
	addi	$2, $2, 1
	lw	$5, 8($3)
	lw	$6, 4($3)
	j	solve_each_element_fast.3017
solve_one_or_network_fast.3021:
	slli	$7, $2, 2
	add	$30, $5, $7
	lw	$7, 0($30)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37763
	jr $31
eq_else.37763:
	slli	$7, $7, 2
	addi	$7, $7, 3324
	lw	$7, 0($7)
	addi	$8, $0, 0
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	mov	$5, $7
	mov	$2, $8
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	solve_each_element_fast.3017
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37765
	jr $31
eq_else.37765:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 12($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37767
	jr $31
eq_else.37767:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 16($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 16($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37769
	jr $31
eq_else.37769:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 20($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37771
	jr $31
eq_else.37771:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 24($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37773
	jr $31
eq_else.37773:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 28($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 28($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37775
	jr $31
eq_else.37775:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 32($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37777
	jr $31
eq_else.37777:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$7, $0, 0
	lw	$8, 0($3)
	sw	$2, 36($3)
	mov	$6, $8
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	solve_each_element_fast.3017
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 36($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	solve_one_or_network_fast.3021
trace_or_matrix_fast.3025:
	slli	$7, $2, 2
	add	$30, $5, $7
	lw	$7, 0($30)
	lw	$8, 0($7)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.37779
	jr $31
eq_else.37779:
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	addi	$30, $0, 99
	bne	$8, $30, eq_else.37781
	lw	$8, 4($7)
	addi	$30, $0, -1
	bne	$8, $30, eq_else.37783
	j	eq_cont.37784
eq_else.37783:
	slli	$8, $8, 2
	addi	$8, $8, 3324
	lw	$8, 0($8)
	addi	$9, $0, 0
	sw	$7, 12($3)
	mov	$5, $8
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37785
	j	eq_cont.37786
eq_else.37785:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37787
	j	eq_cont.37788
eq_else.37787:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37789
	j	eq_cont.37790
eq_else.37789:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37791
	j	eq_cont.37792
eq_else.37791:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37793
	j	eq_cont.37794
eq_else.37793:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 28($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37795
	j	eq_cont.37796
eq_else.37795:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$2, $0, 8
	lw	$5, 12($3)
	lw	$6, 0($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37796:
eq_cont.37794:
eq_cont.37792:
eq_cont.37790:
eq_cont.37788:
eq_cont.37786:
eq_cont.37784:
	j	eq_cont.37782
eq_else.37781:
	slli	$9, $8, 2
	addi	$9, $9, 3004
	lw	$9, 0($9)
	lw	$10, 40($9)
	lf	$f0, 0($10)
	lf	$f1, 8($10)
	lf	$f2, 16($10)
	lw	$11, 4($6)
	slli	$8, $8, 2
	add	$30, $11, $8
	lw	$8, 0($30)
	lw	$11, 4($9)
	sw	$7, 12($3)
	addi	$30, $0, 1
	bne	$11, $30, eq_else.37797
	lw	$10, 0($6)
	mov	$6, $8
	mov	$5, $10
	mov	$2, $9
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solver_rect_fast.2909
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	eq_cont.37798
eq_else.37797:
	addi	$30, $0, 2
	bne	$11, $30, eq_else.37799
	lf	$f0, 0($8)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37801
	addi	$9, $0, 0
	j	eq_cont.37802
eq_else.37801:
	addi	$9, $0, 1
eq_cont.37802:
	bne	$9, $0, eq_else.37803
	addi	$2, $0, 0
	j	eq_cont.37804
eq_else.37803:
	lf	$f0, 0($8)
	lf	$f1, 24($10)
	fmul	$f0, $f0, $f1
	addi	$8, $0, 3528
	sf	$f0, 0($8)
	addi	$2, $0, 1
eq_cont.37804:
	j	eq_cont.37800
eq_else.37799:
	lf	$f3, 0($8)
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.37805
	addi	$11, $0, 1
	j	eq_cont.37806
eq_else.37805:
	addi	$11, $0, 0
eq_cont.37806:
	bne	$11, $0, eq_else.37807
	lf	$f4, 8($8)
	fmul	$f0, $f4, $f0
	lf	$f4, 16($8)
	fmul	$f1, $f4, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 24($8)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 24($10)
	fmul	$f2, $f0, $f0
	fmul	$f1, $f3, $f1
	fsub	$f1, $f2, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37809
	addi	$10, $0, 0
	j	eq_cont.37810
eq_else.37809:
	addi	$10, $0, 1
eq_cont.37810:
	bne	$10, $0, eq_else.37811
	addi	$2, $0, 0
	j	eq_cont.37812
eq_else.37811:
	lw	$9, 24($9)
	bne	$9, $0, eq_else.37813
	sqrt	$f1, $f1
	fsub	$f0, $f0, $f1
	lf	$f1, 32($8)
	fmul	$f0, $f0, $f1
	addi	$8, $0, 3528
	sf	$f0, 0($8)
	j	eq_cont.37814
eq_else.37813:
	sqrt	$f1, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 32($8)
	fmul	$f0, $f0, $f1
	addi	$8, $0, 3528
	sf	$f0, 0($8)
eq_cont.37814:
	addi	$2, $0, 1
eq_cont.37812:
	j	eq_cont.37808
eq_else.37807:
	addi	$2, $0, 0
eq_cont.37808:
eq_cont.37800:
eq_cont.37798:
	bne	$2, $0, eq_else.37815
	j	eq_cont.37816
eq_else.37815:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37817
	addi	$2, $0, 0
	j	eq_cont.37818
eq_else.37817:
	addi	$2, $0, 1
eq_cont.37818:
	bne	$2, $0, eq_else.37819
	addi	$2, $0, 0
	j	eq_cont.37820
eq_else.37819:
	addi	$2, $0, 1
eq_cont.37820:
	bne	$2, $0, eq_else.37821
	j	eq_cont.37822
eq_else.37821:
	lw	$2, 12($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37823
	j	eq_cont.37824
eq_else.37823:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37825
	j	eq_cont.37826
eq_else.37825:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37827
	j	eq_cont.37828
eq_else.37827:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37829
	j	eq_cont.37830
eq_else.37829:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37831
	j	eq_cont.37832
eq_else.37831:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37833
	j	eq_cont.37834
eq_else.37833:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	lw	$5, 28($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37835
	j	eq_cont.37836
eq_else.37835:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_each_element_fast.3017
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$2, $0, 8
	lw	$5, 12($3)
	lw	$6, 0($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.37836:
eq_cont.37834:
eq_cont.37832:
eq_cont.37830:
eq_cont.37828:
eq_cont.37826:
eq_cont.37824:
eq_cont.37822:
eq_cont.37816:
eq_cont.37782:
	lw	$2, 8($3)
	addi	$2, $2, 1
	slli	$5, $2, 2
	lw	$6, 4($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$7, 0($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37837
	jr $31
eq_else.37837:
	sw	$2, 16($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.37839
	lw	$7, 4($5)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.37841
	j	eq_cont.37842
eq_else.37841:
	slli	$7, $7, 2
	addi	$7, $7, 3324
	lw	$7, 0($7)
	addi	$8, $0, 0
	lw	$9, 0($3)
	sw	$5, 20($3)
	mov	$6, $9
	mov	$5, $7
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37843
	j	eq_cont.37844
eq_else.37843:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37845
	j	eq_cont.37846
eq_else.37845:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37847
	j	eq_cont.37848
eq_else.37847:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37849
	j	eq_cont.37850
eq_else.37849:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37851
	j	eq_cont.37852
eq_else.37851:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 7
	lw	$5, 20($3)
	lw	$6, 0($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.37852:
eq_cont.37850:
eq_cont.37848:
eq_cont.37846:
eq_cont.37844:
eq_cont.37842:
	j	eq_cont.37840
eq_else.37839:
	lw	$8, 0($3)
	sw	$5, 20($3)
	mov	$5, $8
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solver_fast2.2946
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.37853
	j	eq_cont.37854
eq_else.37853:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37855
	addi	$2, $0, 0
	j	eq_cont.37856
eq_else.37855:
	addi	$2, $0, 1
eq_cont.37856:
	bne	$2, $0, eq_else.37857
	addi	$2, $0, 0
	j	eq_cont.37858
eq_else.37857:
	addi	$2, $0, 1
eq_cont.37858:
	bne	$2, $0, eq_else.37859
	j	eq_cont.37860
eq_else.37859:
	lw	$2, 20($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37861
	j	eq_cont.37862
eq_else.37861:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37863
	j	eq_cont.37864
eq_else.37863:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37865
	j	eq_cont.37866
eq_else.37865:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37867
	j	eq_cont.37868
eq_else.37867:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 20($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37869
	j	eq_cont.37870
eq_else.37869:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 24($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.37871
	j	eq_cont.37872
eq_else.37871:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 0($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_each_element_fast.3017
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 7
	lw	$5, 20($3)
	lw	$6, 0($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.37872:
eq_cont.37870:
eq_cont.37868:
eq_cont.37866:
eq_cont.37864:
eq_cont.37862:
eq_cont.37860:
eq_cont.37854:
eq_cont.37840:
	lw	$2, 16($3)
	addi	$2, $2, 1
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	trace_or_matrix_fast.3025
get_nvector_second.3035:
	lf	$f0, 3548($0)
	lw	$5, 20($2)
	lf	$f1, 0($5)
	fsub	$f0, $f0, $f1
	lf	$f1, 3556($0)
	lw	$5, 20($2)
	lf	$f2, 8($5)
	fsub	$f1, $f1, $f2
	lf	$f2, 3564($0)
	lw	$5, 20($2)
	lf	$f3, 16($5)
	fsub	$f2, $f2, $f3
	lw	$5, 16($2)
	lf	$f3, 0($5)
	fmul	$f3, $f0, $f3
	lw	$5, 16($2)
	lf	$f4, 8($5)
	fmul	$f4, $f1, $f4
	lw	$5, 16($2)
	lf	$f5, 16($5)
	fmul	$f5, $f2, $f5
	lw	$5, 12($2)
	bne	$5, $0, eq_else.37873
	addi	$5, $0, 3576
	sf	$f3, 0($5)
	addi	$5, $0, 3584
	sf	$f4, 0($5)
	addi	$5, $0, 3592
	sf	$f5, 0($5)
	j	eq_cont.37874
eq_else.37873:
	lw	$5, 36($2)
	lf	$f6, 16($5)
	fmul	$f6, $f1, $f6
	lw	$5, 36($2)
	lf	$f7, 8($5)
	fmul	$f7, $f2, $f7
	fadd	$f6, $f6, $f7
	lf	$f7, 4944($0) # 0.500000
	fmul	$f6, $f6, $f7
	fadd	$f3, $f3, $f6
	addi	$5, $0, 3576
	sf	$f3, 0($5)
	lw	$5, 36($2)
	lf	$f3, 16($5)
	fmul	$f3, $f0, $f3
	lw	$5, 36($2)
	lf	$f6, 0($5)
	fmul	$f2, $f2, $f6
	fadd	$f2, $f3, $f2
	lf	$f3, 4944($0) # 0.500000
	fmul	$f2, $f2, $f3
	fadd	$f2, $f4, $f2
	addi	$5, $0, 3584
	sf	$f2, 0($5)
	lw	$5, 36($2)
	lf	$f2, 8($5)
	fmul	$f0, $f0, $f2
	lw	$5, 36($2)
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4944($0) # 0.500000
	fmul	$f0, $f0, $f1
	fadd	$f0, $f5, $f0
	addi	$5, $0, 3592
	sf	$f0, 0($5)
eq_cont.37874:
	lw	$2, 24($2)
	lf	$f0, 3576($0)
	fmul	$f0, $f0, $f0
	lf	$f1, 3584($0)
	fmul	$f1, $f1, $f1
	fadd	$f0, $f0, $f1
	lf	$f1, 3592($0)
	fmul	$f1, $f1, $f1
	fadd	$f0, $f0, $f1
	sqrt	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.37875
	addi	$5, $0, 1
	j	eq_cont.37876
eq_else.37875:
	addi	$5, $0, 0
eq_cont.37876:
	bne	$5, $0, eq_else.37877
	bne	$2, $0, eq_else.37879
	lf	$f1, 4920($0) # 1.000000
	fdiv	$f0, $f1, $f0
	j	eq_cont.37880
eq_else.37879:
	lf	$f1, 4880($0) # -1.000000
	fdiv	$f0, $f1, $f0
eq_cont.37880:
	j	eq_cont.37878
eq_else.37877:
	lf	$f0, 4920($0) # 1.000000
eq_cont.37878:
	lf	$f1, 3576($0)
	fmul	$f1, $f1, $f0
	sf	$f1, 3576($0)
	lf	$f1, 3584($0)
	fmul	$f1, $f1, $f0
	sf	$f1, 3584($0)
	lf	$f1, 3592($0)
	fmul	$f0, $f1, $f0
	sf	$f0, 3592($0)
	jr $31
utexture.3040:
	lw	$6, 0($2)
	lw	$7, 32($2)
	lf	$f0, 0($7)
	sf	$f0, 3600($0)
	lw	$7, 32($2)
	lf	$f0, 8($7)
	sf	$f0, 3608($0)
	lw	$7, 32($2)
	lf	$f0, 16($7)
	sf	$f0, 3616($0)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.37882
	lf	$f0, 0($5)
	lw	$6, 20($2)
	lf	$f1, 0($6)
	fsub	$f0, $f0, $f1
	lf	$f1, 4672($0) # 0.050000
	fmul	$f1, $f0, $f1
	floor	$f1, $f1
	lf	$f2, 4664($0) # 20.000000
	fmul	$f1, $f1, $f2
	fsub	$f0, $f0, $f1
	lf	$f1, 4688($0) # 10.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37883
	addi	$6, $0, 0
	j	eq_cont.37884
eq_else.37883:
	addi	$6, $0, 1
eq_cont.37884:
	bne	$6, $0, eq_else.37885
	addi	$6, $0, 0
	j	eq_cont.37886
eq_else.37885:
	addi	$6, $0, 1
eq_cont.37886:
	lf	$f0, 16($5)
	lw	$2, 20($2)
	lf	$f1, 16($2)
	fsub	$f0, $f0, $f1
	lf	$f1, 4672($0) # 0.050000
	fmul	$f1, $f0, $f1
	floor	$f1, $f1
	lf	$f2, 4664($0) # 20.000000
	fmul	$f1, $f1, $f2
	fsub	$f0, $f0, $f1
	lf	$f1, 4688($0) # 10.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37887
	addi	$2, $0, 0
	j	eq_cont.37888
eq_else.37887:
	addi	$2, $0, 1
eq_cont.37888:
	bne	$2, $0, eq_else.37889
	addi	$2, $0, 0
	j	eq_cont.37890
eq_else.37889:
	addi	$2, $0, 1
eq_cont.37890:
	bne	$6, $0, eq_else.37891
	bne	$2, $0, eq_else.37893
	lf	$f0, 4704($0) # 255.000000
	j	eq_cont.37894
eq_else.37893:
	lf	$f0, 4888($0) # 0.000000
eq_cont.37894:
	j	eq_cont.37892
eq_else.37891:
	bne	$2, $0, eq_else.37895
	lf	$f0, 4888($0) # 0.000000
	j	eq_cont.37896
eq_else.37895:
	lf	$f0, 4704($0) # 255.000000
eq_cont.37896:
eq_cont.37892:
	addi	$2, $0, 3608
	sf	$f0, 0($2)
	jr $31
eq_else.37882:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.37898
	lf	$f0, 8($5)
	lf	$f1, 4680($0) # 0.250000
	fmul	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37899
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.37900
eq_else.37899:
	lf	$f1, 4920($0) # 1.000000
eq_cont.37900:
	fmul	$f0, $f1, $f0
	lf	$f2, 4800($0) # 6.283185
	fdiv	$f3, $f0, $f2
	floor	$f3, $f3
	fmul	$f2, $f3, $f2
	fsub	$f0, $f0, $f2
	lf	$f2, 4968($0) # 3.141593
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37901
	fsub	$f0, $f0, $f2
	fneg	$f1, $f1
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37903
	fsub	$f0, $f2, $f0
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37905
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37906
eq_else.37905:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37906:
	j	eq_cont.37904
eq_else.37903:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37907
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37908
eq_else.37907:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37908:
eq_cont.37904:
	j	eq_cont.37902
eq_else.37901:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37909
	fsub	$f0, $f2, $f0
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37911
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37912
eq_else.37911:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37912:
	j	eq_cont.37910
eq_else.37909:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37913
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37914
eq_else.37913:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37914:
eq_cont.37910:
eq_cont.37902:
	fmul	$f0, $f0, $f0
	lf	$f1, 4704($0) # 255.000000
	fmul	$f1, $f1, $f0
	addi	$2, $0, 3600
	sf	$f1, 0($2)
	lf	$f1, 4704($0) # 255.000000
	lf	$f2, 4920($0) # 1.000000
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	addi	$2, $0, 3608
	sf	$f0, 0($2)
	jr $31
eq_else.37898:
	addi	$30, $0, 3
	bne	$6, $30, eq_else.37916
	lf	$f0, 0($5)
	lw	$6, 20($2)
	lf	$f1, 0($6)
	fsub	$f0, $f0, $f1
	lf	$f1, 16($5)
	lw	$2, 20($2)
	lf	$f2, 16($2)
	fsub	$f1, $f1, $f2
	fmul	$f0, $f0, $f0
	fmul	$f1, $f1, $f1
	fadd	$f0, $f0, $f1
	sqrt	$f0, $f0
	lf	$f1, 4688($0) # 10.000000
	fdiv	$f0, $f0, $f1
	floor	$f1, $f0
	fsub	$f0, $f0, $f1
	lf	$f1, 4720($0) # 3.141593
	fmul	$f0, $f0, $f1
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37917
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.37918
eq_else.37917:
	lf	$f2, 4920($0) # 1.000000
eq_cont.37918:
	fmul	$f0, $f2, $f0
	lf	$f2, 4800($0) # 6.283185
	fdiv	$f3, $f0, $f2
	floor	$f3, $f3
	fmul	$f2, $f3, $f2
	fsub	$f0, $f0, $f2
	lf	$f2, 4968($0) # 3.141593
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.37919
	fsub	$f0, $f0, $f2
	lf	$f1, 4880($0) # -1.000000
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37921
	fsub	$f0, $f2, $f0
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37923
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37924
eq_else.37923:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37924:
	j	eq_cont.37922
eq_else.37921:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37925
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37926
eq_else.37925:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37926:
eq_cont.37922:
	j	eq_cont.37920
eq_else.37919:
	lf	$f2, 4968($0) # 3.141593
	lf	$f3, 4960($0) # 1.570796
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37927
	fsub	$f0, $f2, $f0
	lf	$f1, 4880($0) # -1.000000
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37929
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37930
eq_else.37929:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37930:
	j	eq_cont.37928
eq_else.37927:
	lf	$f2, 4952($0) # 0.785398
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.37931
	lf	$f2, 4944($0) # 0.500000
	lf	$f3, 4936($0) # 0.041664
	lf	$f4, 4928($0) # 0.001370
	lf	$f5, 4920($0) # 1.000000
	fmul	$f6, $f0, $f0
	fmul	$f2, $f6, $f2
	fsub	$f2, $f5, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f0, $f0, $f0
	fmul	$f3, $f0, $f0
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37932
eq_else.37931:
	lf	$f2, 4960($0) # 1.570796
	fsub	$f0, $f2, $f0
	lf	$f2, 4912($0) # 0.166667
	lf	$f3, 4904($0) # 0.008333
	lf	$f4, 4896($0) # 0.000196
	fmul	$f5, $f0, $f0
	fmul	$f5, $f0, $f5
	fmul	$f2, $f5, $f2
	fsub	$f2, $f0, $f2
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f3, $f5, $f3
	fadd	$f2, $f2, $f3
	fmul	$f3, $f0, $f0
	fmul	$f5, $f3, $f3
	fmul	$f0, $f0, $f3
	fmul	$f0, $f0, $f5
	fmul	$f0, $f0, $f4
	fsub	$f0, $f2, $f0
	fmul	$f0, $f1, $f0
eq_cont.37932:
eq_cont.37928:
eq_cont.37920:
	fmul	$f0, $f0, $f0
	lf	$f1, 4704($0) # 255.000000
	fmul	$f1, $f0, $f1
	addi	$2, $0, 3608
	sf	$f1, 0($2)
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4704($0) # 255.000000
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3616
	sf	$f0, 0($2)
	jr $31
eq_else.37916:
	addi	$30, $0, 4
	bne	$6, $30, eq_else.37934
	lf	$f0, 0($5)
	lw	$6, 20($2)
	lf	$f1, 0($6)
	fsub	$f0, $f0, $f1
	lw	$6, 16($2)
	lf	$f1, 0($6)
	sqrt	$f1, $f1
	fmul	$f0, $f0, $f1
	lf	$f1, 16($5)
	lw	$6, 20($2)
	lf	$f2, 16($6)
	fsub	$f1, $f1, $f2
	lw	$6, 16($2)
	lf	$f2, 16($6)
	sqrt	$f2, $f2
	fmul	$f1, $f1, $f2
	fmul	$f2, $f0, $f0
	fmul	$f3, $f1, $f1
	fadd	$f2, $f2, $f3
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37935
	addi	$6, $0, 0
	j	eq_cont.37936
eq_else.37935:
	addi	$6, $0, 1
eq_cont.37936:
	bne	$6, $0, eq_else.37937
	fneg	$f3, $f0
	j	eq_cont.37938
eq_else.37937:
	movf	$f3, $f0
eq_cont.37938:
	lf	$f4, 4744($0) # 0.000100
	fsub	$f3, $f4, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37939
	addi	$6, $0, 0
	j	eq_cont.37940
eq_else.37939:
	addi	$6, $0, 1
eq_cont.37940:
	bne	$6, $0, eq_else.37941
	addi	$6, $0, 0
	j	eq_cont.37942
eq_else.37941:
	addi	$6, $0, 1
eq_cont.37942:
	bne	$6, $0, eq_else.37943
	fdiv	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37945
	addi	$6, $0, 0
	j	eq_cont.37946
eq_else.37945:
	addi	$6, $0, 1
eq_cont.37946:
	bne	$6, $0, eq_else.37947
	fneg	$f0, $f0
	j	eq_cont.37948
eq_else.37947:
eq_cont.37948:
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.37949
	lf	$f1, 4880($0) # -1.000000
	j	eq_cont.37950
eq_else.37949:
	lf	$f1, 4920($0) # 1.000000
eq_cont.37950:
	fmul	$f0, $f1, $f0
	lf	$f3, 4872($0) # 0.437500
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37951
	lf	$f3, 4816($0) # 2.437500
	sltf	$30, $f0, $f3
	bne	$30, $0, eq_else.37953
	lf	$f3, 4960($0) # 1.570796
	lf	$f4, 4920($0) # 1.000000
	fdiv	$f0, $f4, $f0
	fmul	$f4, $f0, $f0
	fmul	$f4, $f0, $f4
	lf	$f5, 4864($0) # 0.333333
	fmul	$f4, $f4, $f5
	fsub	$f4, $f0, $f4
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	lf	$f6, 4856($0) # 0.200000
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f6, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f5, $f5, $f6
	lf	$f6, 4848($0) # 0.142857
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	lf	$f6, 4840($0) # 0.111111
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f6, $f5, $f5
	fmul	$f6, $f6, $f6
	fmul	$f6, $f6, $f0
	fmul	$f5, $f6, $f5
	lf	$f6, 4832($0) # 0.089764
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f6, $f5, $f5
	fmul	$f0, $f6, $f0
	fmul	$f0, $f0, $f5
	lf	$f5, 4824($0) # 0.060035
	fmul	$f0, $f0, $f5
	fadd	$f0, $f4, $f0
	fsub	$f0, $f3, $f0
	fmul	$f0, $f1, $f0
	j	eq_cont.37954
eq_else.37953:
	lf	$f3, 4952($0) # 0.785398
	lf	$f4, 4920($0) # 1.000000
	fsub	$f4, $f0, $f4
	lf	$f5, 4920($0) # 1.000000
	fadd	$f0, $f0, $f5
	fdiv	$f0, $f4, $f0
	fmul	$f4, $f0, $f0
	fmul	$f4, $f0, $f4
	lf	$f5, 4864($0) # 0.333333
	fmul	$f4, $f4, $f5
	fsub	$f4, $f0, $f4
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	lf	$f6, 4856($0) # 0.200000
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f6, $f5, $f5
	fmul	$f5, $f0, $f5
	fmul	$f5, $f5, $f6
	lf	$f6, 4848($0) # 0.142857
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f5
	fmul	$f5, $f0, $f5
	lf	$f6, 4840($0) # 0.111111
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f6, $f5, $f5
	fmul	$f6, $f6, $f6
	fmul	$f6, $f6, $f0
	fmul	$f5, $f6, $f5
	lf	$f6, 4832($0) # 0.089764
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f0, $f0
	fmul	$f5, $f5, $f5
	fmul	$f6, $f5, $f5
	fmul	$f0, $f6, $f0
	fmul	$f0, $f0, $f5
	lf	$f5, 4824($0) # 0.060035
	fmul	$f0, $f0, $f5
	fadd	$f0, $f4, $f0
	fadd	$f0, $f3, $f0
	fmul	$f0, $f1, $f0
eq_cont.37954:
	j	eq_cont.37952
eq_else.37951:
	fmul	$f3, $f0, $f0
	fmul	$f3, $f0, $f3
	lf	$f4, 4864($0) # 0.333333
	fmul	$f3, $f3, $f4
	fsub	$f3, $f0, $f3
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4856($0) # 0.200000
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f4, $f0, $f4
	fmul	$f4, $f4, $f5
	lf	$f5, 4848($0) # 0.142857
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f4, $f4, $f4
	fmul	$f4, $f0, $f4
	lf	$f5, 4840($0) # 0.111111
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f5, $f4, $f4
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f0
	fmul	$f4, $f5, $f4
	lf	$f5, 4832($0) # 0.089764
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f0, $f0
	fmul	$f4, $f4, $f4
	fmul	$f5, $f4, $f4
	fmul	$f0, $f5, $f0
	fmul	$f0, $f0, $f4
	lf	$f4, 4824($0) # 0.060035
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	fmul	$f0, $f1, $f0
eq_cont.37952:
	lf	$f1, 4728($0) # 30.000000
	fmul	$f0, $f0, $f1
	lf	$f1, 4720($0) # 3.141593
	fdiv	$f0, $f0, $f1
	j	eq_cont.37944
eq_else.37943:
	lf	$f0, 4736($0) # 15.000000
eq_cont.37944:
	floor	$f1, $f0
	fsub	$f0, $f0, $f1
	lf	$f1, 8($5)
	lw	$5, 20($2)
	lf	$f3, 8($5)
	fsub	$f1, $f1, $f3
	lw	$2, 16($2)
	lf	$f3, 8($2)
	sqrt	$f3, $f3
	fmul	$f1, $f1, $f3
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f2
	bne	$30, $0, eq_else.37955
	addi	$2, $0, 0
	j	eq_cont.37956
eq_else.37955:
	addi	$2, $0, 1
eq_cont.37956:
	bne	$2, $0, eq_else.37957
	fneg	$f3, $f2
	j	eq_cont.37958
eq_else.37957:
	movf	$f3, $f2
eq_cont.37958:
	lf	$f4, 4744($0) # 0.000100
	fsub	$f3, $f4, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	bne	$30, $0, eq_else.37959
	addi	$2, $0, 0
	j	eq_cont.37960
eq_else.37959:
	addi	$2, $0, 1
eq_cont.37960:
	bne	$2, $0, eq_else.37961
	addi	$2, $0, 0
	j	eq_cont.37962
eq_else.37961:
	addi	$2, $0, 1
eq_cont.37962:
	bne	$2, $0, eq_else.37963
	fdiv	$f1, $f1, $f2
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37965
	addi	$2, $0, 0
	j	eq_cont.37966
eq_else.37965:
	addi	$2, $0, 1
eq_cont.37966:
	bne	$2, $0, eq_else.37967
	fneg	$f1, $f1
	j	eq_cont.37968
eq_else.37967:
eq_cont.37968:
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37969
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.37970
eq_else.37969:
	lf	$f2, 4920($0) # 1.000000
eq_cont.37970:
	fmul	$f1, $f2, $f1
	lf	$f3, 4872($0) # 0.437500
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.37971
	lf	$f3, 4816($0) # 2.437500
	sltf	$30, $f1, $f3
	bne	$30, $0, eq_else.37973
	lf	$f3, 4960($0) # 1.570796
	lf	$f4, 4920($0) # 1.000000
	fdiv	$f1, $f4, $f1
	fmul	$f4, $f1, $f1
	fmul	$f4, $f1, $f4
	lf	$f5, 4864($0) # 0.333333
	fmul	$f4, $f4, $f5
	fsub	$f4, $f1, $f4
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	lf	$f6, 4856($0) # 0.200000
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f6, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f5, $f5, $f6
	lf	$f6, 4848($0) # 0.142857
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	lf	$f6, 4840($0) # 0.111111
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f6, $f5, $f5
	fmul	$f6, $f6, $f6
	fmul	$f6, $f6, $f1
	fmul	$f5, $f6, $f5
	lf	$f6, 4832($0) # 0.089764
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f6, $f5, $f5
	fmul	$f1, $f6, $f1
	fmul	$f1, $f1, $f5
	lf	$f5, 4824($0) # 0.060035
	fmul	$f1, $f1, $f5
	fadd	$f1, $f4, $f1
	fsub	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
	j	eq_cont.37974
eq_else.37973:
	lf	$f3, 4952($0) # 0.785398
	lf	$f4, 4920($0) # 1.000000
	fsub	$f4, $f1, $f4
	lf	$f5, 4920($0) # 1.000000
	fadd	$f1, $f1, $f5
	fdiv	$f1, $f4, $f1
	fmul	$f4, $f1, $f1
	fmul	$f4, $f1, $f4
	lf	$f5, 4864($0) # 0.333333
	fmul	$f4, $f4, $f5
	fsub	$f4, $f1, $f4
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	lf	$f6, 4856($0) # 0.200000
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f6, $f5, $f5
	fmul	$f5, $f1, $f5
	fmul	$f5, $f5, $f6
	lf	$f6, 4848($0) # 0.142857
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f5
	fmul	$f5, $f1, $f5
	lf	$f6, 4840($0) # 0.111111
	fmul	$f5, $f5, $f6
	fadd	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f6, $f5, $f5
	fmul	$f6, $f6, $f6
	fmul	$f6, $f6, $f1
	fmul	$f5, $f6, $f5
	lf	$f6, 4832($0) # 0.089764
	fmul	$f5, $f5, $f6
	fsub	$f4, $f4, $f5
	fmul	$f5, $f1, $f1
	fmul	$f5, $f5, $f5
	fmul	$f6, $f5, $f5
	fmul	$f1, $f6, $f1
	fmul	$f1, $f1, $f5
	lf	$f5, 4824($0) # 0.060035
	fmul	$f1, $f1, $f5
	fadd	$f1, $f4, $f1
	fadd	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.37974:
	j	eq_cont.37972
eq_else.37971:
	fmul	$f3, $f1, $f1
	fmul	$f3, $f1, $f3
	lf	$f4, 4864($0) # 0.333333
	fmul	$f3, $f3, $f4
	fsub	$f3, $f1, $f3
	fmul	$f4, $f1, $f1
	fmul	$f4, $f4, $f4
	fmul	$f4, $f1, $f4
	lf	$f5, 4856($0) # 0.200000
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f5, $f4, $f4
	fmul	$f4, $f1, $f4
	fmul	$f4, $f4, $f5
	lf	$f5, 4848($0) # 0.142857
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f4, $f4, $f4
	fmul	$f4, $f4, $f4
	fmul	$f4, $f1, $f4
	lf	$f5, 4840($0) # 0.111111
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f5, $f4, $f4
	fmul	$f5, $f5, $f5
	fmul	$f5, $f5, $f1
	fmul	$f4, $f5, $f4
	lf	$f5, 4832($0) # 0.089764
	fmul	$f4, $f4, $f5
	fsub	$f3, $f3, $f4
	fmul	$f4, $f1, $f1
	fmul	$f4, $f4, $f4
	fmul	$f5, $f4, $f4
	fmul	$f1, $f5, $f1
	fmul	$f1, $f1, $f4
	lf	$f4, 4824($0) # 0.060035
	fmul	$f1, $f1, $f4
	fadd	$f1, $f3, $f1
	fmul	$f1, $f2, $f1
eq_cont.37972:
	lf	$f2, 4728($0) # 30.000000
	fmul	$f1, $f1, $f2
	lf	$f2, 4720($0) # 3.141593
	fdiv	$f1, $f1, $f2
	j	eq_cont.37964
eq_else.37963:
	lf	$f1, 4736($0) # 15.000000
eq_cont.37964:
	floor	$f2, $f1
	fsub	$f1, $f1, $f2
	lf	$f2, 4712($0) # 0.150000
	lf	$f3, 4944($0) # 0.500000
	fsub	$f0, $f3, $f0
	fmul	$f0, $f0, $f0
	fsub	$f0, $f2, $f0
	lf	$f2, 4944($0) # 0.500000
	fsub	$f1, $f2, $f1
	fmul	$f1, $f1, $f1
	fsub	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37975
	addi	$2, $0, 0
	j	eq_cont.37976
eq_else.37975:
	addi	$2, $0, 1
eq_cont.37976:
	bne	$2, $0, eq_else.37977
	j	eq_cont.37978
eq_else.37977:
	lf	$f0, 4888($0) # 0.000000
eq_cont.37978:
	lf	$f1, 4704($0) # 255.000000
	fmul	$f0, $f1, $f0
	lf	$f1, 4696($0) # 0.300000
	fdiv	$f0, $f0, $f1
	addi	$2, $0, 3616
	sf	$f0, 0($2)
	jr $31
eq_else.37934:
	jr $31
add_light.3043:
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.37981
	addi	$2, $0, 0
	j	eq_cont.37982
eq_else.37981:
	addi	$2, $0, 1
eq_cont.37982:
	bne	$2, $0, eq_else.37983
	j	eq_cont.37984
eq_else.37983:
	addi	$2, $0, 3648
	lf	$f3, 0($2)
	addi	$2, $0, 3600
	lf	$f4, 0($2)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$2, $0, 3648
	sf	$f3, 0($2)
	addi	$2, $0, 3656
	lf	$f3, 0($2)
	addi	$2, $0, 3608
	lf	$f4, 0($2)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$2, $0, 3656
	sf	$f3, 0($2)
	addi	$2, $0, 3664
	lf	$f3, 0($2)
	addi	$2, $0, 3616
	lf	$f4, 0($2)
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.37984:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.37985
	addi	$2, $0, 0
	j	eq_cont.37986
eq_else.37985:
	addi	$2, $0, 1
eq_cont.37986:
	bne	$2, $0, eq_else.37987
	jr $31
eq_else.37987:
	fmul	$f0, $f1, $f1
	fmul	$f0, $f0, $f0
	fmul	$f0, $f0, $f2
	addi	$2, $0, 3648
	lf	$f1, 0($2)
	fadd	$f1, $f1, $f0
	addi	$2, $0, 3648
	sf	$f1, 0($2)
	addi	$2, $0, 3656
	lf	$f1, 0($2)
	fadd	$f1, $f1, $f0
	addi	$2, $0, 3656
	sf	$f1, 0($2)
	addi	$2, $0, 3664
	lf	$f1, 0($2)
	fadd	$f0, $f1, $f0
	addi	$2, $0, 3664
	sf	$f0, 0($2)
	jr $31
trace_reflections.3047:
	bge	$2, $0, ge.37990
	jr $31
ge.37990:
	slli	$6, $2, 2
	addi	$6, $6, 3868
	lw	$6, 0($6)
	lw	$7, 4($6)
	lf	$f2, 4656($0) # 1000000000.000000
	addi	$8, $0, 3540
	sf	$f2, 0($8)
	addi	$8, $0, 0
	addi	$9, $0, 3524
	lw	$9, 0($9)
	sw	$2, 0($3)
	sf	$f1, 8($3)
	sw	$5, 16($3)
	sf	$f0, 24($3)
	sw	$7, 32($3)
	sw	$6, 36($3)
	mov	$6, $7
	mov	$5, $9
	mov	$2, $8
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.37994
	addi	$2, $0, 0
	j	eq_cont.37995
eq_else.37994:
	addi	$2, $0, 1
eq_cont.37995:
	bne	$2, $0, eq_else.37996
	addi	$2, $0, 0
	j	eq_cont.37997
eq_else.37996:
	addi	$2, $0, 1
eq_cont.37997:
	bne	$2, $0, eq_else.37998
	addi	$2, $0, 0
	j	eq_cont.37999
eq_else.37998:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38000
	addi	$2, $0, 0
	j	eq_cont.38001
eq_else.38000:
	addi	$2, $0, 1
eq_cont.38001:
	bne	$2, $0, eq_else.38002
	addi	$2, $0, 0
	j	eq_cont.38003
eq_else.38002:
	addi	$2, $0, 1
eq_cont.38003:
eq_cont.37999:
	bne	$2, $0, eq_else.38004
	j	eq_cont.38005
eq_else.38004:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	addi	$5, $0, 4
	mul	$2, $2, $5
	addi	$5, $0, 3536
	lw	$5, 0($5)
	add	$2, $2, $5
	lw	$5, 36($3)
	lw	$6, 0($5)
	bne	$2, $6, eq_else.38006
	addi	$2, $0, 3524
	lw	$2, 0($2)
	lw	$6, 0($2)
	lw	$7, 0($6)
	addi	$30, $0, -1
	bne	$7, $30, eq_else.38008
	addi	$2, $0, 0
	j	eq_cont.38009
eq_else.38008:
	sw	$6, 40($3)
	sw	$2, 44($3)
	addi	$30, $0, 99
	bne	$7, $30, eq_else.38010
	addi	$2, $0, 1
	j	eq_cont.38011
eq_else.38010:
	addi	$8, $0, 3860
	addi	$9, $0, 3548
	mov	$6, $9
	mov	$5, $8
	mov	$2, $7
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	solver_fast.2928
	addi	$3, $3, -56
	lw	$31, 52($3)
	bne	$2, $0, eq_else.38012
	addi	$2, $0, 0
	j	eq_cont.38013
eq_else.38012:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38014
	addi	$2, $0, 0
	j	eq_cont.38015
eq_else.38014:
	addi	$2, $0, 1
eq_cont.38015:
	bne	$2, $0, eq_else.38016
	addi	$2, $0, 0
	j	eq_cont.38017
eq_else.38016:
	addi	$2, $0, 1
eq_cont.38017:
	bne	$2, $0, eq_else.38018
	addi	$2, $0, 0
	j	eq_cont.38019
eq_else.38018:
	addi	$2, $0, 1
	lw	$5, 40($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -56
	lw	$31, 52($3)
	bne	$2, $0, eq_else.38020
	addi	$2, $0, 0
	j	eq_cont.38021
eq_else.38020:
	addi	$2, $0, 1
eq_cont.38021:
eq_cont.38019:
eq_cont.38013:
eq_cont.38011:
	bne	$2, $0, eq_else.38022
	addi	$2, $0, 1
	lw	$5, 44($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.38023
eq_else.38022:
	addi	$2, $0, 1
	lw	$5, 40($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -56
	lw	$31, 52($3)
	bne	$2, $0, eq_else.38024
	addi	$2, $0, 1
	lw	$5, 44($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.38025
eq_else.38024:
	addi	$2, $0, 1
eq_cont.38025:
eq_cont.38023:
eq_cont.38009:
	bne	$2, $0, eq_else.38026
	lw	$2, 32($3)
	lw	$5, 0($2)
	addi	$6, $0, 3576
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	fmul	$f0, $f0, $f1
	addi	$6, $0, 3584
	lf	$f1, 0($6)
	lf	$f2, 8($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3592
	lf	$f1, 0($6)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lw	$5, 36($3)
	lf	$f1, 8($5)
	lf	$f2, 24($3)
	fmul	$f3, $f1, $f2
	fmul	$f0, $f3, $f0
	lw	$2, 0($2)
	lw	$5, 16($3)
	lf	$f3, 0($5)
	lf	$f4, 0($2)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($5)
	lf	$f5, 8($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($5)
	lf	$f5, 16($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f3
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.38028
	addi	$2, $0, 0
	j	eq_cont.38029
eq_else.38028:
	addi	$2, $0, 1
eq_cont.38029:
	bne	$2, $0, eq_else.38030
	j	eq_cont.38031
eq_else.38030:
	addi	$2, $0, 3648
	lf	$f3, 0($2)
	addi	$2, $0, 3600
	lf	$f4, 0($2)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$2, $0, 3648
	sf	$f3, 0($2)
	addi	$2, $0, 3656
	lf	$f3, 0($2)
	addi	$2, $0, 3608
	lf	$f4, 0($2)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$2, $0, 3656
	sf	$f3, 0($2)
	addi	$2, $0, 3664
	lf	$f3, 0($2)
	addi	$2, $0, 3616
	lf	$f4, 0($2)
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38031:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38032
	addi	$2, $0, 0
	j	eq_cont.38033
eq_else.38032:
	addi	$2, $0, 1
eq_cont.38033:
	bne	$2, $0, eq_else.38034
	j	eq_cont.38035
eq_else.38034:
	fmul	$f0, $f1, $f1
	fmul	$f0, $f0, $f0
	lf	$f1, 8($3)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3648
	lf	$f3, 0($2)
	fadd	$f3, $f3, $f0
	addi	$2, $0, 3648
	sf	$f3, 0($2)
	addi	$2, $0, 3656
	lf	$f3, 0($2)
	fadd	$f3, $f3, $f0
	addi	$2, $0, 3656
	sf	$f3, 0($2)
	addi	$2, $0, 3664
	lf	$f3, 0($2)
	fadd	$f0, $f3, $f0
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38035:
	j	eq_cont.38027
eq_else.38026:
eq_cont.38027:
	j	eq_cont.38007
eq_else.38006:
eq_cont.38007:
eq_cont.38005:
	lw	$2, 0($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38036
	jr $31
ge.38036:
	slli	$5, $2, 2
	addi	$5, $5, 3868
	lw	$5, 0($5)
	lw	$6, 4($5)
	lf	$f0, 4656($0) # 1000000000.000000
	addi	$7, $0, 3540
	sf	$f0, 0($7)
	addi	$7, $0, 3524
	lw	$7, 0($7)
	lw	$8, 0($7)
	lw	$9, 0($8)
	sw	$2, 48($3)
	sw	$6, 52($3)
	sw	$5, 56($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.38038
	j	eq_cont.38039
eq_else.38038:
	sw	$7, 60($3)
	addi	$30, $0, 99
	bne	$9, $30, eq_else.38040
	lw	$9, 4($8)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.38042
	j	eq_cont.38043
eq_else.38042:
	slli	$9, $9, 2
	addi	$9, $9, 3324
	lw	$9, 0($9)
	addi	$10, $0, 0
	sw	$8, 64($3)
	mov	$5, $9
	mov	$2, $10
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38044
	j	eq_cont.38045
eq_else.38044:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38046
	j	eq_cont.38047
eq_else.38046:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38048
	j	eq_cont.38049
eq_else.38048:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	addi	$2, $0, 5
	lw	$5, 64($3)
	lw	$6, 52($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -72
	lw	$31, 68($3)
eq_cont.38049:
eq_cont.38047:
eq_cont.38045:
eq_cont.38043:
	j	eq_cont.38041
eq_else.38040:
	sw	$8, 64($3)
	mov	$5, $6
	mov	$2, $9
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solver_fast2.2946
	addi	$3, $3, -72
	lw	$31, 68($3)
	bne	$2, $0, eq_else.38050
	j	eq_cont.38051
eq_else.38050:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38052
	addi	$2, $0, 0
	j	eq_cont.38053
eq_else.38052:
	addi	$2, $0, 1
eq_cont.38053:
	bne	$2, $0, eq_else.38054
	addi	$2, $0, 0
	j	eq_cont.38055
eq_else.38054:
	addi	$2, $0, 1
eq_cont.38055:
	bne	$2, $0, eq_else.38056
	j	eq_cont.38057
eq_else.38056:
	lw	$2, 64($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38058
	j	eq_cont.38059
eq_else.38058:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38060
	j	eq_cont.38061
eq_else.38060:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38062
	j	eq_cont.38063
eq_else.38062:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$2, 64($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38064
	j	eq_cont.38065
eq_else.38064:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 52($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_each_element_fast.3017
	addi	$3, $3, -72
	lw	$31, 68($3)
	addi	$2, $0, 5
	lw	$5, 64($3)
	lw	$6, 52($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -72
	lw	$31, 68($3)
eq_cont.38065:
eq_cont.38063:
eq_cont.38061:
eq_cont.38059:
eq_cont.38057:
eq_cont.38051:
eq_cont.38041:
	addi	$2, $0, 1
	lw	$5, 60($3)
	lw	$6, 52($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -72
	lw	$31, 68($3)
eq_cont.38039:
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38066
	addi	$2, $0, 0
	j	eq_cont.38067
eq_else.38066:
	addi	$2, $0, 1
eq_cont.38067:
	bne	$2, $0, eq_else.38068
	addi	$2, $0, 0
	j	eq_cont.38069
eq_else.38068:
	addi	$2, $0, 1
eq_cont.38069:
	bne	$2, $0, eq_else.38070
	addi	$2, $0, 0
	j	eq_cont.38071
eq_else.38070:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38072
	addi	$2, $0, 0
	j	eq_cont.38073
eq_else.38072:
	addi	$2, $0, 1
eq_cont.38073:
	bne	$2, $0, eq_else.38074
	addi	$2, $0, 0
	j	eq_cont.38075
eq_else.38074:
	addi	$2, $0, 1
eq_cont.38075:
eq_cont.38071:
	bne	$2, $0, eq_else.38076
	j	eq_cont.38077
eq_else.38076:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	addi	$5, $0, 4
	mul	$2, $2, $5
	addi	$5, $0, 3536
	lw	$5, 0($5)
	add	$2, $2, $5
	lw	$5, 56($3)
	lw	$6, 0($5)
	bne	$2, $6, eq_else.38078
	addi	$2, $0, 0
	addi	$6, $0, 3524
	lw	$6, 0($6)
	mov	$5, $6
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -72
	lw	$31, 68($3)
	bne	$2, $0, eq_else.38080
	lw	$2, 52($3)
	lw	$5, 0($2)
	addi	$6, $0, 3576
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	fmul	$f0, $f0, $f1
	addi	$6, $0, 3584
	lf	$f1, 0($6)
	lf	$f2, 8($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3592
	lf	$f1, 0($6)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lw	$5, 56($3)
	lf	$f1, 8($5)
	lf	$f2, 24($3)
	fmul	$f3, $f1, $f2
	fmul	$f0, $f3, $f0
	lw	$2, 0($2)
	lw	$5, 16($3)
	lf	$f3, 0($5)
	lf	$f4, 0($2)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($5)
	lf	$f5, 8($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($5)
	lf	$f5, 16($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f3
	lf	$f3, 8($3)
	movf	$f2, $f3
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	add_light.3043
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.38081
eq_else.38080:
eq_cont.38081:
	j	eq_cont.38079
eq_else.38078:
eq_cont.38079:
eq_cont.38077:
	lw	$2, 48($3)
	addi	$2, $2, -1
	lf	$f0, 24($3)
	lf	$f1, 8($3)
	lw	$5, 16($3)
	j	trace_reflections.3047
trace_ray.3052:
	addi	$30, $0, 4
	ble	$2, $30, le.38082
	jr $31
le.38082:
	lw	$7, 8($6)
	lf	$f2, 4656($0) # 1000000000.000000
	addi	$8, $0, 3540
	sf	$f2, 0($8)
	addi	$8, $0, 0
	addi	$9, $0, 3524
	lw	$9, 0($9)
	sf	$f1, 0($3)
	sw	$6, 8($3)
	sf	$f0, 16($3)
	sw	$5, 24($3)
	sw	$7, 28($3)
	sw	$2, 32($3)
	mov	$6, $5
	mov	$2, $8
	mov	$5, $9
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_or_matrix.3011
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38085
	addi	$2, $0, 0
	j	eq_cont.38086
eq_else.38085:
	addi	$2, $0, 1
eq_cont.38086:
	bne	$2, $0, eq_else.38087
	addi	$2, $0, 0
	j	eq_cont.38088
eq_else.38087:
	addi	$2, $0, 1
eq_cont.38088:
	bne	$2, $0, eq_else.38089
	addi	$2, $0, 0
	j	eq_cont.38090
eq_else.38089:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38091
	addi	$2, $0, 0
	j	eq_cont.38092
eq_else.38091:
	addi	$2, $0, 1
eq_cont.38092:
	bne	$2, $0, eq_else.38093
	addi	$2, $0, 0
	j	eq_cont.38094
eq_else.38093:
	addi	$2, $0, 1
eq_cont.38094:
eq_cont.38090:
	bne	$2, $0, eq_else.38095
	addi	$2, $0, -1
	lw	$5, 32($3)
	slli	$6, $5, 2
	lw	$7, 28($3)
	add $30, $7, $6
	sw	$2, 0($30)
	bne	$5, $0, eq_else.38096
	jr $31
eq_else.38096:
	lw	$2, 24($3)
	lf	$f0, 0($2)
	addi	$5, $0, 3292
	lf	$f1, 0($5)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($2)
	addi	$5, $0, 3300
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($2)
	addi	$2, $0, 3308
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38098
	addi	$2, $0, 0
	j	eq_cont.38099
eq_else.38098:
	addi	$2, $0, 1
eq_cont.38099:
	bne	$2, $0, eq_else.38100
	jr $31
eq_else.38100:
	fmul	$f1, $f0, $f0
	fmul	$f0, $f1, $f0
	lf	$f1, 16($3)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3316
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3648
	lf	$f1, 0($2)
	fadd	$f1, $f1, $f0
	addi	$2, $0, 3648
	sf	$f1, 0($2)
	addi	$2, $0, 3656
	lf	$f1, 0($2)
	fadd	$f1, $f1, $f0
	addi	$2, $0, 3656
	sf	$f1, 0($2)
	addi	$2, $0, 3664
	lf	$f1, 0($2)
	fadd	$f0, $f1, $f0
	addi	$2, $0, 3664
	sf	$f0, 0($2)
	jr $31
eq_else.38095:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	slli	$5, $2, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lw	$6, 8($5)
	lw	$7, 28($5)
	lf	$f0, 0($7)
	lf	$f1, 16($3)
	fmul	$f0, $f0, $f1
	lw	$7, 4($5)
	sw	$6, 36($3)
	sf	$f0, 40($3)
	sw	$2, 48($3)
	sw	$5, 52($3)
	addi	$30, $0, 1
	bne	$7, $30, eq_else.38103
	addi	$7, $0, 3536
	lw	$7, 0($7)
	lf	$f2, 4888($0) # 0.000000
	addi	$8, $0, 3576
	sf	$f2, 0($8)
	addi	$8, $0, 3584
	sf	$f2, 0($8)
	addi	$8, $0, 3592
	sf	$f2, 0($8)
	addi	$8, $7, -1
	addi	$7, $7, -1
	slli	$7, $7, 3
	lw	$9, 24($3)
	add	$30, $9, $7
	lf	$f2, 0($30)
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f2
	sltf	$29, $f2, $f3
	bne	$30, $29, eq_else.38105
	addi	$7, $0, 1
	j	eq_cont.38106
eq_else.38105:
	addi	$7, $0, 0
eq_cont.38106:
	bne	$7, $0, eq_else.38107
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f2
	bne	$30, $0, eq_else.38109
	addi	$7, $0, 0
	j	eq_cont.38110
eq_else.38109:
	addi	$7, $0, 1
eq_cont.38110:
	bne	$7, $0, eq_else.38111
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.38112
eq_else.38111:
	lf	$f2, 4920($0) # 1.000000
eq_cont.38112:
	j	eq_cont.38108
eq_else.38107:
	lf	$f2, 4888($0) # 0.000000
eq_cont.38108:
	fneg	$f2, $f2
	slli	$7, $8, 3
	addi	$7, $7, 3576
	sf	$f2, 0($7)
	j	eq_cont.38104
eq_else.38103:
	addi	$30, $0, 2
	bne	$7, $30, eq_else.38113
	lw	$7, 16($5)
	lf	$f2, 0($7)
	fneg	$f2, $f2
	addi	$7, $0, 3576
	sf	$f2, 0($7)
	lw	$7, 16($5)
	lf	$f2, 8($7)
	fneg	$f2, $f2
	addi	$7, $0, 3584
	sf	$f2, 0($7)
	lw	$7, 16($5)
	lf	$f2, 16($7)
	fneg	$f2, $f2
	addi	$7, $0, 3592
	sf	$f2, 0($7)
	j	eq_cont.38114
eq_else.38113:
	mov	$2, $5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	get_nvector_second.3035
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38114:
eq_cont.38104:
	addi	$2, $0, 3548
	lf	$f0, 0($2)
	addi	$2, $0, 3696
	sf	$f0, 0($2)
	addi	$2, $0, 3556
	lf	$f0, 0($2)
	addi	$2, $0, 3704
	sf	$f0, 0($2)
	addi	$2, $0, 3564
	lf	$f0, 0($2)
	addi	$2, $0, 3712
	sf	$f0, 0($2)
	addi	$5, $0, 3548
	lw	$2, 52($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	utexture.3040
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$2, $0, 4
	lw	$5, 48($3)
	mul	$2, $5, $2
	addi	$5, $0, 3536
	lw	$5, 0($5)
	add	$2, $2, $5
	lw	$5, 32($3)
	slli	$6, $5, 2
	lw	$7, 28($3)
	add $30, $7, $6
	sw	$2, 0($30)
	lw	$2, 8($3)
	lw	$6, 4($2)
	slli	$8, $5, 2
	add	$30, $6, $8
	lw	$6, 0($30)
	addi	$8, $0, 3548
	lf	$f0, 0($8)
	sf	$f0, 0($6)
	addi	$8, $0, 3556
	lf	$f0, 0($8)
	sf	$f0, 8($6)
	addi	$8, $0, 3564
	lf	$f0, 0($8)
	sf	$f0, 16($6)
	lw	$6, 12($2)
	lw	$8, 52($3)
	lw	$9, 28($8)
	lf	$f0, 0($9)
	lf	$f1, 4944($0) # 0.500000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38115
	addi	$9, $0, 0
	j	eq_cont.38116
eq_else.38115:
	addi	$9, $0, 1
eq_cont.38116:
	bne	$9, $0, eq_else.38117
	addi	$9, $0, 0
	j	eq_cont.38118
eq_else.38117:
	addi	$9, $0, 1
eq_cont.38118:
	bne	$9, $0, eq_else.38119
	addi	$9, $0, 1
	slli	$10, $5, 2
	add $30, $6, $10
	sw	$9, 0($30)
	lw	$6, 16($2)
	slli	$9, $5, 2
	add	$30, $6, $9
	lw	$9, 0($30)
	addi	$10, $0, 3600
	lf	$f0, 0($10)
	sf	$f0, 0($9)
	addi	$10, $0, 3608
	lf	$f0, 0($10)
	sf	$f0, 8($9)
	addi	$10, $0, 3616
	lf	$f0, 0($10)
	sf	$f0, 16($9)
	slli	$9, $5, 2
	add	$30, $6, $9
	lw	$6, 0($30)
	lf	$f0, 4640($0) # 0.003906
	lf	$f1, 40($3)
	fmul	$f0, $f0, $f1
	lf	$f2, 0($6)
	fmul	$f2, $f2, $f0
	sf	$f2, 0($6)
	lf	$f2, 8($6)
	fmul	$f2, $f2, $f0
	sf	$f2, 8($6)
	lf	$f2, 16($6)
	fmul	$f0, $f2, $f0
	sf	$f0, 16($6)
	lw	$6, 28($2)
	slli	$9, $5, 2
	add	$30, $6, $9
	lw	$6, 0($30)
	addi	$9, $0, 3576
	lf	$f0, 0($9)
	sf	$f0, 0($6)
	addi	$9, $0, 3584
	lf	$f0, 0($9)
	sf	$f0, 8($6)
	addi	$9, $0, 3592
	lf	$f0, 0($9)
	sf	$f0, 16($6)
	j	eq_cont.38120
eq_else.38119:
	addi	$9, $0, 0
	slli	$10, $5, 2
	add $30, $6, $10
	sw	$9, 0($30)
eq_cont.38120:
	lf	$f0, 4632($0) # -2.000000
	lw	$6, 24($3)
	lf	$f1, 0($6)
	addi	$9, $0, 3576
	lf	$f2, 0($9)
	fmul	$f1, $f1, $f2
	lf	$f2, 8($6)
	addi	$9, $0, 3584
	lf	$f3, 0($9)
	fmul	$f2, $f2, $f3
	fadd	$f1, $f1, $f2
	lf	$f2, 16($6)
	addi	$9, $0, 3592
	lf	$f3, 0($9)
	fmul	$f2, $f2, $f3
	fadd	$f1, $f1, $f2
	fmul	$f0, $f0, $f1
	lf	$f1, 0($6)
	addi	$9, $0, 3576
	lf	$f2, 0($9)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	sf	$f1, 0($6)
	lf	$f1, 8($6)
	addi	$9, $0, 3584
	lf	$f2, 0($9)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	sf	$f1, 8($6)
	lf	$f1, 16($6)
	addi	$9, $0, 3592
	lf	$f2, 0($9)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f1, $f0
	sf	$f0, 16($6)
	lw	$9, 28($8)
	lf	$f0, 8($9)
	lf	$f1, 16($3)
	fmul	$f0, $f1, $f0
	addi	$9, $0, 3524
	lw	$9, 0($9)
	lw	$10, 0($9)
	lw	$11, 0($10)
	sf	$f0, 56($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.38121
	addi	$2, $0, 0
	j	eq_cont.38122
eq_else.38121:
	sw	$10, 64($3)
	sw	$9, 68($3)
	addi	$30, $0, 99
	bne	$11, $30, eq_else.38123
	addi	$2, $0, 1
	j	eq_cont.38124
eq_else.38123:
	addi	$12, $0, 3860
	addi	$13, $0, 3548
	mov	$6, $13
	mov	$5, $12
	mov	$2, $11
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	solver_fast.2928
	addi	$3, $3, -80
	lw	$31, 76($3)
	bne	$2, $0, eq_else.38125
	addi	$2, $0, 0
	j	eq_cont.38126
eq_else.38125:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38127
	addi	$2, $0, 0
	j	eq_cont.38128
eq_else.38127:
	addi	$2, $0, 1
eq_cont.38128:
	bne	$2, $0, eq_else.38129
	addi	$2, $0, 0
	j	eq_cont.38130
eq_else.38129:
	addi	$2, $0, 1
eq_cont.38130:
	bne	$2, $0, eq_else.38131
	addi	$2, $0, 0
	j	eq_cont.38132
eq_else.38131:
	addi	$2, $0, 1
	lw	$5, 64($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -80
	lw	$31, 76($3)
	bne	$2, $0, eq_else.38133
	addi	$2, $0, 0
	j	eq_cont.38134
eq_else.38133:
	addi	$2, $0, 1
eq_cont.38134:
eq_cont.38132:
eq_cont.38126:
eq_cont.38124:
	bne	$2, $0, eq_else.38135
	addi	$2, $0, 1
	lw	$5, 68($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.38136
eq_else.38135:
	addi	$2, $0, 1
	lw	$5, 64($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -80
	lw	$31, 76($3)
	bne	$2, $0, eq_else.38137
	addi	$2, $0, 1
	lw	$5, 68($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -80
	lw	$31, 76($3)
	j	eq_cont.38138
eq_else.38137:
	addi	$2, $0, 1
eq_cont.38138:
eq_cont.38136:
eq_cont.38122:
	bne	$2, $0, eq_else.38139
	addi	$2, $0, 3576
	lf	$f0, 0($2)
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3584
	lf	$f1, 0($2)
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3592
	lf	$f1, 0($2)
	addi	$2, $0, 3308
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	lf	$f1, 40($3)
	fmul	$f0, $f0, $f1
	lw	$2, 24($3)
	lf	$f2, 0($2)
	addi	$5, $0, 3292
	lf	$f3, 0($5)
	fmul	$f2, $f2, $f3
	lf	$f3, 8($2)
	addi	$5, $0, 3300
	lf	$f4, 0($5)
	fmul	$f3, $f3, $f4
	fadd	$f2, $f2, $f3
	lf	$f3, 16($2)
	addi	$5, $0, 3308
	lf	$f4, 0($5)
	fmul	$f3, $f3, $f4
	fadd	$f2, $f2, $f3
	fneg	$f2, $f2
	lf	$f3, 4888($0) # 0.000000
	sltf	$30, $f3, $f0
	bne	$30, $0, eq_else.38141
	addi	$5, $0, 0
	j	eq_cont.38142
eq_else.38141:
	addi	$5, $0, 1
eq_cont.38142:
	bne	$5, $0, eq_else.38143
	j	eq_cont.38144
eq_else.38143:
	addi	$5, $0, 3648
	lf	$f3, 0($5)
	addi	$5, $0, 3600
	lf	$f4, 0($5)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$5, $0, 3648
	sf	$f3, 0($5)
	addi	$5, $0, 3656
	lf	$f3, 0($5)
	addi	$5, $0, 3608
	lf	$f4, 0($5)
	fmul	$f4, $f0, $f4
	fadd	$f3, $f3, $f4
	addi	$5, $0, 3656
	sf	$f3, 0($5)
	addi	$5, $0, 3664
	lf	$f3, 0($5)
	addi	$5, $0, 3616
	lf	$f4, 0($5)
	fmul	$f0, $f0, $f4
	fadd	$f0, $f3, $f0
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38144:
	lf	$f0, 4888($0) # 0.000000
	sltf	$30, $f0, $f2
	bne	$30, $0, eq_else.38145
	addi	$5, $0, 0
	j	eq_cont.38146
eq_else.38145:
	addi	$5, $0, 1
eq_cont.38146:
	bne	$5, $0, eq_else.38147
	j	eq_cont.38148
eq_else.38147:
	fmul	$f0, $f2, $f2
	fmul	$f0, $f0, $f0
	lf	$f2, 56($3)
	fmul	$f0, $f0, $f2
	addi	$5, $0, 3648
	lf	$f3, 0($5)
	fadd	$f3, $f3, $f0
	addi	$5, $0, 3648
	sf	$f3, 0($5)
	addi	$5, $0, 3656
	lf	$f3, 0($5)
	fadd	$f3, $f3, $f0
	addi	$5, $0, 3656
	sf	$f3, 0($5)
	addi	$5, $0, 3664
	lf	$f3, 0($5)
	fadd	$f0, $f3, $f0
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38148:
	j	eq_cont.38140
eq_else.38139:
eq_cont.38140:
	addi	$2, $0, 3548
	lf	$f0, 0($2)
	addi	$2, $0, 3720
	sf	$f0, 0($2)
	addi	$2, $0, 3556
	lf	$f0, 0($2)
	addi	$2, $0, 3728
	sf	$f0, 0($2)
	addi	$2, $0, 3564
	lf	$f0, 0($2)
	addi	$2, $0, 3736
	sf	$f0, 0($2)
	addi	$2, $0, 3000
	lw	$2, 0($2)
	addi	$5, $2, -1
	addi	$2, $0, 3548
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	setup_startp_constants.2963
	addi	$3, $3, -80
	lw	$31, 76($3)
	addi	$2, $0, 4588
	lw	$2, 0($2)
	addi	$2, $2, -1
	bge	$2, $0, ge_else.38149
	j	ge_cont.38150
ge_else.38149:
	slli	$5, $2, 2
	addi	$5, $5, 3868
	lw	$5, 0($5)
	lw	$6, 4($5)
	lf	$f0, 4656($0) # 1000000000.000000
	addi	$7, $0, 3540
	sf	$f0, 0($7)
	addi	$7, $0, 3524
	lw	$7, 0($7)
	lw	$8, 0($7)
	lw	$9, 0($8)
	sw	$2, 72($3)
	sw	$6, 76($3)
	sw	$5, 80($3)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.38151
	j	eq_cont.38152
eq_else.38151:
	sw	$7, 84($3)
	addi	$30, $0, 99
	bne	$9, $30, eq_else.38153
	lw	$9, 4($8)
	addi	$30, $0, -1
	bne	$9, $30, eq_else.38155
	j	eq_cont.38156
eq_else.38155:
	slli	$9, $9, 2
	addi	$9, $9, 3324
	lw	$9, 0($9)
	addi	$10, $0, 0
	sw	$8, 88($3)
	mov	$5, $9
	mov	$2, $10
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38157
	j	eq_cont.38158
eq_else.38157:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38159
	j	eq_cont.38160
eq_else.38159:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38161
	j	eq_cont.38162
eq_else.38161:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	addi	$2, $0, 5
	lw	$5, 88($3)
	lw	$6, 76($3)
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -96
	lw	$31, 92($3)
eq_cont.38162:
eq_cont.38160:
eq_cont.38158:
eq_cont.38156:
	j	eq_cont.38154
eq_else.38153:
	sw	$8, 88($3)
	mov	$5, $6
	mov	$2, $9
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solver_fast2.2946
	addi	$3, $3, -96
	lw	$31, 92($3)
	bne	$2, $0, eq_else.38163
	j	eq_cont.38164
eq_else.38163:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38165
	addi	$2, $0, 0
	j	eq_cont.38166
eq_else.38165:
	addi	$2, $0, 1
eq_cont.38166:
	bne	$2, $0, eq_else.38167
	addi	$2, $0, 0
	j	eq_cont.38168
eq_else.38167:
	addi	$2, $0, 1
eq_cont.38168:
	bne	$2, $0, eq_else.38169
	j	eq_cont.38170
eq_else.38169:
	lw	$2, 88($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38171
	j	eq_cont.38172
eq_else.38171:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38173
	j	eq_cont.38174
eq_else.38173:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38175
	j	eq_cont.38176
eq_else.38175:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	lw	$2, 88($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38177
	j	eq_cont.38178
eq_else.38177:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 76($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_each_element_fast.3017
	addi	$3, $3, -96
	lw	$31, 92($3)
	addi	$2, $0, 5
	lw	$5, 88($3)
	lw	$6, 76($3)
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -96
	lw	$31, 92($3)
eq_cont.38178:
eq_cont.38176:
eq_cont.38174:
eq_cont.38172:
eq_cont.38170:
eq_cont.38164:
eq_cont.38154:
	addi	$2, $0, 1
	lw	$5, 84($3)
	lw	$6, 76($3)
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -96
	lw	$31, 92($3)
eq_cont.38152:
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38179
	addi	$2, $0, 0
	j	eq_cont.38180
eq_else.38179:
	addi	$2, $0, 1
eq_cont.38180:
	bne	$2, $0, eq_else.38181
	addi	$2, $0, 0
	j	eq_cont.38182
eq_else.38181:
	addi	$2, $0, 1
eq_cont.38182:
	bne	$2, $0, eq_else.38183
	addi	$2, $0, 0
	j	eq_cont.38184
eq_else.38183:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38185
	addi	$2, $0, 0
	j	eq_cont.38186
eq_else.38185:
	addi	$2, $0, 1
eq_cont.38186:
	bne	$2, $0, eq_else.38187
	addi	$2, $0, 0
	j	eq_cont.38188
eq_else.38187:
	addi	$2, $0, 1
eq_cont.38188:
eq_cont.38184:
	bne	$2, $0, eq_else.38189
	j	eq_cont.38190
eq_else.38189:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	addi	$5, $0, 4
	mul	$2, $2, $5
	addi	$5, $0, 3536
	lw	$5, 0($5)
	add	$2, $2, $5
	lw	$5, 80($3)
	lw	$6, 0($5)
	bne	$2, $6, eq_else.38191
	addi	$2, $0, 0
	addi	$6, $0, 3524
	lw	$6, 0($6)
	mov	$5, $6
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -96
	lw	$31, 92($3)
	bne	$2, $0, eq_else.38193
	lw	$2, 76($3)
	lw	$5, 0($2)
	addi	$6, $0, 3576
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	fmul	$f0, $f0, $f1
	addi	$6, $0, 3584
	lf	$f1, 0($6)
	lf	$f2, 8($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3592
	lf	$f1, 0($6)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lw	$5, 80($3)
	lf	$f1, 8($5)
	lf	$f2, 40($3)
	fmul	$f3, $f1, $f2
	fmul	$f0, $f3, $f0
	lw	$2, 0($2)
	lw	$5, 24($3)
	lf	$f3, 0($5)
	lf	$f4, 0($2)
	fmul	$f3, $f3, $f4
	lf	$f4, 8($5)
	lf	$f5, 8($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	lf	$f4, 16($5)
	lf	$f5, 16($2)
	fmul	$f4, $f4, $f5
	fadd	$f3, $f3, $f4
	fmul	$f1, $f1, $f3
	lf	$f3, 56($3)
	movf	$f2, $f3
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	add_light.3043
	addi	$3, $3, -96
	lw	$31, 92($3)
	j	eq_cont.38194
eq_else.38193:
eq_cont.38194:
	j	eq_cont.38192
eq_else.38191:
eq_cont.38192:
eq_cont.38190:
	lw	$2, 72($3)
	addi	$2, $2, -1
	lf	$f0, 40($3)
	lf	$f1, 56($3)
	lw	$5, 24($3)
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	trace_reflections.3047
	addi	$3, $3, -96
	lw	$31, 92($3)
ge_cont.38150:
	lf	$f0, 4624($0) # 0.100000
	lf	$f1, 16($3)
	fsub	$f0, $f1, $f0
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.38195
	addi	$2, $0, 0
	j	eq_cont.38196
eq_else.38195:
	addi	$2, $0, 1
eq_cont.38196:
	bne	$2, $0, eq_else.38197
	addi	$2, $0, 0
	j	eq_cont.38198
eq_else.38197:
	addi	$2, $0, 1
eq_cont.38198:
	bne	$2, $0, eq_else.38199
	jr $31
eq_else.38199:
	lw	$2, 32($3)
	addi	$30, $0, 4
	bge	$2, $30, ge_else.38201
	addi	$5, $2, 1
	addi	$6, $0, -1
	slli	$5, $5, 2
	lw	$7, 28($3)
	add $30, $7, $5
	sw	$6, 0($30)
	j	ge_cont.38202
ge_else.38201:
ge_cont.38202:
	lw	$5, 36($3)
	addi	$30, $0, 2
	bne	$5, $30, eq_else.38203
	lf	$f0, 4920($0) # 1.000000
	lw	$5, 52($3)
	lw	$5, 28($5)
	lf	$f2, 0($5)
	fsub	$f0, $f0, $f2
	fmul	$f0, $f1, $f0
	addi	$2, $2, 1
	addi	$5, $0, 3540
	lf	$f1, 0($5)
	lf	$f2, 0($3)
	fadd	$f1, $f2, $f1
	lw	$5, 24($3)
	lw	$6, 8($3)
	j	trace_ray.3052
eq_else.38203:
	jr $31
trace_diffuse_ray.3058:
	lf	$f1, 4656($0) # 1000000000.000000
	sf	$f1, 3540($0)
	addi	$5, $0, 0
	lw	$6, 3524($0)
	sf	$f0, 0($3)
	sw	$2, 8($3)
	mov	$27, $6
	mov	$6, $2
	mov	$2, $5
	mov	$5, $27
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -16
	lw	$31, 12($3)
	lf	$f0, 3540($0)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38205
	addi	$2, $0, 0
	j	eq_cont.38206
eq_else.38205:
	addi	$2, $0, 1
eq_cont.38206:
	bne	$2, $0, eq_else.38207
	addi	$2, $0, 0
	j	eq_cont.38208
eq_else.38207:
	addi	$2, $0, 1
eq_cont.38208:
	bne	$2, $0, eq_else.38209
	addi	$2, $0, 0
	j	eq_cont.38210
eq_else.38209:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38211
	addi	$2, $0, 0
	j	eq_cont.38212
eq_else.38211:
	addi	$2, $0, 1
eq_cont.38212:
	bne	$2, $0, eq_else.38213
	addi	$2, $0, 0
	j	eq_cont.38214
eq_else.38213:
	addi	$2, $0, 1
eq_cont.38214:
eq_cont.38210:
	bne	$2, $0, eq_else.38215
	jr $31
eq_else.38215:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$5, 8($3)
	lw	$5, 0($5)
	lw	$6, 4($2)
	sw	$2, 12($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.38217
	addi	$6, $0, 3536
	lw	$6, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$7, $0, 3576
	sf	$f0, 0($7)
	addi	$7, $0, 3584
	sf	$f0, 0($7)
	addi	$7, $0, 3592
	sf	$f0, 0($7)
	addi	$7, $6, -1
	addi	$6, $6, -1
	slli	$6, $6, 3
	add	$30, $5, $6
	lf	$f0, 0($30)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.38219
	addi	$5, $0, 1
	j	eq_cont.38220
eq_else.38219:
	addi	$5, $0, 0
eq_cont.38220:
	bne	$5, $0, eq_else.38221
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38223
	addi	$5, $0, 0
	j	eq_cont.38224
eq_else.38223:
	addi	$5, $0, 1
eq_cont.38224:
	bne	$5, $0, eq_else.38225
	lf	$f0, 4880($0) # -1.000000
	j	eq_cont.38226
eq_else.38225:
	lf	$f0, 4920($0) # 1.000000
eq_cont.38226:
	j	eq_cont.38222
eq_else.38221:
	lf	$f0, 4888($0) # 0.000000
eq_cont.38222:
	fneg	$f0, $f0
	slli	$5, $7, 3
	addi	$5, $5, 3576
	sf	$f0, 0($5)
	j	eq_cont.38218
eq_else.38217:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.38227
	lw	$5, 16($2)
	lf	$f0, 0($5)
	fneg	$f0, $f0
	addi	$5, $0, 3576
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 8($5)
	fneg	$f0, $f0
	addi	$5, $0, 3584
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 16($5)
	fneg	$f0, $f0
	addi	$5, $0, 3592
	sf	$f0, 0($5)
	j	eq_cont.38228
eq_else.38227:
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	get_nvector_second.3035
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.38228:
eq_cont.38218:
	addi	$5, $0, 3548
	lw	$2, 12($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	utexture.3040
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$2, $0, 3524
	lw	$5, 0($2)
	lw	$2, 0($5)
	lw	$6, 0($2)
	addi	$30, $0, -1
	bne	$6, $30, eq_else.38229
	addi	$2, $0, 0
	j	eq_cont.38230
eq_else.38229:
	sw	$2, 16($3)
	sw	$5, 20($3)
	addi	$30, $0, 99
	bne	$6, $30, eq_else.38231
	addi	$2, $0, 1
	j	eq_cont.38232
eq_else.38231:
	addi	$7, $0, 3860
	addi	$8, $0, 3548
	mov	$5, $7
	mov	$2, $6
	mov	$6, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	solver_fast.2928
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.38233
	addi	$2, $0, 0
	j	eq_cont.38234
eq_else.38233:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38235
	addi	$2, $0, 0
	j	eq_cont.38236
eq_else.38235:
	addi	$2, $0, 1
eq_cont.38236:
	bne	$2, $0, eq_else.38237
	addi	$2, $0, 0
	j	eq_cont.38238
eq_else.38237:
	addi	$2, $0, 1
eq_cont.38238:
	bne	$2, $0, eq_else.38239
	addi	$2, $0, 0
	j	eq_cont.38240
eq_else.38239:
	addi	$2, $0, 1
	lw	$5, 16($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.38241
	addi	$2, $0, 0
	j	eq_cont.38242
eq_else.38241:
	addi	$2, $0, 1
eq_cont.38242:
eq_cont.38240:
eq_cont.38234:
eq_cont.38232:
	bne	$2, $0, eq_else.38243
	addi	$2, $0, 1
	lw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.38244
eq_else.38243:
	addi	$2, $0, 1
	lw	$5, 16($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_group.2997
	addi	$3, $3, -32
	lw	$31, 28($3)
	bne	$2, $0, eq_else.38245
	addi	$2, $0, 1
	lw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.38246
eq_else.38245:
	addi	$2, $0, 1
eq_cont.38246:
eq_cont.38244:
eq_cont.38230:
	bne	$2, $0, eq_else.38247
	addi	$2, $0, 3576
	lf	$f0, 0($2)
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3584
	lf	$f1, 0($2)
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3592
	lf	$f1, 0($2)
	addi	$2, $0, 3308
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38248
	addi	$2, $0, 0
	j	eq_cont.38249
eq_else.38248:
	addi	$2, $0, 1
eq_cont.38249:
	bne	$2, $0, eq_else.38250
	lf	$f0, 4888($0) # 0.000000
	j	eq_cont.38251
eq_else.38250:
eq_cont.38251:
	lf	$f1, 0($3)
	fmul	$f0, $f1, $f0
	lw	$2, 12($3)
	lw	$2, 28($2)
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3624
	lf	$f1, 0($2)
	addi	$2, $0, 3600
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3624
	sf	$f1, 0($2)
	addi	$2, $0, 3632
	lf	$f1, 0($2)
	addi	$2, $0, 3608
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3632
	sf	$f1, 0($2)
	addi	$2, $0, 3640
	lf	$f1, 0($2)
	addi	$2, $0, 3616
	lf	$f2, 0($2)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f1, $f0
	addi	$2, $0, 3640
	sf	$f0, 0($2)
	jr $31
eq_else.38247:
	jr $31
iter_trace_diffuse_rays.3061:
	bge	$7, $0, ge.38254
	jr $31
ge.38254:
	slli	$8, $7, 2
	add	$30, $2, $8
	lw	$8, 0($30)
	lw	$8, 0($8)
	lf	$f0, 0($8)
	lf	$f1, 0($5)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($8)
	lf	$f2, 8($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($8)
	lf	$f2, 16($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38256
	addi	$8, $0, 0
	j	eq_cont.38257
eq_else.38256:
	addi	$8, $0, 1
eq_cont.38257:
	sw	$6, 0($3)
	sw	$5, 4($3)
	sw	$2, 8($3)
	sw	$7, 12($3)
	bne	$8, $0, eq_else.38258
	slli	$8, $7, 2
	add	$30, $2, $8
	lw	$8, 0($30)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	lf	$f1, 4656($0) # 1000000000.000000
	addi	$9, $0, 3540
	sf	$f1, 0($9)
	addi	$9, $0, 3524
	lw	$9, 0($9)
	lw	$10, 0($9)
	lw	$11, 0($10)
	sf	$f0, 16($3)
	sw	$8, 24($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.38260
	j	eq_cont.38261
eq_else.38260:
	sw	$9, 28($3)
	addi	$30, $0, 99
	bne	$11, $30, eq_else.38262
	lw	$11, 4($10)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.38264
	j	eq_cont.38265
eq_else.38264:
	slli	$11, $11, 2
	addi	$11, $11, 3324
	lw	$11, 0($11)
	addi	$12, $0, 0
	sw	$10, 32($3)
	mov	$6, $8
	mov	$5, $11
	mov	$2, $12
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38266
	j	eq_cont.38267
eq_else.38266:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38268
	j	eq_cont.38269
eq_else.38268:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38270
	j	eq_cont.38271
eq_else.38270:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$2, $0, 5
	lw	$5, 32($3)
	lw	$6, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38271:
eq_cont.38269:
eq_cont.38267:
eq_cont.38265:
	j	eq_cont.38263
eq_else.38262:
	sw	$10, 32($3)
	mov	$5, $8
	mov	$2, $11
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solver_fast2.2946
	addi	$3, $3, -40
	lw	$31, 36($3)
	bne	$2, $0, eq_else.38272
	j	eq_cont.38273
eq_else.38272:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38274
	addi	$2, $0, 0
	j	eq_cont.38275
eq_else.38274:
	addi	$2, $0, 1
eq_cont.38275:
	bne	$2, $0, eq_else.38276
	addi	$2, $0, 0
	j	eq_cont.38277
eq_else.38276:
	addi	$2, $0, 1
eq_cont.38277:
	bne	$2, $0, eq_else.38278
	j	eq_cont.38279
eq_else.38278:
	lw	$2, 32($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38280
	j	eq_cont.38281
eq_else.38280:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38282
	j	eq_cont.38283
eq_else.38282:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38284
	j	eq_cont.38285
eq_else.38284:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38286
	j	eq_cont.38287
eq_else.38286:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_each_element_fast.3017
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$2, $0, 5
	lw	$5, 32($3)
	lw	$6, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38287:
eq_cont.38285:
eq_cont.38283:
eq_cont.38281:
eq_cont.38279:
eq_cont.38273:
eq_cont.38263:
	addi	$2, $0, 1
	lw	$5, 28($3)
	lw	$6, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38261:
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38288
	addi	$2, $0, 0
	j	eq_cont.38289
eq_else.38288:
	addi	$2, $0, 1
eq_cont.38289:
	bne	$2, $0, eq_else.38290
	addi	$2, $0, 0
	j	eq_cont.38291
eq_else.38290:
	addi	$2, $0, 1
eq_cont.38291:
	bne	$2, $0, eq_else.38292
	addi	$2, $0, 0
	j	eq_cont.38293
eq_else.38292:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38294
	addi	$2, $0, 0
	j	eq_cont.38295
eq_else.38294:
	addi	$2, $0, 1
eq_cont.38295:
	bne	$2, $0, eq_else.38296
	addi	$2, $0, 0
	j	eq_cont.38297
eq_else.38296:
	addi	$2, $0, 1
eq_cont.38297:
eq_cont.38293:
	bne	$2, $0, eq_else.38298
	j	eq_cont.38299
eq_else.38298:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$5, 24($3)
	lw	$5, 0($5)
	lw	$6, 4($2)
	sw	$2, 36($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.38300
	addi	$6, $0, 3536
	lw	$6, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$7, $0, 3576
	sf	$f0, 0($7)
	addi	$7, $0, 3584
	sf	$f0, 0($7)
	addi	$7, $0, 3592
	sf	$f0, 0($7)
	addi	$7, $6, -1
	addi	$6, $6, -1
	slli	$6, $6, 3
	add	$30, $5, $6
	lf	$f0, 0($30)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.38302
	addi	$5, $0, 1
	j	eq_cont.38303
eq_else.38302:
	addi	$5, $0, 0
eq_cont.38303:
	bne	$5, $0, eq_else.38304
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38306
	addi	$5, $0, 0
	j	eq_cont.38307
eq_else.38306:
	addi	$5, $0, 1
eq_cont.38307:
	bne	$5, $0, eq_else.38308
	lf	$f0, 4880($0) # -1.000000
	j	eq_cont.38309
eq_else.38308:
	lf	$f0, 4920($0) # 1.000000
eq_cont.38309:
	j	eq_cont.38305
eq_else.38304:
	lf	$f0, 4888($0) # 0.000000
eq_cont.38305:
	fneg	$f0, $f0
	slli	$5, $7, 3
	addi	$5, $5, 3576
	sf	$f0, 0($5)
	j	eq_cont.38301
eq_else.38300:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.38310
	lw	$5, 16($2)
	lf	$f0, 0($5)
	fneg	$f0, $f0
	addi	$5, $0, 3576
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 8($5)
	fneg	$f0, $f0
	addi	$5, $0, 3584
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 16($5)
	fneg	$f0, $f0
	addi	$5, $0, 3592
	sf	$f0, 0($5)
	j	eq_cont.38311
eq_else.38310:
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	get_nvector_second.3035
	addi	$3, $3, -48
	lw	$31, 44($3)
eq_cont.38311:
eq_cont.38301:
	addi	$5, $0, 3548
	lw	$2, 36($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	utexture.3040
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$2, $0, 0
	addi	$5, $0, 3524
	lw	$5, 0($5)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -48
	lw	$31, 44($3)
	bne	$2, $0, eq_else.38312
	addi	$2, $0, 3576
	lf	$f0, 0($2)
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3584
	lf	$f1, 0($2)
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3592
	lf	$f1, 0($2)
	addi	$2, $0, 3308
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38314
	addi	$2, $0, 0
	j	eq_cont.38315
eq_else.38314:
	addi	$2, $0, 1
eq_cont.38315:
	bne	$2, $0, eq_else.38316
	lf	$f0, 4888($0) # 0.000000
	j	eq_cont.38317
eq_else.38316:
eq_cont.38317:
	lf	$f1, 16($3)
	fmul	$f0, $f1, $f0
	lw	$2, 36($3)
	lw	$2, 28($2)
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3624
	lf	$f1, 0($2)
	addi	$2, $0, 3600
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3624
	sf	$f1, 0($2)
	addi	$2, $0, 3632
	lf	$f1, 0($2)
	addi	$2, $0, 3608
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3632
	sf	$f1, 0($2)
	addi	$2, $0, 3640
	lf	$f1, 0($2)
	addi	$2, $0, 3616
	lf	$f2, 0($2)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f1, $f0
	addi	$2, $0, 3640
	sf	$f0, 0($2)
	j	eq_cont.38313
eq_else.38312:
eq_cont.38313:
eq_cont.38299:
	j	eq_cont.38259
eq_else.38258:
	addi	$8, $7, 1
	slli	$8, $8, 2
	add	$30, $2, $8
	lw	$8, 0($30)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	lf	$f1, 4656($0) # 1000000000.000000
	addi	$9, $0, 3540
	sf	$f1, 0($9)
	addi	$9, $0, 3524
	lw	$9, 0($9)
	lw	$10, 0($9)
	lw	$11, 0($10)
	sf	$f0, 40($3)
	sw	$8, 48($3)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.38318
	j	eq_cont.38319
eq_else.38318:
	sw	$9, 52($3)
	addi	$30, $0, 99
	bne	$11, $30, eq_else.38320
	lw	$11, 4($10)
	addi	$30, $0, -1
	bne	$11, $30, eq_else.38322
	j	eq_cont.38323
eq_else.38322:
	slli	$11, $11, 2
	addi	$11, $11, 3324
	lw	$11, 0($11)
	addi	$12, $0, 0
	sw	$10, 56($3)
	mov	$6, $8
	mov	$5, $11
	mov	$2, $12
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38324
	j	eq_cont.38325
eq_else.38324:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38326
	j	eq_cont.38327
eq_else.38326:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38328
	j	eq_cont.38329
eq_else.38328:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$2, $0, 5
	lw	$5, 56($3)
	lw	$6, 48($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38329:
eq_cont.38327:
eq_cont.38325:
eq_cont.38323:
	j	eq_cont.38321
eq_else.38320:
	sw	$10, 56($3)
	mov	$5, $8
	mov	$2, $11
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solver_fast2.2946
	addi	$3, $3, -64
	lw	$31, 60($3)
	bne	$2, $0, eq_else.38330
	j	eq_cont.38331
eq_else.38330:
	addi	$2, $0, 3528
	lf	$f0, 0($2)
	addi	$2, $0, 3540
	lf	$f1, 0($2)
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38332
	addi	$2, $0, 0
	j	eq_cont.38333
eq_else.38332:
	addi	$2, $0, 1
eq_cont.38333:
	bne	$2, $0, eq_else.38334
	addi	$2, $0, 0
	j	eq_cont.38335
eq_else.38334:
	addi	$2, $0, 1
eq_cont.38335:
	bne	$2, $0, eq_else.38336
	j	eq_cont.38337
eq_else.38336:
	lw	$2, 56($3)
	lw	$5, 4($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38338
	j	eq_cont.38339
eq_else.38338:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 8($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38340
	j	eq_cont.38341
eq_else.38340:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 12($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38342
	j	eq_cont.38343
eq_else.38342:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	lw	$5, 16($2)
	addi	$30, $0, -1
	bne	$5, $30, eq_else.38344
	j	eq_cont.38345
eq_else.38344:
	slli	$5, $5, 2
	addi	$5, $5, 3324
	lw	$5, 0($5)
	addi	$6, $0, 0
	lw	$7, 48($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_each_element_fast.3017
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$2, $0, 5
	lw	$5, 56($3)
	lw	$6, 48($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	solve_one_or_network_fast.3021
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38345:
eq_cont.38343:
eq_cont.38341:
eq_cont.38339:
eq_cont.38337:
eq_cont.38331:
eq_cont.38321:
	addi	$2, $0, 1
	lw	$5, 52($3)
	lw	$6, 48($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	trace_or_matrix_fast.3025
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38319:
	addi	$2, $0, 3540
	lf	$f0, 0($2)
	lf	$f1, 4752($0) # -0.100000
	fsub	$f1, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f1
	bne	$30, $0, eq_else.38346
	addi	$2, $0, 0
	j	eq_cont.38347
eq_else.38346:
	addi	$2, $0, 1
eq_cont.38347:
	bne	$2, $0, eq_else.38348
	addi	$2, $0, 0
	j	eq_cont.38349
eq_else.38348:
	addi	$2, $0, 1
eq_cont.38349:
	bne	$2, $0, eq_else.38350
	addi	$2, $0, 0
	j	eq_cont.38351
eq_else.38350:
	lf	$f1, 4648($0) # 100000000.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38352
	addi	$2, $0, 0
	j	eq_cont.38353
eq_else.38352:
	addi	$2, $0, 1
eq_cont.38353:
	bne	$2, $0, eq_else.38354
	addi	$2, $0, 0
	j	eq_cont.38355
eq_else.38354:
	addi	$2, $0, 1
eq_cont.38355:
eq_cont.38351:
	bne	$2, $0, eq_else.38356
	j	eq_cont.38357
eq_else.38356:
	addi	$2, $0, 3572
	lw	$2, 0($2)
	slli	$2, $2, 2
	addi	$2, $2, 3004
	lw	$2, 0($2)
	lw	$5, 48($3)
	lw	$5, 0($5)
	lw	$6, 4($2)
	sw	$2, 60($3)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.38358
	addi	$6, $0, 3536
	lw	$6, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$7, $0, 3576
	sf	$f0, 0($7)
	addi	$7, $0, 3584
	sf	$f0, 0($7)
	addi	$7, $0, 3592
	sf	$f0, 0($7)
	addi	$7, $6, -1
	addi	$6, $6, -1
	slli	$6, $6, 3
	add	$30, $5, $6
	lf	$f0, 0($30)
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	sltf	$29, $f0, $f1
	bne	$30, $29, eq_else.38360
	addi	$5, $0, 1
	j	eq_cont.38361
eq_else.38360:
	addi	$5, $0, 0
eq_cont.38361:
	bne	$5, $0, eq_else.38362
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38364
	addi	$5, $0, 0
	j	eq_cont.38365
eq_else.38364:
	addi	$5, $0, 1
eq_cont.38365:
	bne	$5, $0, eq_else.38366
	lf	$f0, 4880($0) # -1.000000
	j	eq_cont.38367
eq_else.38366:
	lf	$f0, 4920($0) # 1.000000
eq_cont.38367:
	j	eq_cont.38363
eq_else.38362:
	lf	$f0, 4888($0) # 0.000000
eq_cont.38363:
	fneg	$f0, $f0
	slli	$5, $7, 3
	addi	$5, $5, 3576
	sf	$f0, 0($5)
	j	eq_cont.38359
eq_else.38358:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.38368
	lw	$5, 16($2)
	lf	$f0, 0($5)
	fneg	$f0, $f0
	addi	$5, $0, 3576
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 8($5)
	fneg	$f0, $f0
	addi	$5, $0, 3584
	sf	$f0, 0($5)
	lw	$5, 16($2)
	lf	$f0, 16($5)
	fneg	$f0, $f0
	addi	$5, $0, 3592
	sf	$f0, 0($5)
	j	eq_cont.38369
eq_else.38368:
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	get_nvector_second.3035
	addi	$3, $3, -72
	lw	$31, 68($3)
eq_cont.38369:
eq_cont.38359:
	addi	$5, $0, 3548
	lw	$2, 60($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	utexture.3040
	addi	$3, $3, -72
	lw	$31, 68($3)
	addi	$2, $0, 0
	addi	$5, $0, 3524
	lw	$5, 0($5)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	shadow_check_one_or_matrix.3000
	addi	$3, $3, -72
	lw	$31, 68($3)
	bne	$2, $0, eq_else.38370
	addi	$2, $0, 3576
	lf	$f0, 0($2)
	addi	$2, $0, 3292
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3584
	lf	$f1, 0($2)
	addi	$2, $0, 3300
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3592
	lf	$f1, 0($2)
	addi	$2, $0, 3308
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	fneg	$f0, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38372
	addi	$2, $0, 0
	j	eq_cont.38373
eq_else.38372:
	addi	$2, $0, 1
eq_cont.38373:
	bne	$2, $0, eq_else.38374
	lf	$f0, 4888($0) # 0.000000
	j	eq_cont.38375
eq_else.38374:
eq_cont.38375:
	lf	$f1, 40($3)
	fmul	$f0, $f1, $f0
	lw	$2, 60($3)
	lw	$2, 28($2)
	lf	$f1, 0($2)
	fmul	$f0, $f0, $f1
	addi	$2, $0, 3624
	lf	$f1, 0($2)
	addi	$2, $0, 3600
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3624
	sf	$f1, 0($2)
	addi	$2, $0, 3632
	lf	$f1, 0($2)
	addi	$2, $0, 3608
	lf	$f2, 0($2)
	fmul	$f2, $f0, $f2
	fadd	$f1, $f1, $f2
	addi	$2, $0, 3632
	sf	$f1, 0($2)
	addi	$2, $0, 3640
	lf	$f1, 0($2)
	addi	$2, $0, 3616
	lf	$f2, 0($2)
	fmul	$f0, $f0, $f2
	fadd	$f0, $f1, $f0
	addi	$2, $0, 3640
	sf	$f0, 0($2)
	j	eq_cont.38371
eq_else.38370:
eq_cont.38371:
eq_cont.38357:
eq_cont.38259:
	lw	$2, 12($3)
	addi	$2, $2, -2
	bge	$2, $0, ge.38376
	jr $31
ge.38376:
	slli	$5, $2, 2
	lw	$6, 8($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$7, 4($3)
	lf	$f1, 0($7)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($7)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($7)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38378
	addi	$5, $0, 0
	j	eq_cont.38379
eq_else.38378:
	addi	$5, $0, 1
eq_cont.38379:
	sw	$2, 64($3)
	bne	$5, $0, eq_else.38380
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -72
	lw	$31, 68($3)
	j	eq_cont.38381
eq_else.38380:
	addi	$5, $2, 1
	slli	$5, $5, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -72
	lw	$31, 68($3)
eq_cont.38381:
	lw	$2, 64($3)
	addi	$7, $2, -2
	lw	$2, 8($3)
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	iter_trace_diffuse_rays.3061
trace_diffuse_ray_80percent.3070:
	sw	$5, 0($3)
	sw	$6, 4($3)
	sw	$2, 8($3)
	bne	$2, $0, eq_else.38382
	j	eq_cont.38383
eq_else.38382:
	addi	$7, $0, 3840
	lw	$7, 0($7)
	lf	$f0, 0($6)
	addi	$8, $0, 3720
	sf	$f0, 0($8)
	lf	$f0, 8($6)
	addi	$8, $0, 3728
	sf	$f0, 0($8)
	lf	$f0, 16($6)
	addi	$8, $0, 3736
	sf	$f0, 0($8)
	addi	$8, $0, 3000
	lw	$8, 0($8)
	addi	$8, $8, -1
	sw	$7, 12($3)
	mov	$5, $8
	mov	$2, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_startp_constants.2963
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$7, $0, 118
	lw	$2, 12($3)
	lw	$5, 0($3)
	lw	$6, 4($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.38383:
	lw	$2, 8($3)
	addi	$30, $0, 1
	bne	$2, $30, eq_else.38384
	j	eq_cont.38385
eq_else.38384:
	addi	$5, $0, 3844
	lw	$5, 0($5)
	lw	$6, 4($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 16($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_startp_constants.2963
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$7, $0, 118
	lw	$2, 16($3)
	lw	$5, 0($3)
	lw	$6, 4($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.38385:
	lw	$2, 8($3)
	addi	$30, $0, 2
	bne	$2, $30, eq_else.38386
	j	eq_cont.38387
eq_else.38386:
	addi	$5, $0, 3848
	lw	$5, 0($5)
	lw	$6, 4($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 20($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_startp_constants.2963
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$7, $0, 118
	lw	$2, 20($3)
	lw	$5, 0($3)
	lw	$6, 4($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38387:
	lw	$2, 8($3)
	addi	$30, $0, 3
	bne	$2, $30, eq_else.38388
	j	eq_cont.38389
eq_else.38388:
	addi	$5, $0, 3852
	lw	$5, 0($5)
	lw	$6, 4($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 24($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_startp_constants.2963
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$7, $0, 118
	lw	$2, 24($3)
	lw	$5, 0($3)
	lw	$6, 4($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38389:
	lw	$2, 8($3)
	addi	$30, $0, 4
	bne	$2, $30, eq_else.38390
	jr $31
eq_else.38390:
	addi	$2, $0, 3856
	lw	$2, 0($2)
	lw	$5, 4($3)
	lf	$f0, 0($5)
	addi	$6, $0, 3720
	sf	$f0, 0($6)
	lf	$f0, 8($5)
	addi	$6, $0, 3728
	sf	$f0, 0($6)
	lf	$f0, 16($5)
	addi	$6, $0, 3736
	sf	$f0, 0($6)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	sw	$2, 28($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$7, $0, 118
	lw	$2, 28($3)
	lw	$5, 0($3)
	lw	$6, 4($3)
	j	iter_trace_diffuse_rays.3061
calc_diffuse_using_1point.3074:
	lw	$6, 20($2)
	lw	$7, 28($2)
	lw	$8, 4($2)
	lw	$9, 16($2)
	slli	$10, $5, 2
	add	$30, $6, $10
	lw	$6, 0($30)
	lf	$f0, 0($6)
	sf	$f0, 3624($0)
	lf	$f0, 8($6)
	sf	$f0, 3632($0)
	lf	$f0, 16($6)
	sf	$f0, 3640($0)
	lw	$2, 24($2)
	lw	$2, 0($2)
	slli	$6, $5, 2
	add	$30, $7, $6
	lw	$6, 0($30)
	slli	$7, $5, 2
	add	$30, $8, $7
	lw	$7, 0($30)
	sw	$9, 0($3)
	sw	$5, 4($3)
	sw	$6, 8($3)
	sw	$7, 12($3)
	sw	$2, 16($3)
	bne	$2, $0, eq_else.38392
	j	eq_cont.38393
eq_else.38392:
	addi	$8, $0, 3840
	lw	$8, 0($8)
	lf	$f0, 0($7)
	addi	$10, $0, 3720
	sf	$f0, 0($10)
	lf	$f0, 8($7)
	addi	$10, $0, 3728
	sf	$f0, 0($10)
	lf	$f0, 16($7)
	addi	$10, $0, 3736
	sf	$f0, 0($10)
	addi	$10, $0, 3000
	lw	$10, 0($10)
	addi	$10, $10, -1
	sw	$8, 20($3)
	mov	$5, $10
	mov	$2, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_startp_constants.2963
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 20($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 8($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38394
	addi	$5, $0, 0
	j	eq_cont.38395
eq_else.38394:
	addi	$5, $0, 1
eq_cont.38395:
	bne	$5, $0, eq_else.38396
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.38397
eq_else.38396:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38397:
	addi	$7, $0, 116
	lw	$2, 20($3)
	lw	$5, 8($3)
	lw	$6, 12($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38393:
	lw	$2, 16($3)
	addi	$30, $0, 1
	bne	$2, $30, eq_else.38398
	j	eq_cont.38399
eq_else.38398:
	addi	$5, $0, 3844
	lw	$5, 0($5)
	lw	$6, 12($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 24($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_startp_constants.2963
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 8($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38400
	addi	$5, $0, 0
	j	eq_cont.38401
eq_else.38400:
	addi	$5, $0, 1
eq_cont.38401:
	bne	$5, $0, eq_else.38402
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	eq_cont.38403
eq_else.38402:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38403:
	addi	$7, $0, 116
	lw	$2, 24($3)
	lw	$5, 8($3)
	lw	$6, 12($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38399:
	lw	$2, 16($3)
	addi	$30, $0, 2
	bne	$2, $30, eq_else.38404
	j	eq_cont.38405
eq_else.38404:
	addi	$5, $0, 3848
	lw	$5, 0($5)
	lw	$6, 12($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 28($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 28($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 8($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38406
	addi	$5, $0, 0
	j	eq_cont.38407
eq_else.38406:
	addi	$5, $0, 1
eq_cont.38407:
	bne	$5, $0, eq_else.38408
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.38409
eq_else.38408:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38409:
	addi	$7, $0, 116
	lw	$2, 28($3)
	lw	$5, 8($3)
	lw	$6, 12($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38405:
	lw	$2, 16($3)
	addi	$30, $0, 3
	bne	$2, $30, eq_else.38410
	j	eq_cont.38411
eq_else.38410:
	addi	$5, $0, 3852
	lw	$5, 0($5)
	lw	$6, 12($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 32($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 8($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38412
	addi	$5, $0, 0
	j	eq_cont.38413
eq_else.38412:
	addi	$5, $0, 1
eq_cont.38413:
	bne	$5, $0, eq_else.38414
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.38415
eq_else.38414:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38415:
	addi	$7, $0, 116
	lw	$2, 32($3)
	lw	$5, 8($3)
	lw	$6, 12($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38411:
	lw	$2, 16($3)
	addi	$30, $0, 4
	bne	$2, $30, eq_else.38416
	j	eq_cont.38417
eq_else.38416:
	addi	$2, $0, 3856
	lw	$2, 0($2)
	lw	$5, 12($3)
	lf	$f0, 0($5)
	addi	$6, $0, 3720
	sf	$f0, 0($6)
	lf	$f0, 8($5)
	addi	$6, $0, 3728
	sf	$f0, 0($6)
	lf	$f0, 16($5)
	addi	$6, $0, 3736
	sf	$f0, 0($6)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	sw	$2, 36($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_startp_constants.2963
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 36($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 8($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38418
	addi	$5, $0, 0
	j	eq_cont.38419
eq_else.38418:
	addi	$5, $0, 1
eq_cont.38419:
	bne	$5, $0, eq_else.38420
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -48
	lw	$31, 44($3)
	j	eq_cont.38421
eq_else.38420:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -48
	lw	$31, 44($3)
eq_cont.38421:
	addi	$7, $0, 116
	lw	$2, 36($3)
	lw	$5, 8($3)
	lw	$6, 12($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -48
	lw	$31, 44($3)
eq_cont.38417:
	lw	$2, 4($3)
	slli	$2, $2, 2
	lw	$5, 0($3)
	add	$30, $5, $2
	lw	$2, 0($30)
	lf	$f0, 3648($0)
	lf	$f1, 0($2)
	lf	$f2, 3624($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3648($0)
	lf	$f0, 3656($0)
	lf	$f1, 8($2)
	lf	$f2, 3632($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3656($0)
	lf	$f0, 3664($0)
	lf	$f1, 16($2)
	lf	$f2, 3640($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3664($0)
	jr $31
calc_diffuse_using_5points.3077:
	slli	$9, $2, 2
	add	$30, $5, $9
	lw	$5, 0($30)
	lw	$5, 20($5)
	addi	$9, $2, -1
	slli	$9, $9, 2
	add	$30, $6, $9
	lw	$9, 0($30)
	lw	$9, 20($9)
	slli	$10, $2, 2
	add	$30, $6, $10
	lw	$10, 0($30)
	lw	$10, 20($10)
	addi	$11, $2, 1
	slli	$11, $11, 2
	add	$30, $6, $11
	lw	$11, 0($30)
	lw	$11, 20($11)
	slli	$12, $2, 2
	add	$30, $7, $12
	lw	$7, 0($30)
	lw	$7, 20($7)
	slli	$12, $8, 2
	add	$30, $5, $12
	lw	$5, 0($30)
	lf	$f0, 0($5)
	sf	$f0, 3624($0)
	lf	$f0, 8($5)
	sf	$f0, 3632($0)
	lf	$f0, 16($5)
	sf	$f0, 3640($0)
	slli	$5, $8, 2
	add	$30, $9, $5
	lw	$5, 0($30)
	lf	$f0, 3624($0)
	lf	$f1, 0($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3624($0)
	lf	$f0, 3632($0)
	lf	$f1, 8($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3632($0)
	lf	$f0, 3640($0)
	lf	$f1, 16($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3640($0)
	slli	$5, $8, 2
	add	$30, $10, $5
	lw	$5, 0($30)
	lf	$f0, 3624($0)
	lf	$f1, 0($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3624($0)
	lf	$f0, 3632($0)
	lf	$f1, 8($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3632($0)
	lf	$f0, 3640($0)
	lf	$f1, 16($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3640($0)
	slli	$5, $8, 2
	add	$30, $11, $5
	lw	$5, 0($30)
	lf	$f0, 3624($0)
	lf	$f1, 0($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3624($0)
	lf	$f0, 3632($0)
	lf	$f1, 8($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3632($0)
	lf	$f0, 3640($0)
	lf	$f1, 16($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3640($0)
	slli	$5, $8, 2
	add	$30, $7, $5
	lw	$5, 0($30)
	lf	$f0, 3624($0)
	lf	$f1, 0($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3624($0)
	lf	$f0, 3632($0)
	lf	$f1, 8($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3632($0)
	lf	$f0, 3640($0)
	lf	$f1, 16($5)
	fadd	$f0, $f0, $f1
	sf	$f0, 3640($0)
	slli	$2, $2, 2
	add	$30, $6, $2
	lw	$2, 0($30)
	lw	$2, 16($2)
	slli	$5, $8, 2
	add	$30, $2, $5
	lw	$2, 0($30)
	lf	$f0, 3648($0)
	lf	$f1, 0($2)
	lf	$f2, 3624($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3648($0)
	lf	$f0, 3656($0)
	lf	$f1, 8($2)
	lf	$f2, 3632($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3656($0)
	lf	$f0, 3664($0)
	lf	$f1, 16($2)
	lf	$f2, 3640($0)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	sf	$f0, 3664($0)
	jr $31
do_without_neighbors.3083:
	addi	$30, $0, 4
	ble	$5, $30, le.38424
	jr $31
le.38424:
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38426
	jr $31
ge.38426:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$2, 0($3)
	sw	$5, 4($3)
	bne	$6, $0, eq_else.38428
	j	eq_cont.38429
eq_else.38428:
	lw	$6, 20($2)
	lw	$7, 28($2)
	lw	$8, 4($2)
	lw	$9, 16($2)
	slli	$10, $5, 2
	add	$30, $6, $10
	lw	$6, 0($30)
	lf	$f0, 0($6)
	addi	$10, $0, 3624
	sf	$f0, 0($10)
	lf	$f0, 8($6)
	addi	$10, $0, 3632
	sf	$f0, 0($10)
	lf	$f0, 16($6)
	addi	$6, $0, 3640
	sf	$f0, 0($6)
	lw	$6, 24($2)
	lw	$6, 0($6)
	slli	$10, $5, 2
	add	$30, $7, $10
	lw	$7, 0($30)
	slli	$10, $5, 2
	add	$30, $8, $10
	lw	$8, 0($30)
	sw	$9, 8($3)
	sw	$7, 12($3)
	sw	$8, 16($3)
	sw	$6, 20($3)
	bne	$6, $0, eq_else.38430
	j	eq_cont.38431
eq_else.38430:
	addi	$10, $0, 3840
	lw	$10, 0($10)
	lf	$f0, 0($8)
	addi	$11, $0, 3720
	sf	$f0, 0($11)
	lf	$f0, 8($8)
	addi	$11, $0, 3728
	sf	$f0, 0($11)
	lf	$f0, 16($8)
	addi	$11, $0, 3736
	sf	$f0, 0($11)
	addi	$11, $0, 3000
	lw	$11, 0($11)
	addi	$11, $11, -1
	sw	$10, 24($3)
	mov	$5, $11
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_startp_constants.2963
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$7, $0, 118
	lw	$2, 24($3)
	lw	$5, 12($3)
	lw	$6, 16($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38431:
	lw	$2, 20($3)
	addi	$30, $0, 1
	bne	$2, $30, eq_else.38432
	j	eq_cont.38433
eq_else.38432:
	addi	$5, $0, 3844
	lw	$5, 0($5)
	lw	$6, 16($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 28($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$7, $0, 118
	lw	$2, 28($3)
	lw	$5, 12($3)
	lw	$6, 16($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38433:
	lw	$2, 20($3)
	addi	$30, $0, 2
	bne	$2, $30, eq_else.38434
	j	eq_cont.38435
eq_else.38434:
	addi	$5, $0, 3848
	lw	$5, 0($5)
	lw	$6, 16($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 32($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$7, $0, 118
	lw	$2, 32($3)
	lw	$5, 12($3)
	lw	$6, 16($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38435:
	lw	$2, 20($3)
	addi	$30, $0, 3
	bne	$2, $30, eq_else.38436
	j	eq_cont.38437
eq_else.38436:
	addi	$5, $0, 3852
	lw	$5, 0($5)
	lw	$6, 16($3)
	lf	$f0, 0($6)
	addi	$7, $0, 3720
	sf	$f0, 0($7)
	lf	$f0, 8($6)
	addi	$7, $0, 3728
	sf	$f0, 0($7)
	lf	$f0, 16($6)
	addi	$7, $0, 3736
	sf	$f0, 0($7)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$5, 36($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_startp_constants.2963
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$7, $0, 118
	lw	$2, 36($3)
	lw	$5, 12($3)
	lw	$6, 16($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -48
	lw	$31, 44($3)
eq_cont.38437:
	lw	$2, 20($3)
	addi	$30, $0, 4
	bne	$2, $30, eq_else.38438
	j	eq_cont.38439
eq_else.38438:
	addi	$2, $0, 3856
	lw	$2, 0($2)
	lw	$5, 16($3)
	lf	$f0, 0($5)
	addi	$6, $0, 3720
	sf	$f0, 0($6)
	lf	$f0, 8($5)
	addi	$6, $0, 3728
	sf	$f0, 0($6)
	lf	$f0, 16($5)
	addi	$6, $0, 3736
	sf	$f0, 0($6)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	sw	$2, 40($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_startp_constants.2963
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$7, $0, 118
	lw	$2, 40($3)
	lw	$5, 12($3)
	lw	$6, 16($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -48
	lw	$31, 44($3)
eq_cont.38439:
	lw	$2, 4($3)
	slli	$5, $2, 2
	lw	$6, 8($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$6, $0, 3648
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	addi	$6, $0, 3624
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	addi	$6, $0, 3656
	lf	$f0, 0($6)
	lf	$f1, 8($5)
	addi	$6, $0, 3632
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	addi	$6, $0, 3664
	lf	$f0, 0($6)
	lf	$f1, 16($5)
	addi	$5, $0, 3640
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38429:
	lw	$2, 4($3)
	addi	$5, $2, 1
	addi	$30, $0, 4
	ble	$5, $30, le.38440
	jr $31
le.38440:
	lw	$2, 0($3)
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38442
	jr $31
ge.38442:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 44($3)
	bne	$6, $0, eq_else.38444
	j	eq_cont.38445
eq_else.38444:
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.38445:
	lw	$2, 44($3)
	addi	$2, $2, 1
	addi	$30, $0, 4
	ble	$2, $30, le.38446
	jr $31
le.38446:
	lw	$5, 0($3)
	lw	$6, 8($5)
	slli	$7, $2, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38448
	jr $31
ge.38448:
	lw	$6, 12($5)
	slli	$7, $2, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bne	$6, $0, eq_else.38450
	j	eq_cont.38451
eq_else.38450:
	lw	$6, 20($5)
	lw	$7, 28($5)
	lw	$8, 4($5)
	lw	$9, 16($5)
	slli	$10, $2, 2
	add	$30, $6, $10
	lw	$6, 0($30)
	lf	$f0, 0($6)
	addi	$10, $0, 3624
	sf	$f0, 0($10)
	lf	$f0, 8($6)
	addi	$10, $0, 3632
	sf	$f0, 0($10)
	lf	$f0, 16($6)
	addi	$6, $0, 3640
	sf	$f0, 0($6)
	lw	$6, 24($5)
	lw	$6, 0($6)
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$7, 0($30)
	slli	$10, $2, 2
	add	$30, $8, $10
	lw	$8, 0($30)
	sw	$9, 48($3)
	sw	$2, 52($3)
	mov	$5, $7
	mov	$2, $6
	mov	$6, $8
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 52($3)
	slli	$5, $2, 2
	lw	$6, 48($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$6, $0, 3648
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	addi	$6, $0, 3624
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	addi	$6, $0, 3656
	lf	$f0, 0($6)
	lf	$f1, 8($5)
	addi	$6, $0, 3632
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	addi	$6, $0, 3664
	lf	$f0, 0($6)
	lf	$f1, 16($5)
	addi	$5, $0, 3640
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38451:
	addi	$5, $2, 1
	addi	$30, $0, 4
	ble	$5, $30, le.38452
	jr $31
le.38452:
	lw	$2, 0($3)
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38454
	jr $31
ge.38454:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 56($3)
	bne	$6, $0, eq_else.38456
	j	eq_cont.38457
eq_else.38456:
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38457:
	lw	$2, 56($3)
	addi	$5, $2, 1
	lw	$2, 0($3)
	j	do_without_neighbors.3083
try_exploit_neighbors.3099:
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$10, 0($30)
	addi	$30, $0, 4
	ble	$9, $30, le.38458
	jr $31
le.38458:
	lw	$11, 8($10)
	slli	$12, $9, 2
	add	$30, $11, $12
	lw	$11, 0($30)
	bge	$11, $0, ge.38460
	jr $31
ge.38460:
	slli	$11, $2, 2
	add	$30, $7, $11
	lw	$11, 0($30)
	lw	$11, 8($11)
	slli	$12, $9, 2
	add	$30, $11, $12
	lw	$11, 0($30)
	slli	$12, $2, 2
	add	$30, $6, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38462
	slli	$12, $2, 2
	add	$30, $8, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38464
	addi	$12, $2, -1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38466
	addi	$12, $2, 1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38468
	addi	$11, $0, 1
	j	eq_cont.38469
eq_else.38468:
	addi	$11, $0, 0
eq_cont.38469:
	j	eq_cont.38467
eq_else.38466:
	addi	$11, $0, 0
eq_cont.38467:
	j	eq_cont.38465
eq_else.38464:
	addi	$11, $0, 0
eq_cont.38465:
	j	eq_cont.38463
eq_else.38462:
	addi	$11, $0, 0
eq_cont.38463:
	bne	$11, $0, eq_else.38470
	slli	$2, $2, 2
	add	$30, $7, $2
	lw	$2, 0($30)
	addi	$30, $0, 4
	ble	$9, $30, le.38471
	jr $31
le.38471:
	lw	$5, 8($2)
	slli	$6, $9, 2
	add	$30, $5, $6
	lw	$5, 0($30)
	bge	$5, $0, ge.38473
	jr $31
ge.38473:
	lw	$5, 12($2)
	slli	$6, $9, 2
	add	$30, $5, $6
	lw	$5, 0($30)
	sw	$2, 0($3)
	sw	$9, 4($3)
	bne	$5, $0, eq_else.38475
	j	eq_cont.38476
eq_else.38475:
	mov	$5, $9
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -16
	lw	$31, 12($3)
eq_cont.38476:
	lw	$2, 4($3)
	addi	$2, $2, 1
	addi	$30, $0, 4
	ble	$2, $30, le.38477
	jr $31
le.38477:
	lw	$5, 0($3)
	lw	$6, 8($5)
	slli	$7, $2, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38479
	jr $31
ge.38479:
	lw	$6, 12($5)
	slli	$7, $2, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bne	$6, $0, eq_else.38481
	j	eq_cont.38482
eq_else.38481:
	lw	$6, 20($5)
	lw	$7, 28($5)
	lw	$8, 4($5)
	lw	$9, 16($5)
	slli	$10, $2, 2
	add	$30, $6, $10
	lw	$6, 0($30)
	lf	$f0, 0($6)
	addi	$10, $0, 3624
	sf	$f0, 0($10)
	lf	$f0, 8($6)
	addi	$10, $0, 3632
	sf	$f0, 0($10)
	lf	$f0, 16($6)
	addi	$6, $0, 3640
	sf	$f0, 0($6)
	lw	$6, 24($5)
	lw	$6, 0($6)
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$7, 0($30)
	slli	$10, $2, 2
	add	$30, $8, $10
	lw	$8, 0($30)
	sw	$9, 8($3)
	sw	$2, 12($3)
	mov	$5, $7
	mov	$2, $6
	mov	$6, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 12($3)
	slli	$5, $2, 2
	lw	$6, 8($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$6, $0, 3648
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	addi	$6, $0, 3624
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	addi	$6, $0, 3656
	lf	$f0, 0($6)
	lf	$f1, 8($5)
	addi	$6, $0, 3632
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	addi	$6, $0, 3664
	lf	$f0, 0($6)
	lf	$f1, 16($5)
	addi	$5, $0, 3640
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38482:
	addi	$5, $2, 1
	addi	$30, $0, 4
	ble	$5, $30, le.38483
	jr $31
le.38483:
	lw	$2, 0($3)
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38485
	jr $31
ge.38485:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 16($3)
	bne	$6, $0, eq_else.38487
	j	eq_cont.38488
eq_else.38487:
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -24
	lw	$31, 20($3)
eq_cont.38488:
	lw	$2, 16($3)
	addi	$5, $2, 1
	lw	$2, 0($3)
	j	do_without_neighbors.3083
eq_else.38470:
	lw	$10, 12($10)
	slli	$11, $9, 2
	add	$30, $10, $11
	lw	$10, 0($30)
	bne	$10, $0, eq_else.38489
	j	eq_cont.38490
eq_else.38489:
	slli	$10, $2, 2
	add	$30, $6, $10
	lw	$10, 0($30)
	lw	$10, 20($10)
	addi	$11, $2, -1
	slli	$11, $11, 2
	add	$30, $7, $11
	lw	$11, 0($30)
	lw	$11, 20($11)
	slli	$12, $2, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 20($12)
	addi	$13, $2, 1
	slli	$13, $13, 2
	add	$30, $7, $13
	lw	$13, 0($30)
	lw	$13, 20($13)
	slli	$14, $2, 2
	add	$30, $8, $14
	lw	$14, 0($30)
	lw	$14, 20($14)
	slli	$15, $9, 2
	add	$30, $10, $15
	lw	$10, 0($30)
	lf	$f0, 0($10)
	addi	$15, $0, 3624
	sf	$f0, 0($15)
	lf	$f0, 8($10)
	addi	$15, $0, 3632
	sf	$f0, 0($15)
	lf	$f0, 16($10)
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	slli	$10, $9, 2
	add	$30, $11, $10
	lw	$10, 0($30)
	addi	$11, $0, 3624
	lf	$f0, 0($11)
	lf	$f1, 0($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3624
	sf	$f0, 0($11)
	addi	$11, $0, 3632
	lf	$f0, 0($11)
	lf	$f1, 8($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3632
	sf	$f0, 0($11)
	addi	$11, $0, 3640
	lf	$f0, 0($11)
	lf	$f1, 16($10)
	fadd	$f0, $f0, $f1
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	slli	$10, $9, 2
	add	$30, $12, $10
	lw	$10, 0($30)
	addi	$11, $0, 3624
	lf	$f0, 0($11)
	lf	$f1, 0($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3624
	sf	$f0, 0($11)
	addi	$11, $0, 3632
	lf	$f0, 0($11)
	lf	$f1, 8($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3632
	sf	$f0, 0($11)
	addi	$11, $0, 3640
	lf	$f0, 0($11)
	lf	$f1, 16($10)
	fadd	$f0, $f0, $f1
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	slli	$10, $9, 2
	add	$30, $13, $10
	lw	$10, 0($30)
	addi	$11, $0, 3624
	lf	$f0, 0($11)
	lf	$f1, 0($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3624
	sf	$f0, 0($11)
	addi	$11, $0, 3632
	lf	$f0, 0($11)
	lf	$f1, 8($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3632
	sf	$f0, 0($11)
	addi	$11, $0, 3640
	lf	$f0, 0($11)
	lf	$f1, 16($10)
	fadd	$f0, $f0, $f1
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	slli	$10, $9, 2
	add	$30, $14, $10
	lw	$10, 0($30)
	addi	$11, $0, 3624
	lf	$f0, 0($11)
	lf	$f1, 0($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3624
	sf	$f0, 0($11)
	addi	$11, $0, 3632
	lf	$f0, 0($11)
	lf	$f1, 8($10)
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3632
	sf	$f0, 0($11)
	addi	$11, $0, 3640
	lf	$f0, 0($11)
	lf	$f1, 16($10)
	fadd	$f0, $f0, $f1
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$10, 0($30)
	lw	$10, 16($10)
	slli	$11, $9, 2
	add	$30, $10, $11
	lw	$10, 0($30)
	addi	$11, $0, 3648
	lf	$f0, 0($11)
	lf	$f1, 0($10)
	addi	$11, $0, 3624
	lf	$f2, 0($11)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3648
	sf	$f0, 0($11)
	addi	$11, $0, 3656
	lf	$f0, 0($11)
	lf	$f1, 8($10)
	addi	$11, $0, 3632
	lf	$f2, 0($11)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$11, $0, 3656
	sf	$f0, 0($11)
	addi	$11, $0, 3664
	lf	$f0, 0($11)
	lf	$f1, 16($10)
	addi	$10, $0, 3640
	lf	$f2, 0($10)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$10, $0, 3664
	sf	$f0, 0($10)
eq_cont.38490:
	addi	$9, $9, 1
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$10, 0($30)
	addi	$30, $0, 4
	ble	$9, $30, le.38491
	jr $31
le.38491:
	lw	$11, 8($10)
	slli	$12, $9, 2
	add	$30, $11, $12
	lw	$11, 0($30)
	bge	$11, $0, ge.38493
	jr $31
ge.38493:
	slli	$11, $2, 2
	add	$30, $7, $11
	lw	$11, 0($30)
	lw	$11, 8($11)
	slli	$12, $9, 2
	add	$30, $11, $12
	lw	$11, 0($30)
	slli	$12, $2, 2
	add	$30, $6, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38495
	slli	$12, $2, 2
	add	$30, $8, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38497
	addi	$12, $2, -1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38499
	addi	$12, $2, 1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	slli	$13, $9, 2
	add	$30, $12, $13
	lw	$12, 0($30)
	bne	$12, $11, eq_else.38501
	addi	$11, $0, 1
	j	eq_cont.38502
eq_else.38501:
	addi	$11, $0, 0
eq_cont.38502:
	j	eq_cont.38500
eq_else.38499:
	addi	$11, $0, 0
eq_cont.38500:
	j	eq_cont.38498
eq_else.38497:
	addi	$11, $0, 0
eq_cont.38498:
	j	eq_cont.38496
eq_else.38495:
	addi	$11, $0, 0
eq_cont.38496:
	bne	$11, $0, eq_else.38503
	slli	$2, $2, 2
	add	$30, $7, $2
	lw	$2, 0($30)
	addi	$30, $0, 4
	ble	$9, $30, le.38504
	jr $31
le.38504:
	lw	$5, 8($2)
	slli	$6, $9, 2
	add	$30, $5, $6
	lw	$5, 0($30)
	bge	$5, $0, ge.38506
	jr $31
ge.38506:
	lw	$5, 12($2)
	slli	$6, $9, 2
	add	$30, $5, $6
	lw	$5, 0($30)
	sw	$2, 20($3)
	sw	$9, 24($3)
	bne	$5, $0, eq_else.38508
	j	eq_cont.38509
eq_else.38508:
	lw	$5, 20($2)
	lw	$6, 28($2)
	lw	$7, 4($2)
	lw	$8, 16($2)
	slli	$10, $9, 2
	add	$30, $5, $10
	lw	$5, 0($30)
	lf	$f0, 0($5)
	addi	$10, $0, 3624
	sf	$f0, 0($10)
	lf	$f0, 8($5)
	addi	$10, $0, 3632
	sf	$f0, 0($10)
	lf	$f0, 16($5)
	addi	$5, $0, 3640
	sf	$f0, 0($5)
	lw	$5, 24($2)
	lw	$5, 0($5)
	slli	$10, $9, 2
	add	$30, $6, $10
	lw	$6, 0($30)
	slli	$10, $9, 2
	add	$30, $7, $10
	lw	$7, 0($30)
	sw	$8, 28($3)
	mov	$2, $5
	mov	$5, $6
	mov	$6, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 24($3)
	slli	$5, $2, 2
	lw	$6, 28($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$6, $0, 3648
	lf	$f0, 0($6)
	lf	$f1, 0($5)
	addi	$6, $0, 3624
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	addi	$6, $0, 3656
	lf	$f0, 0($6)
	lf	$f1, 8($5)
	addi	$6, $0, 3632
	lf	$f2, 0($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	addi	$6, $0, 3664
	lf	$f0, 0($6)
	lf	$f1, 16($5)
	addi	$5, $0, 3640
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3664
	sf	$f0, 0($5)
eq_cont.38509:
	lw	$2, 24($3)
	addi	$5, $2, 1
	addi	$30, $0, 4
	ble	$5, $30, le.38510
	jr $31
le.38510:
	lw	$2, 20($3)
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38512
	jr $31
ge.38512:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 32($3)
	bne	$6, $0, eq_else.38514
	j	eq_cont.38515
eq_else.38514:
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38515:
	lw	$2, 32($3)
	addi	$5, $2, 1
	lw	$2, 20($3)
	j	do_without_neighbors.3083
eq_else.38503:
	lw	$10, 12($10)
	slli	$11, $9, 2
	add	$30, $10, $11
	lw	$10, 0($30)
	sw	$8, 36($3)
	sw	$7, 40($3)
	sw	$6, 44($3)
	sw	$5, 48($3)
	sw	$2, 52($3)
	sw	$9, 24($3)
	bne	$10, $0, eq_else.38516
	j	eq_cont.38517
eq_else.38516:
	mov	$5, $6
	mov	$6, $7
	mov	$7, $8
	mov	$8, $9
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	calc_diffuse_using_5points.3077
	addi	$3, $3, -64
	lw	$31, 60($3)
eq_cont.38517:
	lw	$2, 24($3)
	addi	$9, $2, 1
	lw	$2, 52($3)
	lw	$5, 48($3)
	lw	$6, 44($3)
	lw	$7, 40($3)
	lw	$8, 36($3)
	j	try_exploit_neighbors.3099
pretrace_diffuse_rays.3112:
	addi	$30, $0, 4
	ble	$5, $30, le.38518
	jr $31
le.38518:
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38520
	jr $31
ge.38520:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 0($3)
	bne	$6, $0, eq_else.38522
	j	eq_cont.38523
eq_else.38522:
	lw	$6, 24($2)
	lw	$6, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$7, $0, 3624
	sf	$f0, 0($7)
	addi	$7, $0, 3632
	sf	$f0, 0($7)
	addi	$7, $0, 3640
	sf	$f0, 0($7)
	lw	$7, 28($2)
	lw	$8, 4($2)
	slli	$6, $6, 2
	addi	$6, $6, 3840
	lw	$6, 0($6)
	slli	$9, $5, 2
	add	$30, $7, $9
	lw	$7, 0($30)
	slli	$9, $5, 2
	add	$30, $8, $9
	lw	$8, 0($30)
	lf	$f0, 0($8)
	addi	$9, $0, 3720
	sf	$f0, 0($9)
	lf	$f0, 8($8)
	addi	$9, $0, 3728
	sf	$f0, 0($9)
	lf	$f0, 16($8)
	addi	$9, $0, 3736
	sf	$f0, 0($9)
	addi	$9, $0, 3000
	lw	$9, 0($9)
	addi	$9, $9, -1
	sw	$2, 4($3)
	sw	$8, 8($3)
	sw	$7, 12($3)
	sw	$6, 16($3)
	mov	$5, $9
	mov	$2, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_startp_constants.2963
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$7, $0, 118
	lw	$2, 16($3)
	lw	$5, 12($3)
	lw	$6, 8($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 4($3)
	lw	$5, 20($2)
	lw	$6, 0($3)
	slli	$7, $6, 2
	add	$30, $5, $7
	lw	$5, 0($30)
	addi	$7, $0, 3624
	lf	$f0, 0($7)
	sf	$f0, 0($5)
	addi	$7, $0, 3632
	lf	$f0, 0($7)
	sf	$f0, 8($5)
	addi	$7, $0, 3640
	lf	$f0, 0($7)
	sf	$f0, 16($5)
eq_cont.38523:
	lw	$5, 0($3)
	addi	$5, $5, 1
	addi	$30, $0, 4
	ble	$5, $30, le.38524
	jr $31
le.38524:
	lw	$6, 8($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	bge	$6, $0, ge.38526
	jr $31
ge.38526:
	lw	$6, 12($2)
	slli	$7, $5, 2
	add	$30, $6, $7
	lw	$6, 0($30)
	sw	$5, 20($3)
	bne	$6, $0, eq_else.38528
	j	eq_cont.38529
eq_else.38528:
	lw	$6, 24($2)
	lw	$6, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$7, $0, 3624
	sf	$f0, 0($7)
	addi	$7, $0, 3632
	sf	$f0, 0($7)
	addi	$7, $0, 3640
	sf	$f0, 0($7)
	lw	$7, 28($2)
	lw	$8, 4($2)
	slli	$6, $6, 2
	addi	$6, $6, 3840
	lw	$6, 0($6)
	slli	$9, $5, 2
	add	$30, $7, $9
	lw	$7, 0($30)
	slli	$9, $5, 2
	add	$30, $8, $9
	lw	$8, 0($30)
	lf	$f0, 0($8)
	addi	$9, $0, 3720
	sf	$f0, 0($9)
	lf	$f0, 8($8)
	addi	$9, $0, 3728
	sf	$f0, 0($9)
	lf	$f0, 16($8)
	addi	$9, $0, 3736
	sf	$f0, 0($9)
	addi	$9, $0, 3000
	lw	$9, 0($9)
	addi	$9, $9, -1
	sw	$2, 4($3)
	sw	$8, 24($3)
	sw	$7, 28($3)
	sw	$6, 32($3)
	mov	$5, $9
	mov	$2, $8
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_startp_constants.2963
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 28($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38530
	addi	$5, $0, 0
	j	eq_cont.38531
eq_else.38530:
	addi	$5, $0, 1
eq_cont.38531:
	bne	$5, $0, eq_else.38532
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	eq_cont.38533
eq_else.38532:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38533:
	addi	$7, $0, 116
	lw	$2, 32($3)
	lw	$5, 28($3)
	lw	$6, 24($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 4($3)
	lw	$5, 20($2)
	lw	$6, 20($3)
	slli	$7, $6, 2
	add	$30, $5, $7
	lw	$5, 0($30)
	addi	$7, $0, 3624
	lf	$f0, 0($7)
	sf	$f0, 0($5)
	addi	$7, $0, 3632
	lf	$f0, 0($7)
	sf	$f0, 8($5)
	addi	$7, $0, 3640
	lf	$f0, 0($7)
	sf	$f0, 16($5)
eq_cont.38529:
	lw	$5, 20($3)
	addi	$5, $5, 1
	j	pretrace_diffuse_rays.3112
pretrace_pixels.3115:
	bge	$5, $0, ge.38534
	jr $31
ge.38534:
	addi	$7, $0, 3688
	lf	$f3, 0($7)
	addi	$7, $0, 3680
	lw	$7, 0($7)
	sub	$7, $5, $7
	itof	$f4, $7
	fmul	$f3, $f3, $f4
	addi	$7, $0, 3744
	lf	$f4, 0($7)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f0
	addi	$7, $0, 3816
	sf	$f4, 0($7)
	addi	$7, $0, 3752
	lf	$f4, 0($7)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f1
	addi	$7, $0, 3824
	sf	$f4, 0($7)
	addi	$7, $0, 3760
	lf	$f4, 0($7)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f3, $f2
	addi	$7, $0, 3832
	sf	$f3, 0($7)
	addi	$7, $0, 3816
	lf	$f3, 0($7)
	fmul	$f3, $f3, $f3
	addi	$7, $0, 3824
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f4
	fadd	$f3, $f3, $f4
	addi	$7, $0, 3832
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f4
	fadd	$f3, $f3, $f4
	sqrt	$f3, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.38536
	addi	$7, $0, 1
	j	eq_cont.38537
eq_else.38536:
	addi	$7, $0, 0
eq_cont.38537:
	bne	$7, $0, eq_else.38538
	lf	$f4, 4920($0) # 1.000000
	fdiv	$f3, $f4, $f3
	j	eq_cont.38539
eq_else.38538:
	lf	$f3, 4920($0) # 1.000000
eq_cont.38539:
	addi	$7, $0, 3816
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f3
	addi	$7, $0, 3816
	sf	$f4, 0($7)
	addi	$7, $0, 3824
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f3
	addi	$7, $0, 3824
	sf	$f4, 0($7)
	addi	$7, $0, 3832
	lf	$f4, 0($7)
	fmul	$f3, $f4, $f3
	addi	$7, $0, 3832
	sf	$f3, 0($7)
	lf	$f3, 4888($0) # 0.000000
	addi	$7, $0, 3648
	sf	$f3, 0($7)
	addi	$7, $0, 3656
	sf	$f3, 0($7)
	addi	$7, $0, 3664
	sf	$f3, 0($7)
	addi	$7, $0, 3268
	lf	$f3, 0($7)
	addi	$7, $0, 3696
	sf	$f3, 0($7)
	addi	$7, $0, 3276
	lf	$f3, 0($7)
	addi	$7, $0, 3704
	sf	$f3, 0($7)
	addi	$7, $0, 3284
	lf	$f3, 0($7)
	addi	$7, $0, 3712
	sf	$f3, 0($7)
	addi	$7, $0, 0
	lf	$f3, 4920($0) # 1.000000
	slli	$8, $5, 2
	add	$30, $2, $8
	lw	$8, 0($30)
	lf	$f4, 4888($0) # 0.000000
	addi	$9, $0, 3816
	sf	$f2, 0($3)
	sf	$f1, 8($3)
	sf	$f0, 16($3)
	sw	$6, 24($3)
	sw	$2, 28($3)
	sw	$5, 32($3)
	mov	$6, $8
	mov	$5, $9
	mov	$2, $7
	movf	$f1, $f4
	movf	$f0, $f3
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_ray.3052
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	slli	$5, $2, 2
	lw	$6, 28($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	addi	$7, $0, 3648
	lf	$f0, 0($7)
	sf	$f0, 0($5)
	addi	$7, $0, 3656
	lf	$f0, 0($7)
	sf	$f0, 8($5)
	addi	$7, $0, 3664
	lf	$f0, 0($7)
	sf	$f0, 16($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 24($5)
	lw	$7, 24($3)
	sw	$7, 0($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$8, 8($5)
	lw	$8, 0($8)
	bge	$8, $0, ge_else.38540
	j	ge_cont.38541
ge_else.38540:
	lw	$8, 12($5)
	lw	$8, 0($8)
	sw	$5, 36($3)
	bne	$8, $0, eq_else.38542
	j	eq_cont.38543
eq_else.38542:
	lw	$8, 24($5)
	lw	$8, 0($8)
	lf	$f0, 4888($0) # 0.000000
	addi	$9, $0, 3624
	sf	$f0, 0($9)
	addi	$9, $0, 3632
	sf	$f0, 0($9)
	addi	$9, $0, 3640
	sf	$f0, 0($9)
	lw	$9, 28($5)
	lw	$10, 4($5)
	slli	$8, $8, 2
	addi	$8, $8, 3840
	lw	$8, 0($8)
	lw	$9, 0($9)
	lw	$10, 0($10)
	lf	$f0, 0($10)
	addi	$11, $0, 3720
	sf	$f0, 0($11)
	lf	$f0, 8($10)
	addi	$11, $0, 3728
	sf	$f0, 0($11)
	lf	$f0, 16($10)
	addi	$11, $0, 3736
	sf	$f0, 0($11)
	addi	$11, $0, 3000
	lw	$11, 0($11)
	addi	$11, $11, -1
	sw	$10, 40($3)
	sw	$9, 44($3)
	sw	$8, 48($3)
	mov	$5, $11
	mov	$2, $10
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	setup_startp_constants.2963
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$2, 48($3)
	lw	$5, 472($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	lw	$6, 44($3)
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 8($5)
	lf	$f2, 8($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 16($5)
	lf	$f2, 16($6)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f0, $f1
	bne	$30, $0, eq_else.38544
	addi	$5, $0, 0
	j	eq_cont.38545
eq_else.38544:
	addi	$5, $0, 1
eq_cont.38545:
	bne	$5, $0, eq_else.38546
	lw	$5, 472($2)
	lf	$f1, 4608($0) # 150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -56
	lw	$31, 52($3)
	j	eq_cont.38547
eq_else.38546:
	lw	$5, 476($2)
	lf	$f1, 4616($0) # -150.000000
	fdiv	$f0, $f0, $f1
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	trace_diffuse_ray.3058
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.38547:
	addi	$7, $0, 116
	lw	$2, 48($3)
	lw	$5, 44($3)
	lw	$6, 40($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	iter_trace_diffuse_rays.3061
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$2, 36($3)
	lw	$5, 20($2)
	lw	$5, 0($5)
	addi	$6, $0, 3624
	lf	$f0, 0($6)
	sf	$f0, 0($5)
	addi	$6, $0, 3632
	lf	$f0, 0($6)
	sf	$f0, 8($5)
	addi	$6, $0, 3640
	lf	$f0, 0($6)
	sf	$f0, 16($5)
eq_cont.38543:
	addi	$5, $0, 1
	lw	$2, 36($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	pretrace_diffuse_rays.3112
	addi	$3, $3, -56
	lw	$31, 52($3)
ge_cont.38541:
	lw	$2, 32($3)
	addi	$2, $2, -1
	lw	$5, 24($3)
	addi	$5, $5, 1
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38548
	j	ge_cont.38549
ge_else.38548:
	addi	$5, $5, -5
ge_cont.38549:
	bge	$2, $0, ge.38550
	jr $31
ge.38550:
	addi	$6, $0, 3688
	lf	$f0, 0($6)
	addi	$6, $0, 3680
	lw	$6, 0($6)
	sub	$6, $2, $6
	itof	$f1, $6
	fmul	$f0, $f0, $f1
	addi	$6, $0, 3744
	lf	$f1, 0($6)
	fmul	$f1, $f0, $f1
	lf	$f2, 16($3)
	fadd	$f1, $f1, $f2
	addi	$6, $0, 3816
	sf	$f1, 0($6)
	addi	$6, $0, 3752
	lf	$f1, 0($6)
	fmul	$f1, $f0, $f1
	lf	$f3, 8($3)
	fadd	$f1, $f1, $f3
	addi	$6, $0, 3824
	sf	$f1, 0($6)
	addi	$6, $0, 3760
	lf	$f1, 0($6)
	fmul	$f0, $f0, $f1
	lf	$f1, 0($3)
	fadd	$f0, $f0, $f1
	addi	$6, $0, 3832
	sf	$f0, 0($6)
	addi	$6, $0, 3816
	lf	$f0, 0($6)
	fmul	$f0, $f0, $f0
	addi	$6, $0, 3824
	lf	$f4, 0($6)
	fmul	$f4, $f4, $f4
	fadd	$f0, $f0, $f4
	addi	$6, $0, 3832
	lf	$f4, 0($6)
	fmul	$f4, $f4, $f4
	fadd	$f0, $f0, $f4
	sqrt	$f0, $f0
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f0
	sltf	$29, $f0, $f4
	bne	$30, $29, eq_else.38552
	addi	$6, $0, 1
	j	eq_cont.38553
eq_else.38552:
	addi	$6, $0, 0
eq_cont.38553:
	bne	$6, $0, eq_else.38554
	lf	$f4, 4920($0) # 1.000000
	fdiv	$f0, $f4, $f0
	j	eq_cont.38555
eq_else.38554:
	lf	$f0, 4920($0) # 1.000000
eq_cont.38555:
	addi	$6, $0, 3816
	lf	$f4, 0($6)
	fmul	$f4, $f4, $f0
	addi	$6, $0, 3816
	sf	$f4, 0($6)
	addi	$6, $0, 3824
	lf	$f4, 0($6)
	fmul	$f4, $f4, $f0
	addi	$6, $0, 3824
	sf	$f4, 0($6)
	addi	$6, $0, 3832
	lf	$f4, 0($6)
	fmul	$f0, $f4, $f0
	addi	$6, $0, 3832
	sf	$f0, 0($6)
	lf	$f0, 4888($0) # 0.000000
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	addi	$6, $0, 3664
	sf	$f0, 0($6)
	addi	$6, $0, 3268
	lf	$f0, 0($6)
	addi	$6, $0, 3696
	sf	$f0, 0($6)
	addi	$6, $0, 3276
	lf	$f0, 0($6)
	addi	$6, $0, 3704
	sf	$f0, 0($6)
	addi	$6, $0, 3284
	lf	$f0, 0($6)
	addi	$6, $0, 3712
	sf	$f0, 0($6)
	addi	$6, $0, 0
	lf	$f0, 4920($0) # 1.000000
	slli	$7, $2, 2
	lw	$8, 28($3)
	add	$30, $8, $7
	lw	$7, 0($30)
	lf	$f4, 4888($0) # 0.000000
	addi	$9, $0, 3816
	sw	$5, 52($3)
	sw	$2, 56($3)
	mov	$5, $9
	mov	$2, $6
	mov	$6, $7
	movf	$f1, $f4
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	trace_ray.3052
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	slli	$5, $2, 2
	lw	$6, 28($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	addi	$7, $0, 3648
	lf	$f0, 0($7)
	sf	$f0, 0($5)
	addi	$7, $0, 3656
	lf	$f0, 0($7)
	sf	$f0, 8($5)
	addi	$7, $0, 3664
	lf	$f0, 0($7)
	sf	$f0, 16($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 24($5)
	lw	$7, 52($3)
	sw	$7, 0($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$8, $0, 0
	mov	$2, $5
	mov	$5, $8
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	pretrace_diffuse_rays.3112
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$2, 56($3)
	addi	$5, $2, -1
	lw	$2, 52($3)
	addi	$2, $2, 1
	addi	$30, $0, 5
	bge	$2, $30, ge_else.38556
	mov	$6, $2
	j	ge_cont.38557
ge_else.38556:
	addi	$6, $2, -5
ge_cont.38557:
	lf	$f0, 16($3)
	lf	$f1, 8($3)
	lf	$f2, 0($3)
	lw	$2, 28($3)
	j	pretrace_pixels.3115
pretrace_line.3122:
	lf	$f0, 3688($0)
	lw	$7, 3684($0)
	sub	$5, $5, $7
	itof	$f1, $5
	fmul	$f0, $f0, $f1
	lf	$f1, 3768($0)
	fmul	$f1, $f0, $f1
	lf	$f2, 3792($0)
	fadd	$f1, $f1, $f2
	lf	$f2, 3776($0)
	fmul	$f2, $f0, $f2
	lf	$f3, 3800($0)
	fadd	$f2, $f2, $f3
	lf	$f3, 3784($0)
	fmul	$f0, $f0, $f3
	lf	$f3, 3808($0)
	fadd	$f0, $f0, $f3
	lw	$5, 3672($0)
	addi	$5, $5, -1
	bge	$5, $0, ge.38558
	jr $31
ge.38558:
	addi	$7, $0, 3688
	lf	$f3, 0($7)
	addi	$7, $0, 3680
	lw	$7, 0($7)
	sub	$7, $5, $7
	itof	$f4, $7
	fmul	$f3, $f3, $f4
	addi	$7, $0, 3744
	lf	$f4, 0($7)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f1
	addi	$7, $0, 3816
	sf	$f4, 0($7)
	addi	$7, $0, 3752
	lf	$f4, 0($7)
	fmul	$f4, $f3, $f4
	fadd	$f4, $f4, $f2
	addi	$7, $0, 3824
	sf	$f4, 0($7)
	addi	$7, $0, 3760
	lf	$f4, 0($7)
	fmul	$f3, $f3, $f4
	fadd	$f3, $f3, $f0
	addi	$7, $0, 3832
	sf	$f3, 0($7)
	addi	$7, $0, 3816
	lf	$f3, 0($7)
	fmul	$f3, $f3, $f3
	addi	$7, $0, 3824
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f4
	fadd	$f3, $f3, $f4
	addi	$7, $0, 3832
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f4
	fadd	$f3, $f3, $f4
	sqrt	$f3, $f3
	lf	$f4, 4888($0) # 0.000000
	sltf	$30, $f4, $f3
	sltf	$29, $f3, $f4
	bne	$30, $29, eq_else.38560
	addi	$7, $0, 1
	j	eq_cont.38561
eq_else.38560:
	addi	$7, $0, 0
eq_cont.38561:
	bne	$7, $0, eq_else.38562
	lf	$f4, 4920($0) # 1.000000
	fdiv	$f3, $f4, $f3
	j	eq_cont.38563
eq_else.38562:
	lf	$f3, 4920($0) # 1.000000
eq_cont.38563:
	addi	$7, $0, 3816
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f3
	addi	$7, $0, 3816
	sf	$f4, 0($7)
	addi	$7, $0, 3824
	lf	$f4, 0($7)
	fmul	$f4, $f4, $f3
	addi	$7, $0, 3824
	sf	$f4, 0($7)
	addi	$7, $0, 3832
	lf	$f4, 0($7)
	fmul	$f3, $f4, $f3
	addi	$7, $0, 3832
	sf	$f3, 0($7)
	lf	$f3, 4888($0) # 0.000000
	addi	$7, $0, 3648
	sf	$f3, 0($7)
	addi	$7, $0, 3656
	sf	$f3, 0($7)
	addi	$7, $0, 3664
	sf	$f3, 0($7)
	addi	$7, $0, 3268
	lf	$f3, 0($7)
	addi	$7, $0, 3696
	sf	$f3, 0($7)
	addi	$7, $0, 3276
	lf	$f3, 0($7)
	addi	$7, $0, 3704
	sf	$f3, 0($7)
	addi	$7, $0, 3284
	lf	$f3, 0($7)
	addi	$7, $0, 3712
	sf	$f3, 0($7)
	addi	$7, $0, 0
	lf	$f3, 4920($0) # 1.000000
	slli	$8, $5, 2
	add	$30, $2, $8
	lw	$8, 0($30)
	lf	$f4, 4888($0) # 0.000000
	addi	$9, $0, 3816
	sf	$f0, 0($3)
	sf	$f2, 8($3)
	sf	$f1, 16($3)
	sw	$6, 24($3)
	sw	$2, 28($3)
	sw	$5, 32($3)
	mov	$6, $8
	mov	$5, $9
	mov	$2, $7
	movf	$f1, $f4
	movf	$f0, $f3
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_ray.3052
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	slli	$5, $2, 2
	lw	$6, 28($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	addi	$7, $0, 3648
	lf	$f0, 0($7)
	sf	$f0, 0($5)
	addi	$7, $0, 3656
	lf	$f0, 0($7)
	sf	$f0, 8($5)
	addi	$7, $0, 3664
	lf	$f0, 0($7)
	sf	$f0, 16($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	lw	$5, 24($5)
	lw	$7, 24($3)
	sw	$7, 0($5)
	slli	$5, $2, 2
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$8, $0, 0
	mov	$2, $5
	mov	$5, $8
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	pretrace_diffuse_rays.3112
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	addi	$5, $2, -1
	lw	$2, 24($3)
	addi	$2, $2, 1
	addi	$30, $0, 5
	bge	$2, $30, ge_else.38564
	mov	$6, $2
	j	ge_cont.38565
ge_else.38564:
	addi	$6, $2, -5
ge_cont.38565:
	lf	$f0, 16($3)
	lf	$f1, 8($3)
	lf	$f2, 0($3)
	lw	$2, 28($3)
	j	pretrace_pixels.3115
scan_pixel.3126:
	lw	$9, 3672($0)
	ble	$9, $2, le.38566
	slli	$9, $2, 2
	add	$30, $7, $9
	lw	$9, 0($30)
	lw	$9, 0($9)
	lf	$f0, 0($9)
	addi	$10, $0, 3648
	sf	$f0, 0($10)
	lf	$f0, 8($9)
	addi	$10, $0, 3656
	sf	$f0, 0($10)
	lf	$f0, 16($9)
	addi	$9, $0, 3664
	sf	$f0, 0($9)
	addi	$9, $0, 3676
	lw	$9, 0($9)
	addi	$10, $5, 1
	ble	$9, $10, le_else.38567
	ble	$5, $0, le_else.38569
	addi	$9, $0, 3672
	lw	$9, 0($9)
	addi	$10, $2, 1
	ble	$9, $10, le_else.38571
	ble	$2, $0, le_else.38573
	addi	$9, $0, 1
	j	le_cont.38574
le_else.38573:
	addi	$9, $0, 0
le_cont.38574:
	j	le_cont.38572
le_else.38571:
	addi	$9, $0, 0
le_cont.38572:
	j	le_cont.38570
le_else.38569:
	addi	$9, $0, 0
le_cont.38570:
	j	le_cont.38568
le_else.38567:
	addi	$9, $0, 0
le_cont.38568:
	sw	$8, 0($3)
	sw	$6, 4($3)
	sw	$5, 8($3)
	sw	$7, 12($3)
	sw	$2, 16($3)
	bne	$9, $0, eq_else.38575
	slli	$9, $2, 2
	add	$30, $7, $9
	lw	$9, 0($30)
	addi	$10, $0, 0
	lw	$11, 8($9)
	lw	$11, 0($11)
	bge	$11, $0, ge_else.38577
	j	ge_cont.38578
ge_else.38577:
	lw	$11, 12($9)
	lw	$11, 0($11)
	sw	$9, 20($3)
	bne	$11, $0, eq_else.38579
	j	eq_cont.38580
eq_else.38579:
	mov	$5, $10
	mov	$2, $9
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38580:
	lw	$2, 20($3)
	lw	$5, 8($2)
	lw	$5, 4($5)
	bge	$5, $0, ge_else.38581
	j	ge_cont.38582
ge_else.38581:
	lw	$5, 12($2)
	lw	$5, 4($5)
	bne	$5, $0, eq_else.38583
	j	eq_cont.38584
eq_else.38583:
	lw	$5, 20($2)
	lw	$6, 28($2)
	lw	$7, 4($2)
	lw	$8, 16($2)
	lw	$5, 4($5)
	lf	$f0, 0($5)
	addi	$9, $0, 3624
	sf	$f0, 0($9)
	lf	$f0, 8($5)
	addi	$9, $0, 3632
	sf	$f0, 0($9)
	lf	$f0, 16($5)
	addi	$5, $0, 3640
	sf	$f0, 0($5)
	lw	$5, 24($2)
	lw	$5, 0($5)
	lw	$6, 4($6)
	lw	$7, 4($7)
	sw	$8, 24($3)
	mov	$2, $5
	mov	$5, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	lw	$2, 4($2)
	addi	$5, $0, 3648
	lf	$f0, 0($5)
	lf	$f1, 0($2)
	addi	$5, $0, 3624
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3648
	sf	$f0, 0($5)
	addi	$5, $0, 3656
	lf	$f0, 0($5)
	lf	$f1, 8($2)
	addi	$5, $0, 3632
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3656
	sf	$f0, 0($5)
	addi	$5, $0, 3664
	lf	$f0, 0($5)
	lf	$f1, 16($2)
	addi	$2, $0, 3640
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38584:
	addi	$5, $0, 2
	lw	$2, 20($3)
	lw	$6, 8($2)
	lw	$6, 8($6)
	bge	$6, $0, ge_else.38585
	j	ge_cont.38586
ge_else.38585:
	lw	$6, 12($2)
	lw	$6, 8($6)
	bne	$6, $0, eq_else.38587
	j	eq_cont.38588
eq_else.38587:
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38588:
	addi	$5, $0, 3
	lw	$2, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	do_without_neighbors.3083
	addi	$3, $3, -32
	lw	$31, 28($3)
ge_cont.38586:
ge_cont.38582:
ge_cont.38578:
	j	eq_cont.38576
eq_else.38575:
	addi	$9, $0, 0
	slli	$10, $2, 2
	add	$30, $7, $10
	lw	$10, 0($30)
	lw	$11, 8($10)
	lw	$11, 0($11)
	bge	$11, $0, ge_else.38589
	j	ge_cont.38590
ge_else.38589:
	slli	$11, $2, 2
	add	$30, $7, $11
	lw	$11, 0($30)
	lw	$11, 8($11)
	lw	$11, 0($11)
	slli	$12, $2, 2
	add	$30, $6, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	lw	$12, 0($12)
	bne	$12, $11, eq_else.38591
	slli	$12, $2, 2
	add	$30, $8, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	lw	$12, 0($12)
	bne	$12, $11, eq_else.38593
	addi	$12, $2, -1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	lw	$12, 0($12)
	bne	$12, $11, eq_else.38595
	addi	$12, $2, 1
	slli	$12, $12, 2
	add	$30, $7, $12
	lw	$12, 0($30)
	lw	$12, 8($12)
	lw	$12, 0($12)
	bne	$12, $11, eq_else.38597
	addi	$11, $0, 1
	j	eq_cont.38598
eq_else.38597:
	addi	$11, $0, 0
eq_cont.38598:
	j	eq_cont.38596
eq_else.38595:
	addi	$11, $0, 0
eq_cont.38596:
	j	eq_cont.38594
eq_else.38593:
	addi	$11, $0, 0
eq_cont.38594:
	j	eq_cont.38592
eq_else.38591:
	addi	$11, $0, 0
eq_cont.38592:
	bne	$11, $0, eq_else.38599
	slli	$9, $2, 2
	add	$30, $7, $9
	lw	$9, 0($30)
	lw	$10, 8($9)
	lw	$10, 0($10)
	bge	$10, $0, ge_else.38601
	j	ge_cont.38602
ge_else.38601:
	lw	$10, 12($9)
	lw	$10, 0($10)
	sw	$9, 28($3)
	bne	$10, $0, eq_else.38603
	j	eq_cont.38604
eq_else.38603:
	lw	$10, 20($9)
	lw	$11, 28($9)
	lw	$12, 4($9)
	lw	$13, 16($9)
	lw	$10, 0($10)
	lf	$f0, 0($10)
	addi	$14, $0, 3624
	sf	$f0, 0($14)
	lf	$f0, 8($10)
	addi	$14, $0, 3632
	sf	$f0, 0($14)
	lf	$f0, 16($10)
	addi	$10, $0, 3640
	sf	$f0, 0($10)
	lw	$10, 24($9)
	lw	$10, 0($10)
	lw	$11, 0($11)
	lw	$12, 0($12)
	sw	$13, 32($3)
	mov	$6, $12
	mov	$5, $11
	mov	$2, $10
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	lw	$2, 0($2)
	addi	$5, $0, 3648
	lf	$f0, 0($5)
	lf	$f1, 0($2)
	addi	$5, $0, 3624
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3648
	sf	$f0, 0($5)
	addi	$5, $0, 3656
	lf	$f0, 0($5)
	lf	$f1, 8($2)
	addi	$5, $0, 3632
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3656
	sf	$f0, 0($5)
	addi	$5, $0, 3664
	lf	$f0, 0($5)
	lf	$f1, 16($2)
	addi	$2, $0, 3640
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38604:
	addi	$5, $0, 1
	lw	$2, 28($3)
	lw	$6, 8($2)
	lw	$6, 4($6)
	bge	$6, $0, ge_else.38605
	j	ge_cont.38606
ge_else.38605:
	lw	$6, 12($2)
	lw	$6, 4($6)
	bne	$6, $0, eq_else.38607
	j	eq_cont.38608
eq_else.38607:
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38608:
	addi	$5, $0, 2
	lw	$2, 28($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	do_without_neighbors.3083
	addi	$3, $3, -40
	lw	$31, 36($3)
ge_cont.38606:
ge_cont.38602:
	j	eq_cont.38600
eq_else.38599:
	lw	$10, 12($10)
	lw	$10, 0($10)
	bne	$10, $0, eq_else.38609
	j	eq_cont.38610
eq_else.38609:
	mov	$5, $6
	mov	$6, $7
	mov	$7, $8
	mov	$8, $9
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	calc_diffuse_using_5points.3077
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38610:
	addi	$9, $0, 1
	lw	$2, 16($3)
	lw	$5, 8($3)
	lw	$6, 4($3)
	lw	$7, 12($3)
	lw	$8, 0($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	try_exploit_neighbors.3099
	addi	$3, $3, -40
	lw	$31, 36($3)
eq_cont.38600:
ge_cont.38590:
eq_cont.38576:
	addi	$2, $0, 3648
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38611
	addi	$2, $0, 255
	j	le_cont.38612
le_else.38611:
	bge	$2, $0, ge_else.38613
	addi	$2, $0, 0
	j	ge_cont.38614
ge_else.38613:
ge_cont.38614:
le_cont.38612:
	outb	$2
	addi	$2, $0, 3656
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38615
	addi	$2, $0, 255
	j	le_cont.38616
le_else.38615:
	bge	$2, $0, ge_else.38617
	addi	$2, $0, 0
	j	ge_cont.38618
ge_else.38617:
ge_cont.38618:
le_cont.38616:
	outb	$2
	addi	$2, $0, 3664
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38619
	addi	$2, $0, 255
	j	le_cont.38620
le_else.38619:
	bge	$2, $0, ge_else.38621
	addi	$2, $0, 0
	j	ge_cont.38622
ge_else.38621:
ge_cont.38622:
le_cont.38620:
	outb	$2
	lw	$2, 16($3)
	addi	$2, $2, 1
	addi	$5, $0, 3672
	lw	$5, 0($5)
	ble	$5, $2, le.38623
	slli	$5, $2, 2
	lw	$7, 12($3)
	add	$30, $7, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	lf	$f0, 8($5)
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	lf	$f0, 16($5)
	addi	$5, $0, 3664
	sf	$f0, 0($5)
	addi	$5, $0, 3676
	lw	$5, 0($5)
	lw	$6, 8($3)
	addi	$8, $6, 1
	ble	$5, $8, le_else.38624
	ble	$6, $0, le_else.38626
	addi	$5, $0, 3672
	lw	$5, 0($5)
	addi	$8, $2, 1
	ble	$5, $8, le_else.38628
	ble	$2, $0, le_else.38630
	addi	$5, $0, 1
	j	le_cont.38631
le_else.38630:
	addi	$5, $0, 0
le_cont.38631:
	j	le_cont.38629
le_else.38628:
	addi	$5, $0, 0
le_cont.38629:
	j	le_cont.38627
le_else.38626:
	addi	$5, $0, 0
le_cont.38627:
	j	le_cont.38625
le_else.38624:
	addi	$5, $0, 0
le_cont.38625:
	sw	$2, 36($3)
	bne	$5, $0, eq_else.38632
	slli	$5, $2, 2
	add	$30, $7, $5
	lw	$5, 0($30)
	lw	$8, 8($5)
	lw	$8, 0($8)
	bge	$8, $0, ge_else.38634
	j	ge_cont.38635
ge_else.38634:
	lw	$8, 12($5)
	lw	$8, 0($8)
	sw	$5, 40($3)
	bne	$8, $0, eq_else.38636
	j	eq_cont.38637
eq_else.38636:
	lw	$8, 20($5)
	lw	$9, 28($5)
	lw	$10, 4($5)
	lw	$11, 16($5)
	lw	$8, 0($8)
	lf	$f0, 0($8)
	addi	$12, $0, 3624
	sf	$f0, 0($12)
	lf	$f0, 8($8)
	addi	$12, $0, 3632
	sf	$f0, 0($12)
	lf	$f0, 16($8)
	addi	$8, $0, 3640
	sf	$f0, 0($8)
	lw	$8, 24($5)
	lw	$8, 0($8)
	lw	$9, 0($9)
	lw	$10, 0($10)
	sw	$11, 44($3)
	mov	$6, $10
	mov	$5, $9
	mov	$2, $8
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$2, 44($3)
	lw	$2, 0($2)
	addi	$5, $0, 3648
	lf	$f0, 0($5)
	lf	$f1, 0($2)
	addi	$5, $0, 3624
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3648
	sf	$f0, 0($5)
	addi	$5, $0, 3656
	lf	$f0, 0($5)
	lf	$f1, 8($2)
	addi	$5, $0, 3632
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3656
	sf	$f0, 0($5)
	addi	$5, $0, 3664
	lf	$f0, 0($5)
	lf	$f1, 16($2)
	addi	$2, $0, 3640
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38637:
	addi	$5, $0, 1
	lw	$2, 40($3)
	lw	$6, 8($2)
	lw	$6, 4($6)
	bge	$6, $0, ge_else.38638
	j	ge_cont.38639
ge_else.38638:
	lw	$6, 12($2)
	lw	$6, 4($6)
	bne	$6, $0, eq_else.38640
	j	eq_cont.38641
eq_else.38640:
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.38641:
	addi	$5, $0, 2
	lw	$2, 40($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	do_without_neighbors.3083
	addi	$3, $3, -56
	lw	$31, 52($3)
ge_cont.38639:
ge_cont.38635:
	j	eq_cont.38633
eq_else.38632:
	addi	$9, $0, 0
	lw	$5, 4($3)
	lw	$8, 0($3)
	mov	$27, $6
	mov	$6, $5
	mov	$5, $27
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	try_exploit_neighbors.3099
	addi	$3, $3, -56
	lw	$31, 52($3)
eq_cont.38633:
	addi	$2, $0, 3648
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38642
	addi	$2, $0, 255
	j	le_cont.38643
le_else.38642:
	bge	$2, $0, ge_else.38644
	addi	$2, $0, 0
	j	ge_cont.38645
ge_else.38644:
ge_cont.38645:
le_cont.38643:
	outb	$2
	addi	$2, $0, 3656
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38646
	addi	$2, $0, 255
	j	le_cont.38647
le_else.38646:
	bge	$2, $0, ge_else.38648
	addi	$2, $0, 0
	j	ge_cont.38649
ge_else.38648:
ge_cont.38649:
le_cont.38647:
	outb	$2
	addi	$2, $0, 3664
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38650
	addi	$2, $0, 255
	j	le_cont.38651
le_else.38650:
	bge	$2, $0, ge_else.38652
	addi	$2, $0, 0
	j	ge_cont.38653
ge_else.38652:
ge_cont.38653:
le_cont.38651:
	outb	$2
	lw	$2, 36($3)
	addi	$2, $2, 1
	lw	$5, 8($3)
	lw	$6, 4($3)
	lw	$7, 12($3)
	lw	$8, 0($3)
	j	scan_pixel.3126
le.38623:
	jr $31
le.38566:
	jr $31
scan_line.3132:
	lw	$9, 3676($0)
	ble	$9, $2, le.38656
	addi	$9, $0, 3676
	lw	$9, 0($9)
	addi	$9, $9, -1
	sw	$8, 0($3)
	sw	$7, 4($3)
	sw	$5, 8($3)
	sw	$2, 12($3)
	sw	$6, 16($3)
	ble	$9, $2, le_else.38657
	addi	$9, $2, 1
	addi	$10, $0, 3688
	lf	$f0, 0($10)
	addi	$10, $0, 3684
	lw	$10, 0($10)
	sub	$9, $9, $10
	itof	$f1, $9
	fmul	$f0, $f0, $f1
	addi	$9, $0, 3768
	lf	$f1, 0($9)
	fmul	$f1, $f0, $f1
	addi	$9, $0, 3792
	lf	$f2, 0($9)
	fadd	$f1, $f1, $f2
	addi	$9, $0, 3776
	lf	$f2, 0($9)
	fmul	$f2, $f0, $f2
	addi	$9, $0, 3800
	lf	$f3, 0($9)
	fadd	$f2, $f2, $f3
	addi	$9, $0, 3784
	lf	$f3, 0($9)
	fmul	$f0, $f0, $f3
	addi	$9, $0, 3808
	lf	$f3, 0($9)
	fadd	$f0, $f0, $f3
	addi	$9, $0, 3672
	lw	$9, 0($9)
	addi	$9, $9, -1
	mov	$6, $8
	mov	$5, $9
	mov	$2, $7
	movf	$f30, $f2
	movf	$f2, $f0
	movf	$f0, $f1
	movf	$f1, $f30
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	pretrace_pixels.3115
	addi	$3, $3, -24
	lw	$31, 20($3)
	j	le_cont.38658
le_else.38657:
le_cont.38658:
	addi	$2, $0, 0
	addi	$5, $0, 3672
	lw	$5, 0($5)
	ble	$5, $0, le_else.38659
	lw	$7, 16($3)
	lw	$5, 0($7)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	addi	$6, $0, 3648
	sf	$f0, 0($6)
	lf	$f0, 8($5)
	addi	$6, $0, 3656
	sf	$f0, 0($6)
	lf	$f0, 16($5)
	addi	$5, $0, 3664
	sf	$f0, 0($5)
	addi	$5, $0, 3676
	lw	$5, 0($5)
	lw	$6, 12($3)
	addi	$8, $6, 1
	ble	$5, $8, le_else.38661
	ble	$6, $0, le_else.38663
	addi	$5, $0, 3672
	lw	$5, 0($5)
	addi	$30, $0, 1
	ble	$5, $30, le_else.38665
	addi	$5, $0, 0
	j	le_cont.38666
le_else.38665:
	addi	$5, $0, 0
le_cont.38666:
	j	le_cont.38664
le_else.38663:
	addi	$5, $0, 0
le_cont.38664:
	j	le_cont.38662
le_else.38661:
	addi	$5, $0, 0
le_cont.38662:
	bne	$5, $0, eq_else.38667
	lw	$2, 0($7)
	lw	$5, 8($2)
	lw	$5, 0($5)
	bge	$5, $0, ge_else.38669
	j	ge_cont.38670
ge_else.38669:
	lw	$5, 12($2)
	lw	$5, 0($5)
	sw	$2, 20($3)
	bne	$5, $0, eq_else.38671
	j	eq_cont.38672
eq_else.38671:
	lw	$5, 20($2)
	lw	$8, 28($2)
	lw	$9, 4($2)
	lw	$10, 16($2)
	lw	$5, 0($5)
	lf	$f0, 0($5)
	addi	$11, $0, 3624
	sf	$f0, 0($11)
	lf	$f0, 8($5)
	addi	$11, $0, 3632
	sf	$f0, 0($11)
	lf	$f0, 16($5)
	addi	$5, $0, 3640
	sf	$f0, 0($5)
	lw	$5, 24($2)
	lw	$5, 0($5)
	lw	$8, 0($8)
	lw	$9, 0($9)
	sw	$10, 24($3)
	mov	$6, $9
	mov	$2, $5
	mov	$5, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	trace_diffuse_ray_80percent.3070
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	lw	$2, 0($2)
	addi	$5, $0, 3648
	lf	$f0, 0($5)
	lf	$f1, 0($2)
	addi	$5, $0, 3624
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3648
	sf	$f0, 0($5)
	addi	$5, $0, 3656
	lf	$f0, 0($5)
	lf	$f1, 8($2)
	addi	$5, $0, 3632
	lf	$f2, 0($5)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$5, $0, 3656
	sf	$f0, 0($5)
	addi	$5, $0, 3664
	lf	$f0, 0($5)
	lf	$f1, 16($2)
	addi	$2, $0, 3640
	lf	$f2, 0($2)
	fmul	$f1, $f1, $f2
	fadd	$f0, $f0, $f1
	addi	$2, $0, 3664
	sf	$f0, 0($2)
eq_cont.38672:
	addi	$5, $0, 1
	lw	$2, 20($3)
	lw	$6, 8($2)
	lw	$6, 4($6)
	bge	$6, $0, ge_else.38673
	j	ge_cont.38674
ge_else.38673:
	lw	$6, 12($2)
	lw	$6, 4($6)
	bne	$6, $0, eq_else.38675
	j	eq_cont.38676
eq_else.38675:
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_diffuse_using_1point.3074
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38676:
	addi	$5, $0, 2
	lw	$2, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	do_without_neighbors.3083
	addi	$3, $3, -32
	lw	$31, 28($3)
ge_cont.38674:
ge_cont.38670:
	j	eq_cont.38668
eq_else.38667:
	addi	$9, $0, 0
	lw	$5, 8($3)
	lw	$8, 4($3)
	mov	$27, $6
	mov	$6, $5
	mov	$5, $27
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	try_exploit_neighbors.3099
	addi	$3, $3, -32
	lw	$31, 28($3)
eq_cont.38668:
	addi	$2, $0, 3648
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38677
	addi	$2, $0, 255
	j	le_cont.38678
le_else.38677:
	bge	$2, $0, ge_else.38679
	addi	$2, $0, 0
	j	ge_cont.38680
ge_else.38679:
ge_cont.38680:
le_cont.38678:
	outb	$2
	addi	$2, $0, 3656
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38681
	addi	$2, $0, 255
	j	le_cont.38682
le_else.38681:
	bge	$2, $0, ge_else.38683
	addi	$2, $0, 0
	j	ge_cont.38684
ge_else.38683:
ge_cont.38684:
le_cont.38682:
	outb	$2
	addi	$2, $0, 3664
	lf	$f0, 0($2)
	ftoi	$2, $f0
	addi	$30, $0, 255
	ble	$2, $30, le_else.38685
	addi	$2, $0, 255
	j	le_cont.38686
le_else.38685:
	bge	$2, $0, ge_else.38687
	addi	$2, $0, 0
	j	ge_cont.38688
ge_else.38687:
ge_cont.38688:
le_cont.38686:
	outb	$2
	addi	$2, $0, 1
	lw	$5, 12($3)
	lw	$6, 8($3)
	lw	$7, 16($3)
	lw	$8, 4($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	scan_pixel.3126
	addi	$3, $3, -32
	lw	$31, 28($3)
	j	le_cont.38660
le_else.38659:
le_cont.38660:
	lw	$2, 12($3)
	addi	$5, $2, 1
	lw	$2, 0($3)
	addi	$2, $2, 2
	addi	$30, $0, 5
	bge	$2, $30, ge_else.38689
	mov	$6, $2
	j	ge_cont.38690
ge_else.38689:
	addi	$6, $2, -5
ge_cont.38690:
	addi	$2, $0, 3676
	lw	$2, 0($2)
	ble	$2, $5, le.38691
	addi	$2, $0, 3676
	lw	$2, 0($2)
	addi	$2, $2, -1
	sw	$6, 28($3)
	sw	$5, 32($3)
	ble	$2, $5, le_else.38692
	addi	$2, $5, 1
	lw	$7, 8($3)
	mov	$5, $2
	mov	$2, $7
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	pretrace_line.3122
	addi	$3, $3, -40
	lw	$31, 36($3)
	j	le_cont.38693
le_else.38692:
le_cont.38693:
	addi	$2, $0, 0
	lw	$5, 32($3)
	lw	$6, 16($3)
	lw	$7, 4($3)
	lw	$8, 8($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	scan_pixel.3126
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 32($3)
	addi	$2, $2, 1
	lw	$5, 28($3)
	addi	$5, $5, 2
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38694
	mov	$8, $5
	j	ge_cont.38695
ge_else.38694:
	addi	$8, $5, -5
ge_cont.38695:
	lw	$5, 4($3)
	lw	$6, 8($3)
	lw	$7, 16($3)
	j	scan_line.3132
le.38691:
	jr $31
le.38656:
	jr $31
create_pixel.3140:
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 0($3)
	mov	$2, $5
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 4($3)
	mov	$2, $5
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 4($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 8($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 12($3)
	mov	$2, $5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 16($3)
	mov	$2, $5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 20($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 24($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 28($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 24($3)
	sw	$5, 24($2)
	lw	$5, 20($3)
	sw	$5, 20($2)
	lw	$5, 16($3)
	sw	$5, 16($2)
	lw	$5, 12($3)
	sw	$5, 12($2)
	lw	$5, 8($3)
	sw	$5, 8($2)
	lw	$5, 4($3)
	sw	$5, 4($2)
	lw	$5, 0($3)
	sw	$5, 0($2)
	jr $31
init_line_elements.3142:
	bge	$5, $0, ge.38698
	jr $31
ge.38698:
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 0($3)
	sw	$5, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 8($3)
	mov	$2, $5
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 12($3)
	mov	$2, $5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 12($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 12($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 12($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 12($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 16($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 20($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 24($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 24($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 28($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 32($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 36($3)
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 36($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 36($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 36($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 36($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 32($3)
	sw	$5, 24($2)
	lw	$5, 28($3)
	sw	$5, 20($2)
	lw	$5, 24($3)
	sw	$5, 16($2)
	lw	$5, 20($3)
	sw	$5, 12($2)
	lw	$5, 16($3)
	sw	$5, 8($2)
	lw	$5, 12($3)
	sw	$5, 4($2)
	lw	$5, 8($3)
	sw	$5, 0($2)
	lw	$5, 4($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38699
	mov	$2, $7
	jr $31
ge.38699:
	sw	$2, 40($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	create_pixel.3140
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 40($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38700
	mov	$2, $7
	jr $31
ge.38700:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 44($3)
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 48($3)
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 52($3)
	mov	$2, $5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 52($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 52($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 52($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 52($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 56($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 60($3)
	mov	$2, $5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 64($3)
	mov	$2, $5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 64($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 64($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 64($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 64($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 68($3)
	mov	$2, $5
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 72($3)
	mov	$2, $5
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 76($3)
	mov	$2, $5
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 76($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 76($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 76($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 76($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 72($3)
	sw	$5, 24($2)
	lw	$5, 68($3)
	sw	$5, 20($2)
	lw	$5, 64($3)
	sw	$5, 16($2)
	lw	$5, 60($3)
	sw	$5, 12($2)
	lw	$5, 56($3)
	sw	$5, 8($2)
	lw	$5, 52($3)
	sw	$5, 4($2)
	lw	$5, 48($3)
	sw	$5, 0($2)
	lw	$5, 44($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38701
	mov	$2, $7
	jr $31
ge.38701:
	sw	$2, 80($3)
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	create_pixel.3140
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$5, $5, -1
	mov	$2, $7
	j	init_line_elements.3142
calc_dirvec.3152:
	addi	$30, $0, 5
	bge	$2, $30, ge.38702
	fmul	$f0, $f1, $f1
	lf	$f1, 4624($0) # 0.100000
	fadd	$f0, $f0, $f1
	sqrt	$f0, $f0
	lf	$f1, 4920($0) # 1.000000
	fdiv	$f1, $f1, $f0
	sw	$6, 0($3)
	sw	$5, 4($3)
	sf	$f3, 8($3)
	sw	$2, 16($3)
	sf	$f0, 24($3)
	sf	$f2, 32($3)
	movf	$f0, $f1
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	atan.2705
	addi	$3, $3, -48
	lw	$31, 44($3)
	lf	$f1, 32($3)
	fmul	$f0, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.38704
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.38705
eq_else.38704:
	lf	$f2, 4920($0) # 1.000000
eq_cont.38705:
	fmul	$f3, $f2, $f0
	lf	$f4, 4800($0) # 6.283185
	fdiv	$f5, $f3, $f4
	floor	$f5, $f5
	fmul	$f4, $f5, $f4
	fsub	$f3, $f3, $f4
	sf	$f0, 40($3)
	movf	$f1, $f3
	movf	$f0, $f2
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	sin_sub1.2687
	addi	$3, $3, -56
	lw	$31, 52($3)
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	lf	$f3, 40($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.38706
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.38707
eq_else.38706:
	lf	$f2, 4920($0) # 1.000000
eq_cont.38707:
	fmul	$f2, $f2, $f3
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	sf	$f0, 48($3)
	movf	$f0, $f1
	movf	$f1, $f2
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	cos_sub1.2698
	addi	$3, $3, -64
	lw	$31, 60($3)
	lf	$f1, 48($3)
	fdiv	$f0, $f1, $f0
	lf	$f1, 24($3)
	fmul	$f0, $f0, $f1
	lw	$2, 16($3)
	addi	$2, $2, 1
	fmul	$f1, $f0, $f0
	lf	$f2, 4624($0) # 0.100000
	fadd	$f1, $f1, $f2
	sqrt	$f1, $f1
	lf	$f2, 4920($0) # 1.000000
	fdiv	$f2, $f2, $f1
	sf	$f0, 56($3)
	sw	$2, 64($3)
	sf	$f1, 72($3)
	movf	$f0, $f2
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	atan.2705
	addi	$3, $3, -88
	lw	$31, 84($3)
	lf	$f1, 8($3)
	fmul	$f0, $f0, $f1
	lf	$f2, 4888($0) # 0.000000
	sltf	$30, $f2, $f0
	bne	$30, $0, eq_else.38709
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.38710
eq_else.38709:
	lf	$f2, 4920($0) # 1.000000
eq_cont.38710:
	fmul	$f3, $f2, $f0
	lf	$f4, 4800($0) # 6.283185
	fdiv	$f5, $f3, $f4
	floor	$f5, $f5
	fmul	$f4, $f5, $f4
	fsub	$f3, $f3, $f4
	sf	$f0, 80($3)
	movf	$f1, $f3
	movf	$f0, $f2
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	sin_sub1.2687
	addi	$3, $3, -96
	lw	$31, 92($3)
	lf	$f1, 4920($0) # 1.000000
	lf	$f2, 4888($0) # 0.000000
	lf	$f3, 80($3)
	sltf	$30, $f2, $f3
	bne	$30, $0, eq_else.38711
	lf	$f2, 4880($0) # -1.000000
	j	eq_cont.38712
eq_else.38711:
	lf	$f2, 4920($0) # 1.000000
eq_cont.38712:
	fmul	$f2, $f2, $f3
	lf	$f3, 4800($0) # 6.283185
	fdiv	$f4, $f2, $f3
	floor	$f4, $f4
	fmul	$f3, $f4, $f3
	fsub	$f2, $f2, $f3
	sf	$f0, 88($3)
	movf	$f0, $f1
	movf	$f1, $f2
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	cos_sub1.2698
	addi	$3, $3, -104
	lw	$31, 100($3)
	lf	$f1, 88($3)
	fdiv	$f0, $f1, $f0
	lf	$f1, 72($3)
	fmul	$f1, $f0, $f1
	lf	$f0, 56($3)
	lf	$f2, 32($3)
	lf	$f3, 8($3)
	lw	$2, 64($3)
	lw	$5, 4($3)
	lw	$6, 0($3)
	j	calc_dirvec.3152
ge.38702:
	fmul	$f2, $f0, $f0
	fmul	$f3, $f1, $f1
	fadd	$f2, $f2, $f3
	lf	$f3, 4920($0) # 1.000000
	fadd	$f2, $f2, $f3
	sqrt	$f2, $f2
	fdiv	$f0, $f0, $f2
	fdiv	$f1, $f1, $f2
	lf	$f3, 4920($0) # 1.000000
	fdiv	$f2, $f3, $f2
	slli	$2, $5, 2
	addi	$2, $2, 3840
	lw	$2, 0($2)
	slli	$5, $6, 2
	add	$30, $2, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	sf	$f0, 0($5)
	sf	$f1, 8($5)
	sf	$f2, 16($5)
	addi	$5, $6, 40
	slli	$5, $5, 2
	add	$30, $2, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	fneg	$f3, $f1
	sf	$f0, 0($5)
	sf	$f2, 8($5)
	sf	$f3, 16($5)
	addi	$5, $6, 80
	slli	$5, $5, 2
	add	$30, $2, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	fneg	$f3, $f0
	fneg	$f4, $f1
	sf	$f2, 0($5)
	sf	$f3, 8($5)
	sf	$f4, 16($5)
	addi	$5, $6, 1
	slli	$5, $5, 2
	add	$30, $2, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	fneg	$f3, $f0
	fneg	$f4, $f1
	fneg	$f5, $f2
	sf	$f3, 0($5)
	sf	$f4, 8($5)
	sf	$f5, 16($5)
	addi	$5, $6, 41
	slli	$5, $5, 2
	add	$30, $2, $5
	lw	$5, 0($30)
	lw	$5, 0($5)
	fneg	$f3, $f0
	fneg	$f4, $f2
	sf	$f3, 0($5)
	sf	$f4, 8($5)
	sf	$f1, 16($5)
	addi	$5, $6, 81
	slli	$5, $5, 2
	add	$30, $2, $5
	lw	$2, 0($30)
	lw	$2, 0($2)
	fneg	$f2, $f2
	sf	$f2, 0($2)
	sf	$f0, 8($2)
	sf	$f1, 16($2)
	jr $31
calc_dirvecs.3160:
	bge	$2, $0, ge.38714
	jr $31
ge.38714:
	itof	$f1, $2
	lf	$f2, 4856($0) # 0.200000
	fmul	$f1, $f1, $f2
	lf	$f2, 4600($0) # 0.900000
	fsub	$f2, $f1, $f2
	addi	$7, $0, 0
	lf	$f1, 4888($0) # 0.000000
	lf	$f3, 4888($0) # 0.000000
	sf	$f0, 0($3)
	sw	$5, 8($3)
	sw	$6, 12($3)
	sw	$2, 16($3)
	mov	$2, $7
	movf	$f30, $f3
	movf	$f3, $f0
	movf	$f0, $f1
	movf	$f1, $f30
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	calc_dirvec.3152
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 16($3)
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4624($0) # 0.100000
	fadd	$f2, $f0, $f1
	addi	$5, $0, 0
	lf	$f0, 4888($0) # 0.000000
	lf	$f1, 4888($0) # 0.000000
	lw	$6, 12($3)
	addi	$7, $6, 2
	lf	$f3, 0($3)
	lw	$8, 8($3)
	mov	$6, $7
	mov	$2, $5
	mov	$5, $8
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	calc_dirvec.3152
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 16($3)
	addi	$2, $2, -1
	lw	$5, 8($3)
	addi	$5, $5, 1
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38716
	j	ge_cont.38717
ge_else.38716:
	addi	$5, $5, -5
ge_cont.38717:
	bge	$2, $0, ge.38718
	jr $31
ge.38718:
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4600($0) # 0.900000
	fsub	$f2, $f0, $f1
	addi	$6, $0, 0
	lf	$f0, 4888($0) # 0.000000
	lf	$f1, 4888($0) # 0.000000
	lf	$f3, 0($3)
	lw	$7, 12($3)
	sw	$5, 20($3)
	sw	$2, 24($3)
	mov	$2, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_dirvec.3152
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4624($0) # 0.100000
	fadd	$f2, $f0, $f1
	addi	$5, $0, 0
	lf	$f0, 4888($0) # 0.000000
	lf	$f1, 4888($0) # 0.000000
	lw	$6, 12($3)
	addi	$7, $6, 2
	lf	$f3, 0($3)
	lw	$8, 20($3)
	mov	$6, $7
	mov	$2, $5
	mov	$5, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_dirvec.3152
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	addi	$2, $2, -1
	lw	$5, 20($3)
	addi	$5, $5, 1
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38720
	j	ge_cont.38721
ge_else.38720:
	addi	$5, $5, -5
ge_cont.38721:
	lf	$f0, 0($3)
	lw	$6, 12($3)
	j	calc_dirvecs.3160
calc_dirvec_rows.3165:
	bge	$2, $0, ge.38722
	jr $31
ge.38722:
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4600($0) # 0.900000
	fsub	$f3, $f0, $f1
	addi	$7, $0, 4
	itof	$f0, $7
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4600($0) # 0.900000
	fsub	$f2, $f0, $f1
	addi	$8, $0, 0
	lf	$f0, 4888($0) # 0.000000
	lf	$f1, 4888($0) # 0.000000
	sw	$2, 0($3)
	sf	$f3, 8($3)
	sw	$5, 16($3)
	sw	$6, 20($3)
	sw	$7, 24($3)
	mov	$2, $8
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_dirvec.3152
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4624($0) # 0.100000
	fadd	$f2, $f0, $f1
	addi	$2, $0, 0
	lf	$f0, 4888($0) # 0.000000
	lf	$f1, 4888($0) # 0.000000
	lw	$5, 20($3)
	addi	$6, $5, 2
	lf	$f3, 8($3)
	lw	$7, 16($3)
	mov	$5, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_dirvec.3152
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 3
	lw	$5, 16($3)
	addi	$6, $5, 1
	addi	$30, $0, 5
	bge	$6, $30, ge_else.38725
	j	ge_cont.38726
ge_else.38725:
	addi	$6, $6, -5
ge_cont.38726:
	lf	$f0, 8($3)
	lw	$7, 20($3)
	mov	$5, $6
	mov	$6, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	calc_dirvecs.3160
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 0($3)
	addi	$2, $2, -1
	lw	$5, 16($3)
	addi	$5, $5, 2
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38727
	j	ge_cont.38728
ge_else.38727:
	addi	$5, $5, -5
ge_cont.38728:
	lw	$6, 20($3)
	addi	$6, $6, 4
	bge	$2, $0, ge.38729
	jr $31
ge.38729:
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4600($0) # 0.900000
	fsub	$f0, $f0, $f1
	addi	$7, $0, 4
	sw	$6, 28($3)
	sw	$5, 32($3)
	sw	$2, 36($3)
	mov	$2, $7
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	calc_dirvecs.3160
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 36($3)
	addi	$2, $2, -1
	lw	$5, 32($3)
	addi	$5, $5, 2
	addi	$30, $0, 5
	bge	$5, $30, ge_else.38731
	j	ge_cont.38732
ge_else.38731:
	addi	$5, $5, -5
ge_cont.38732:
	lw	$6, 28($3)
	addi	$6, $6, 4
	j	calc_dirvec_rows.3165
create_dirvec_elements.3171:
	bge	$5, $0, ge.38733
	jr $31
ge.38733:
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 0($3)
	sw	$5, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 8($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 8($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 4($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38735
	jr $31
ge.38735:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 12($3)
	mov	$2, $5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 16($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 16($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38737
	jr $31
ge.38737:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 20($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 24($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 24($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 20($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38739
	jr $31
ge.38739:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 28($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 32($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 32($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 28($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38741
	jr $31
ge.38741:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 36($3)
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 40($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 40($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 36($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38743
	jr $31
ge.38743:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 44($3)
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 48($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 48($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 44($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38745
	jr $31
ge.38745:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 52($3)
	mov	$2, $5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 56($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 56($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 52($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$2, $5, -1
	bge	$2, $0, ge.38747
	jr $31
ge.38747:
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 60($3)
	mov	$2, $5
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 64($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 64($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 60($3)
	slli	$6, $5, 2
	lw	$7, 0($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$5, $5, -1
	mov	$2, $7
	j	create_dirvec_elements.3171
create_dirvecs.3174:
	bge	$2, $0, ge.38749
	jr $31
ge.38749:
	addi	$5, $0, 120
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 0($3)
	sw	$5, 4($3)
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 8($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 8($3)
	sw	$2, 0($5)
	lw	$2, 4($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$5, 0($3)
	slli	$6, $5, 2
	addi	$6, $6, 3840
	sw	$2, 0($6)
	slli	$2, $5, 2
	addi	$2, $2, 3840
	lw	$2, 0($2)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 12($3)
	mov	$2, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 16($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 16($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 472($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 20($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 468($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 24($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 24($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 464($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 28($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 28($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 460($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 32($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 32($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 456($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 36($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 36($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 452($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 40($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 40($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 12($3)
	sw	$2, 448($5)
	addi	$2, $0, 111
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	create_dirvec_elements.3171
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 0($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38751
	jr $31
ge.38751:
	addi	$5, $0, 120
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 44($3)
	sw	$5, 48($3)
	mov	$2, $6
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 52($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 52($3)
	sw	$2, 0($5)
	lw	$2, 48($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 44($3)
	slli	$6, $5, 2
	addi	$6, $6, 3840
	sw	$2, 0($6)
	slli	$2, $5, 2
	addi	$2, $2, 3840
	lw	$2, 0($2)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 56($3)
	mov	$2, $6
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 60($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 60($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 472($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 64($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 64($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 468($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	min_caml_create_float_array
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 68($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 68($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 464($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 72($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 72($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 460($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 76($3)
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 76($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 456($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 80($3)
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 80($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 56($3)
	sw	$2, 452($5)
	addi	$2, $0, 112
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	create_dirvec_elements.3171
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$2, 44($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38753
	jr $31
ge.38753:
	addi	$5, $0, 120
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 84($3)
	sw	$5, 88($3)
	mov	$2, $6
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_float_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 92($3)
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 92($3)
	sw	$2, 0($5)
	lw	$2, 88($3)
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 84($3)
	slli	$6, $5, 2
	addi	$6, $6, 3840
	sw	$2, 0($6)
	slli	$2, $5, 2
	addi	$2, $2, 3840
	lw	$2, 0($2)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 96($3)
	mov	$2, $6
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 100($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 100($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 96($3)
	sw	$2, 472($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 104($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 104($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 96($3)
	sw	$2, 468($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 108($3)
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 108($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 96($3)
	sw	$2, 464($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_float_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 112($3)
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 112($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 96($3)
	sw	$2, 460($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	min_caml_create_float_array
	addi	$3, $3, -120
	lw	$31, 116($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 116($3)
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	min_caml_create_array
	addi	$3, $3, -128
	lw	$31, 124($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 116($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 96($3)
	sw	$2, 456($5)
	addi	$2, $0, 113
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	create_dirvec_elements.3171
	addi	$3, $3, -128
	lw	$31, 124($3)
	lw	$2, 84($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38755
	jr $31
ge.38755:
	addi	$5, $0, 120
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 120($3)
	sw	$5, 124($3)
	mov	$2, $6
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 128($3)
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 128($3)
	sw	$2, 0($5)
	lw	$2, 124($3)
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 120($3)
	slli	$6, $5, 2
	addi	$6, $6, 3840
	sw	$2, 0($6)
	slli	$2, $5, 2
	addi	$2, $2, 3840
	lw	$2, 0($2)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 132($3)
	mov	$2, $6
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_float_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 136($3)
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 136($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 132($3)
	sw	$2, 472($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_float_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 140($3)
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 140($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 132($3)
	sw	$2, 468($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 144($3)
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 144($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 132($3)
	sw	$2, 464($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 148($3)
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 148($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 132($3)
	sw	$2, 460($5)
	addi	$2, $0, 114
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	create_dirvec_elements.3171
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$2, 120($3)
	addi	$2, $2, -1
	j	create_dirvecs.3174
init_dirvec_constants.3176:
	bge	$5, $0, ge.38757
	jr $31
ge.38757:
	slli	$6, $5, 2
	add	$30, $2, $6
	lw	$6, 0($30)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 0($3)
	sw	$5, 4($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -16
	lw	$31, 12($3)
	lw	$2, 4($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38759
	jr $31
ge.38759:
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 8($3)
	bge	$7, $0, ge_else.38761
	j	ge_cont.38762
ge_else.38761:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	lw	$9, 4($5)
	lw	$10, 0($5)
	lw	$11, 4($8)
	sw	$5, 12($3)
	addi	$30, $0, 1
	bne	$11, $30, eq_else.38763
	sw	$9, 16($3)
	sw	$7, 20($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_rect_table.2949
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	slli	$6, $5, 2
	lw	$7, 16($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38764
eq_else.38763:
	addi	$30, $0, 2
	bne	$11, $30, eq_else.38765
	sw	$9, 16($3)
	sw	$7, 20($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_surface_table.2952
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	slli	$6, $5, 2
	lw	$7, 16($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38766
eq_else.38765:
	sw	$9, 16($3)
	sw	$7, 20($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	setup_second_table.2955
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$5, 20($3)
	slli	$6, $5, 2
	lw	$7, 16($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38766:
eq_cont.38764:
	addi	$5, $5, -1
	lw	$2, 12($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -32
	lw	$31, 28($3)
ge_cont.38762:
	lw	$2, 8($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38767
	jr $31
ge.38767:
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 24($3)
	mov	$2, $5
	mov	$5, $7
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -32
	lw	$31, 28($3)
	lw	$2, 24($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38769
	jr $31
ge.38769:
	slli	$5, $2, 2
	lw	$6, 0($3)
	add	$30, $6, $5
	lw	$5, 0($30)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 28($3)
	bge	$7, $0, ge_else.38771
	j	ge_cont.38772
ge_else.38771:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	lw	$9, 4($5)
	lw	$10, 0($5)
	lw	$11, 4($8)
	sw	$5, 32($3)
	addi	$30, $0, 1
	bne	$11, $30, eq_else.38773
	sw	$9, 36($3)
	sw	$7, 40($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_rect_table.2949
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 40($3)
	slli	$6, $5, 2
	lw	$7, 36($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38774
eq_else.38773:
	addi	$30, $0, 2
	bne	$11, $30, eq_else.38775
	sw	$9, 36($3)
	sw	$7, 40($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_surface_table.2952
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 40($3)
	slli	$6, $5, 2
	lw	$7, 36($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38776
eq_else.38775:
	sw	$9, 36($3)
	sw	$7, 40($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	setup_second_table.2955
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$5, 40($3)
	slli	$6, $5, 2
	lw	$7, 36($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38776:
eq_cont.38774:
	addi	$5, $5, -1
	lw	$2, 32($3)
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -48
	lw	$31, 44($3)
ge_cont.38772:
	lw	$2, 28($3)
	addi	$5, $2, -1
	lw	$2, 0($3)
	j	init_dirvec_constants.3176
init_vecset_constants.3179:
	bge	$2, $0, ge.38777
	jr $31
ge.38777:
	slli	$5, $2, 2
	addi	$5, $5, 3840
	lw	$5, 0($5)
	lw	$6, 476($5)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 0($3)
	sw	$5, 4($3)
	bge	$7, $0, ge_else.38779
	j	ge_cont.38780
ge_else.38779:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	lw	$9, 4($6)
	lw	$10, 0($6)
	lw	$11, 4($8)
	sw	$6, 8($3)
	addi	$30, $0, 1
	bne	$11, $30, eq_else.38781
	sw	$9, 12($3)
	sw	$7, 16($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_rect_table.2949
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	slli	$6, $5, 2
	lw	$7, 12($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38782
eq_else.38781:
	addi	$30, $0, 2
	bne	$11, $30, eq_else.38783
	sw	$9, 12($3)
	sw	$7, 16($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_surface_table.2952
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	slli	$6, $5, 2
	lw	$7, 12($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38784
eq_else.38783:
	sw	$9, 12($3)
	sw	$7, 16($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	setup_second_table.2955
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$5, 16($3)
	slli	$6, $5, 2
	lw	$7, 12($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38784:
eq_cont.38782:
	addi	$5, $5, -1
	lw	$2, 8($3)
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -24
	lw	$31, 20($3)
ge_cont.38780:
	lw	$2, 4($3)
	lw	$5, 472($2)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	mov	$2, $5
	mov	$5, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -24
	lw	$31, 20($3)
	lw	$2, 4($3)
	lw	$5, 468($2)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	bge	$6, $0, ge_else.38785
	j	ge_cont.38786
ge_else.38785:
	slli	$7, $6, 2
	addi	$7, $7, 3004
	lw	$7, 0($7)
	lw	$8, 4($5)
	lw	$9, 0($5)
	lw	$10, 4($7)
	sw	$5, 20($3)
	addi	$30, $0, 1
	bne	$10, $30, eq_else.38787
	sw	$8, 24($3)
	sw	$6, 28($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_rect_table.2949
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	slli	$6, $5, 2
	lw	$7, 24($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38788
eq_else.38787:
	addi	$30, $0, 2
	bne	$10, $30, eq_else.38789
	sw	$8, 24($3)
	sw	$6, 28($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_surface_table.2952
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	slli	$6, $5, 2
	lw	$7, 24($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38790
eq_else.38789:
	sw	$8, 24($3)
	sw	$6, 28($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	setup_second_table.2955
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 28($3)
	slli	$6, $5, 2
	lw	$7, 24($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38790:
eq_cont.38788:
	addi	$5, $5, -1
	lw	$2, 20($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -40
	lw	$31, 36($3)
ge_cont.38786:
	addi	$5, $0, 116
	lw	$2, 4($3)
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	init_dirvec_constants.3176
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$2, 0($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38791
	jr $31
ge.38791:
	slli	$5, $2, 2
	addi	$5, $5, 3840
	lw	$5, 0($5)
	lw	$6, 476($5)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 32($3)
	sw	$5, 36($3)
	mov	$5, $7
	mov	$2, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -48
	lw	$31, 44($3)
	lw	$2, 36($3)
	lw	$5, 472($2)
	addi	$6, $0, 3000
	lw	$6, 0($6)
	addi	$6, $6, -1
	bge	$6, $0, ge_else.38793
	j	ge_cont.38794
ge_else.38793:
	slli	$7, $6, 2
	addi	$7, $7, 3004
	lw	$7, 0($7)
	lw	$8, 4($5)
	lw	$9, 0($5)
	lw	$10, 4($7)
	sw	$5, 40($3)
	addi	$30, $0, 1
	bne	$10, $30, eq_else.38795
	sw	$8, 44($3)
	sw	$6, 48($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	setup_rect_table.2949
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	slli	$6, $5, 2
	lw	$7, 44($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38796
eq_else.38795:
	addi	$30, $0, 2
	bne	$10, $30, eq_else.38797
	sw	$8, 44($3)
	sw	$6, 48($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	setup_surface_table.2952
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	slli	$6, $5, 2
	lw	$7, 44($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38798
eq_else.38797:
	sw	$8, 44($3)
	sw	$6, 48($3)
	mov	$5, $7
	mov	$2, $9
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	setup_second_table.2955
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	slli	$6, $5, 2
	lw	$7, 44($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38798:
eq_cont.38796:
	addi	$5, $5, -1
	lw	$2, 40($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -56
	lw	$31, 52($3)
ge_cont.38794:
	addi	$5, $0, 117
	lw	$2, 36($3)
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	init_dirvec_constants.3176
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$2, 32($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38799
	jr $31
ge.38799:
	slli	$5, $2, 2
	addi	$5, $5, 3840
	lw	$5, 0($5)
	lw	$6, 476($5)
	addi	$7, $0, 3000
	lw	$7, 0($7)
	addi	$7, $7, -1
	sw	$2, 52($3)
	sw	$5, 56($3)
	bge	$7, $0, ge_else.38801
	j	ge_cont.38802
ge_else.38801:
	slli	$8, $7, 2
	addi	$8, $8, 3004
	lw	$8, 0($8)
	lw	$9, 4($6)
	lw	$10, 0($6)
	lw	$11, 4($8)
	sw	$6, 60($3)
	addi	$30, $0, 1
	bne	$11, $30, eq_else.38803
	sw	$9, 64($3)
	sw	$7, 68($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	setup_rect_table.2949
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	slli	$6, $5, 2
	lw	$7, 64($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38804
eq_else.38803:
	addi	$30, $0, 2
	bne	$11, $30, eq_else.38805
	sw	$9, 64($3)
	sw	$7, 68($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	setup_surface_table.2952
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	slli	$6, $5, 2
	lw	$7, 64($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38806
eq_else.38805:
	sw	$9, 64($3)
	sw	$7, 68($3)
	mov	$5, $8
	mov	$2, $10
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	setup_second_table.2955
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$5, 68($3)
	slli	$6, $5, 2
	lw	$7, 64($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38806:
eq_cont.38804:
	addi	$5, $5, -1
	lw	$2, 60($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -80
	lw	$31, 76($3)
ge_cont.38802:
	addi	$5, $0, 118
	lw	$2, 56($3)
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	init_dirvec_constants.3176
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$2, 52($3)
	addi	$2, $2, -1
	bge	$2, $0, ge.38807
	jr $31
ge.38807:
	slli	$5, $2, 2
	addi	$5, $5, 3840
	lw	$5, 0($5)
	addi	$6, $0, 119
	sw	$2, 72($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	init_dirvec_constants.3176
	addi	$3, $3, -80
	lw	$31, 76($3)
	lw	$2, 72($3)
	addi	$2, $2, -1
	j	init_vecset_constants.3179
setup_reflections.3196:
	bge	$2, $0, ge.38809
	jr $31
ge.38809:
	slli	$5, $2, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	lw	$6, 8($5)
	addi	$30, $0, 2
	bne	$6, $30, eq_else.38811
	lw	$6, 28($5)
	lf	$f0, 0($6)
	lf	$f1, 4920($0) # 1.000000
	fsub	$f0, $f1, $f0
	lf	$f1, 4888($0) # 0.000000
	sltf	$30, $f1, $f0
	bne	$30, $0, eq_else.38812
	addi	$6, $0, 0
	j	eq_cont.38813
eq_else.38812:
	addi	$6, $0, 1
eq_cont.38813:
	bne	$6, $0, eq_else.38814
	addi	$6, $0, 0
	j	eq_cont.38815
eq_else.38814:
	addi	$6, $0, 1
eq_cont.38815:
	bne	$6, $0, eq_else.38816
	jr $31
eq_else.38816:
	lw	$6, 4($5)
	addi	$30, $0, 1
	bne	$6, $30, eq_else.38818
	addi	$6, $0, 4
	mul	$2, $2, $6
	addi	$6, $0, 4588
	lw	$6, 0($6)
	lf	$f0, 4920($0) # 1.000000
	lw	$5, 28($5)
	lf	$f1, 0($5)
	fsub	$f0, $f0, $f1
	addi	$5, $0, 3292
	lf	$f1, 0($5)
	fneg	$f1, $f1
	addi	$5, $0, 3300
	lf	$f2, 0($5)
	fneg	$f2, $f2
	addi	$5, $0, 3308
	lf	$f3, 0($5)
	fneg	$f3, $f3
	addi	$5, $2, 1
	addi	$7, $0, 3292
	lf	$f4, 0($7)
	addi	$7, $0, 3
	lf	$f5, 4888($0) # 0.000000
	sf	$f1, 0($3)
	sw	$2, 8($3)
	sw	$6, 12($3)
	sw	$5, 16($3)
	sf	$f0, 24($3)
	sf	$f3, 32($3)
	sf	$f2, 40($3)
	sf	$f4, 48($3)
	mov	$2, $7
	movf	$f0, $f5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 56($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 56($3)
	sw	$2, 0($5)
	lf	$f0, 48($3)
	sf	$f0, 0($2)
	lf	$f0, 40($3)
	sf	$f0, 8($2)
	lf	$f1, 32($3)
	sf	$f1, 16($2)
	addi	$2, $0, 3000
	lw	$2, 0($2)
	addi	$2, $2, -1
	sw	$5, 60($3)
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -72
	lw	$31, 68($3)
	mov	$2, $4
	addi	$4, $4, 16
	lf	$f0, 24($3)
	sf	$f0, 8($2)
	lw	$5, 60($3)
	sw	$5, 4($2)
	lw	$5, 16($3)
	sw	$5, 0($2)
	lw	$5, 12($3)
	slli	$6, $5, 2
	addi	$6, $6, 3868
	sw	$2, 0($6)
	addi	$2, $5, 1
	lw	$6, 8($3)
	addi	$7, $6, 2
	addi	$8, $0, 3300
	lf	$f1, 0($8)
	addi	$8, $0, 3
	lf	$f2, 4888($0) # 0.000000
	sw	$2, 64($3)
	sw	$7, 68($3)
	sf	$f1, 72($3)
	mov	$2, $8
	movf	$f0, $f2
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 80($3)
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 80($3)
	sw	$2, 0($5)
	lf	$f0, 0($3)
	sf	$f0, 0($2)
	lf	$f1, 72($3)
	sf	$f1, 8($2)
	lf	$f1, 32($3)
	sf	$f1, 16($2)
	addi	$2, $0, 3000
	lw	$2, 0($2)
	addi	$2, $2, -1
	sw	$5, 84($3)
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -96
	lw	$31, 92($3)
	mov	$2, $4
	addi	$4, $4, 16
	lf	$f0, 24($3)
	sf	$f0, 8($2)
	lw	$5, 84($3)
	sw	$5, 4($2)
	lw	$5, 68($3)
	sw	$5, 0($2)
	lw	$5, 64($3)
	slli	$5, $5, 2
	addi	$5, $5, 3868
	sw	$2, 0($5)
	lw	$2, 12($3)
	addi	$5, $2, 2
	lw	$6, 8($3)
	addi	$6, $6, 3
	addi	$7, $0, 3308
	lf	$f1, 0($7)
	addi	$7, $0, 3
	lf	$f2, 4888($0) # 0.000000
	sw	$5, 88($3)
	sw	$6, 92($3)
	sf	$f1, 96($3)
	mov	$2, $7
	movf	$f0, $f2
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 104($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 104($3)
	sw	$2, 0($5)
	lf	$f0, 0($3)
	sf	$f0, 0($2)
	lf	$f0, 40($3)
	sf	$f0, 8($2)
	lf	$f0, 96($3)
	sf	$f0, 16($2)
	addi	$2, $0, 3000
	lw	$2, 0($2)
	addi	$2, $2, -1
	sw	$5, 108($3)
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -120
	lw	$31, 116($3)
	mov	$2, $4
	addi	$4, $4, 16
	lf	$f0, 24($3)
	sf	$f0, 8($2)
	lw	$5, 108($3)
	sw	$5, 4($2)
	lw	$5, 92($3)
	sw	$5, 0($2)
	lw	$5, 88($3)
	slli	$5, $5, 2
	addi	$5, $5, 3868
	sw	$2, 0($5)
	lw	$2, 12($3)
	addi	$2, $2, 3
	addi	$5, $0, 4588
	sw	$2, 0($5)
	jr $31
eq_else.38818:
	addi	$30, $0, 2
	bne	$6, $30, eq_else.38821
	addi	$6, $0, 4
	mul	$2, $2, $6
	addi	$2, $2, 1
	addi	$6, $0, 4588
	lw	$6, 0($6)
	lf	$f0, 4920($0) # 1.000000
	lw	$7, 28($5)
	lf	$f1, 0($7)
	fsub	$f0, $f0, $f1
	lw	$7, 16($5)
	addi	$8, $0, 3292
	lf	$f1, 0($8)
	lf	$f2, 0($7)
	fmul	$f1, $f1, $f2
	addi	$8, $0, 3300
	lf	$f2, 0($8)
	lf	$f3, 8($7)
	fmul	$f2, $f2, $f3
	fadd	$f1, $f1, $f2
	addi	$8, $0, 3308
	lf	$f2, 0($8)
	lf	$f3, 16($7)
	fmul	$f2, $f2, $f3
	fadd	$f1, $f1, $f2
	lf	$f2, 4776($0) # 2.000000
	lw	$7, 16($5)
	lf	$f3, 0($7)
	fmul	$f2, $f2, $f3
	fmul	$f2, $f2, $f1
	addi	$7, $0, 3292
	lf	$f3, 0($7)
	fsub	$f2, $f2, $f3
	lf	$f3, 4776($0) # 2.000000
	lw	$7, 16($5)
	lf	$f4, 8($7)
	fmul	$f3, $f3, $f4
	fmul	$f3, $f3, $f1
	addi	$7, $0, 3300
	lf	$f4, 0($7)
	fsub	$f3, $f3, $f4
	lf	$f4, 4776($0) # 2.000000
	lw	$5, 16($5)
	lf	$f5, 16($5)
	fmul	$f4, $f4, $f5
	fmul	$f1, $f4, $f1
	addi	$5, $0, 3308
	lf	$f4, 0($5)
	fsub	$f1, $f1, $f4
	addi	$5, $0, 3
	lf	$f4, 4888($0) # 0.000000
	sw	$6, 112($3)
	sw	$2, 116($3)
	sf	$f0, 120($3)
	sf	$f1, 128($3)
	sf	$f3, 136($3)
	sf	$f2, 144($3)
	mov	$2, $5
	movf	$f0, $f4
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	mov	$5, $2
	addi	$2, $0, 3000
	lw	$2, 0($2)
	sw	$5, 152($3)
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 152($3)
	sw	$2, 0($5)
	lf	$f0, 144($3)
	sf	$f0, 0($2)
	lf	$f0, 136($3)
	sf	$f0, 8($2)
	lf	$f0, 128($3)
	sf	$f0, 16($2)
	addi	$2, $0, 3000
	lw	$2, 0($2)
	addi	$2, $2, -1
	sw	$5, 156($3)
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 164($3)
	addi	$3, $3, 168
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -168
	lw	$31, 164($3)
	mov	$2, $4
	addi	$4, $4, 16
	lf	$f0, 120($3)
	sf	$f0, 8($2)
	lw	$5, 156($3)
	sw	$5, 4($2)
	lw	$5, 116($3)
	sw	$5, 0($2)
	lw	$5, 112($3)
	slli	$6, $5, 2
	addi	$6, $6, 3868
	sw	$2, 0($6)
	addi	$2, $5, 1
	addi	$5, $0, 4588
	sw	$2, 0($5)
	jr $31
eq_else.38821:
	jr $31
eq_else.38811:
	jr $31
#	main program starts
Main:
	addi	$5, $0, 1
	addi	$6, $0, 0
	addi	$2, $0, 3000
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$2, $0, 0
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 60
	addi	$6, $0, 0
	addi	$7, $0, 0
	addi	$8, $0, 0
	addi	$9, $0, 0
	addi	$10, $0, 0
	mov	$11, $4
	addi	$4, $4, 48
	sw	$2, 40($11)
	sw	$2, 36($11)
	sw	$2, 32($11)
	sw	$2, 28($11)
	sw	$10, 24($11)
	sw	$2, 20($11)
	sw	$2, 16($11)
	sw	$9, 12($11)
	sw	$8, 8($11)
	sw	$7, 4($11)
	sw	$6, 0($11)
	mov	$6, $11
	addi	$2, $0, 3004
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3244
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3268
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3292
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$5, $0, 1
	lf	$f0, 4704($0) # 255.000000
	addi	$2, $0, 3316
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_float_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$2, $0, 50
	addi	$5, $0, 1
	addi	$6, $0, -1
	sw	$2, 0($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	mov	$6, $2
	addi	$2, $0, 3324
	lw	$5, 0($3)
	sw	$31, 4($3)
	addi	$3, $3, 8
	jal	min_caml_create_global_array
	addi	$3, $3, -8
	lw	$31, 4($3)
	addi	$2, $0, 1
	addi	$5, $0, 1
	lw	$6, 3324($0)
	sw	$2, 4($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$6, $2
	addi	$2, $0, 3524
	lw	$5, 4($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 1
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3528
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 1
	addi	$6, $0, 0
	addi	$2, $0, 3536
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 1
	lf	$f0, 4656($0) # 1000000000.000000
	addi	$2, $0, 3540
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3548
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 1
	addi	$6, $0, 0
	addi	$2, $0, 3572
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3576
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3600
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3624
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3648
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 2
	addi	$6, $0, 0
	addi	$2, $0, 3672
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 2
	addi	$6, $0, 0
	addi	$2, $0, 3680
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 1
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3688
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3696
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3720
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3744
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3768
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3792
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	addi	$2, $0, 3816
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$2, $0, 0
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$5, $2
	addi	$2, $0, 0
	sw	$5, 8($3)
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 0
	mov	$6, $4
	addi	$4, $4, 8
	sw	$2, 4($6)
	lw	$2, 8($3)
	sw	$2, 0($6)
	mov	$2, $6
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	mov	$6, $2
	addi	$5, $0, 5
	addi	$2, $0, 3840
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_global_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$2, $0, 0
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 12($3)
	addi	$3, $3, 16
	jal	min_caml_create_float_array
	addi	$3, $3, -16
	lw	$31, 12($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 12($3)
	mov	$2, $5
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	addi	$5, $0, 60
	lw	$6, 12($3)
	sw	$2, 16($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	sw	$2, 3864($0)
	lw	$2, 16($3)
	sw	$2, 3860($0)
	addi	$2, $0, 0
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 20($3)
	addi	$3, $3, 24
	jal	min_caml_create_float_array
	addi	$3, $3, -24
	lw	$31, 20($3)
	mov	$5, $2
	addi	$2, $0, 0
	sw	$5, 20($3)
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 20($3)
	sw	$2, 0($5)
	mov	$2, $5
	addi	$5, $0, 180
	addi	$6, $0, 0
	lf	$f0, 4888($0) # 0.000000
	mov	$7, $4
	addi	$4, $4, 16
	sf	$f0, 8($7)
	sw	$2, 4($7)
	sw	$6, 0($7)
	mov	$6, $7
	addi	$2, $0, 3868
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_global_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 1
	addi	$6, $0, 0
	addi	$2, $0, 4588
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_global_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$2, $0, 128
	addi	$5, $0, 128
	sw	$2, 3672($0)
	sw	$5, 3676($0)
	addi	$5, $0, 64
	sw	$5, 3680($0)
	addi	$5, $0, 64
	sw	$5, 3684($0)
	lf	$f0, 4592($0) # 128.000000
	itof	$f1, $2
	fdiv	$f0, $f0, $f1
	sf	$f0, 3688($0)
	lw	$2, 3672($0)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 24($3)
	mov	$2, $5
	sw	$31, 28($3)
	addi	$3, $3, 32
	jal	min_caml_create_float_array
	addi	$3, $3, -32
	lw	$31, 28($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 28($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 32($3)
	mov	$2, $5
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 32($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 32($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 32($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_float_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	lw	$5, 32($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 36($3)
	addi	$3, $3, 40
	jal	min_caml_create_array
	addi	$3, $3, -40
	lw	$31, 36($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 36($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 40($3)
	mov	$2, $5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_float_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 44($3)
	addi	$3, $3, 48
	jal	min_caml_create_array
	addi	$3, $3, -48
	lw	$31, 44($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 44($3)
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 44($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 44($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 44($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 44($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 48($3)
	mov	$2, $5
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_float_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	lw	$5, 48($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 52($3)
	addi	$3, $3, 56
	jal	min_caml_create_array
	addi	$3, $3, -56
	lw	$31, 52($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 52($3)
	mov	$2, $5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 56($3)
	mov	$2, $5
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 56($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 56($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 56($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_float_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 56($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 52($3)
	sw	$5, 24($2)
	lw	$5, 48($3)
	sw	$5, 20($2)
	lw	$5, 44($3)
	sw	$5, 16($2)
	lw	$5, 40($3)
	sw	$5, 12($2)
	lw	$5, 36($3)
	sw	$5, 8($2)
	lw	$5, 32($3)
	sw	$5, 4($2)
	lw	$5, 28($3)
	sw	$5, 0($2)
	mov	$5, $2
	lw	$2, 24($3)
	sw	$31, 60($3)
	addi	$3, $3, 64
	jal	min_caml_create_array
	addi	$3, $3, -64
	lw	$31, 60($3)
	lw	$5, 3672($0)
	addi	$5, $5, -2
	bge	$5, $0, ge_else.38825
	j	ge_cont.38826
ge_else.38825:
	sw	$2, 60($3)
	sw	$5, 64($3)
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	create_pixel.3140
	addi	$3, $3, -72
	lw	$31, 68($3)
	lw	$5, 64($3)
	slli	$6, $5, 2
	lw	$7, 60($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$5, $5, -1
	mov	$2, $7
	sw	$31, 68($3)
	addi	$3, $3, 72
	jal	init_line_elements.3142
	addi	$3, $3, -72
	lw	$31, 68($3)
ge_cont.38826:
	lw	$5, 3672($0)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 68($3)
	sw	$5, 72($3)
	mov	$2, $6
	sw	$31, 76($3)
	addi	$3, $3, 80
	jal	min_caml_create_float_array
	addi	$3, $3, -80
	lw	$31, 76($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 76($3)
	mov	$2, $5
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 80($3)
	mov	$2, $5
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_float_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	lw	$5, 80($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 84($3)
	addi	$3, $3, 88
	jal	min_caml_create_array
	addi	$3, $3, -88
	lw	$31, 84($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 84($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 88($3)
	mov	$2, $5
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_float_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 92($3)
	addi	$3, $3, 96
	jal	min_caml_create_array
	addi	$3, $3, -96
	lw	$31, 92($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 92($3)
	mov	$2, $5
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 92($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 92($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 92($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 92($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 96($3)
	mov	$2, $5
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 96($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 96($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 96($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_float_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	lw	$5, 96($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 100($3)
	addi	$3, $3, 104
	jal	min_caml_create_array
	addi	$3, $3, -104
	lw	$31, 100($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 100($3)
	mov	$2, $5
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 104($3)
	mov	$2, $5
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 104($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 104($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 104($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_float_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 104($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 100($3)
	sw	$5, 24($2)
	lw	$5, 96($3)
	sw	$5, 20($2)
	lw	$5, 92($3)
	sw	$5, 16($2)
	lw	$5, 88($3)
	sw	$5, 12($2)
	lw	$5, 84($3)
	sw	$5, 8($2)
	lw	$5, 80($3)
	sw	$5, 4($2)
	lw	$5, 76($3)
	sw	$5, 0($2)
	mov	$5, $2
	lw	$2, 72($3)
	sw	$31, 108($3)
	addi	$3, $3, 112
	jal	min_caml_create_array
	addi	$3, $3, -112
	lw	$31, 108($3)
	lw	$5, 3672($0)
	addi	$5, $5, -2
	bge	$5, $0, ge_else.38827
	j	ge_cont.38828
ge_else.38827:
	sw	$2, 108($3)
	sw	$5, 112($3)
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	create_pixel.3140
	addi	$3, $3, -120
	lw	$31, 116($3)
	lw	$5, 112($3)
	slli	$6, $5, 2
	lw	$7, 108($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$5, $5, -1
	mov	$2, $7
	sw	$31, 116($3)
	addi	$3, $3, 120
	jal	init_line_elements.3142
	addi	$3, $3, -120
	lw	$31, 116($3)
ge_cont.38828:
	lw	$5, 3672($0)
	addi	$6, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 116($3)
	sw	$5, 120($3)
	mov	$2, $6
	sw	$31, 124($3)
	addi	$3, $3, 128
	jal	min_caml_create_float_array
	addi	$3, $3, -128
	lw	$31, 124($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 124($3)
	mov	$2, $5
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 128($3)
	mov	$2, $5
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 128($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 128($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 128($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_float_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	lw	$5, 128($3)
	sw	$2, 16($5)
	addi	$2, $0, 5
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 132($3)
	addi	$3, $3, 136
	jal	min_caml_create_array
	addi	$3, $3, -136
	lw	$31, 132($3)
	addi	$5, $0, 5
	addi	$6, $0, 0
	sw	$2, 132($3)
	mov	$2, $5
	mov	$5, $6
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 136($3)
	mov	$2, $5
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_float_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 140($3)
	addi	$3, $3, 144
	jal	min_caml_create_array
	addi	$3, $3, -144
	lw	$31, 140($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 140($3)
	mov	$2, $5
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 140($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 140($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 140($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 140($3)
	sw	$2, 16($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 144($3)
	mov	$2, $5
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 144($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 144($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 144($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_float_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	lw	$5, 144($3)
	sw	$2, 16($5)
	addi	$2, $0, 1
	addi	$6, $0, 0
	mov	$5, $6
	sw	$31, 148($3)
	addi	$3, $3, 152
	jal	min_caml_create_array
	addi	$3, $3, -152
	lw	$31, 148($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 148($3)
	mov	$2, $5
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	mov	$5, $2
	addi	$2, $0, 5
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 152($3)
	mov	$2, $5
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 152($3)
	sw	$2, 4($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 152($3)
	sw	$2, 8($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 152($3)
	sw	$2, 12($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_float_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 152($3)
	sw	$2, 16($5)
	mov	$2, $4
	addi	$4, $4, 32
	sw	$5, 28($2)
	lw	$5, 148($3)
	sw	$5, 24($2)
	lw	$5, 144($3)
	sw	$5, 20($2)
	lw	$5, 140($3)
	sw	$5, 16($2)
	lw	$5, 136($3)
	sw	$5, 12($2)
	lw	$5, 132($3)
	sw	$5, 8($2)
	lw	$5, 128($3)
	sw	$5, 4($2)
	lw	$5, 124($3)
	sw	$5, 0($2)
	mov	$5, $2
	lw	$2, 120($3)
	sw	$31, 156($3)
	addi	$3, $3, 160
	jal	min_caml_create_array
	addi	$3, $3, -160
	lw	$31, 156($3)
	lw	$5, 3672($0)
	addi	$5, $5, -2
	bge	$5, $0, ge_else.38829
	j	ge_cont.38830
ge_else.38829:
	sw	$2, 156($3)
	sw	$5, 160($3)
	sw	$31, 164($3)
	addi	$3, $3, 168
	jal	create_pixel.3140
	addi	$3, $3, -168
	lw	$31, 164($3)
	lw	$5, 160($3)
	slli	$6, $5, 2
	lw	$7, 156($3)
	add $30, $7, $6
	sw	$2, 0($30)
	addi	$5, $5, -1
	mov	$2, $7
	sw	$31, 164($3)
	addi	$3, $3, 168
	jal	init_line_elements.3142
	addi	$3, $3, -168
	lw	$31, 164($3)
ge_cont.38830:
	sw	$2, 164($3)
	sw	$31, 172($3)
	addi	$3, $3, 176
	jal	read_parameter.2863
	addi	$3, $3, -176
	lw	$31, 172($3)
	addi	$2, $0, 80
	outb	$2
	addi	$2, $0, 54
	outb	$2
	addi	$2, $0, 10
	outb	$2
	lw	$2, 3672($0)
	bne	$2, $0, eq_else.38831
	addi	$2, $0, 48
	outb	$2
	j	eq_cont.38832
eq_else.38831:
	bne	$2, $0, eq_else.38833
	j	eq_cont.38834
eq_else.38833:
	div10	$5, $2
	addi	$6, $0, 10
	mul	$6, $5, $6
	sub	$2, $2, $6
	bne	$5, $0, eq_else.38835
	j	eq_cont.38836
eq_else.38835:
	div10	$6, $5
	addi	$7, $0, 10
	mul	$7, $6, $7
	sub	$5, $5, $7
	bne	$6, $0, eq_else.38837
	j	eq_cont.38838
eq_else.38837:
	div10	$7, $6
	addi	$8, $0, 10
	mul	$7, $7, $8
	sub	$6, $6, $7
	addi	$6, $6, 48
	outb	$6
eq_cont.38838:
	addi	$5, $5, 48
	outb	$5
eq_cont.38836:
	addi	$2, $2, 48
	outb	$2
eq_cont.38834:
eq_cont.38832:
	addi	$2, $0, 32
	outb	$2
	lw	$2, 3676($0)
	bne	$2, $0, eq_else.38839
	addi	$2, $0, 48
	outb	$2
	j	eq_cont.38840
eq_else.38839:
	bne	$2, $0, eq_else.38841
	j	eq_cont.38842
eq_else.38841:
	div10	$5, $2
	addi	$6, $0, 10
	mul	$6, $5, $6
	sub	$2, $2, $6
	bne	$5, $0, eq_else.38843
	j	eq_cont.38844
eq_else.38843:
	div10	$6, $5
	addi	$7, $0, 10
	mul	$7, $6, $7
	sub	$5, $5, $7
	bne	$6, $0, eq_else.38845
	j	eq_cont.38846
eq_else.38845:
	div10	$7, $6
	addi	$8, $0, 10
	mul	$7, $7, $8
	sub	$6, $6, $7
	addi	$6, $6, 48
	outb	$6
eq_cont.38846:
	addi	$5, $5, 48
	outb	$5
eq_cont.38844:
	addi	$2, $2, 48
	outb	$2
eq_cont.38842:
eq_cont.38840:
	addi	$2, $0, 32
	outb	$2
	addi	$2, $0, 50
	outb	$2
	addi	$2, $0, 53
	outb	$2
	addi	$2, $0, 53
	outb	$2
	addi	$2, $0, 10
	outb	$2
	addi	$2, $0, 120
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 168($3)
	mov	$2, $5
	sw	$31, 172($3)
	addi	$3, $3, 176
	jal	min_caml_create_float_array
	addi	$3, $3, -176
	lw	$31, 172($3)
	mov	$5, $2
	lw	$2, 3000($0)
	sw	$5, 172($3)
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	min_caml_create_array
	addi	$3, $3, -184
	lw	$31, 180($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 172($3)
	sw	$2, 0($5)
	lw	$2, 168($3)
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	min_caml_create_array
	addi	$3, $3, -184
	lw	$31, 180($3)
	sw	$2, 3856($0)
	lw	$2, 3856($0)
	addi	$5, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$2, 176($3)
	mov	$2, $5
	sw	$31, 180($3)
	addi	$3, $3, 184
	jal	min_caml_create_float_array
	addi	$3, $3, -184
	lw	$31, 180($3)
	mov	$5, $2
	lw	$2, 3000($0)
	sw	$5, 180($3)
	sw	$31, 188($3)
	addi	$3, $3, 192
	jal	min_caml_create_array
	addi	$3, $3, -192
	lw	$31, 188($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 180($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 176($3)
	sw	$2, 472($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 188($3)
	addi	$3, $3, 192
	jal	min_caml_create_float_array
	addi	$3, $3, -192
	lw	$31, 188($3)
	mov	$5, $2
	lw	$2, 3000($0)
	sw	$5, 184($3)
	sw	$31, 188($3)
	addi	$3, $3, 192
	jal	min_caml_create_array
	addi	$3, $3, -192
	lw	$31, 188($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 184($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 176($3)
	sw	$2, 468($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 188($3)
	addi	$3, $3, 192
	jal	min_caml_create_float_array
	addi	$3, $3, -192
	lw	$31, 188($3)
	mov	$5, $2
	lw	$2, 3000($0)
	sw	$5, 188($3)
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	min_caml_create_array
	addi	$3, $3, -200
	lw	$31, 196($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 188($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 176($3)
	sw	$2, 464($5)
	addi	$2, $0, 3
	lf	$f0, 4888($0) # 0.000000
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	min_caml_create_float_array
	addi	$3, $3, -200
	lw	$31, 196($3)
	mov	$5, $2
	lw	$2, 3000($0)
	sw	$5, 192($3)
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	min_caml_create_array
	addi	$3, $3, -200
	lw	$31, 196($3)
	mov	$5, $4
	addi	$4, $4, 8
	sw	$2, 4($5)
	lw	$2, 192($3)
	sw	$2, 0($5)
	mov	$2, $5
	lw	$5, 176($3)
	sw	$2, 460($5)
	addi	$2, $0, 114
	mov	$27, $5
	mov	$5, $2
	mov	$2, $27
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	create_dirvec_elements.3171
	addi	$3, $3, -200
	lw	$31, 196($3)
	addi	$2, $0, 3
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	create_dirvecs.3174
	addi	$3, $3, -200
	lw	$31, 196($3)
	addi	$2, $0, 9
	addi	$5, $0, 0
	addi	$6, $0, 0
	itof	$f0, $2
	lf	$f1, 4856($0) # 0.200000
	fmul	$f0, $f0, $f1
	lf	$f1, 4600($0) # 0.900000
	fsub	$f0, $f0, $f1
	addi	$2, $0, 4
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	calc_dirvecs.3160
	addi	$3, $3, -200
	lw	$31, 196($3)
	addi	$2, $0, 8
	addi	$5, $0, 2
	addi	$6, $0, 4
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	calc_dirvec_rows.3165
	addi	$3, $3, -200
	lw	$31, 196($3)
	lw	$2, 3856($0)
	addi	$5, $0, 119
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	init_dirvec_constants.3176
	addi	$3, $3, -200
	lw	$31, 196($3)
	addi	$2, $0, 3
	sw	$31, 196($3)
	addi	$3, $3, 200
	jal	init_vecset_constants.3179
	addi	$3, $3, -200
	lw	$31, 196($3)
	lw	$2, 3860($0)
	lf	$f0, 3292($0)
	sf	$f0, 0($2)
	lf	$f0, 3300($0)
	sf	$f0, 8($2)
	lf	$f0, 3308($0)
	sf	$f0, 16($2)
	lw	$2, 3000($0)
	addi	$2, $2, -1
	bge	$2, $0, ge_else.38847
	j	ge_cont.38848
ge_else.38847:
	slli	$5, $2, 2
	addi	$5, $5, 3004
	lw	$5, 0($5)
	addi	$6, $0, 3860
	lw	$6, 4($6)
	addi	$7, $0, 3860
	lw	$7, 0($7)
	lw	$8, 4($5)
	addi	$30, $0, 1
	bne	$8, $30, eq_else.38849
	sw	$6, 196($3)
	sw	$2, 200($3)
	mov	$2, $7
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	setup_rect_table.2949
	addi	$3, $3, -208
	lw	$31, 204($3)
	lw	$5, 200($3)
	slli	$6, $5, 2
	lw	$7, 196($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38850
eq_else.38849:
	addi	$30, $0, 2
	bne	$8, $30, eq_else.38851
	sw	$6, 196($3)
	sw	$2, 200($3)
	mov	$2, $7
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	setup_surface_table.2952
	addi	$3, $3, -208
	lw	$31, 204($3)
	lw	$5, 200($3)
	slli	$6, $5, 2
	lw	$7, 196($3)
	add $30, $7, $6
	sw	$2, 0($30)
	j	eq_cont.38852
eq_else.38851:
	sw	$6, 196($3)
	sw	$2, 200($3)
	mov	$2, $7
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	setup_second_table.2955
	addi	$3, $3, -208
	lw	$31, 204($3)
	lw	$5, 200($3)
	slli	$6, $5, 2
	lw	$7, 196($3)
	add $30, $7, $6
	sw	$2, 0($30)
eq_cont.38852:
eq_cont.38850:
	addi	$5, $5, -1
	addi	$2, $0, 3860
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	iter_setup_dirvec_constants.2958
	addi	$3, $3, -208
	lw	$31, 204($3)
ge_cont.38848:
	lw	$2, 3000($0)
	addi	$2, $2, -1
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	setup_reflections.3196
	addi	$3, $3, -208
	lw	$31, 204($3)
	addi	$5, $0, 0
	addi	$6, $0, 0
	lw	$2, 116($3)
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	pretrace_line.3122
	addi	$3, $3, -208
	lw	$31, 204($3)
	addi	$2, $0, 0
	addi	$8, $0, 2
	lw	$5, 68($3)
	lw	$6, 116($3)
	lw	$7, 164($3)
	sw	$31, 204($3)
	addi	$3, $3, 208
	jal	scan_line.3132
	addi	$3, $3, -208
	lw	$31, 204($3)
	j Exit
#	main program ends
# floor

#  min_caml_floor:
#    sf $f0, 0($4)
#    lw $2, 0($4)
#    ori $10, $0, 32640 # sisubu mask $10
#    slli $10, $10, 16
#    ori $11, $0, 127 # kasubu mask $11
#    slli $11, $11, 16
#    ori $11, $11, 65535
#    and $9, $2, $10 # formal sisubu $9
#    srli $5, $9, 23
#    addi $5, $5, -127 # actual sisubu A $5
#    and $12, $2, $11 # kasubu $12
#    srli $8, $2, 31 # sgn $8
#    slt $6, $5, $0
#    bne $6, $0, floor_tiny # A < 0
#    beq $5, $0, floor_exp_zero # A = 0
#    addi $7, $0, 23
#    slt $6, $5, $7
#    bne $6, $0, floor_main # A < 23
#  
#  floor_end:
#    jr $31
#  
#  floor_tiny:
#    bne $8, $0, floor_tiny_neg
#    sw $0, 0($4)  # return 0.0
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_tiny_neg:
#    ori $13, $0, 49024
#    slli $13, $13, 16 # return -1.0
#    sw $13, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_exp_zero:
#    bne $8, $0, floor_exp_zero_neg
#    ori $13, $0, 16256  
#    slli $13, $13, 16 # return 1.0
#    sw $13, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_exp_zero_neg:
#    beq $12, $0, floor_exp_zero_neg_just
#    ori $13, $0, 49152   
#    slli $13, $13, 16 # return -2.0
#    sw $13, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_exp_zero_neg_just:
#    ori $13, $0, 49024  
#    slli $13, $13, 16 # return -1.0
#    sw $13, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_main:
#    addi $7, $0, 23
#    sub $6, $7, $5 # 23-A
#    srl $7, $12, $6
#    sll $7, $7, $6
#    bne $8, $0, floor_neg
#    or $10, $9, $7
#    sw $10, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_neg: # kai bit tattetara kiriage
#    slli $8, $8, 31
#    bne $12, $7, floor_ceil
#    or $10, $9, $7
#    or $10, $10, $8
#    sw $10, 0($4)
#    lf $f0, 0($4)
#    j floor_end
#  
#  floor_ceil:
#    ori $10, $0, 1
#    sll $10, $10, $6
#    add $7, $7, $10
#    or $10, $9, $7
#    or $10, $10, $8
#    sw $10, 0($4)
#    lf $f0, 0($4)
#    j floor_end 


# float_of_int (less than 8388608)


# 2nd instructions 

min_caml_sqrt:
  sqrt $f0, $f0
  jr $31

min_caml_float_of_int:
  itof $f0, $2
  jr $31

min_caml_int_of_float:
  ftoi $2, $f0
  jr $31

min_caml_floor:
  floor $f0, $f0
  jr $31





min_caml_float_of_int_kernel:
  ori $5, $0, 19200
  slli $5, $5, 16 # 8388608.0
  sw $5, 0($4)
  add $5, $5, $2
  sw $5, 4($4)
  lf $f0, 4($4)
  lf $f1, 0($4)
  fsub $f0, $f0, $f1
  jr $31

min_caml_int_of_float_kernel:
  ori $5, $0, 19200
  slli $5, $5, 16 # 8388608.0
  sw $5, 0($4)
  lf $f1, 0($4)
  fadd $f0, $f0, $f1
  sf $f0, 0($4)
  lw $2, 0($4)
  sub $2, $2, $5
  jr $31

#	create_array
min_caml_create_array:	
	mov	$6, $2
	mov	$2, $4
create_array_loop:
	bne	$0, $6, create_array_cont
	j	create_array_exit
create_array_exit:
	jr $31
create_array_cont:
	sw $5, 0($4)
	addi $6, $6, -1
	addi $4, $4, 4
	j	create_array_loop

#	create_float_array
min_caml_create_float_array:
	mov $5, $2
	mov	$2, $4
create_float_array_loop:
	bne	$5, $0, create_float_array_cont
	jr $31
create_float_array_cont:
	sf $f0, 0($4)
	addi $5, $5, -1
	addi $4, $4, 8
	j	create_float_array_loop
  
#	create_global_array
# $2 addr, $5 length, $6 element
min_caml_create_global_array:	
	mov	$7, $2
create_global_array_loop:
	bne	$0, $5, create_global_array_cont
	j	create_array_exit
create_global_array_exit:
	jr $31
create_global_array_cont:
	sw $6, 0($7)
	addi $5, $5, -1
	addi $7, $7, 4
	j	create_global_array_loop

#	create_global_float_array
# $2 addr, $5 length, $f0 element
min_caml_create_global_float_array:	
	mov	$7, $2
create_global_float_array_loop:
	bne	$0, $5, create_global_float_array_cont
	j	create_array_exit
create_global_float_array_exit:
	jr $31
create_global_float_array_cont:
	sf $f0, 0($7)
	addi $5, $5, -1
	addi $7, $7, 8
	j	create_global_float_array_loop

#read_int
min_caml_read_int:
  in $2
  jr $31 

#read_float
min_caml_read_float:
  inf $f0
  jr $31

min_caml_print_float:
  outf $f0
  jr $31

#print_char
min_caml_print_char:
  outb $2
  jr $31
Exit:
