...
and g' oc = function (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  | NonTail(_), Nop -> ()
  | NonTail(x), Li(i) when -32768 <= i && i < 32768 -> add (Addi(x,reg_zero,i))
  | NonTail(x), Li(i) ->
      (*
  ...
  | NonTail(x), SetL(Id.L(y)) -> assert false
  | NonTail(x), Mr(y) when x = y -> ()
  | NonTail(x), Mr(y) when x = "%r0" -> ()
  | NonTail(x), Mr(y) when y = "%r0" -> ()
  | NonTail(x), Mr(y) -> add (Mov(x,y)) (* Printf.fprintf oc "\tmov\t%s, %s\n" (reg x) (reg y) *)
  | NonTail(x), Neg(y) -> add (Sub(x,reg_zero,y))
(*
    Printf.fprintf oc "\tsub\t%s, %s, %s\n" (reg x) (reg reg_zero) (reg y)
*)
  | NonTail(x), Add(y, V(z)) -> add (Add(x,y,z))    (* Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Add(y, C(z)) -> add (Addi(x,y,z))   (* Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg x) (reg y) z      *) 
  | NonTail(x), Sub(y, V(z)) -> add (Sub(x,y,z))    (* Printf.fprintf oc "\tsub\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Sub(y, C(z)) -> add (Addi(x,y,(-z))) (* Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg x) (reg y) (-z)   *)
  | NonTail(x), Mul(y, V(z)) -> add (Mul(x,y,z))    (* Printf.fprintf oc "\tmul\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Mul(y, C(z)) -> add (Addi(reg_tmp,reg_zero,z)); add (Mul(x,y,reg_tmp))
(*
    Printf.fprintf oc "\taddi\t%s, %s, %d\n\tmul\t%s, %s, %s\n" (reg reg_tmp) (reg reg_zero) z (reg x) (reg y) (reg reg_tmp)
*)
  | NonTail(x), Div(y, V(z)) -> assert false (*Printf.fprintf oc "\tdiv\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
      (*
  ...
  | NonTail(x), Div(y, C(z))
    -> (* Printf.fprintf oc "\taddi\t%s, %s, %d\n\tdiv\t%s, %s, %s\n" (reg reg_tmp) (reg reg_zero) z (reg x) (reg y) (reg reg_tmp) *)
       (
         match z with 
         | 2  -> add (Srai(x,y,1)) (* Printf.fprintf oc "\tsrai\t%s, %s, 1\n" (reg x) (reg y) *)
         | 4  -> add (Srai(x,y,2)) (* Printf.fprintf oc "\tsrai\t%s, %s, 2\n" (reg x) (reg y) *)
         | 8  -> add (Srai(x,y,3)) (* Printf.fprintf oc "\tsrai\t%s, %s, 3\n" (reg x) (reg y) *)
         | 10 -> assert false (*  Printf.fprintf oc "\tdiv10\t%s, %s\n" (reg x) (reg y) *)
         | x -> assert false (* Printf.eprintf "found: %d, Division is supported by 2,4,8,and 10 only." x;
                 failwith "Div error" *)
       )
  | NonTail(x), Slw(y, V(z)) -> add (Sll(x,y,z))
      (* Printf.fprintf oc "\tsll\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Slw(y, C(z)) -> add (Slli(x,y,z)) (* Printf.fprintf oc "\tslli\t%s, %s, %d\n" (reg x) (reg y) z *)
  | NonTail(x), Xor(y, V(z)) -> add (Xor(x,y,z))  (* Printf.fprintf oc "\txor\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Xor(y, C(z)) -> add (Xori(x,y,z)) (* Printf.fprintf oc "\txori\t%s, %s, %d\n" (reg x) (reg y) (z) *)
  | NonTail(x), Lwz(y, V(z)) -> 
      (* add (Add(reg_tmp,y,z)); add (Lw(x,0,reg_tmp)) *)
      add (Addlw(x,y,z))
(*
...
*)
  | NonTail(x), Lwz(y, C(z)) -> add (Lw(x,z,y)) (* Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg x) z (reg y) *)
  | NonTail(_), Stw(x, y, V(z)) -> 
      (* oukyu shochi *)
      add (Add(reg_tmp,y,z)); add (Sw(x,0,reg_tmp))
(*
      Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg reg_tmp) (reg y) (reg z);
      Printf.fprintf oc "\tsw\t%s, 0(%s)\n" (reg x) (reg reg_tmp)
*)
  | NonTail(_), Stw(x, y, C(z)) -> add (Sw(x,z,y)) (* Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg x) z (reg y) *)
  | NonTail(x), FMr(y) when x = y -> ()
  | NonTail(x), FMr(y) -> add (Movf(x,y)) (* Printf.fprintf oc "\tmovf\t%s, %s\n" (reg x) (reg y) *)
  | NonTail(x), FNeg(y) -> add (Fneg(x,y)) (* Printf.fprintf oc "\tfneg\t%s, %s\n" (reg x) (reg y) *)
  | NonTail(x), FAdd(y, z) -> add (Fadd(x,y,z)) (* Printf.fprintf oc "\tfadd\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), FSub(y, z) -> add (Fsub(x,y,z)) (* Printf.fprintf oc "\tfsub\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), FMul(y, z) -> add (Fmul(x,y,z)) (* Printf.fprintf oc "\tfmul\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), FDiv(y, z) -> add (Fdiv(x,y,z)) (* Printf.fprintf oc "\tfdiv\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Sltf(y, z) -> add (Sltf(x,y,z)) (* Printf.fprintf oc "\tsltf\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Slt(y, z) -> add (Slt(x,y,z)) (* Printf.fprintf oc "\tslt\t%s, %s, %s\n" (reg x) (reg y) (reg z) *)
  | NonTail(x), Lfd(y, V(z)) -> (* add (Add(reg_tmp,y,z)); add (Lf(x,0,reg_tmp)) *) add (Addlf(x,y,z))
      (* Printf.fprintf oc "\tadd\t%s, %s, %s\n\tlf\t%s, 0(%s)\n" (reg reg_tmp) (reg y) (reg z) (reg x) (reg reg_tmp) *)
(*
      Printf.fprintf oc "\taddlf\t%s, %s, %s\n" x y z
*)
  | NonTail(x), Lfd(y, C(z)) -> add (Lf(x,z,y)) (* Printf.fprintf oc "\tlf\t%s, %d(%s)\n" (reg x) z (reg y) *)
  | NonTail(_), Stfd(x, y, V(z)) -> add (Add(reg_tmp,y,z)); add (Sf(x,0,reg_tmp))
    (* Printf.fprintf oc "\tadd\t%s, %s, %s\n\tsf\t%s, 0(%s)\n" (reg reg_tmp) (reg y) (reg z) (reg x) (reg reg_tmp) *)
  | NonTail(_), Stfd(x, y, C(z)) -> add (Sf(x,z,y)) (* Printf.fprintf oc "\tsf\t%s, %d(%s)\n" (reg x) z (reg y) *)
  | NonTail(_), Comment(s) -> add (Comment(s)) (* Printf.fprintf oc "#\t%s\n" s *)
  (* 退避の仮想命令の実装 (caml2html: emit_save) *)
  | NonTail(_), Save(x, y) when List.mem x allregs && not (S.mem y !stackset) ->
      save y;
      add (Sw(x,(offset y),reg_sp))
      (* Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) *)
  | NonTail(_), Save(x, y) when List.mem x allfregs && not (S.mem y !stackset) ->
      savef y;
      add (Sf(x,(offset y),reg_sp))
      (* Printf.fprintf oc "\tsf\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) *)
  | NonTail(_), Save(x, y) -> (* assert (S.mem y !stackset); *) ()
  (* 復帰の仮想命令の実装 (caml2html: emit_restore) *)
  | NonTail(x), Restore(y) when List.mem x allregs ->
      add (Lw(x,(offset y),reg_sp))
      (* Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) *)
  | NonTail(x), Restore(y) ->
      assert (List.mem x allfregs);
      add (Lf(x,(offset y),reg_sp))
...
(* new instructions *)
  ...
  | Tail, CallCls(x, ys, zs) -> (* 末尾呼び出し (caml2html: emit_tailcall) *)
      ...
      add (Lw(reg_tmp,0,reg_cl)); add (Jr(reg_tmp))
  ...
  | NonTail(a), CallCls(x, ys, zs) ->
      ...
      add (Sw(reg_lr,(ss-1),reg_sp));
      add (Addi(reg_sp,reg_sp,ss));
      add (Lw(reg_tmp,0,reg_cl));
      add (Jalr(reg_lr,reg_tmp));
      add (Addi(reg_sp,reg_sp,(-ss)));
      add (Lw(reg_lr,(ss-1),reg_sp));
      (* 
...
and g'_args oc x_reg_cl ys zs =
  ...
  let (i, yrs) =
    List.fold_left
      ...
      ys in
  List.iter
    (fun (y, r) -> add (Mov(r,y));(* Printf.fprintf oc "\tmov\t%s, %s\n" (reg r) (reg y) *) )
    (shuffle reg_sw yrs);
  let (d, zfrs) =
    List.fold_left
      ...
      zs in
  List.iter
    (fun (z, fr) -> add (Movf(fr,z)) (* Printf.fprintf oc "\tmovf\t%s, %s\n" (reg fr) (reg z) *) )
...
let rec arrange_float_imm oc data n = 
  ...
  | ((Id.L(x),d)::rest) ->
      ...
      add (Sw(reg_tmp,0,reg_hp));
      add (Addi(reg_hp,reg_hp,1));
      (* Printf.fprintf oc "\tsw\t%s, 0(%s)\n" (reg reg_tmp) (reg reg_hp);
