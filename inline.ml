...
let recursive_threshold = ref 200
...
let rec is_recursive e x =
  match e with
  | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2)
  | Let(_, e1, e2) | LetRec({ body = e1 }, e2) -> is_recursive e1 x || is_recursive e2 x
  | LetTuple(_, _, e) -> is_recursive e x
  | App(x',ys) when (x = x') -> true
  | _ -> false
...
let rec g env = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) -> (* 関数定義の場合 (caml2html: inline_letrec) *)
      if is_recursive e1 x then (
        let env = if size e1 > !recursive_threshold then env else M.add x (yts,e1) env in
