...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | Asm("sqrt",[x]) when memf x env ->  Printf.printf "fold sqrt\n";Float(sqrt (findf x env))
  | Asm("ftoi",[x]) when memf x env ->  Printf.printf "fold ftoi\n";Int(int_of_float ((findf x env) +. 0.5))
  | Asm("itof",[x]) when memi x env ->  Printf.printf "fold itof\n";Float(float_of_int (findi x env))
  | Asm("floor",[x]) when memf x env -> Printf.printf "fold floor\n";Float(floor (findf x env))
